...
async function boot(): Promise<void> {
	const [{ mount }, { default: App }, { installFlightScope }] = await Promise.all([
		import('svelte'),
		import('./App.svelte'),
		import('$lib/state/flightScope'),
	]);
	// Hand the route workspace to the surfaces that read a plan without
	// importing one (state/planScope.svelte.ts). It rides the same dynamic
	// window as the App import for the same reason, and lands before mount
	// so nothing paints against an unregistered scope.
	installFlightScope();
