<script lang="ts">
	...
	import { ui, MOBILE_UI_MEDIA, PHONE_LANDSCAPE_MEDIA } from './lib/state/ui.svelte';
	...
	onMount(() => {
		...
		const sync = (): void => {
			...
			if (entering) {
				...
				// landscape stays mobile, so the page survives rotation.)
				ui.page = null;
			}
			if (flipped || turned) {
...
<div class="app">
	...
	<LazySurface id="detail" open={isOpen('detail')} load={() => import('$lib/components/DetailSurface.svelte')} />
	<LazySurface
		id="account"
