...
import { viewFile } from '$lib/native/openDocument';
...
/** Hand a file to ANOTHER APP on the device rather than to storage: Android's
 *  "open with" chooser, which is how an exported flight plan reaches SDVFR.
 *
 *  False on the web, in a shell without the plugin, and on a phone where
 *  nothing opens the type; the caller then falls back to `downloadBlob`, whose
 *  share sheet still saves the file wherever the pilot wants it. That fallback
 *  is not decoration: the navigation apps this exists for register ACTION_VIEW
 *  and no ACTION_SEND filter at all, so the share sheet CANNOT list them, and
 *  the chooser cannot list an app that only saves. Neither gesture reaches
 *  everywhere on its own (docs/route-files.md, docs/android.md). */
export async function openFileWith(
	...
	try {
		...
		const written = await Filesystem.writeFile({
			path: flatName(filename),
			directory: Directory.Cache,
			data,
			encoding: Encoding.UTF8,
		});
		return await viewFile(written.uri, mime);
	} catch {
		return false;
	}
}
