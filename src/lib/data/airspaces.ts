...
/** The shared `id` behind a per-row `key`, the inverse of the `id|name`
 *  construction below. A caller holding one row's key and a structure keyed by
 *  id (the activation hatch, which is drawn once per activated id) needs this
 *  to make the two meet: several rows share an id, so comparing keys across
 *  that boundary silently misses. */
export function airspaceIdOfKey(key: string): string {
...
const AIRSPACE_CATEGORY: Record<string, AirspaceCategory> = {
	...
	FBZ: 'activity',
	// The kinds cmd/fr now reads off the SIA's txtLocalType rather than
	// folding into the generic bucket (docs/fr-aip.md).
	MODEL: 'activity', AEROBATIC: 'activity', UAS: 'activity',
...
export const AIRSPACE_TYPE_LABELS = {
	...
	AEROBATIC: 'Aerobatic activity',
	MODEL: 'Model aircraft flying site',
	UAS: 'Unmanned aircraft activity',
...
/** The vertical profiles' airspace filter rows, the Garmin navigators'
 *  Airspace page ported to the vocabulary this data actually carries
 *  (GTN Xi Pilot's Guide 190-02327-03 "Airspace Selections": Class B/TMA,
 *  Class C/CTA, Class A/D, Restricted, MOA (Military), Other). Garmin
 *  splits controlled airspace by CLASS and everything else by KIND, which
 *  is the right axis here too, except that class A and class D are kept
 *  apart: in Europe class A forbids VFR outright while class D only wants
 *  a clearance, so Garmin's "Class A/D" pairing would put the one row a
 *  VFR pilot must never lose beside an ordinary one. Class E earns its own
 *  row for the mirror reason: Garmin buries it in "Other" and it is where
 *  French VFR actually flies.
 *
 *  Two rows Garmin has no equivalent for. `trafficmgmt` (RMZ / TMZ) is
 *  split out because it is the ONE airspace family the regulation makes
 *  you talk to: SERA.6005(a) wants a continuous watch and "before entering
 *  a radio mandatory zone, an initial call ... shall be made". Filing it
 *  in the catch-all would let one tick hide the only volume a pilot is
 *  required to call. And `fir` / `siv` name the structural background,
 *  And `fir` / `siv` name the structural background. The FIR row stays
 *  column-only, a FIR being one slab across a whole leg; the FIS row is
 *  offered everywhere, because 94 of the 110 French sectors have their
 *  ceiling inside the VFR band and a profile is the only surface that can
 *  show a climb leaving one. */
export type AirspaceFilterGroup =
	| 'classA'
	| 'classB'
	| 'classC'
	| 'classD'
...
/** Display order of the filter rows: the five ICAO classes, then the
 *  families a flight ENTERS, then the two background rows, the FIR (which
 *  the column profiles alone offer) and the FIS sectors. Exhaustive over
 *  AirspaceFilterGroup by construction. */
export const AIRSPACE_FILTER_GROUPS = [
	'classA',
	'classB',
	'classC',
	'classD',
	...
	'fir',
	'siv',
] as const satisfies readonly AirspaceFilterGroup[];

/** The rows the two CHART profiles (route, trace) offer: everything but
 *  `fir`, whose volumes their walk never produces. A FIR really is one slab
 *  across the whole leg with no vertical structure a VFR flight meets; a
 *  FIS sector is the opposite, and its row is offered here. */
export const PROFILE_BAND_FILTER_GROUPS = AIRSPACE_FILTER_GROUPS.filter((g) => g !== 'fir');

/** The rows the TRACE profile offers: the band rows less the FIS sectors.
 *  Drawing those honestly needs the in-force emphasis, since they nest, and
 *  that needs an altitude-aware schedule walk the trace cannot feed yet -
 *  `traceAsWaypoints` carries `alt: 0` on every vertex and `inBand`'s floor
 *  test is strict, so the walk answers an empty timeline. A row the surface
 *  cannot serve is absent, never disabled. */
export const TRACE_BAND_FILTER_GROUPS = PROFILE_BAND_FILTER_GROUPS.filter((g) => g !== 'siv');

/** The ICAO classes that name a filter row. F and G do not: class G is
 *  already dropped from the profiles as clutter, and class F is two rows
 *  in the whole coverage. */
const CLASS_GROUP: Record<string, AirspaceFilterGroup> = {
	A: 'classA', B: 'classB', C: 'classC', D: 'classD', E: 'classE',
};

/** Special-use types whose KIND names their row whatever class they carry:
 *  Belgium files W as class B / C / E, two ADIZ rows carry class A, and
 *  five danger areas carry class G. A type that says what a volume IS
 *  outranks a class that says how it is served. */
const TYPE_GROUP: Record<string, AirspaceFilterGroup> = {
	// A temporary flight restriction is a RESTRICTION, not an "other": it
	// files with R / D rather than beside unclassed controlled airspace.
	R: 'restricted', D: 'restricted', CBA: 'restricted', TFR: 'restricted',
	MOA: 'military', W: 'military', A: 'military', TRA: 'military', TSA: 'military',
	RMZ: 'trafficmgmt', TMZ: 'trafficmgmt', 'TMZ-RMZ': 'trafficmgmt',
	ADIZ: 'other',
};

/** The one filter row an airspace answers to, or null when it answers to
 *  NONE, i.e. when no row can hide it. (The toolbar's LEVEL BAND is a
 *  separate, app-wide condition and still applies to a row-less volume,
 *  exactly as it does on the map.)
 *
 *  The resolution ORDER is the contract:
 *
 *  1. A PROHIBITED area names no row. Garmin's own rule, stated flatly in
 *     both pilot's guides ("Airspace alerts for Prohibited airspace cannot
 *     be disabled"), and on its map P is the last thing decluttered.
 *  2. Aerial activity answers to its own row, `activity`, whatever class
 *     it carries.
 *  3. The structural background answers to its OWN row, `fir` or `siv`,
 *     never to a class row. A FIR that carries a class (18 rows do) must
 *     not vanish from a COLUMN profile when Class A is unticked, the frame
 *     going with the picture; and the two rows exist at all because those
 *     profiles draw FIR / UIR / SIV / FIC columns and nothing else could
 *     hide them.
 *  4. Type before class, per TYPE_GROUP: type `A` is an Alert Area, not
 *     class A.
 *  5. Then the ICAO class letter.
 *  6. Anything controlled that is left carries no A-E class (the field is
 *     '', 'G' or the FAA rows' literal 'Other'), so it is Other. Unknown
 *     future types land here too, `airspaceCategory` defaulting to
 *     controlled.
 *  7. Everything else fails OPEN, which is the only safe default for a
 *     dataset that changes every 28 days.
 *
 *  Pure; the shape is a Pick, so Airspace, AirspaceCorridorBand and
 *  PlacedBand all satisfy it with no adapter. */
export function airspaceFilterGroup(
	a: Pick<Airspace, 'type' | 'airClass' | 'category'>,
): AirspaceFilterGroup | null {
	if (a.type === 'P') {
		return null;
	}
	if (a.category === 'activity') {
		return 'activity';
	}
	if (a.category === 'fir') {
		return 'fir';
	...
	const byType = TYPE_GROUP[a.type];
	if (byType) {
		return byType;
	}
	const byClass = CLASS_GROUP[a.airClass];
	if (byClass) {
		return byClass;
	}
	return a.category === 'controlled' ? 'other' : null;
}

/** Does the pilot's row choice let this airspace through? A row-less
 *  airspace (a prohibited area) always passes; the level band, being
 *  nobody's row, is applied by the caller. */
export function airspaceGroupShown(
	a: Pick<Airspace, 'type' | 'airClass' | 'category'>,
	on: Readonly<Record<AirspaceFilterGroup, boolean>>,
): boolean {
	const g = airspaceFilterGroup(a);
	return g == null || on[g];
}
