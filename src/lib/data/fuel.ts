...
export interface FuelOffering<S = unknown> {
	...
	withdrawnAll?: S;
	/** The conditions the AIP attaches to the pump, where it states any.
	 *  Optional so a structural caller need not fabricate one; absent reads
	 *  as no condition stated. */
	cond?: readonly FuelCondition[];
}

/** What a VISITING aeroplane has to do to uplift here, as one answer.
 *
 *  A row's flags are about the pump and not about a grade, so one aerodrome
 *  gets one answer: ten rows carry `or` and `hx` together and would otherwise
 *  say two things about one field. `h24` is the ABSENCE of a condition and so
 *  only wins standing alone. */
export type FuelAccess = 'h24' | 'onRequest' | 'noHours' | 'basedOnly';

/** The strongest condition on the row, or null where it states none.
 *
 *  `or` is the one flag that REOPENS a field reserved to its based aircraft,
 *  and that is read off the corpus rather than assumed: LFKG is the single
 *  row carrying both, and its prose says why ("réservé aux ACFT basés -
 *  disponible avec préavis ou sur demande"). `hx` beside `based` is not an
 *  escape hatch, the hours qualifying the reservation rather than excusing
 *  it ("HX réservé aux ACFT basés", LFGE / LFHJ / LFIT), and no row in the
 *  corpus carries `based` beside `h24` at all. */
export function fuelAccess(cond: readonly FuelCondition[] | undefined): FuelAccess | null {
	const has = (c: FuelCondition): boolean => cond?.includes(c) ?? false;
	if (has('based') && !has('or')) {
		return 'basedOnly';
	}
	if (has('or')) {
 ...
 *
 *  The five answers are not interchangeable to a pilot. Three are the AIP's:
 *  this field has no fuel at all, its fuel is reserved for the aircraft based
 *  on it, or none of the grades it publishes is one the aeroplane takes; all
 *  are permanent until the next cycle, and the answer is to plan around them. Two are a NOTAM's, carry a date and an id,
...
export type FuelRefusal<S = unknown> =
	| { kind: 'aipNone' }
	/** The AIP reserves this field's fuel for the aircraft based on it. */
	| { kind: 'aipBased' }
	| { kind: 'aipGrade' }
	| { kind: 'notamAll'; by: S }
...
export function fuelRefusal<S>(
	...
	// The AIP states this pump is for the aircraft based on the field. That
	// is about the aerodrome and not about the aeroplane, so it sits with the
	// two reasons above rather than the two below: it holds whatever grade
	// the visitor burns, it holds with no aeroplane selected, and it holds at
	// the three fields that reserve their fuel while naming no grade at all
	// (LFEP, LFHR, LFOY), which the unknown-grade guard below would swallow.
	if (fuelAccess(f.cond) === 'basedOnly') {
		return { kind: 'aipBased' };
	}
	// Past the three reasons above, every answer is about THIS aeroplane's
