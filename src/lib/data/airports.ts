...
export interface Airport {
	...
	charts: AirportChart[];
	/** Published helipads (AIXM <Tla>), France only. A helistation's own
	 *  reference point is usually the centre of its pad, so this is mostly
	 *  the SIZE that is new; where the two differ, the pad is the truth
	 *  about the pad. Always an array, empty for the 600-odd aerodromes
	 *  that publish none. */
	pads: HeliPad[];
}

/** One published helipad. `lat` / `lon` are the touchdown area's own
 *  centre; the two squares are the touchdown and lift-off area and the
 *  final approach and take-off area around it, in metres, either of which
 *  the AIP may leave unstated. */
export interface HeliPad {
	lat: number;
	lon: number;
	tlofLenM: number | null;
	tlofWidM: number | null;
	fatoLenM: number | null;
	fatoWidM: number | null;
}
...
function rowToAirport(r: AirportRow): Airport {
	return {
		...
		pads: (r[18] ?? []).map(rowToPad),
	};
}

function rowToPad(p: PadRow): HeliPad {
	return { lat: p[0], lon: p[1], tlofLenM: p[2], tlofWidM: p[3], fatoLenM: p[4], fatoWidM: p[5] };
}
