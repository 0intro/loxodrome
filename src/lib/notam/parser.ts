 ...
 */
/** A coordinate and, within the same sentence, the radius drawn round it,
 *  on EITHER side: the corpus writes "500336N 0041313E ... RADIUS 400M" and
 *  "RADIUS 3NM AROUND 500739N0094213E" alike, which is why radius.ts reads
 *  both directions too. Bounded to 70 characters and stopped by a full stop
 *  so it reads one statement; newlines are allowed because the
 *  reconstruction wraps a line wherever the briefing did. */
const COORD_SRC = String.raw`\d{4,7}(?:[.,]\d+)?\s*[NS][\s,\-/]*\d{5,8}(?:[.,]\d+)?\s*[EW]`;
const radiusBesideCoordPattern = new RegExp(
	`${COORD_SRC}[^.]{0,70}?\\b(?:RADIUS|RAYON)\\b|\\b(?:RADIUS|RAYON)\\b[^.]{0,70}?${COORD_SRC}`,
	'i',
);
...
function hasCoordinateExtractionCue(
	...
	// A coordinate with a RADIUS stated beside it is a position: the text has
	// drawn a circle round it, which is the same evidence radius.ts reads to
	// size that circle. Belgium and Germany write their warnings as one line
	// with no marker at all ("MACON 500336N 0041313E MODEL ACFT EVENT RADIUS
	// 400M", "MEEUWEN GRUITRODE PJE 510212N 0053121E RADIUS 04NM"), and
	// without this cue the position AND its radius were both dropped for the
	// Q-line's own centre and its coarse 5 NM disc.
	const hasRadiusBesideCoord = radiusBesideCoordPattern.test(eContent);
	return (
		hasPsnKeyword ||
		hasCentreKeyword ||
		hasObstKeyword ||
		hasObstQCode ||
		hasRadiusBesideCoord ||
...
function scanCoordinates(
	...
	// Coordinate-like patterns in the E) section. The pair separator accepts
	// a SLASH beside the space, comma and dash: the SIA writes an obstacle's
	// position "PSN : 442958.700N / 0044224.902E" (P1801/26 LFLQ, P0464/26
	// LFLP), and without it the whole E) reading is lost and the entry falls
	// back to the Q-line centre, half a mile off the crane it is about.
	const coordPattern =
...
export function parseNotams(text: string, opts: ParseOptions = {}): Notam[] {
	...
	const notams: Notam[] = [];
	// id -> the body it was first seen with. The dedupe exists because one
	// source NOTAM recurs across a briefing's groupings and across per-route
	// responses, which is an IDENTICAL repeat. Series+number+year is unique
	// only WITHIN a state, though, and both fetch paths brief an AREA that
	// crosses borders, so a border briefing carries genuinely different
	// NOTAMs under one id (measured over France on 2026-09-18: ten ids, all
	// cross-FIR, two of them dropping a French NOTAM outright, one of which
	// was creating temporary restricted areas). Comparing the body tells the
	// two apart: a repeat collapses, a collision keeps both.
	...
	for (const piece of pieces) {
		let notamId = piece.id;
		const body = piece.content.replace(/\s+/g, ' ').trim();
		const prior = seenIds.get(notamId);
		if (prior != null) {
			if (prior === body) {
				continue;
			}
			// A different NOTAM under the same id: qualify it with its own
			// FILING LOCATION, the prefixed form the splitter and relations.ts
			// already read ("LFFA-W2578/25"). Item A) rather than the Q-line
			// FIR, because a number is unique to the office that issued it and
			// the US issues per AERODROME: KGXY and KENV both file A0002/26
			// inside KZDV. The first occurrence keeps the bare id, so an
			// unambiguous briefing is untouched.
			const at = /(?:^|\s)A\)\s*([A-Z]{4})/.exec(piece.content);
			const fir = /(?:^|\s)Q\)\s*([A-Z]{4})/.exec(piece.content);
			const where = at?.[1] ?? fir?.[1] ?? '';
			notamId = where ? `${where}-${piece.id}` : piece.id;
			if (seenIds.has(notamId)) {
				continue;
			}
		}
		seenIds.set(notamId, body);
