...
export function extractRadiusFromText(
	...
	// A radius stated a few WORDS after the coordinate, which is how Belgium
	// and Germany write a one-line warning: "MACON 500336N 0041313E MODEL
	// ACFT EVENT RADIUS 400M", "LOCHRISTI 510527N 0034953E ASCENTS AND
	// EVOLUTIONS OF ABOUT 24 HOT AIR BALLOONS RADIUS 10NM". Bounded to the
	// sentence and to 80 characters, and refused when another coordinate
	// sits between the two, so a radius can never be read off its neighbour.
	const separated = eContent.substring(matchEnd, matchEnd + 80);
	const sepMatch =
		/^([^.]*?)\b(?:RADIUS|RAYON)\s*(?::|DE\s+|OF\s+)?\s*(\d+(?:[.,]\d+)?)\s*(NM|KM|METRES?|M)\b/i.exec(
			separated,
		) ??
		/^([^.]*?)\b(\d+(?:[.,]\d+)?)\s*(NM|KM|METRES?|M)\s+RADIUS\b/i.exec(separated);
	if (sepMatch && !/\d{4,7}\s*[NS]/i.test(sepMatch[1])) {
		return {
			radius: parseFloat(sepMatch[2].replace(',', '.')),
			radiusUnit: normaliseUnit(sepMatch[3]),
		};
	}
