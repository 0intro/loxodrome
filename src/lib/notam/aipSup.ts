 ...
 *  is neither France's nor Spain's. The Q-line FIR is the evidence (and a
 *  French NOTAM's own LF series), deliberately not the airspace links' wider
 *  rule (notamPublisher, which falls back to the A) locations): the
 *  overseas French FIRs number their supplements in series of their own,
 *  which a metropolitan supplement number must not be matched against. */
