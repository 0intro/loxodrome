...
/** A degree² shoelace area scaled to square metres at `meanLat`. THE common
 *  unit the click resolver ranks by: an airspace and a SIGMET measure
 *  themselves in degree², a NOTAM circle and a SUP AIP circle in metres, and
 *  only one unit lets the smallest DRAWN shape win whatever kind drew it
 *  (docs/map-hit-testing.md). */
export function degAreaToM2(deg2: number, meanLat: number): number {
	return deg2 * M_PER_DEG * M_PER_DEG * Math.cos((meanLat * Math.PI) / 180);
}

/** Approximate polygon area in m²; the shoelace deg² area scaled by
 *  cos(meanLat). Used to sort features smallest-first so click resolution
 *  lands on the smallest containing feature. */
export function polygonAreaM2(coords: LatLon[]): number {
	const meanLat = coords.reduce((s, c) => s + c.lat, 0) / coords.length;
	return degAreaToM2(computePolygonArea(coords), meanLat);
}

/** The same, for a `[[lat, lon], …]` ring: the tuple twin of polygonAreaM2,
 *  which takes `{lat, lon}` objects. Airspace and SIGMET rings are tuples. */
export function ringAreaM2(ring: readonly [number, number][]): number {
	...
	for (let i = 0, j = ring.length - 1; i < ring.length; j = i++) {
		deg2 += ring[j][1] * ring[i][0] - ring[i][1] * ring[j][0];
		latSum += ring[i][0];
	}
	return degAreaToM2(Math.abs(deg2) / 2, latSum / ring.length);
}

/** Area of a circle in m². Named beside the ring helpers so a caller ranking
 *  drawn shapes never writes the formula itself. */
...
/** The position a NOTAM is ABOUT, for anything that needs one point rather
 *  than a shape: the Q-line centre, which every briefing carries, else the
 *  first coordinate it drew. Null only for an entry with neither. */
export function notamAnchor(notam: Notam): LatLon | null {
	const q = notam.qualifier;
	if (q && Number.isFinite(q.lat) && Number.isFinite(q.lon)) {
		return { lat: q.lat, lon: q.lon };
	}
	const c = notam.coordinates[0];
...
/** Bounding box of the points a NOTAM is DRAWN at: the polygon's ring, or the
 *  position(s) the pins sit on, never padded by a radius. The difference is
 *  not cosmetic. Measured over three real briefings, the radius-padded box
 *  came out at the SAME 18.1 x 23.4 degrees every time, because two 460 NM
 *  navaid outages (F1932/26, F1665/25) reach into every French briefing there
 *  is; a box that identical across three different routes carries no
 *  information about where any of them is. The markers came out at 5.2 x 5.9,
 *  5.0 x 11.3 and 6.5 x 8.6, which is the answer to "where is this briefing".
 *
 *  Use this to frame MANY NOTAMs. To frame ONE, notamFocusBbox is still right:
 *  you picked that entry and its area is what you asked to see. */
export function notamMarkerBbox(notam: Notam): Bbox | null {
	if (notam.coordinates.length === 0) {
		return null;
	}
	return coordsBbox(notam.coordinates, false);
}
...
/** One DRAWN shape of a NOTAM entry: the ring a polygon entry draws, or one
 *  of the radius circles a position entry draws. The click resolver ranks
 *  SHAPES, not entries: an entry carrying a 1 NM circle far away and a 30 NM
 *  circle over the cursor draws both, and the one under the cursor is the one
 *  whose size has to decide what the click selects
 *  (docs/map-hit-testing.md). Q-line radius circles are excluded throughout:
 *  they only render when selected and would otherwise blanket the map with
 *  thousands of square-km hits. */
export type NotamShape =
	| { kind: 'ring'; ring: [number, number][]; bbox: Bbox; areaM2: number }
	| {
			kind: 'circle';
			lat: number;
			lon: number;
...
/** Every shape the entry draws, in coordinate order; empty for an entry that
 *  draws nothing but its pin(s). */
export function notamShapes(notam: Notam): NotamShape[] {
	if (isPolygonEntry(notam)) {
		...
		return [
			{
				...
				ring,
				bbox: ringBbox(ring),
				areaM2: polygonAreaM2(notam.coordinates),
			},
		];
	}
	const out: NotamShape[] = [];
	for (const c of notam.coordinates) {
		...
		out.push({
			...
			areaM2: circleAreaM2(radiusM),
		});
	}
	return out;
}

/** Is the point inside this drawn shape? Rings use pointInRing, circles a
 *  planar distance against the radius. */
export function shapeContainsPoint(
	...
	if (shape.kind === 'ring') {
		return pointInRing(lat, lon, shape.ring);
	}
	const cosLat = Math.cos((shape.lat * Math.PI) / 180) || 1;
	const dy = (lat - shape.lat) * M_PER_DEG;
	const dx = (lon - shape.lon) * M_PER_DEG * cosLat;
	return dx * dx + dy * dy <= shape.radiusM * shape.radiusM;
}
