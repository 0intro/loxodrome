...
export const about = {
	...
	headFinland: 'Finlande\u202f: registre des obstacles de Fintraffic ANS',
	...
	operaFetched:
		'en direct depuis le cache de 24 h du service Open Radar Data, les tuiles de la vue de la carte pour la dernière heure ou les deux dernières',
	operaFetchedAside:
	...
	privacyPosition:
		'Votre position est lue sur le GPS de l’appareil, dessinée sur la carte et écrite dans la trace. Aucune requête ne la porte, sauf qu’un compte, si vous vous y connectez, synchronise vos vols enregistrés avec leurs traces (voir plus bas). Sous Android, l’enregistrement se poursuit dans un service de premier plan, avec la notification qui l’accompagne, écran éteint si besoin\u202f; l’application déclare les autorisations d’accès à internet, de localisation, de service de premier plan, de notification et de maintien en éveil, et aucune autre. Les requêtes ci-dessous portent des coordonnées de carte, de route ou d’aérodrome, pas votre point GPS, mais tant que la carte suit l’aéronef elles disent bien où vous êtes.',
	privacyStored:
		'Ce que vous construisez reste dans ce navigateur\u202f: les routes, les aéronefs, les préférences, et la bibliothèque des vols avec ses traces. L’onglet Réglages les efface par groupe avec "Réinitialiser l’application…"\u202f; les paquets de cartes et de documents téléchargés et les tuiles en cache y survivent à dessein, chaque paquet ayant sa propre suppression dans le gestionnaire des données hors ligne, ouvert depuis l’onglet Couches.',
	privacyTiles:
	...
	privacyTerrain:
		'les tuiles d’altitude du sol, depuis le Worker décrit ci-dessous\u202f: dès qu’un profil de route ou de trace, un profil d’altitude (ouvert depuis un panneau d’espace aérien ou de NOTAM, ou depuis le menu contextuel de la carte), un couloir d’altitude minimale ou une alerte d’espace aérien a besoin de la hauteur du terrain sous et autour de votre route, de votre trace ou du point, par couloir entier lorsque vous épinglez le terrain d’un plan pour l’usage hors ligne, la tuile sous le pointeur dès qu’il s’arrête sur la carte, et, pendant l’enregistrement ou le rejeu d’un vol, les tuiles de la vue de la carte pour la couche Relief et celles de la trajectoire à venir pour les alertes relief',
	privacyProxy:
		'Deux Workers Cloudflare exploités par l’auteur se placent devant ces services, aucun d’eux n’envoyant les en-têtes CORS dont un navigateur a besoin pour lire la réponse\u202f: proxy.loxodrome.fr relaie les requêtes NOTAM, météo et radar ainsi que les cartes VAC, et charts.loxodrome.fr sert les tuiles de cartes, les tuiles d’altitude du sol et les paquets hors ligne. Chacun voit ce que le service verrait lui-même, la requête de route, les indicateurs d’aérodrome ou les coordonnées des tuiles, ainsi que votre adresse IP. Ils appliquent une limitation de débit par adresse IP et mettent brièvement les réponses en cache. Le journal du relais ne consigne que la route, le code de statut et le texte d’erreur amont d’une requête en échec, jamais le corps de la requête ni l’adresse dont elle provient, et le serveur de cartes ne tient aucun journal. Aucun compte ni identifiant n’y est associé.',
	...
	privacyToggleOff:
		'La bascule "Météo en direct" de l’onglet Réglages arrête les requêtes météo, c’est-à-dire les METAR et TAF, les avis SIGMET, les vents en altitude, le radar de précipitations et le catalogue TEMSI / WINTEM. Elle n’affecte ni les tuiles de la carte ni le terrain, dont la carte et le profil de route ont besoin pour fonctionner.',
	privacyWeather:
	...
	privacyVac:
		'activer la couche des cartes VAC (France, onglet Couches) lit la carte VAC du SIA de chaque aérodrome de la vue de la carte\u202f: dans un paquet de documents téléchargé s’il y en a un, sinon par lectures partielles du paquet publié depuis le relais de cartes ci-dessous, et à défaut carte par carte par le relais ci-dessous, qui la demande au SIA, si bien que ces requêtes nomment les aérodromes de la vue',
	privacyRadar:
		'activer le radar de précipitations demande au relais ci-dessous les images du composite couvrant la vue de la carte, quelques tuiles à la fois, et le relais, qui récupère chaque image une seule fois dans le cache de données ouvertes d’EUMETNET, ne sait de ce que vous regardez que ces tuiles',
	...
	tagline:
		'Loxodrome est une application de préparation du vol et de navigation pour l’aviation générale, bâtie sur les AIP nationales d’Europe et des États-Unis. Elle les dessine en carte aéronautique, prépare le vol jusqu’au bilan carburant, à la masse et centrage et aux distances de décollage et d’atterrissage, le briefe, puis le suit en vol. Libre et open source, elle s’exécute entièrement dans votre navigateur, s’installe sur l’appareil pour un usage hors ligne et ne demande aucun compte. Un compte facultatif peut synchroniser vos plans de vol, vos vols et vos avions entre vos appareils.',
	...
	doesAccount:
		'\u202f: connectez-vous par un code reçu par courriel. Vos plans de vol, vos vols avec leurs traces et vos fiches avion vous suivent entre vos appareils et l’ordinateur du club. Entièrement facultatif\u202f: tout fonctionne sans compte, et rien ne quitte l’appareil tant que vous ne vous connectez pas. Supprimez-le à tout moment depuis l’application.',
	...
	terrainFetched:
		'à la demande, depuis le même Worker qui sert les tuiles de cartes et les paquets hors ligne\u202f; mises en cache hors ligne, et non concernées par la bascule Météo en direct',
	terrainData:
