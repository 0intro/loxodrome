...
export const weather = {
	...
	anchorAria: 'Heure suivie par la carte',
	...
	radar: {
		caution:
			'Les images radar ne sont jamais actuelles\u202f: la plus récente a déjà plusieurs minutes à l’affichage, et la boucle remonte plus loin encore. Elles montrent où étaient les précipitations, pas où elles sont. Servez-vous-en pour éviter une zone, jamais pour vous y frayer un chemin, ni à la place du briefing.',
		caveat: {
			DBZH: 'Réflectivité observée par le composite EUMETNET OPERA de plus de 160 radars au sol européens, rafraîchi toutes les 5 min et en général vieux de 5 à 10 min à l’affichage, les mesures qu’il contient l’étant davantage. Pour la préparation du vol et la planification à longue échéance seulement\u202f: ne jamais s’en servir pour se frayer un chemin dans ou près des précipitations, ni à la place du briefing réglementaire. Les alertes ne le lisent jamais.',
			RATE: 'Intensité de pluie instantanée du composite EUMETNET OPERA, convertie de la réflectivité par la relation de Marshall-Palmer (une incertitude d’un facteur deux), rafraîchie toutes les 15 min et en général vieille de 10 à 25 min à l’affichage, sur une grille de 2 km. Pour la préparation du vol et la planification à longue échéance seulement\u202f: ne jamais s’en servir pour se frayer un chemin dans ou près des précipitations, ni à la place du briefing réglementaire. Les alertes ne le lisent jamais.',
		},
		coverageLimits: 'Afficher les limites de couverture',
		credit: {
			DBZH: 'Composite radar EUMETNET OPERA (CC BY 4.0), produit par Météo-France.',
			RATE: 'Composite radar EUMETNET OPERA (CC BY 4.0), produit par GeoSphere Austria.',
		},
		expired: (p: { time: string; min: number }) =>
			`Radar périmé\u202f: dernière image ${p.time}, plus de ${p.min} min, échos retirés.`,
		frameAge: (p: { time: string; age: string }) => `${p.time}, ${p.age}`,
		framesHeld: (p: { loaded: number; wanted: number }) => `${p.loaded} images sur ${p.wanted} chargées`,
		hidden: 'La couche radar est masquée.',
		legend: 'Radar de précipitations',
		loading: 'Chargement des images radar.',
		frameLoading: 'chargement',
		tierTitle: {
			fresh: 'Image récente',
			aging: 'Image vieillissante',
			stale: 'Image ancienne',
			expired: 'Image au-delà de l’expiration',
		},
		loopAria: 'Durée de l’animation radar',
		loopCapped: (n: number) => `Animation limitée à ${n} images à ce zoom.`,
		loopLabel: 'Animation',
		loopLabels: { 30: '30 min', 60: '1 h', 120: '2 h' },
		newestFrame: (p: { time: string; published: string; age: string }) =>
			`Dernière image ${p.time}, publiée ${p.published}, ${p.age}`,
		newestFrameNoPub: (p: { time: string; age: string }) => `Dernière image ${p.time}, ${p.age}`,
		noCoverage: 'Pas de couverture radar',
		noFrames: 'Aucune image radar listée pour les dernières heures.',
		now: 'maintenant',
		opacity: 'Opacité',
		outside: 'La carte est hors du composite européen.',
		productAria: 'Produit radar',
		productLabel: 'Produit',
		productLabels: { DBZH: 'dBZ', RATE: 'mm/h' },
		publishedAt: (time: string) => `publiée ${time}`,
		radar: 'Radar',
		relAge: (min: number) => `-${min} min`,
		scaleUnits: {
			DBZH: 'Réflectivité en dBZ, avec l’intensité de pluie de Marshall-Palmer en mm/h.',
			RATE: 'Intensité de pluie en mm/h, avec la réflectivité de Marshall-Palmer en dBZ.',
		},
		scrubAria: 'Image radar',
		showRadar: 'Afficher le radar de précipitations sur la carte',
	...
	valid: 'Validité',
	validBeyondReach: (reach: string) => `Prévision jusqu’à ${reach}, rien n’est tracé au-delà.`,
	validClamped: (briefed: string) =>
		`La prévision ne va pas jusqu’à ${briefed}, sa dernière heure est donc affichée.`,
	validDateAria: 'Date de validité (UTC)',
	validFromCustom: 'Elle suit votre période, son début.',
	validFromFlight: 'Elle suit votre période, le départ de votre vol.',
	validFromFlightDay: 'Elle suit votre période, le jour de votre vol.',
	validFromNow: 'Elle suit votre période, l’heure courante.',
	validPinned: 'Heure réglée manuellement, elle ne suit plus votre période.',
