...
export const about = {
	...
	headFinland: 'Finland: Fintraffic ANS obstacle register',
	...
	operaFetched:
		'live from the Open Radar Data service\u2019s 24-hour cache, the tiles of the map view for the last hour or two',
	operaFetchedAside:
	...
	privacyAccount:
		'An account, if you create one, syncs your flight plans, flights and aircraft between your devices through api.loxodrome.fr, a third Cloudflare Worker run by the author. It stores your e-mail address, per-device session records (a device name and when it was last seen) and your documents as opaque content it never parses, in Cloudflare\u2019s European storage (the database in Western Europe, the trace files under EU jurisdiction). Sign-in codes reach your address through Cloudflare\u2019s e-mail service, the sign-in form uses Cloudflare Turnstile to keep bots out, and an address that asks for a code without ever becoming an account is kept only as a hash. Everything is self-serve behind the toolbar\u2019s account button, also reachable at loxodrome.fr/?account=: export a copy of your data, sign out, or delete the account, which erases the server copy after a 7-day grace (deletion records linger up to 90 days so your other devices learn of them). Daily backups of the service, kept on the author\u2019s own computer for up to two months, hold the same data, so erased data leaves the last of them within two months. The steps are also set out at loxodrome.fr/account-deletion.html. Deleting the account never touches the copies on your devices. Each call to api.loxodrome.fr is kept for 7 days in Cloudflare\u2019s request log, which the author can read: the address called and the status of the answer, the request headers with your IP address among them (Cloudflare masks the session token), and the approximate location Cloudflare derives from that address. The request body is never logged, so neither your documents nor a sign-in code is, and the service\u2019s own log lines record only its failures and, by their internal identifier, the accounts it erases.',
	privacyAutorouter:
		'on request, the autorouter fetch sends a route or map-view query, through the relay below, to the public service',
	privacyHeading: 'Privacy',
	privacyIntro:
		'There is no tracking and no advertising, and the application sets no cookie of its own. The briefing you paste or open is parsed in your browser and is never uploaded. An account is entirely optional and everything works without one. The application does make the network requests below: the first two on their own while you use the map, the others only when you ask for them.',
	...
	privacyPosition:
		'Your position is read from the device GPS, drawn on the map and written to the trace. No request carries it, except that an account, if you sign in to one, syncs your recorded flights with their traces (see below). On Android the recording continues in a foreground service, with the notification that goes with it, so the screen may go off; the application declares the internet, location, foreground-service, notification and wake-lock permissions, and no others. The requests below carry map, route or aerodrome coordinates rather than your fix, but while the map follows the aircraft they do describe where you are.',
	privacyStored:
		'What you build stays in this browser: the routes, the aircraft, the preferences, and the flights library with its traces. The Settings tab erases them by group with "Reset application…"; the downloaded chart and document packs and the cached tiles survive it on purpose, each pack having its own delete in the offline data manager, opened from the Layers tab.',
	privacyTiles:
	...
	privacyTerrain:
		'ground elevation tiles, from the worker described below: whenever a route or trace profile, an altitude profile (opened from an airspace or NOTAM panel, or from the map right-click menu), a minimum-altitude corridor or an airspace alert needs the height of the terrain under and around your route, trace or point, a corridor at a time when you pin a plan\u2019s terrain for offline use, the tile under the pointer once it comes to rest on the map, and, while a flight is recorded or replayed, the tiles of the map view for the Terrain layer and those along the path ahead for the terrain alerts',
	privacyProxy:
		'Two Cloudflare Workers run by the author sit in front of those services, because none of them sends the CORS headers a browser needs to read the response: proxy.loxodrome.fr relays the NOTAM, weather and radar requests and the VAC plates, and charts.loxodrome.fr serves the chart tiles, the ground elevation tiles and the offline packs. Each sees what the service itself would see, the route query, the aerodrome idents or the tile coordinates, together with your IP address. They apply a per-IP rate limit and cache responses briefly. The relay\u2019s log records the route, the status and the upstream error text of a failed request only, never the request body and never the address it came from, and the chart server keeps no log at all. No account and no identifier is attached to any of it.',
	...
	privacyToggleOff:
		'The Settings tab\'s "Live weather" toggle stops the weather requests, which are the METAR and TAF, the SIGMET advisories, the winds aloft, the precipitation radar and the TEMSI / WINTEM catalog. It does not affect the map tiles or the terrain, which the map and the route profile need to work at all.',
	privacyWeather:
	...
	privacyVac:
		'turning the VAC charts layer on (France, Layers tab) reads the SIA VAC plate of each aerodrome in the map view: from a downloaded document pack when there is one, otherwise by ranged reads of the published pack from the chart relay below, and failing that one plate at a time through the relay below, which fetches it from the SIA, so these requests name the aerodromes in view',
	privacyRadar:
		'turning the precipitation radar on asks the relay below for the composite frames covering the map view, a few tiles at a time, and the relay, fetching each frame once from the EUMETNET open-data cache, learns nothing of where you look beyond those tiles',
	refresh: 'Refresh',
	...
	// rest hangs off.
	tagline:
		'Loxodrome is a flight-preparation and in-flight navigation application for general aviation, built on the national AIPs of Europe and the United States. It draws them as an aeronautical chart, plans the flight down to the fuel, the mass and balance and the takeoff and landing distances, briefs it, and follows it in the cockpit. It is free and open source, runs entirely in your browser, installs on the device for offline use, and needs no account. An optional account can sync your flight plans, flights and aircraft between your devices.',
	...
	doesAccount:
		': sign in with a code received by e-mail. Your flight plans, flights with their traces and aircraft sheets follow you between your devices and the club computer. Entirely optional: everything works without an account, and nothing leaves the device until you sign in. Delete it at any time from the application.',
	terrainFetched:
		'on demand, from the same worker that serves the chart tiles and the offline packs; cached offline, and not covered by the Live weather toggle',
	terrainData:
