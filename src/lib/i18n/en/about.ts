...
export const about = {
	...
	privacyTerrain:
		'ground elevation tiles, fetched whenever a route profile, a minimum-altitude corridor or an airspace alert needs the height of the terrain under and around your route, and a corridor at a time when you pin a plan\u2019s terrain for offline use, from the worker described below; no preference suppresses these',
	privacyProxy:
		'Two Cloudflare Workers run by the author sit in front of those services, because none of them sends the CORS headers a browser needs to read the response: proxy.loxodrome.fr relays the NOTAM and weather requests, and charts.loxodrome.fr serves the chart tiles, the ground elevation tiles and the offline packs. Each sees what the service itself would see, the route query, the aerodrome idents or the tile coordinates, together with your IP address. They apply a per-IP rate limit and cache responses briefly; their logs record the route, the status and the upstream error text of a failed request only, never the request body and never the address it came from. No account and no identifier is attached to any of it.',
	...
	terrainData:
		'ground elevation, and the highest and lowest ground around each point, sampled from elevation tiles (the readout under the cursor, the route ground profile, minimum safe altitudes, height above ground for the airspace alerts, and the printed dossier)',
	egm96Data:
