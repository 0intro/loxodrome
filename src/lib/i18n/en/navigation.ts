...
export const navigation = {
	...
	locationDisclosure:
		'Loxodrome reads your precise position to record the flight trace, follow the route and warn you about the airspace ahead. The recording continues with the screen off or the application in the background, and its notification stays up for as long as it runs. Your position is written to the trace on this device and is not transmitted, unless you sign in to the optional account, which then uploads your recorded flights, traces included, to sync them between your devices.',
	locationAgree: 'Agree',
	...
	xtkRight: 'R',
	onCourse: 'on course',
	/* The in-flight band's cells (docs/nav-live.md "The in-flight strip"):
	   the heading to steer and the readings a tap cycles to. The abbreviations
	   themselves (MH, BRG, DTK, AGL, MSA, ETE) stay locale-invariant Doc 8400
	   literals in the markup; these are their spelled-out tooltips. */
	headingTip:
		'Magnetic heading to steer to the active waypoint: the bearing from the aircraft, corrected for the forecast wind on this leg at the planned true airspeed.',
	dtkTip: 'Desired track: the planned magnetic course of the leg being flown, as the navigation log prints it.',
	aglTip:
		'Height above the ground under the aircraft, from the terrain model: the GPS altitude less the ground elevation.',
	msaLegTip:
		'Minimum safe altitude of the leg being flown, the navigation log’s MSA column: terrain and obstacles within the corridor, plus the margin.',
	navlogEteTip: 'Time to the destination at the live ground speed.',
	destination: 'Destination',
	/** The reading a tap shows next (the ring cue's accessible name). */
	ringNext: 'Show the next reading',
	/** The next contact's lead, on the cell: "in 8.6 NM · 6 min". */
	contactIn: (lead: string) => `in ${lead}`,
	/** The overflown field's short label, beside its name. */
	contactOverShort: 'Overflown',
	contactHandoverTip:
		'Frequency to set before the next boundary, with the distance and time left to it.',
	freqMore: 'Hold to see every frequency at this unit. Each copies on a tap.',
	/** The plan-only muting, in the width a cell can spare. */
	planOnlyShort: 'plan',
	navlogTakeoff: 'Takeoff',
	navlogTakeoffBeforeTrace: 'before the trace',
	navlogTakeoffBeforeTraceTip:
		'The trace begins in flight: the takeoff happened before its first fix and is not recorded.',
	...
	/* The terrain and obstacle rows of the same stack
	   (docs/terrain-awareness.md): the lead states the threat's top against
	   the projected path and the seconds to it, the action the clearance the
	   phase of flight requires. Facts, never a manoeuvre: nothing says
	   "pull up". */
	terrainAhead: (height: string, s: string) => `Terrain ahead: ${height} in ${s} s`,
	terrainWarning: (height: string, s: string) => `TERRAIN: ${height} in ${s} s`,
	obstacleAhead: (type: string, height: string, s: string) => `${type} ahead: ${height} in ${s} s`,
	obstacleWarning: (type: string, height: string, s: string) =>
		`OBSTACLE: ${type}, ${height} in ${s} s`,
	terrainAbovePath: (ft: string) => `${ft} ft above the flight path`,
	terrainBelowPath: (ft: string) => `${ft} ft below the flight path`,
	terrainAtPath: 'level with the flight path',
	terrainClearance: (ft: string) => `required clearance ${ft} ft`,
	terrainAnnounceCaution: 'Terrain ahead',
	terrainAnnounceWarning: 'Terrain warning',
	obstacleAnnounceCaution: 'Obstacle ahead',
	obstacleAnnounceWarning: 'Obstacle warning',
	/* The channel chip and the alert panel (the record half of the alert
	   ...
	   agreement ("Alerts: 3, acknowledged: 2"). */
	alertPanelOpenTip: 'Open the alert panel',
	alertChipActive: (n: string) => `Alerts: ${n}`,
	alertChipAcked: (n: string) => `acknowledged: ${n}`,
	alertChipAllAcked: 'All alerts acknowledged',
	alertChipSuspended: 'Alerts suspended',
	...
	alertSuspendedLost: 'Alerts suspended: position lost',
	alertSuspendedLostTip:
		'No position for over 15 seconds: a frozen position must not be graded against airspace or terrain. Evaluation resumes with the next fix.',
	alertSuspendedNoFix: 'Alerts waiting for a position',
	...
	alertsTip:
		'Grades what lies around and ahead of the aircraft from the GNSS position: forbidden zones, clearance boundaries, equipment zones and hazards, with activation read over the projected path, then the terrain and the obstacles along the next minute of flight.',
	alertsEnable: 'Evaluate airspace, terrain and obstacles around the aircraft',
	...
	alertTierTerrain: 'Terrain ahead',
	alertTierTerrainTip:
		'Alerts when the path ahead comes within 100 ft of rising ground, the red of the terrain layer: a caution a minute ahead, a warning half a minute ahead. Under IFR the clearance is the one a terrain awareness system applies by phase of flight: 700 ft en route, 350 ft near an aerodrome, 150 ft on approach, 100 ft after takeoff (500, 300 and 100 ft while descending). Ground that does not rise above the ground under the aircraft alerts only where it stands above the aircraft on its track, and anything standing less than 500 ft above an aerodrome within 2 NM of its runways, which its circuit clears and its chart shows, never alerts.',
	alertTierObstacle: 'Obstacles ahead',
	alertTierObstacleTip:
		'Alerts when a published obstacle on the path ahead stands within the same clearance, outside an aerodrome’s surroundings.',
	alertTerrainInhibit: 'Inhibit terrain and obstacle alerts',
	alertTerrainInhibitTip:
		'For a deliberate low-level segment or an aerodrome the data does not list. Lifted by the next recording or trace.',
	alertTerrainInhibited: 'Terrain and obstacle alerts inhibited',
	alertTerrainUnread: 'Terrain ahead not read yet',
	alertTerrainUnreadTip:
		'Part of the ground along the path ahead has not loaded, or failed to load. Nothing is alerted from what is missing.',
	alertObstaclesMissing: 'Obstacle data not loaded: obstacles are not evaluated',
	alertTerrainAdvisory:
		'Terrain and obstacle alerts are an aid to situational awareness only, not a certified terrain awareness and warning system (TAWS).',
	...
	alertAudioCaution: 'Chime for airspace cautions too',
