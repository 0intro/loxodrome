...
/** The call's own deadline, widened by the CALLER's signal when it has
 *  one: a blob carries a 120 s deadline, and a sign-out joining the
 *  trace download must not sit behind it. */
function abortWith(timeoutMs: number, caller: AbortSignal | undefined): AbortSignal | undefined {
	...
	try {
		return AbortSignal.any([caller, limit]);
	} catch {
		return caller; // no `any` here: the caller's stop is the one that must land
	}
}
...
async function call(
	...
	const signal = abortWith(init.timeoutMs ?? 30_000, init.signal ?? undefined);
	let res: Response;
	try {
 ...
 *  inflates and VERIFIES the hash before anything is applied. `signal`
 *  is the trace download's stop: without it a wipe joining the download
 *  would wait out the 120 s deadline of whatever is in flight. */
export async function getBlob(
	token: string,
	hash: string,
	signal?: AbortSignal,
): Promise<Uint8Array> {
	const res = await call(`/v1/blobs/${hash}`, {
		method: 'GET',
		token,
		timeoutMs: 120_000,
		...(signal ? { signal } : {}),
