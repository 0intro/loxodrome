...
async function wxFetch<T>(
	...
	// NEVER the browser's HTTP cache for live weather, whatever the headers
	// say: the edge is the sharing layer (a minute), the caches here pace
	// their own refetch, and the zone stretches a cached answer's browser
	// TTL to four hours (measured on /wx: max-age=60 on the miss, 14400 on
	// the hit two seconds later), which held a METAR that long.
