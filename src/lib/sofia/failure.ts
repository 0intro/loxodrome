 ...
 *
 *  ONE well-spaced retry is the general answer: a transport failure can cost
 *  the whole briefing budget per attempt, so a second one buys little against
 *  a service that has just failed twice.
 *
 *  `refused` and `malformed` were once worth two, on a measurement of
 *  "roughly one briefing in six refused spuriously" with
 *  "Le format d'un des paramètres n'est pas bon : [departure_date,
 *  departure_time, lang]". That refusal was never spurious: it is SOFIA
 *  declining a departure earlier than the current minute, DETERMINISTICALLY
 *  (measured 2026-09-06, one minute in the past refused three times out of
 *  three), which this app sent whenever the viewing period opened in the
 *  past, which the Flight period always does within an hour of the ETD. The
 *  client now resolves the departure per attempt (sofiaDepartureMs), so the
 *  cause is gone and the extra attempts were only ever three doomed POSTs at
 *  a flood-prone government service. `refused` is back to meaning what this
 *  file says it means: SOFIA read the request and would refuse it again.
 *
 *  A proxy refusal (`busy`) and the pilot's Stop (`cancelled`) are decisions,
 *  not accidents: retrying either would only add load to what just turned us
 *  away, and `noAerodrome` was settled here without asking, so there is
