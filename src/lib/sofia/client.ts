...
export interface NarrowRouteOptions {
	 ...
	 *  Never in the past: SOFIA refuses a past departure outright, so callers
	 *  resolve this through `sofiaDepartureMs`. */
	validFrom: Date;
}
...
/** How far ahead of now a clamped departure is placed, at least.
 *
 *  SOFIA refuses a `departure_date` / `departure_time` strictly EARLIER than
 *  the current UTC minute, and would refuse it again: measured 2026-09-06,
 *  one minute in the past was refused three times out of three, while a
 *  request a month ahead was accepted. It caps NO look-ahead, and its own
 *  date picker agrees (minDate today, no maxDate), so a past departure is
 *  the only one it will not read. The margin covers the proxy hop, the
 *  worker's own handshake and a device clock a minute slow. */
export const SOFIA_DEPARTURE_GUARD_MS = 60_000;

/** The departure instant to ask SOFIA for, given the start of the period the
 *  pilot is looking at. Only ever moves a PAST request forward, never a
 *  future one, so a flight planned for next week is still briefed for next
 *  week.
 *
 *  The app's evaluation window cannot be sent as it stands because it is a
 *  DISPLAY window: it opens an hour before the ETD (the ramp hour a briefing
 *  is read in) and is floored to the past minute in `now` mode, so its start
 *  is routinely behind the wall clock. Resolve it per REQUEST rather than
 *  once per briefing: a multi-route briefing paces its POSTs seconds apart
 *  and retries seconds after that, so a value frozen at the start goes stale
 *  mid-loop and every route behind the boundary is refused.
 *
 *  Rejecting a past period outright is the CALLER's job (fetch.ts): moving
 *  the departure forward is honest only while the period still has a future
 *  half for the briefing to land in. */
export function sofiaDepartureMs(windowFromMs: number, nowMs: number): number {
	// Ceil to the whole minute, not floor: buildNarrowRouteBody prints HHMM, so
	// the minute this value lands in is the minute SOFIA reads, and that minute
	// has to still be ahead when the request arrives. Ceil plus the guard
	// leaves between 60 and 120 s of margin.
	const earliest = Math.ceil((nowMs + SOFIA_DEPARTURE_GUARD_MS) / 60_000) * 60_000;
	// An ordered test rather than Math.max, which answers NaN for one: a window
	// start that failed to resolve would then reach Date.toISOString inside the
	// body builder and throw. This way it simply briefs from now.
	return windowFromMs > earliest ? windowFromMs : earliest;
}
