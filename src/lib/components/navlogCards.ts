/* Pure paper geometry and chunking for the printed nav logs AND the
 * printed radio/airspace schedules (the NavLogModal all-routes kneeboard,
 * its single-route A4 document and the flight dossier's per-trip cards):
 * content that outgrows one panel is split into numbered continuation
 * parts instead of being left to fragment mid-row across sheets, which on
 * one CSS grid tears a band in half, holes the page before it, and drops
 * the column header off the page after. Kept free of Svelte and I/O (the
 * $lib/state/navlogSchedule.ts pattern) so it is unit-testable in Node;
 * the DOM measuring mounts live in the navlogMeasure.ts sibling.
 *
...
/** The content box one printed part is measured in and rendered into. Two
 *  exist, and a caller states which paper it is packing for: measuring a
 *  page against the card's geometry is exactly the drift these constants
 *  exist to prevent. */
export interface PrintGeometry {
	/** Printed content width, in CSS px. */
	widthPx: number;
	/** Height allowance for that content, in CSS px. */
	budgetPx: number;
	/** The rendition prop the sheet / schedule mounts take for this paper. */
	rendition: 'kneeboard' | 'portrait';
}

/** The A5 kneeboard card, two per landscape sheet: A4 landscape 297 mm
 *  minus the sheet's two 8 mm paddings and the 16 mm central gap, halved
 *  (132.5 mm) by 210 mm minus those two paddings (194 mm, ~733 px). */
export const KNEEBOARD_CARD: PrintGeometry = {
	widthPx: ((297 - 2 * 8 - 16) / 2) * PX_PER_MM,
	budgetPx: (210 - 2 * 8) * PX_PER_MM,
	rendition: 'kneeboard',
};

/** Slack left under the A4 page's content box. The split is measured at
 *  the very geometry the page prints at, so it fits by construction; a
 *  page packed to the last pixel, though, spills a fragment onto a sheet
 *  of its own the moment the print layout rounds a fraction the measuring
 *  mount did not. Two millimetres buys that back and costs no band. */
const PAGE_SLACK_PX = 2 * PX_PER_MM;

/** The A4 portrait page of the surface's own single-route document: 210 by
 *  297 mm minus the 12 mm padding each side that carries the printed
 *  margin (the `@page` margin is zeroed, a print dialog being able to
 *  strip that but not a padding). */
export const PORTRAIT_PAGE: PrintGeometry = {
	widthPx: (210 - 2 * 12) * PX_PER_MM,
	budgetPx: (297 - 2 * 12) * PX_PER_MM - PAGE_SLACK_PX,
	rendition: 'portrait',
};
