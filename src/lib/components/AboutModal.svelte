<script lang="ts">
	...
	import type { AixmObstaclesMeta, NatureMeta, SupAipMeta } from '$lib/data/meta';
	...
	import { fmtInt, t } from '$lib/state/i18n.svelte';
	...
	// the country's dataset metas plus the card's own extras (chart links,
	// licence note, the nature / SUP AIP rows). Built in a $derived so the
	// headings and licence notes re-translate live.
	interface PublisherCard extends AixmCountryMeta {
		...
		bareCounts?: boolean;
		/** Chart-PDF count + the aerodromes carrying them (UK / AT). */
		charts?: { count: number; sites: number } | null;
		/** DE instead counts DFS aerodrome pages (permalinks, no PDFs). */
		chartPages?: number | null;
	...
	const publisherCards = $derived.by((): PublisherCard[] => {
		...
		const cards: PublisherCard[] = [
			{
				...
				...m.aixm.uk,
				charts: m.ukAdCharts
					? { count: m.ukAdCharts.charts, sites: m.ukAdCharts.aerodromes }
					: null,
			...
			},
			{
				...
				...m.aixm.de,
				chartPages: m.deAdCharts ? m.deAdCharts.aerodromes : null,
		...
		return cards.filter((c) => c.airspaces || c.airports || c.obstacles || c.navaids);
	});
...
>
	...
	{#if aboutModal.view === 'license'}
		...
		<div class="body">
				...
				{:else}
					...
					<div class="sources">
						...
						{#if m.faa}
							...
							<DataSourceCard>
								{#snippet heading()}
									<a href="https://adds-faa.opendata.arcgis.com/" target="_blank" rel="noopener noreferrer">{t.about.headFaa}</a>
								{/snippet}
								<dt>AIRAC</dt>
								<dd>AIRAC {airacYYNN(faa.generatedAt)}</dd>
								<dt>{t.about.generated}</dt>
								<dd class={freshnessClass(faa.generatedAt)}>
									{fmtUTC(faa.generatedAt)}
									<span class="aside">({fmtAgo(faa.generatedAt)})</span>
								</dd>
								<!-- i18n-ignore: FAA dataset names, invariant -->
								<dt>Boundary</dt>
								<dd>{t.about.airspacesCount(fmtNumber(faa.boundary.count))}</dd>
								<!-- i18n-ignore: FAA dataset names, invariant -->
								<dt>Special-Use</dt>
								<dd>{t.about.airspacesCount(fmtNumber(faa.specialUse.count))}</dd>
								<dt>{t.about.faaClass}</dt>
								<dd>{t.about.airspacesCount(fmtNumber(faa.class.count))}</dd>
								{#if m.faaAirports}
									<dt>{t.layers.airports}</dt>
									<dd>
										{fmtNumber(m.faaAirports.ahpCount)}
										<span class="aside">({t.about.runwaysAside(fmtNumber(m.faaAirports.runwayCount))})</span>
									</dd>
								{/if}
								{#if m.faaNavaids}
									<dt>{t.layers.navaids}</dt>
									<dd>
										{fmtNumber(m.faaNavaids.navaidCount)}
										<span class="aside">({t.about.navaidsAside(fmtNumber(m.faaNavaids.counts.WAYPOINT ?? 0))})</span>
									</dd>
								{/if}
								{#if m.faaObstacles}
									<dt>{t.layers.obstacles}</dt>
									<dd>
										{fmtNumber(m.faaObstacles.obstacleCount)}
										<span class="aside">({t.about.obstaclesAside({ lit: fmtNumber(m.faaObstacles.litCount), windTurbines: fmtNumber(m.faaObstacles.counts.windturbine ?? 0) })})</span>
									</dd>
								{/if}
								{#if m.usAdCharts}
									<dt>{t.detail.charts}</dt>
									<dd>
										{fmtNumber(m.usAdCharts.charts)}
										<span class="aside">({t.about.chartsAside(fmtNumber(m.usAdCharts.airports))})</span>
									</dd>
								{/if}
								<dt>{t.about.licenseLabel}</dt>
						...
						{#each publisherCards as c (c.key)}
							<DataSourceCard>
								...
								<!-- The loaded cycle is looked up only for a card that
								     already claims one. Switzerland deliberately claims
								     none (FOCA revises as needed and its date is a
								     publication date), and it still has an obstacles
								     cell in the AIRAC matrix, so reading that cell
								     unconditionally would label that date AIRAC. -->
								{@const cell = stamp ? loadedCycle(c.key) : undefined}
								{@const nextCyc = announceNextCycle(cell, undefined)}
								{#if cell?.effective ?? stamp?.effective}
									<dt>AIRAC</dt>
								...
								{/if}
								{#if c.chartPages != null}
									<dt>{t.detail.charts}</dt>
									<dd>{t.about.chartPages(fmtNumber(c.chartPages))}</dd>
								{/if}
								<dt>{t.about.licenseLabel}</dt>
								<dd class="muted">
									{c.license}
			...
			<!-- The live services and the bundled models sit outside the
			     sidecar gate above: none of them reads a meta, and hiding
			     them behind a failed sidecar fetch dropped their attribution
			     for exactly the reader most likely to be offline.

			     They are their own sections rather than more cards in the
			     one above because they say a different KIND of thing. A data
			     set states a cycle and counts its rows, and a shared label
			     column is what lines those figures up; a service states in a
			     sentence what it carries and when it is asked, and the same
			     shared column would hand 10.5rem to three short labels and
			     squeeze the sentences into what is left. Grouping lets each
			     kind size its own labels and still read as aligned. The cut
			     between them, fetched on demand against bundled into the
			     app, is the one the Privacy section above already draws. -->
			<section>
				<h3>{t.about.liveServicesHeading}</h3>
				<div class="sources">
					...
					<!-- The tiles carry no cycle and no count, so the card is
					     ...
					     (src/lib/map/terrain.ts). It sits with the services
					     rather than beside the geoid it reads with, because
					     what a reader needs from this section is which
					     requests leave the device. -->
					<DataSourceCard prose>
						{#snippet heading()}
							<a href="https://registry.opendata.aws/terrain-tiles/" target="_blank" rel="noopener noreferrer">{t.about.headTerrain}</a>
						{/snippet}
						<dt>{t.about.dataLabel}</dt>
			...
			<!-- Compiled into the bundle, so they are the two sources that
			     answer with no network at all. -->
			<section>
				<h3>{t.about.bundledModelsHeading}</h3>
				<div class="sources">
					<DataSourceCard prose>
						{#snippet heading()}
