<script lang="ts">
	 ...
	 *  and the card omits the row until then; a read that settled with
	 *  none says so, since the ground is served from these models anyway. */
	let terrainTiers = $state<TerrainRegion[]>([]);
	let terrainRead = $state(false);
	$effect(() => {
		...
		<div class="body">
			...
			<section>
				...
				<ul class="privacy">
					...
					<li>{t.about.privacyRadar}</li>
					<li>{t.about.privacyVac}</li>
				...
				{:else}
					...
					<div class="sources">
						...
						{#if m.frFuel}
							...
							<DataSourceCard>
								{#snippet heading()}
									<a href="https://www.sia.aviation-civile.gouv.fr/" target="_blank" rel="noopener noreferrer">{t.about.headFuel}</a>
								{/snippet}
								<dt>AIRAC</dt>
								<dd>
									{fmtAiracDate(fu.effective)}
									<!-- The two halves are two cycles: the plates refresh weekly and
									     the AD 2.4 entries only when the SIA export is rebuilt by
									     hand, so the AIP cycle is stated whenever it has fallen
									     behind rather than left to be assumed. -->
									{#if fu.aixmEffective && fu.aixmEffective !== fu.effective}
										<span class="aside">({t.about.fuelAipCycle(fmtAiracDate(fu.aixmEffective))})</span>
									{/if}
								</dd>
								<dt>{t.about.generated}</dt>
								<dd class={freshnessClass(fu.generatedAt)}>
									{fmtUTC(fu.generatedAt)}
									<span class="aside">({fmtAgo(fu.generatedAt)})</span>
								</dd>
								<dt>{t.about.fuelLabel}</dt>
								<dd>
									{fmtNumber(fu.aerodromes)}
									<span class="aside">({t.about.fuelAside(fmtNumber(fu.withGrades))})</span>
								</dd>
								<dt>{t.about.licenseLabel}</dt>
								<dd class="muted">{t.about.licFrance}</dd>
								<dt>{t.about.refresh}</dt>
			...
			<section>
				...
				<div class="sources">
					...
					<DataSourceCard prose>
						{#snippet heading()}
							<!-- The Open Radar Data documentation is EUMETNET's own statement of
							     this data channel and its licence (its Data Policy); the OPERA
							     programme URL only serves the radar-animation page, which speaks
							     of members' and third-party licences, not of the open data. -->
							<a href="https://eumetnet.github.io/openradardata-documentation/" target="_blank" rel="noopener noreferrer">{t.about.headOpera}</a>
						{/snippet}
						<dt>{t.about.dataLabel}</dt>
						<dd>{t.about.operaData}</dd>
						<dt>{t.about.fetchedLabel}</dt>
						<dd>
							{t.about.operaFetched}
							<span class="aside">{t.about.operaFetchedAside}</span>
						</dd>
						<dt>{t.about.licenseLabel}</dt>
						<dd class="muted">
							<!-- What CC BY 4.0 asks for, in order: the licence by name and link,
							     the licensor and where its terms were read, the producers (in the
							     data line above and in the frames' own metadata), and that the
							     material was modified. -->
							<!-- i18n-ignore: licence name, invariant -->
							<a href="https://creativecommons.org/licenses/by/4.0/" target="_blank" rel="noopener noreferrer">CC BY 4.0</a>{t.about.licOpera}
							<a href="https://eumetnet.github.io/openradardata-documentation/1-ORD-API-overview/" target="_blank" rel="noopener noreferrer">{t.about.licOperaDoc}</a>{t.about.licOperaAfter}
						</dd>
					</DataSourceCard>
				</div>
			</section>
			...
			<section>
				...
				<div class="sources">
					...
					<DataSourceCard prose>
						...
						{:else if terrainRead}
							<dt>{t.about.terrainTiersLabel}</dt>
							<dd class="muted">{t.about.terrainCreditsMissing}</dd>
						{/if}
					</DataSourceCard>
