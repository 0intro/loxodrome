<script lang="ts">
	...
	function headerDragDown(e: PointerEvent): void {
		if (!gripEdge || e.button !== 0) {
			return;
		}
		const hdr = e.currentTarget as HTMLElement;
		const startY = e.clientY;
		const disarm = (): void => {
			hdr.removeEventListener('pointermove', armedMove);
			hdr.removeEventListener('pointerup', disarm);
			hdr.removeEventListener('pointercancel', disarm);
		};
		function armedMove(ev: PointerEvent): void {
			...
			onGripDown(ev, false, startY);
		}
		hdr.addEventListener('pointermove', armedMove);
		hdr.addEventListener('pointerup', disarm);
		hdr.addEventListener('pointercancel', disarm);
	}
	 ...
	 * aimed at. */
	function onGripDown(e: PointerEvent, tappable = true, startY?: number): void {
		if (!gripEdge) {
			return;
		}
		...
		startResize(
			...
			(px) => applyRelease(dockRelease(def, 'right', liveStage(), px, pane), 'right'),
		);
	}
			...
			>
				{#if pane}
					...
					<button
						...
						title={t.common.paneDetentTip}
						onpointerdown={onGripDown}
						onclick={ui.isLandscapePhone ? cycleDetent : undefined}
						onkeydown={onGripKey}
					>
