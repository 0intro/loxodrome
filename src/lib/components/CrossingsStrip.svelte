<script lang="ts">
	...
	interface Props {
		...
		interactive?: boolean;
		/** The phone rendition of the summary button: `line` is the full-width
		 *  row above the chart, `chip` the pill the strip overlays on the
		 *  plot's top right (RouteProfileModal positions it), its text one
		 *  count in words. */
		variant?: 'line' | 'chip';
	...
	const { crossings, interactive = false, variant = 'line', surfaceId, onhover, onselect }: Props = $props();
...
{#if interactive && ui.isMobile && crossings.length > 0}
	...
	<button
		class={['cx-sum', 'no-print', variant === 'chip' && 'chip']}
		aria-label={t.route.crossingsOpenAria}
	...
	>
		...
		{/if}
		{#if variant === 'chip'}
			<span class="cx-sum-n">{t.route.crossingsCount(crossings.length)}</span>
		{:else}
...
<style>
	...
	/* The strip's overlay pill (the approved mock's): 26 px over a 92 %
	   surface, the count in words, the NO-GO count in the restricted ink;
	   the touch reach rides the outset pseudo-element, never the pill. */
	.cx-sum.chip,
	:global(:root.touch-ui) .cx-sum.chip {
		position: relative;
		gap: 5px;
		width: auto;
		min-height: 26px;
