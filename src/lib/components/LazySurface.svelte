<script lang="ts">
	...
	interface Props {
		...
		/** Release the surface's OWN reason to be open, for the one surface
		 *  whose open flag is not the workspace slot: the detail pane is
		 *  re-asserted from `ui.detail` by App's bridge, so closing the slot
		 *  alone would have it reopened, re-requested and re-failed on the
		 *  next flush, for as long as the chunk stays unreachable. */
		onFail?: (() => void) | undefined;
	}

	let { id, open, load, onFail }: Props = $props();
