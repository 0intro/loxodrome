<script lang="ts">
	...
	import { closePage } from '$lib/state/ui.svelte';
	...
	/* The banner is in FLOW at the top of the app, and a phone PAGE is fixed
	 * and opaque over the whole stage: a file that failed while one was up
	 * (an "Open with" from another app lands on whatever the pilot left
	 * open) said nothing they could see. The page steps aside ONCE, as the
	 * failure ARRIVES: acting on the standing value instead made a failure
	 * nobody had dismissed close every page the pilot opened afterwards,
	 * which the device pass caught as a nav log that would not open. */
	let lastFailure: OpenFailure | null = null;
	$effect(() => {
		const failure = openFile.failure;
