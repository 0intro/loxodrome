<script lang="ts">
	...
	import { profileScope } from '$lib/state/profile.svelte';
	import VerticalProfileModal from './VerticalProfileModal.svelte';
	import type { VerticalOverlay } from './verticalProfile';
	import { aboveIsComplete, airspaceAbove, airspaceColumns, profileWindow } from './verticalProfile';
	...
	// The airspace stack at the picked point, ordered highest-band first, run
	// through the filter rows, the level band and the global "all airspaces vs
	// only those on the map" scope. The scope carries WHAT EACH DROPPED, which
	// the chart cannot work out for itself: an emptied stack and an empty sky
	// are the same column list, and the header would read "nothing above" over
	// a point under a class A TMA.
	const stack = $derived(
		point ? airspacesOver(getAirspaces() ?? [], point.lat, point.lng) : [],
	);
	/* Short-circuited rather than run over the empty stack, which reads the
	   same but is not: profileScope subscribes to the rows, the level band, the
	   scope and both layer records, and this module is meant to be INERT while
	   its surface is closed (the `point` comment above). */
	const scope = $derived(
		point ? profileScope(stack) : { shown: [], hiddenRows: 0, hiddenBand: 0, hiddenScope: 0 },
	);
	const airspaces = $derived(scope.shown);
	...
	// Built over the WHOLE stack and then filtered: one pass, and it is what
	// lets the header ask whether hiding any of them moved the airspace-above
	// answer rather than merely whether anything was hidden.
	const allColumns = $derived(
		// A single probed point, so the band's two ends are one reading.
		airspaceColumns(stack, groundBandAtPoint(groundFt), {
			unknownLimit: t.detail.unknownLimit,
			airspaceTypes: t.data.airspaceTypes,
		}),
	);
	const shownKeys = $derived(new Set(airspaces.map((a) => a.key)));
	const columns = $derived(allColumns.filter((c) => shownKeys.has(c.id)));
	const aboveComplete = $derived(
		aboveIsComplete(airspaceAbove(columns, groundFt), airspaceAbove(allColumns, groundFt)),
	);
