<script lang="ts">
	...
	groundFill?: TerrainSample[] | undefined;
		...
		widthPx: number;
		heightPx: number;
		/** The phone strip rendition (docs/route-profile.md "Phones: the
		 *  strip"): no gutters, the axis labels and the waypoint idents drawn
		 *  INSIDE the plot over a halo, no axis grab zones, and one finger
		 *  scrolls the strip (touch-action pan-x) instead of panning the
		 *  window. Honoured on the INTERACTIVE instances alone (compactMode),
		 *  so the dossier / measuring mounts keep the gutters by
		 *  construction whatever they are passed. */
		compact?: boolean;
		...
		/** The activated band keys UNGATED by the NOTAMs overlay toggle: what
		 *  the FORBIDDEN penetration verdict is graded against, so the chart
		 *  and the crossings strip cannot disagree about a NO-GO when the
		 *  overlay is off (the strip has always read the ungated set). The
		 *  hatch and the tooltip stay on `bandActivations`, which is NOTAM
		 *  CONTENT and rightly follows its toggle. Absent: fall back to the
		 *  gated map, which is what every caller did before this existed. */
		activeBandKeys?: ReadonlySet<string> | undefined;
		/** Per band key, the stretches over which that band's unit is THE
		 *  radio contact in force, with the line to print already composed
		 *  (the `obstacles & { tip }` idiom: no catalog reaches the chart).
		 *  Drawn for the FIS sectors, which NEST - a flight is inside its
		 *  SIV and the FIC underneath at once - so a band alone would show
		 *  two frequencies and answer neither. The emphasised stretch is
		 *  the answer, and its edges are the handovers, resolved at the
		 *  right distance AND the right altitude. */
		contactStretches?: ReadonlyMap<string, ContactStretchView> | null;
	...
	const {
		...
		activeBandKeys = undefined,
		contactStretches = null,
		onNotamClick,
		onObstacleClick,
	...
	/* The strip rendition, interactive instances only (the print mounts
	 * never acquire it). Its gutters are zero: every geometry read below
	 * goes through these four rather than the PAD_* constants, so one flag
	 * moves the axes inside the plot and the desktop chart is byte-identical
	 * at compact=false. */
	const compactMode = $derived(compact && interactive);
	const padL = $derived(compactMode ? 0 : PAD_L);
	const padR = $derived(compactMode ? 0 : PAD_R);
	const padT = $derived(compactMode ? 0 : PAD_T);
	const padB = $derived(compactMode ? 0 : PAD_B);
	...
	function distAtClientX(clientX: number): number {
		...
		const distNM = fromNM + ((sx - padL) / innerW) * (toNM - fromNM);
	...
	function altAtClientY(clientY: number): number {
		...
		const altFt = ceilingFt - ((sy - padT) / innerH) * (ceilingFt - floorFt);
	...
	function cursorHoverMove(e: PointerEvent): void {
		...
		const inside = ux >= padL && ux <= plotRight && uy >= padT && uy <= baseY;
	...
	const innerW = $derived(Math.max(1, widthPx - padL - padR));
	const innerH = $derived(Math.max(1, heightPx - padT - padB));
	const plotRight = $derived(padL + innerW);
	const baseY = $derived(padT + innerH);

	// Local transform closures (read the reactive props at call time).
	const x = (nm: number): number => xOf(nm, fromNM, toNM, innerW, padL);
	const y = (ft: number): number => yOf(ft, floorFt, ceilingFt, innerH, padT);
	...
	const windBarbViews = $derived.by<BarbView[]>(() => {
		...
		for (let i = 0; i + 1 < waypoints.length; i++) {
			...
			// inside the plot.
			const cy = Math.max(padT + BARB_PX + 6, y(legAltsFt[i] ?? 0) - 16);
	...
	const bandLabels = $derived.by<BandLabel[]>(() => {
		...
		for (const band of notamBands ? [...bands, ...notamBands] : bands) {
			...
			band.spans.forEach((span, si) => {
				const bandTop = Math.max(padT, band.knownExtent ? y(span.topFt) : y(ceilingFt));
				const bandBot = Math.min(baseY, band.knownExtent ? y(span.botFt) : y(floorFt));
				const bh = bandBot - bandTop;
				const vx1 = Math.max(x(span.enterNM), padL);
	...
	// Inside the plot on the strip (no top gutter to stack into), the
	// waypoint idents' own line.
	const FEAT_Y0 = $derived(compactMode ? 11 : padT - 4);
...
>
	...
	<!-- horizontal FL/GND gridlines + altitude labels in the left gutter.
	 The strip has no gutter and scrolls under its box, so its altitude
	 labels are the modal's pinned overlay (ProfileAxisOverlay), not the
	 chart's: here only the gridlines. -->
	{#each yt as t (t.ft)}
		<line class="grid" x1={padL} x2={plotRight} y1={y(t.ft)} y2={y(t.ft)} />
		{#if !compactMode}
			<line class="ay-tick" x1={padL - 4} x2={padL} y1={y(t.ft)} y2={y(t.ft)} />
			<text class="ay-label" x={padL - 6} y={y(t.ft) + 3} text-anchor="end">{t.label}</text>
		{/if}
	{/each}

	<!-- vertical distance gridlines + labels (the strip's labels paint at
	 the end of the document instead, over the ground) -->
	{#each xt as d (d)}
		<line class="grid" x1={x(d)} x2={x(d)} y1={padT} y2={baseY} />
		{#if !compactMode}
	...
	<!-- axis unit captions in the gutter corners: the distance unit on the
	 x-label row (the right gutter would collide with the centered end label),
	...
	{#if !compactMode}
		...
		<text class="axis-unit" x={padL - 6} y={padT - 8} text-anchor="end">{transitionAltFt != null ? 'ft / FL' : 'ft'}</text>
	{/if}
	...
	{#if onPan && !compactMode}
		<rect class="gutter gy" x="0" y="0" width={padL} height={H} />
	...
	<g clip-path="url(#rp-clip-{uid})">
		...
		{#each bands as band (band.key)}
			{#if !band.knownExtent || (band.maxFt >= floorFt && band.minFt <= ceilingFt)}
				{#each band.spans as span, si (si)}
					...
					{@const contactLine = contactStretches?.get(band.key)?.line ?? ''}
					<g
						class="band"
						class:outline={band.category === 'activity'}
					...
					>
						...
						{#each penetrations.get(band.key) ?? [] as pen (pen.fromNM)}
							{#if pen.fromNM < span.leaveNM - 1e-9 && pen.toNM > span.enterNM + 1e-9}
								<path
									...
									class:forbidden={isForbiddenCrossing(band, {
										vfr,
										active:
											activeBandKeys?.has(band.key) ??
											bandActivations?.has(band.key) ??
											false,
									})}
							...
							{/if}
						{/each}
						<!-- The stretch over which this sector is the contact in
						 force. Same shape as a penetration and for the same
						 reason: the band says where the volume is, the emphasis
						 says where it applies to you. -->
						{#each contactStretches?.get(band.key)?.spans ?? [] as cs (cs.fromNM)}
							{#if cs.fromNM < span.leaveNM - 1e-9 && cs.toNM > span.enterNM + 1e-9}
								<path class="inforce" d={bandPathD(band, clipSpanToRange(span, cs.fromNM, cs.toNM))} />
							{/if}
						{/each}
						<path class:pecked={band.rtba} d={bandPathD(band, span)} />
						<title>{contactLine ? `${bandTip}\n${contactLine}` : bandTip}</title>
					</g>
				{/each}
			{/if}
		{/each}
		...
		{#if dimFlown && playheadNM != null}
			...
			<rect
				...
				x={padL}
				y={padT}
				width={Math.max(0, x(playheadNM) - padL)}
				height={innerH}
			/>
		{/if}
	...
	{#if taRuleFt != null}
		<line class="ta-line" x1={padL} x2={plotRight} y1={y(taRuleFt)} y2={y(taRuleFt)} />
		<text class="ta-label" x={padL + 4} y={y(taRuleFt) - 4}>TA {fmtAlt(taRuleFt)} ft</text>
	{/if}
	...
	{#if dragLeg !== null}
		...
		{@const lx = Math.min(plotRight - 60, Math.max(padL + 6, dragX))}
		<line class="drag-guide" x1={padL} x2={plotRight} y1={gy} y2={gy} />
		<text class="drag-label" x={lx} y={gy < padT + 18 ? gy + 16 : gy - 7}
	...
	{#if inspectNM != null && dragLeg === null}
		...
		<line class="inspect-line" x1={x(inspectNM)} x2={x(inspectNM)} y1={padT} y2={baseY} />
	...
	{#each visibleWps as v, j (v.idx)}
		...
		<line class="wp-tick" x1={px} x2={px} y1={padT} y2={baseY} />
		{#if onWaypointClick}
			<g
				class="wp clickable"
				role="button"
			...
			>
				...
				<rect class="wp-hit" x={px - 6} y={padT} width={12} height={innerH} />
		...
		{#if wpLabels[j] != null}
			...
			<text
				class="wp-label"
				class:halo={compactMode}
				x={px + (L.anchor === 'start' ? 2 : L.anchor === 'end' ? -2 : 0)}
				y={compactMode ? 11 : padT - 16}
	...
	{#if features}
		{#each features as f, i (i)}
			{#if f.distNM >= fromNM - 1e-6 && f.distNM <= toNM + 1e-6}
				<line class="feat-tick" x1={x(f.distNM)} x2={x(f.distNM)} y1={padT} y2={baseY} />
			{/if}
		{/each}
	...
	{#if playheadNM != null}
		...
		{#if phx >= padL && phx <= plotRight}
			{@const py = altAtDistNM(playheadNM)}
			<line class="playhead" x1={phx} x2={phx} y1={padT} y2={baseY} />
			{#if py != null}
			...
			{#if playheadAltFt != null}
				...
				{#if ay >= padT && ay <= baseY}
					{#if py != null && y(py) >= padT && y(py) <= baseY}
	...
	{#if onCursor && hoverNM != null && !pinned}
		...
		<line class="cursor-line" x1={x(hoverNM)} x2={x(hoverNM)} y1={padT} y2={baseY} />
		{#if ay != null}
			<circle class="cursor-dot" cx={x(hoverNM)} cy={y(ay)} r={3.5} />
		{/if}
	{/if}

	<!-- The strip's distance labels: inside the plot along its bottom edge,
	 painted last so they read over the ground and the bands (the desktop's
	 sit in a gutter nothing paints over); the end labels anchor inward so
	 the box never cuts them, and the last one carries the unit. -->
	{#if compactMode}
		{#each xt as d, i (d)}
			{@const last = i === xt.length - 1}
			<text
				class="xlabel halo"
				x={x(d) + (i === 0 ? 3 : last ? -3 : 0)}
				y={H - 4}
				text-anchor={i === 0 ? 'start' : last ? 'end' : 'middle'}>{fmtNMTick(d)}{last ? ' NM' : ''}</text
			>
		{/each}
	{/if}
</svg>
...
<style>
	...
	/* The strip: one finger is the scroller's own horizontal pan (with the
	   browser's momentum), so the chart keeps only the vertical pan, the
	   pinch and the long press; a horizontal drag reaches it as a
	   pointercancel, which the gesture machine settles like a release. */
	.plot.compact {
		touch-action: pan-x;
	}

	/* Labels drawn over the plot rather than in a gutter read through the
	   bands and the ground behind them by their surface halo. */
	.plot.compact .halo {
		font-size: 10px;
		paint-order: stroke;
		stroke: var(--surface);
	...
	/* Aerial activity: the SIA marks these zones with a thin outline and a
	   pictogram, never a tint (airspaceSymbology's spec is `band: null`).
	   The profile has no room for the pictogram, so it keeps the outline
	   and the label, and the zone stays visually subordinate to the volumes
	   a flight actually has to clear. */
	.band.outline path {
		fill: none;
		stroke-opacity: 0.75;
	}
	...
	/* ... except where the plan actually goes THROUGH one. `.band.outline
	   path` above kills the fill for the whole group, and the `.pen` rules
	   set only `fill-opacity`, so without this an activity crossing would
	   read exactly like an activity the flight clears - the one thing the
	   pilot who turned the row on is looking for. Written at 0,3,1 so it
	   outranks both, rather than leaning on the source order that decides
	   the `stroke-opacity` tie between the two 0,2,1 rules. */
	.band.outline path.pen {
		fill: var(--as-color);
		stroke-opacity: 1;
	}

	/* The FIS sector in force over this stretch: a filled tint inside the
	   sector's own outline, quieter than a penetration because it is not a
	   containment the pilot must act on, only the answer to "who am I
	   talking to here". */
	.band path.inforce {
		fill: var(--as-color);
		fill-opacity: 0.22;
