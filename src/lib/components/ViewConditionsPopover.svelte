<script lang="ts">
	...
	// Null with no route workspace at all, which is a different thing from a
	// workspace holding nothing flyable: the Flight period is then not OFFERED
	// rather than offered and disabled, the same rule SurfaceShell's actions
	// row follows. A row the surface cannot serve is absent.
	const flight = $derived(planScope.flight);
	const hasRoute = $derived(flight?.flyable() ?? false);
	const flightWin = $derived(mode === 'flight' ? (flight?.window() ?? null) : null);
	// Without an explicit ETD the flight window is the flight's whole DAY
	// (state/timeWindow.svelte.ts); the popover says so rather than
	// presenting a span nobody stated as an exact one.
	const etdSet = $derived(flight?.etdStated() ?? true);
