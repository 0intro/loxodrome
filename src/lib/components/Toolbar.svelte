<script lang="ts">
	...
	const menuActions = $derived(
		offlineEntry
			? [searchAction, printAction, fullscreenAction, offlineAction, aboutAction]
			: [searchAction, printAction, fullscreenAction, aboutAction],
	);
...
{#if ui.isMobile && moreOpen}
	...
	<div class="more-menu" id="{uid}-more-menu">
		<!-- The identity row first (the standard account position), then the
		     surface launchers (the desktop cluster's phone home), then the
		     utilities under a hairline. -->
		<button class="more-item" onclick={openAccount} aria-label={accountLabel}>
