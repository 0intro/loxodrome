<script lang="ts">
	...
	import { ui, toggleSidebar, closeDetail, openPage } from '$lib/state/ui.svelte';
	...
	import { openAccountModal } from '$lib/state/accountModal.svelte';
	...
	import { acknowledgeRadarCaution, dismissRadarCaution, radar } from '$lib/state/radar.svelte';
	...
	/* The account presence (docs/accounts-sync.md, "UI surface"): a QUIET
	 * identity chip, never a sign-in nag. Signed out it is the bare person
	 * icon; signed in it wears the address's initial, and the dot carries
	 * the attention states the Settings group words (a sync refusal, an
	 * expired session). Signed OUT (or expired) the click opens the login
	 * DIALOG; signed in it opens the account MANAGEMENT surface. */
	const accountAttention = $derived(
		sync.errorCode !== null ? 'error' : account.authExpired ? 'warn' : null,
	);
	const accountLabel = $derived(signedIn() ? (account.email ?? '') : t.account.title);
	...
	/* The phone's toolbar folds away while a recording runs when the pilot
	 * chose so (Settings > Interface): the band's contact row then carries
	 * Stop and Layers. A template condition, never an effect on
	 * nav.recording (the goFlying doctrine). */
	const folded = $derived(ui.isMobile && nav.recording && display.toolbarInFlight === 'folded');
	/* The phone's fit button: the route when one is planned, else the NOTAMs. */
	const phoneFitDisabled = $derived(!mapUsable || (fitCount === 0 && activeRoute().waypoints.length < 2));
	...
	/* The offline manager rides the UTILITY set, not the launcher cluster:
	 * that cluster is reserved for the flight tools (the nav log, the route
	 * profile, flight prep, the flights library), and this is a desk task.
	 * One record, two renditions (the desktop overflow row and
	 * the phone More menu), so the percentage reaches both. It shows only
	 * where a download can be made or something is already held. */
	const offlineLabel = $derived(
		offlineQueue.busy
			? `${t.offline.title} ${Math.round(offlineQueue.progress * 100)} %`
			: t.offline.title,
	);

	const offlineAction: ToolbarAction = $derived({
		id: 'offline',
		icon: 'download',
		label: offlineLabel,
		// Announce what is SHOWN: a bare title would hide the percentage the
		// button exists to carry while a queue drains.
		aria: offlineLabel,
		disabled: false,
		pressed: offlineModal.open || offlineQueue.busy,
		run: () => toggleOffline(),
	});
	...
	/* The phone app menu's rows (the logo's caret): the library and the two
	 * pages the bar has no destination for, then the utilities. Export-map
	 * and full screen left the phone (full screen rides the recording,
	 * docs/mobile-ui-review.md). */
	const settingsAction: ToolbarAction = $derived({
		id: 'settings',
		icon: 'sliders',
		label: t.tabs.settings,
		aria: t.tabs.settings,
		disabled: false,
		run: () => openPage('settings'),
	});
	const flightsAction = $derived(launcherActions.find((a) => a.id === 'flights') ?? launcherActions[launcherActions.length - 1]);
	/* No Layers row: the toolbar's own button opens the same page, and the
	   chrome is the launchers' ONE home, by decision. A second door to one
	   destination is the duplication the redesign took out of the tabs. */
	const phoneMenuActions = $derived(
		offlineEntry
			? [flightsAction, settingsAction, offlineAction, aboutAction]
			: [flightsAction, settingsAction, aboutAction],
	);
...
{#if !folded}
<header class="toolbar" inert={ui.page !== null || undefined}>
	{#if ui.isMobile}
		<!-- The app menu on the phone: the logo, with a caret, opens the
		     app-level rows (account, flights, settings, offline data, about,
		     language, night), the mock's decision; the sidebar toggle has no
		     sidebar to toggle there. -->
		<button
			class="logo-btn"
		...
		>
			<!-- i18n-ignore: brand name, invariant in both languages -->
	...
	<div class="brand" class:hidden={ui.isMobile}>
	...
	{#if ui.isMobile}
		<!-- Search opens the Airports page on its Search section: the phone's
		     search is an aerodrome search first (an alternate in flight). -->
		<button
			class="icon-btn"
			onclick={() => openPage('airports', 'search')}
	...
	>
		...
		<!-- TRANSIENT on the desktop bar, deliberately: TOOLBAR_LAUNCHERS_MEDIA
		     is 1360px because the French bar measures 1352 natural, so a
		     permanently-present icon would push that calibrated constant. It
		     shows while the queue has something to say, which is also when a
		     pilot wants it, and the phone More menu carries the permanent
		     row. -->
		{#if offlineEntry && offlineQueue.busy}
			{@render actionBtn(offlineAction)}
		{/if}
		<LanguageToggle />
		<ThemeToggle />
		{@render actionBtn(aboutAction)}
		<button
			class="icon-btn acct-btn"
			onclick={openAccount}
			title={accountLabel}
			aria-label={accountLabel}
		>
			{#if signedIn()}
				<span class="acct-initial" aria-hidden="true">
			...
			{/if}
			{#if accountAttention}
				<span class="acct-dot" class:warn={accountAttention === 'warn'} aria-hidden="true"
				></span>
			{/if}
		</button>
...
{#if ui.isMobile}
	<!-- The app menu is the phone's own BOTTOM SHEET, on the shared
	     HeadOverlay chrome: the same presentation, Escape and system Back as
	     every other phone sheet, and with them the swipe down to close
	     (ui/sheet.ts). It hangs off the logo in the mock and reads as a menu
	     there, but a top-left dropdown on a 791 px screen sits at the far
	     end of the thumb's reach, and this menu is opened one-handed in the
	     cockpit. HeadOverlay's own backdrop dismisses on CLICK, which is
	     what keeps the same tap from falling through to the map (measured on
	     the Redmi: it opened EGKK). -->
	<HeadOverlay
		open={moreOpen}
		x={0}
		y={0}
	...
	>
		<!-- The app-level rows, the mock's list: the identity row first (the
		     standard account position), the flights library, the settings
		     and layers pages, offline data, About, then the two toggles. The
		     surface launchers left this menu: the Plan page carries them. -->
		<button class="item more-item" onclick={openAccount} aria-label={accountLabel}>
			<Icon name="user" size={16} />
			<span class="acct-more-label">{accountLabel}</span>
			{#if accountAttention}
				<span class="acct-dot in-row" class:warn={accountAttention === 'warn'} aria-hidden="true"
				></span>
			{/if}
		</button>
		<div class="more-sep" aria-hidden="true"></div>
		{#each phoneMenuActions as a (a.id)}
			<button
...
<!-- The radar's in-flight caution is hosted here, beside the flight
     confirms, because the bar is always mounted (this block sits outside
     the folded in-flight form) and because the Fly path raises the caution
     and may then offer the battery settings: the caution first, the
     pending confirm once it is answered, never two dialogs stacked. -->
{#if radar.cautionDue}
	<ConfirmDialog
		mode="acknowledge"
		message={t.weather.radar.caution}
		confirmLabel={t.common.understood}
		onConfirm={acknowledgeRadarCaution}
		onCancel={dismissRadarCaution}
	/>
{:else if flightConfirm}
	<ConfirmDialog
...
<style>
	...
	/* The account chip: the initial stands in for the icon when signed in,
	   and the dot flags what the Account group will word (danger for a
	   sync refusal, amber for an expired session). Positioned against the
	   button, the flight-dot idiom. */
	.acct-btn {
		position: relative;
	}
