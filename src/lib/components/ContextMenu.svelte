<script lang="ts">
	import { decodeQ, t, fmtDecimal } from '$lib/state/i18n.svelte';
	...
	import { RADAR_SCALES, cellClass, radarTipLines, sampleCell, slotHHMM } from '$lib/weather/opera';
	import { NO_COVERAGE_COLOR } from '$lib/weather/opera';
	...
	import { navRouteId, setNavLegPin, setNavRoutePin } from '$lib/state/navRoute.svelte';
	...
	/* Direct-To here (route/directTo.ts, Garmin's rule for a point not in
	 * the plan): an alternate route from the present position to the point,
	 * pinned as the route being flown; the plan itself is untouched. Only
	 * with a position, and while the plan has room for one more route. */
	const pose = $derived(currentPose());
	const canDirect = $derived(pose !== null && canAddRoute());
	...
	function onDirectTo(): void {
		...
		const snap = snapLatLng(contextMenu.lat, contextMenu.lng);
		const r = directTo(snap ?? { lat: contextMenu.lat, lon: contextMenu.lng, kind: 'free' }, here, navRouteId());
		if (r) {
			setNavRoutePin(r.id);
		}
		closeContextMenu();
	...
	/* Activate leg (the leg pin, docs/nav-live.md "Telling it which segment
	 * is being flown"): the log reads this segment from now on. The leg the
	 * pointer is on belongs to the active route, the pin's own index. */
	function onActivateLeg(index: number | null): void {
		if (index === null) {
			return;
		}
		setNavLegPin(activeRoute().id, index);
		closeContextMenu();
	}
...
<!-- HeadOverlay: the desktop's anchored popup, and on a phone the bottom
     sheet every long-press menu is there (swipe down to close, Back). -->
<HeadOverlay
	open={contextMenu.open}
	x={contextMenu.x}
	y={contextMenu.y}
	title={t.map.whatsHere}
...
>
		...
		{/if}
		{#if canDirect}
			<button class="item action" onclick={onDirectTo} title={t.map.directToHereTip}>
				<Icon name="navigation" size={14} />
				<span class="action-label">{t.map.directToHere}</span>
			</button>
		{/if}
		{#if contextMenu.leg !== null && nav.points.length > 0}
			<button
				class="item action"
				onclick={() => onActivateLeg(contextMenu.leg)}
				title={t.map.activateLegTip}
			>
				<Icon name="navlog" size={14} />
				<span class="action-label">{t.map.activateLeg}</span>
			</button>
		{/if}
		...
		{/each}
		<!-- The UNGATED stack, which is the same one the Airspaces section below
		     lists: this is a "what is here?" query and a display preference may
		     not take a door off it, the menu listing area features from the
		     datasets whatever the toggles, so the full stack over a point is
		     always one right-click away. Counting the FILTERED set instead hid the action
		     outright whenever the on-map scope was on with no category ticked,
		     which is the state a fresh install is in, and made the two counts on
		     one menu disagree for reasons neither of them stated. What the chart
		     then draws is its own header's business, and it says so. -->
		{#if contextMenu.airspaces.length > 0}
			...
			>
				...
				<span class="action-label">{t.map.altitudeProfile(contextMenu.airspaces.length)}</span>
			</button>
		{/if}
		...
		{#if contextMenu.stations.length > 0}
			...
			{/each}
		{/if}
		{#if contextMenu.radar && contextMenu.radar.kind !== 'none'}
			{@const rs = contextMenu.radar}
			{@const scale = RADAR_SCALES[rs.product]}
			{@const cell = sampleCell(rs)}
			{@const k = cell != null ? cellClass(cell, scale) : -1}
			<!-- A reading, not an action: the radar has no panel to open. Off
			     paper, like the radar itself (app.css). -->
			<div class="section-title no-print">{t.map.radarHeading}</div>
			<div class="item radar-row no-print">
				<span class="swatch" style:background={k >= 0 ? scale.steps[k].color : NO_COVERAGE_COLOR}></span>
				<span class="id">{radarTipLines(rs, { radar: t.weather.radar.radar, noCoverage: t.weather.radar.noCoverage, words: t.weather.radar.words, number: fmtDecimal }).join(' ')}</span>
				<span class="name">{slotHHMM(rs.t)}</span>
			</div>
		{/if}
...
<style>
	...
	/* The radar reading: a row with nothing to click. */
	.item.radar-row {
		cursor: default;
	}
