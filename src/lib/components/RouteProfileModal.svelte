<script lang="ts" module>
	...
	interface SavedProfileView {
		...
		altTouched: boolean;
		totalNM: number;
		/** The phone strip's own view: its pinch zoom and the NM at the
		 *  box's left edge (absent = the strip opens at 0 / on the
		 *  aircraft). */
		stripZoom?: number;
		scrollNM?: number | undefined;
	}
...
<script lang="ts">
	...
	import { PROFILE_BAND_FILTER_GROUPS } from '$lib/data/airspaces';
	import { buildContactSpans, contactStretches as foldContactStretches } from '$lib/route/airspaces';
	...
	import {
		...
		closeRouteProfile,
		openRouteProfile,
	} from '$lib/state/routeProfileModal.svelte';
	...
	// Per-leg MSA for the step-line, the NavLogSheet recipe verbatim (the same
	// min-alt corridor and the same flight-rules margin, so the line, the
	// nav-log column and the obstacle marks agree by construction). The
	// terrain pass is keyed on the waypoints' COORDINATES + the corridor
	// half-width only, so altitude edits, window moves and the VFR/IFR flag
	// never refetch it; abort on supersede; no spinner, the line pops in like
	// the lazily loaded obstacles.
	...
	// ONE pass over the minimum-altitude corridor's terrain, whatever the MSA
	// layer is doing: the drawn ground is that corridor's envelope, and the
	// MSA line is a pure fold of the same result, so toggling the layer costs
	// nothing and the line, the ground and the obstacle marks cannot disagree.
	let corridor = $state<CorridorGround | null>(null);
	$effect(() => {
		const halfWidthNM = routeSettings.minAltCorridorRadiusNM;
		const pts = activeRoute().waypoints.map((w) => ({ lat: w.lat, lon: w.lon }));
		if (!routeProfileModal.open || pts.length < 2) {
			corridor = null;
			msaSettled = true;
			return;
		}
		...
		void computeCorridorGround(pts, { signal: ctrl.signal, halfWidthNM })
			.then((ground) => {
	...
	// The corridor's ground for the MSA column needs the obstacles too; the
	// dataset lazy-loads and the fold re-runs when it lands.
	$effect(() => {
		if (routeProfileModal.open && profileLayers.msa && !dataState.obstaclesLoaded) {
			void ensureObstacles().catch(() => {
				/* surfaced via dataState.obstaclesError */
			});
		}
	...
	$effect(() => {
		...
		const pts = activeRoute().waypoints.map((w) => ({ lat: w.lat, lon: w.lon }));
		profileMsa =
			ground && profileLayers.msa && pts.length >= 2
				? minAltitudesFrom(pts, getObstacles() ?? [], ground, {
	...
	/** The corridor envelope as a drawable series: the highest ground within
	 *  the minimum-altitude corridor at each station. */
	...
	const overlays = createProfileOverlays({
		...
		terrain: () => terrain,
		// RAW, always. Feeding the gated arrays here would make the
		// activation join lose a hidden band, which would drop it from
		// activeBandKeys, which would make isForbiddenCrossing grade it
		// not-forbidden, which would stop the gate rescuing it: a circular,
		// self-confirming failure that silently disables the rescue for
		// exactly the active R / RTBA zones it exists for.
	 ...
	 * On a PHONE nothing survives: the detail is the pane's third occupant
	 * and takes the dock this chart is sitting in, so the breadcrumb is what
	 * gets the pilot back to it (surfaceSurvivesDetail). */
	function leaveForDetail(): void {
		if (surfaceSurvivesDetail('routeProfile')) {
			return;
		}
		markDetailReturn({
			label: () => t.detail.backToVerticalProfile,
			reopen: openRouteProfile,
		});
		closeRouteProfile();
	}
	...
	// The RAW store value, null included: it is what every other
	// cachedAirspaceSchedule caller passes, and that memo compares terrain by
	// REFERENCE, so a `?? []` here would miss the nav log's entry for as long
	// as the samples are not ready and take its own 1000-sample walk (and a
	// second of the sixteen slots). The walk itself cannot tell the two apart
	// (groundAtSamples returns null for either), so this is purely about
	// sharing the answer.
	const terrainSamples = $derived.by<TerrainSample[] | null>(() => {
		const r = activeRoute();
		return routeTerrainSamples(r.id, r.waypoints);
	});
	const terrain = $derived<TerrainSample[]>(terrainSamples ?? []);
	...
	const dataCeilingFt = $derived(
		Math.max(
			...
			computeCeilingFt(
				...
				// The bands actually drawn: zooming out reaches what is on the
			// chart, and hiding the upper stack is half the point of the
			// filter. (It already followed the NOTAMs toggle this way.)
			[...overlays.shownBands, ...(overlays.profileNotamBands ?? [])],
			),
		),
	);
	...
	/* ---- the phone strip (docs/route-profile.md "Phones: the strip") ----
	 * The distance axis leaves the window model: the chart is drawn at the
	 * px-per-NM floor times the pinch zoom inside a native horizontal
	 * scroller, always the whole plan [0, totalNM]; the altitude axis keeps
	 * the window (its pinch factor, pan and Fit). Off while printing, so
	 * paper keeps the gutters and the fitted width by construction. */
	...
	/* Follow: while armed the strip holds the aircraft mid-box (the map's
	 * follow idiom), the effect re-aiming on every pose, zoom and box
	 * resize. Only a scroll of the pilot's OWN releases it: a scroll event
	 * counts as theirs while a finger is down or during the momentum after
	 * it, or right after a wheel, and never one the follow, the pinch
	 * anchor or a resize clamp produced (those land at the aimed position).
	 * The chip re-arms it; Fit too. */
	...
	$effect(() => {
		if (!compact || !followArmed || livePlayheadNM == null || !scrollEl || stripW <= 0) {
			return;
		}
		scrollStripTo(followScrollLeft(livePlayheadNM * stripPxPerNM, win.plotW, stripW));
	});
	function onStripScroll(): void {
		...
		if (own && Math.abs(scrollEl.scrollLeft - aimedLeft) > 1) {
			if (followArmed) {
				followArmed = false;
			}
			saveView();
	...
	/** Pinch on the strip's distance axis: the scale changes about the
	 *  fingers' midpoint (zoomScrollLeft) unless the follow is holding the
	 *  aircraft, which then simply stays mid-box at the new scale. */
	function onStripZoom(anchorNM: number, factor: number): void {
		...
		const following = followArmed && livePlayheadNM != null;
		stripZoom = next;
		saveView();
		if (following) {
			return; // the follow effect re-aims at the new scale
		}
	...
	/** The chart's pan on the strip: the altitude half goes to the window,
	 *  the distance half (the keyboard's, a two-finger drag's) scrolls. */
	...
	// The saved scroll position, applied once the strip has a width (the
	// box measures after the open); the follow wins while a playhead exists.
	let pendingScrollNM = $state<number | null>(null);
	$effect(() => {
		if (!compact || !scrollEl || !win.ready || stripW <= 0 || pendingScrollNM == null) {
			return;
		}
		const nm = pendingScrollNM;
		pendingScrollNM = null;
		if (livePlayheadNM == null) {
			scrollStripTo(nm * stripPxPerNM);
		}
	});
	...
	$effect(() => {
		...
		untrack(() => {
			if (restoring) {
				...
				stripZoom = saved?.stripZoom ?? 1;
				followArmed = true;
				pendingScrollNM = saved?.scrollNM ?? 0;
	...
	function saveView(): void {
		savedViews.set(routes.activeId, {
			...
			totalNM,
			stripZoom,
			scrollNM: scrollEl && stripPxPerNM > 0 ? scrollEl.scrollLeft / stripPxPerNM : undefined,
		});
	}
	...
	// Over the FULL band set and the UNGATED activation map: a crossing's
	// active / forbidden verdict must not change when a display toggle
	// flips, so the strip keeps listing a volume the airspace filter has
	// decluttered off the chart. The chart declutters; the strip never lies.
	/* Which FIS sector is the contact in force over each stretch, so the
	 * chart can emphasise the one a pilot actually calls. The sectors nest
	 * (a flight is inside its SIV and the FIC underneath at once), so the
	 * band alone would show two frequencies and answer neither.
	 *
	 * Computed only while the FIS row is on, and off the SAME memoised walk
	 * the nav log uses: identical argument tuple, so this is a cache hit
	 * rather than a second 1000-sample pass. resolveScheduleRadios is what
	 * empties a sector that is off watch, so an unmanned sector simply has
	 * no stretch.
	 */
	const contactStretchMap = $derived.by<Map<string, ContactStretchView> | null>(() => {
		if (!profileAirspaceGroups.siv || !airspaces || !enough) {
			return null;
		}
		...
		const spans = buildContactSpans(
			resolveScheduleRadios(schedule, airspaces),
			!routeSettings.vfr,
		);
		// eslint-disable-next-line svelte/prefer-svelte-reactivity -- derived output, rebuilt whole
		const out = new Map<string, ContactStretchView>();
		for (const st of foldContactStretches(spans, doc.totalNM)) {
			const span = spans.find((sp) => sp.ev.key === st.key);
			if (!span || span.ev.category !== 'siv') {
				continue;
			}
			const line = contactLines(span.ev.radio, 'airspace')[0];
			const entry = out.get(st.key) ?? {
				spans: [],
				line: line ? `${line.label} ${line.freq}` : '',
			};
			entry.spans.push({ fromNM: st.fromNM, toNM: st.toNM });
			out.set(st.key, entry);
		}
		return out.size > 0 ? out : null;
	});

	const crossings = $derived(overlays.crossings);
	...
	function onProfileContextMenu(distNM: number, altFt: number, clientX: number, clientY: number): void {
		...
		// Over the UNGATED set: "Classe G (uncharted)" is an assertion about
		// the AIP, and an absence the pilot's own filter created must never
...
{#snippet stripActions({ close }: { close: () => void })}
	...
	<button
		...
		onclick={() => {
			close();
			void printSurface('routeProfile');
		}}
	>
...
>
	{#snippet header()}
		...
		{/if}
		<!-- The phone pane's head is 36 px on a 392 px screen: the title
		     yields whenever the switcher or the profile hop shows, and Fit
		     and Print fold into the actions sheet; the layers button alone
		     stays, its popover anchoring on it. Desktop and paper keep all
		     of it. -->
		{#if !compact || (routes.list.length < 2 && nav.points.length < 2)}
	...
	<div class="body" class:strip={compact}>
				...
				{:else}
					...
					<div class="cx-scroll" class:chip={compact}>
						<CrossingsStrip
							{crossings}
							interactive
							variant={compact ? 'chip' : 'line'}
					...
					<div class="plot-stack">
						<div class="plot-row">
							...
							<div class="plot-area print-plot-ink" bind:clientWidth={win.plotW} bind:clientHeight={win.plotH}>
								{#if win.ready}
									<!-- The strip's scroller (inert on desktop): its own
									     scroll is the distance pan, so the follow reads it
									     for a release and the touch / wheel marks say
									     whose scroll it was. -->
								...
								contactStretches={contactStretchMap}
									...
									{/if}
									{#if compact && !followArmed && livePlayheadNM != null}
										<button
	...
	{#snippet extra()}
		...
		{#if airspacesOpen}
			<ProfileAirspacesPopover
				rows={PROFILE_BAND_FILTER_GROUPS}
...
<style>
	...
	/* ---- phones: the strip ----------------------------------------------
	   ...
	   doctrine), and off the `strip` class the template stamps while the
	   surface is NOT printing, so paper needs no counter-rules: the print
	   re-render drops the class and the desktop rules apply. The sliders
	   go: the strip scrolls sideways at the px-per-NM floor (pinch above
	   it) and the chart's own gestures window the altitude axis. No
	   border, margin or legend row: the chart owns the whole pane, the
	   crossings become an overlay chip at the plot's top right, and the
	   title line yields except while it carries the loading / degradation
	   cap. */
	...
	/* The native horizontal scroller the strip lives in: the SVG is wider
	   than the box, one finger pans it with the browser's own momentum,
	   the scrollbar stays hidden (the profile itself is the position) and
	   the over-scroll never reaches the page or Android's back gesture.
	   Inert on desktop, where the SVG is the box's own size. */
	.plot-scroll.strip {
		height: 100%;
		overflow-x: auto;
		overflow-y: hidden;
	...
	/* The follow chip: shown while the strip was scrolled off the aircraft
	   (a scroll of the pilot's own releases the follow), one tap re-arms
	   it. Bottom left, the gesture hint's mirror; the 44 px reach rides
	   the outset pseudo-element, the band's action-button idiom. */
	.follow-chip {
		position: absolute;
		bottom: 8px;
		left: 8px;
