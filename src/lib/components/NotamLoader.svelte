<script lang="ts">
	...
	import { stopSofiaFetch } from '$lib/sofia/fetch';
	import { fetchRouteNotamsFromSofia } from '$lib/state/routeNotamFetch';
	...
	/* How much later than the period a briefing may open before that reads as a
	 * gap rather than as the shape a briefing structurally has. Two things put
	 * a head there and neither is missing coverage: a request carries a forward
	 * margin, no service briefing a departure already behind the clock, and the
	 * Flight period deliberately opens the RAMP HOUR before the ETD, the hour a
	 * briefing is read in. An hour covers both and leaves the caution for a
	 * period genuinely reaching into the past: a flight dated without an hour,
	 * a range typed over a flight already flown. Tighter than that and it fires
	 * on every ordinary pre-flight briefing, which is the one moment a caution
	 * has to still mean something. */
	const BRIEFING_HEAD_TOLERANCE_MS = 60 * 60_000;
	/* The mirror of periodGap at the head: the instant the loaded briefing
	 * STARTS covering, when the viewing period opens materially before it. What
	 * it reports is the part of the period lying in a past no source can brief,
	 * which is what a flight whose date is stated without an hour, or a period
	 * typed over a flight already under way, is looking at. */
	const periodHeadGap = $derived.by(() => {
		const b = lastFetch?.briefed;
		if (!b) {
			return null;
		}
		return activeEvalWindow().from < b.from - BRIEFING_HEAD_TOLERANCE_MS
			? formatZulu(new Date(b.from))
			: null;
	});
	...
	async function run(action: () => void | Promise<void>): Promise<void> {
		// Every parse stamps parsedAt (parseInput), so an UNCHANGED stamp means
		// the action produced no briefing at all: a fetch its own pre-flight
		// guard declined, which leaves the previous briefing standing. Reading
		// the count alone would then see THAT briefing and hand the panel to
		// the list, carrying off screen the error line this click just raised.
		// The stamp and not the array, because it is a plain number: comparing
		// `notamState.notams` would rest on a $state proxy handing back the
		// same reference on two reads, which is not a documented guarantee.
	...
	// and the fetch modules' own guards: a second click costs nothing but a
	// declined call, and run()'s identity test (not the count) is what stops
	// it settling the panel on a briefing this click did not produce.
	// autorouter needs no account: the shared credential lives in the proxy,
	// so the fetch is always available.
	...
	<fieldset class="group">
		...
		{#if fetchableRoutes > 0}
			...
			<!-- The other half of coverage, which was only ever stated in
			     ...
			     Both ends of it, since a PIB cannot open in the past
			     either. Same alert family as the route gaps below. -->
			{#if periodHeadGap}
				<p class="gaps-head period-gap" title={t.input.periodNotCoveredFromTip}>
					<Icon name="alert-triangle" size={13} />
					{t.input.periodNotCoveredFrom({ from: periodHeadGap })}
				</p>
			{/if}
			{#if periodGap}
...
<style>
	...
	/* The one section that gives. The other fieldsets keep their automatic
	   minimum, so the shrink lands here rather than squashing the buttons
	   above; inside, the only child with a floor low enough to absorb it is
	   the paste box. */
	.loader .group.paste {
		min-block-size: 0;
	}
	...
	.notam-input {
		...
		/* THE BOX TAKES WHAT IS LEFT, between a cap and a floor.
		   ...
		   440px is the SIZE and so the CAP: on a screen with room to spare the
		   box stops here rather than growing to fill, which is what keeps a
		   very large window from showing a paste box the size of a page. It
		   was a `min-height`, which is precisely the thing a layout may not
		   shrink, so the card grew instead of the box giving way: measured on
		   the device at 392x735, the page's scroller held 827px of content in
		   627 and the whole actions row, Display on map included, sat 121px
		   below the fold.

		   140px is the floor, about nine lines of this face, so a very small
		   screen gets a paste box still worth the name and scrolls for the
		   rest rather than a usable card with a useless box in it.
		   ...
		   Nothing here is measured against the rest of the card, which is what
		   a hand-fitted constant did until a loaded route grew the route
		   section by 125px and put the actions back off the screen. */
		height: 440px;
		min-height: 140px;
		padding: 8px 10px;
