<script lang="ts">
	...
	import { obstaclesForNotam } from '$lib/state/notamObstacleLinks.svelte';
	import { navaidsForNotam } from '$lib/state/notamNavaidLinks.svelte';
	import { ensureNotamLinkData } from '$lib/state/notamLinkData';
	import { activatesSupZones } from '$lib/state/supaipLinks.svelte';
	import { airportByIdent, dataState } from '$lib/state/data.svelte';
	...
	// layers were never enabled; never in the NOTAM Viewer, which reads
	// neither (state/notamLinkData.ts). Airspaces already load once a briefing
	// exists; airports default-on.
	$effect(() => {
		ensureNotamLinkData(notam.qCode);
	});
	...
	// Structured view of a frequency-change NOTAM (isFrequencyChangeQCode): the
	// parsed assignments (aerodrome services) or the single new value (FIS/SIV),
	// so the panel states what moved, not just the raw E) text. Aerodrome
	// changes link their A) airport (whose Frequencies panel now shows the
	// override); a SIV change is reached through "Affected airspaces" above.
	// null when not a frequency change or nothing parsed.
	const freqChange = $derived.by(() => {
		if (!isFrequencyChangeQCode(notam.qCode)) {
			return null;
		}
		const e = sections.E ?? '';
		const assignments = parseFreqAssignments(e);
		// FIS/SIV change: a stated new/old pair ("READ <new> INSTEAD OF <old>"),
		// else a lone new value. The lone value asserts a move on its own, so it
		// is read only under a condition that says one happened: the QCACS
		// ("installed") admission routinely describes a NEW facility, and the
		// card would otherwise head an installation's own frequency, guard
		// included, with "Frequency change" (A2113/23 OITK).
		...
		if (assignments.length === 0) {
			...
			if (repl) {
				single = repl.freq;
				was = repl.was;
			} else if (isFrequencyChangeCondition(notam.qCode)) {
				single = singleFreq(e);
			}
		}
