<script lang="ts">
	...
	import { currentPose } from '$lib/state/navRecording.svelte';
	import { MAX_ROUTES, canAddRoute, directTo } from '$lib/state/route.svelte';
	import { navRouteId, setNavRoutePin } from '$lib/state/navRoute.svelte';
	...
	import { gradeLabel, type FuelCondition } from '$lib/data/fuel';
	import {
		ensureAerodromeFuel,
		fuelNoteKey,
		fuelNoteText,
	...
	import { aircraftKey } from '$lib/aircraft/schema';
	...
	$effect(() => {
		...
		void ensureAerodromeFuel(airport.source).catch(() => {});
	});
	...
	/* The fuel entry, typed out of the AIP's prose by cmd/fuel. Null for a
	 * publisher with no such dataset, for an aerodrome the AIP says nothing
	 * about, and while it loads: all three mean "show nothing", which is the
	 * app's idiom for absent data and the only honest one here (an "unknown"
	 * line would appear on every aerodrome in Europe). */
	const fuelRes = $derived(resolveAerodromeFuel(airport.ident, airport.source));
	const fuel = $derived(fuelRes.fuel);

	/* The grades this aeroplane may take, so the panel can say which of the
	 * pumps is for it. The data sheet's whole accepted list, not the grade
	 * currently in the tanks: what a fuel stop asks is what the aeroplane
	 * MAY uplift. */
	const myGrades = $derived(selectedAircraft()?.fuel?.types ?? []);
	const myPlane = $derived(selectedAircraft());
	const myName = $derived(myPlane ? aircraftKey(myPlane) : '');

	const fuelChips = $derived(
		(fuel?.grades ?? []).map((g) => {
			// A military pump is never a match, however close the product is
			// to a civil grade: a civil pilot cannot uplift from it.
			const mil = g.scope === 'mil';
			// A NOTAM has taken it off the field, so it is not a match
			// however well the aeroplane would have burnt it.
			const withdrawn = g.withdrawn;
			const match = !mil && !withdrawn && myGrades.some((x) => x === g.grade);
			return {
				grade: g.grade,
				...
				match,
				withdrawn,
				// Carried in words, not in the fill alone: which of these an
				// aeroplane may take is DERIVED and appears nowhere in the
				// AIP text below, so a reader who cannot see the colour
				// would otherwise lose it (the AD chart marker's rule).
				note: withdrawn
					? t.detail.fuelWithdrawnTip
					: mil
						? t.detail.fuelMilTip
						: match
							? t.detail.fuelMatchTip(myName)
							: '',
			};
		}),
	);

	const condTips: Record<FuelCondition, () => string> = {
		h24: () => t.detail.fuelH24Tip,
		or: () => t.detail.fuelOrTip,
		hx: () => t.detail.fuelHxTip,
		based: () => t.detail.fuelBasedTip,
	};

	/* H24 / O/R / HX are ICAO Doc 8400 abbreviations and stay as they are in
	 * both languages; "based aircraft" has no such abbreviation and is
	 * worded. Each carries its meaning in a title. */
	const fuelConds = $derived(
		(fuel?.cond ?? []).map((c, i, all) => ({
			key: c,
			// The comma rides the label because Svelte trims a lone ", "
			// between elements, and the row is a flex line whose gap is the
			// space after it.
			label:
				(c === 'based' ? t.detail.fuelBasedOnly : c === 'or' ? 'O/R' : c.toUpperCase()) +
				(i < all.length - 1 ? ',' : ''),
			tip: condTips[c](),
		})),
	);
	...
	// The runways' NOTAM state at the instant the map draws (drawnStateAt,
	// the resolver's default), so the panel strikes what the map would and a
	// nightly closure does not read as shut at noon
	// (state/runwayOverride.svelte.ts). The chip above says a NOTAM concerns
	// the runway; `closed` says the runway is shut. `distances` is what a
	// declared-distance NOTAM has restated, `contested` the ends it
	// contradicts without this being able to read the new figures
	// (mechanism 18).
	const runwayState = $derived(resolveAerodromeRunways(airport));
	const runwayClosures = $derived(runwayState.closed);
	const runwayDistances = $derived(runwayState.distances);
	// The closures the table could NOT strike, said under it (mechanism 16's
	// "said rather than guessed at"): one whose D) schedule is unreadable, and
	// one naming an end this aerodrome does not publish, which no runway row's
	// NOTAM chip can carry since its designator matches none of them. A line
	// that merely RESTRICTS a runway already rides that row's chip, and an
	// unreadable declared distance its contested mark.
	const runwayNotes = $derived(
		runwayState.notes.filter((n) => n.kind === 'schedule' || n.kind === 'unmatched'),
	);

	// Whether a NOTAM has closed the FIELD at that same instant, and what the
	// lines that did not close it say (state/aerodromeState.svelte.ts).
	// Distinct from the `closed` TAG above, which is the AIP's own statement
	// that the aerodrome is disused: this one is shut this week and open
	// again next.
	const adState = $derived(resolveAerodromeState(airport));
	...
	// empty.
	/** One runway's LDA / TORA / TODA / ASDA as the panel should PRINT them
	 *  today: the AIP's figures with a declared-distance NOTAM's own values
	 *  substituted per end and per distance (mechanism 18). The published
	 *  record is still what the panel shows where no NOTAM speaks.
	 *
	 *  Metres, because that is what the column prints; the dataset stores
	 *  feet and the resolver metres. */
	function declaredPair(r: Runway, key: 'lda' | 'tora' | 'toda' | 'asda'): string {
		const ftKey = { lda: 'LdaFt', tora: 'ToraFt', toda: 'TodaFt', asda: 'AsdaFt' }[key];
		const cell = (endName: 'le' | 'he'): string => {
			const id = endName === 'le' ? r.le : r.he;
			const over = id ? runwayDistances.get(normalizeRunwayDesignator(id))?.values[key] : undefined;
			if (over != null) {
				return String(Math.round(over));
			}
			const ft = (r as unknown as Record<string, number | null>)[`${endName}${ftKey}`];
			return ft == null ? '–' : String(Math.round(ft * 0.3048));
		};
		return `${cell('le')}\n${cell('he')}`;
	}

	/** How one declared-distance cell of this runway is marked, per distance
	 *  and across both ends (declaredDistanceMark). A contested end prints
	 *  the AIP's figure, which the NOTAM says is wrong and is usually the
	 *  longer of the two, so the cell has to say so rather than print it
	 *  plain; and a cell the NOTAM did not restate is not styled as its. */
	function declaredMark(
		r: Runway,
		key: 'lda' | 'tora' | 'toda' | 'asda',
	): { kind: 'notam' | 'contested'; id: string } | null {
		const mark = declaredDistanceMark(runwayState, [r.le, r.he], key);
		return mark ? { kind: mark.kind, id: mark.notam.notam.id } : null;
	}
	...
	/* Direct-To from the sheet: only with a position (a flight or a replay). */
	const pose = $derived(currentPose());
	function onDirectTo(): void {
		const here = pose;
		if (!here) {
			return;
		}
		const r = directTo(
			{
				...
				lon: airport.lon,
				kind: 'airport',
				refId: airport.ident.toUpperCase(),
				ident: airport.ident,
				label: airport.name || undefined,
			},
			here,
			navRouteId(),
		);
...
<div class="content">
	...
	{#if isClosed || adState.closed || accessLabel || status === 'military' || status === 'joint' || airport.vfr || airport.ifr}
		<div class="tags">
			<!-- A NOTAM closure leads even the AIP's own tag: it is the one
			     statement about this field that is true TODAY, and the two are
			     kept apart because they mean different things (disused for
			     good, versus shut for the weekend). -->
			{#if adState.closed}
				<span
					class="tag tag--notam-closed"
					title={t.detail.adClosedTip(adState.closed.notam.id)}
				>{t.detail.adClosedTag}</span>
			{/if}
	...
	<!-- What a NOTAM says about the aerodrome that the closure grammar would
	     not act on: a use restricted rather than withdrawn, a closure on a
	     schedule it could not read. Shown rather than applied, which is what
	     lets the grammar refuse. -->
	{#if adState.notes.length > 0}
		<p class="ad-notes-label">{t.detail.adNotesHeading}</p>
		<ul class="ad-notes">
			{#each adState.notes as n (aerodromeNoteKey(n))}
				<li>
					<NotamIdButton item={n.notam} />
					<span class="note-text">{aerodromeNoteText(n)}</span>
				</li>
			{/each}
		</ul>
	{/if}
	...
	<div class="actions">
		...
		{#if pose}
			<!-- Direct-To (route/directTo.ts): the diversion a pilot reads this
			     sheet for. An alternate route from the present position,
			     pinned as the route being flown; the plan is untouched. -->
			<button
				type="button"
				class="act act-direct"
				disabled={!canAddRoute()}
				onclick={onDirectTo}
				title={canAddRoute() ? t.detail.directToTip : t.detail.directToFull(MAX_ROUTES)}
			>
				<Icon name="navigation" size={14} />
				<span>{t.detail.directTo(airport.ident)}</span>
			</button>
		{/if}
	...
	{#snippet runwayEnds(r: Runway)}
	{@const ends = [r.le, r.he].filter(Boolean)}
	{#if ends.length === 0}
		–
	{:else}
		<!-- Keyed by POSITION, never by the designator: 271 runways publish
		     both ends under one spelling (helipads, "H1" / "H1"), 79 of them
		     on fields the map draws, and a duplicate key throws in production
		     (Svelte's each_key_duplicate), taking the panel with it. -->
		{#each ends as end, i (i)}
			{@const closedBy = runwayClosures.get(normalizeRunwayDesignator(end))}
			{#if i > 0}<br />{/if}<span
				class="rwy-end"
				class:closed={closedBy != null}
				title={closedBy ? t.detail.runwayClosedTip(closedBy.notam.id) : undefined}
				>{end}</span
			>{#if closedBy}<span class="rwy-closed">{t.detail.runwayClosed}</span>{/if}
		{/each}
	{/if}
{/snippet}
...
{#snippet lgtEnd(end: string, lgt: RunwayLighting)}
	...
	{#if airport.runways.length > 0}
		<section class="block">
				...
				<table class="runways">
					...
					<tbody>
						{#each airport.runways as r, i (i)}
							...
							<tr>
								<td>{@render runwayEnds(r)}{#if rwNotams.length > 0}<button
											class="rwy-chip"
								...
								{#if hasDeclaredDistances}
									{#each ['lda', 'tora', 'toda', 'asda'] as const as key (key)}
										{@const mark = declaredMark(r, key)}
										<td
											class="num"
											class:dd-notam={mark?.kind === 'notam'}
											class:dd-contested={mark?.kind === 'contested'}
											title={mark
												? mark.kind === 'notam'
													? t.detail.ddFromNotam(mark.id)
													: t.detail.ddContestedTip(mark.id)
												: undefined}>{declaredPair(r, key)}</td
										>
									{/each}
								{/if}
								{#if hasRunwayDetail}
			...
			{#if runwayNotes.length > 0}
				<p class="ad-notes-label">{t.detail.rwyNotesHeading}</p>
				<ul class="ad-notes">
					{#each runwayNotes as n (n.notam.notam.id + '|' + n.kind + '|' + n.ends.join('/'))}
						<li>
							<NotamIdButton item={n.notam} />
							<span class="note-text"
								>{n.kind === 'schedule'
									? t.detail.rwyNoteSchedule(n.ends.join('/'))
									: t.detail.rwyNoteUnmatched(n.ends.join('/'))}</span
							>
						</li>
					{/each}
				</ul>
	...
	<!-- Fuel sits with the ground services, immediately above the AD-2
	     directory, so runways / frequencies / airspaces stay contiguous
	     above it. It is out of that collapsed block on purpose: whether the
	     field sells the grade this aeroplane runs on is a go / no-go fact,
	     not a directory entry. -->
	{#if fuel}
		<section class="block fuel">
			<h3>{t.detail.fuelHeading}</h3>
			{#if fuel.avail === 'no'}
				<p class="fuel-none" title={t.detail.fuelNoneTip}>{t.detail.fuelNone}</p>
			{:else if fuel.withdrawnAll}
				<!-- A NOTAM has taken the whole supply off the field. The AIP
				     entry underneath still stands and still shows: this is a
				     dated fact with a link, not a correction of the record. -->
				<p class="fuel-withdrawn-all" title={t.detail.fuelNotamNoneTip}>
					{t.detail.fuelNotamNone}
					<NotamIdButton item={fuel.withdrawnAll} title={t.detail.fuelWithdrawnTip} />
				</p>
			{/if}
			{#if fuel.avail !== 'no' && fuelChips.length > 0}
				<div class="fuel-grades">
					{#each fuelChips as c (c.grade)}
						<span
							class="fuel-chip"
							class:match={c.match}
							class:mil={c.mil}
							class:withdrawn={!!c.withdrawn}
							title={c.note || null}
						>
							{c.label}{#if c.mil}<span class="mil-tag">MIL</span>{/if}{#if c.note}<span
									class="sr-only">, {c.note}</span
								>{/if}{#if c.withdrawn}<NotamIdButton
									item={c.withdrawn}
									title={t.detail.fuelWithdrawnTip}
								/>{/if}
						</span>
					{/each}
				</div>
			{/if}
			{#if fuelConds.length > 0}
				<p class="fuel-cond">
					{#each fuelConds as c (c.key)}<span title={c.tip}>{c.label}</span>{/each}
				</p>
			{/if}
			<!-- What a NOTAM says about the fuel that this cannot act on: a
			     restriction narrower than a grade, a grade the AIP does not
			     publish here, a schedule it could not read. Shown rather than
			     applied, which is what lets the grammar refuse. -->
			{#if fuelRes.notes.length > 0}
				<p class="fuel-notes-label">{t.detail.fuelNotesHeading}</p>
				<ul class="fuel-notes">
					{#each fuelRes.notes as n (fuelNoteKey(n))}
						<li>
							<NotamIdButton item={n.source} />
							<span class="note-text">{fuelNoteText(n)}</span>
						</li>
					{/each}
				</ul>
			{/if}

			<!-- The AIP's own words, always. The chips are a reading of them
			     and a reading is not a substitute: an entry no grammar could
			     type shows here with no chips at all. -->
			{#if fuel.text}
				<div class="fuel-text" lang={lang}>{formatAipRemark(fuel.text, lang)}</div>
			{/if}
		</section>
	{/if}
...
<style>
	...
	/* A closed end keeps its designator legible and struck, the way a closed
	   radio row reads: the runway is still published, it is just shut. */
	.rwy-end.closed {
	...
	/* A grade the selected aeroplane takes. The fill is the signal; the
	   title says why, since a colour alone is not an answer. */
	.fuel-chip.match {
		color: var(--accent-text);
		background: var(--accent);
		border-color: var(--accent);
	...
	/* A grade a NOTAM has taken off the field: the published value stays
	   readable, since the panel prints the record, but struck and muted -
	   the FrequencyList closed-row rule. */
	.fuel-chip.withdrawn {
		color: var(--text-muted);
		text-decoration: line-through;
	}

	/* The whole supply withdrawn. Warning ink, unlike the AIP's own
	   "No fuel": one is the record, the other is today. */
	.fuel-withdrawn-all {
		display: flex;
		flex-wrap: wrap;
		gap: 0 6px;
	...
	/* The NOTAM closure wears the AIP closure's ink: they say the same thing
	   about landing here, and a second colour would read as a third state. */

	/* A declared distance a NOTAM restated, and one it contradicts without a
	   readable replacement. The second matters more: the figure printed is
	   the AIP's and is usually the LONGER of the two. */
	.dd-notam {
	...
	/* Same geometry as the fuel notes below, the other "shown, not applied"
	   list on this panel. */
	.ad-notes-label {
		margin: 6px 0 2px;
		font-size: var(--fs-2xs);
	...
	.fuel-notes .note-text {
		color: var(--text-muted);
	}

	/* An explicit "no fuel" is a fact, not a hazard: neutral ink. */
	.fuel-none {
		margin: 2px 0 0;
		font-size: 13px;
		color: var(--text-muted);
