<script lang="ts">
	...
	const supRefs = $derived.by(() => {
		...
		for (const r of extractAipSups(notam.fullContent)) {
			...
			out.push({
				...
				number: r.number,
				year: r.year,
				hrefFr: siaSupUrl(r, 'fr'),
				hrefEn: siaSupUrl(r, 'en'),
				series: r.series,
			});
		}
		return out;
	...
	const chips = $derived(
		supRefs.map((r) => {
			const sup = r.series === 'A' ? supaipByRef(r.number, r.year) : null;
			return {
				...r,
				sup,
			...
			{#each chips as r (r.display)}
				<li class="sup-chip">
					<div class="sup-head">
						...
						{/if}
						<SupLangLinks fr={r.pdfFr} en={r.pdfEn} />
					</div>
				</li>
			{/each}
		</ul>
