<script lang="ts">
	...
	//
	// With NO plan there are no flight rules in force, so both published
	// clauses are shown rather than one category picked on the app's own
	// authority: LF-R 275 forbids VFR and admits IFR on radio contact, LF-R
	// 204 the reverse, and answering only one of those to a reader who stated
	// neither would be a statement the AIP did not make.
	const entryRows = $derived.by(() => {
		...
		const inForce = planScope.flightRules?.() ?? null;
		const shown: ('vfr' | 'ifr')[] = inForce ? [inForce] : ['vfr', 'ifr'];
		return shown.flatMap((rules) => {
			const cond = airspace.entry[rules];
			return cond ? [{ rules, label: rules.toUpperCase(), word: words[cond] }] : [];
		});
	});
...
<div class="content">
	...
	{#each entryRows as row (row.rules)}
		<section class="block">
			<h3 title={t.detail.entryConditionsTip}>
				<!-- i18n-ignore: ICAO Doc 8400 abbreviation, locale-invariant -->
				{t.detail.entryConditions(row.label)}
			</h3>
			<p class="block-text">{row.word}</p>
		</section>
	{/each}
