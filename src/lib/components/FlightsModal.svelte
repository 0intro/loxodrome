<script lang="ts">
	...
	import { fetchOutingOnDemand, sync } from '$lib/state/sync.svelte';
	...
	<div class="body">
		...
		{#if traceStubs.length > 0 && signedIn()}
			<!-- Synced rows whose trace blob was never fetched here (the
			     ...
			     appear as each trace lands and re-derives. While the
			     personal-mode download is draining they land by themselves,
			     so it reports rather than offering a tap that would race it. -->
			<p class="muted no-print" role="status">
				{#if sync.traces !== null}
					{t.account.statusFetchingTraces(sync.traces.done, sync.traces.total)}
				{:else}
