<script lang="ts">
	...
	import { ensureRouteMsa, routeMsaLegs, routeMsaLoading } from '$lib/state/routeMsa.svelte';
	import {
		fmtTrack,
		fmtNM,
		fmtEte,
	...
	import { resolveAirportRadios, freqChangeIdents } from '$lib/state/freqOverride.svelte';
	...
	interface Props {
		 ...
		 *  the printed ones. */
		kneeboard?: boolean;
		/** A4-portrait page rendition, the geometry the surface's own
		 *  single-route print document paginates into: the same grid the
		 *  `@media print` block gives an unprepared print, scoped by PROP and
		 *  media-independent for the kneeboard's own reason, so
		 *  navlogMeasure's print-prep mount lays the sheet out on screen at
		 *  the printed page's width and the packed band heights are the ones
		 *  that page prints. */
		portrait?: boolean;
		/** Live plog overlay (navigation mode; state/navLive.svelte.ts):
		 *  fills the ETO / ATO slots, dims passed rows, highlights the current
		 *  leg and its target waypoint. The interactive mount passes it, and
		 *  so do the A4 page mounts and their measuring mount (that print is
		 *  the screen's own record, measured with the overlay it carries);
		 *  the kneeboard / dossier mounts and their measuring mount never,
		 *  and a kneeboard mount nulls it besides (the gate below), so that
		 *  paper is unchanged. */
	...
	// Per-leg minimum safe altitude: the caller's precomputed values when
	// supplied (print), else the shared memo the in-flight band reads too
	// (state/routeMsa, the nav-log recipe), so the sheet's MSA column and the
	// band's "MSA leg" are one answer. Keyed on the waypoints' COORDINATES
	// plus the corridor half-width (the terrain + obstacle swath) and the
	// flight rules (the margin: msaMarginForTerrainFt, 500 ft VFR / 1000 ft
	// IFR, 2000 ft IFR over a mountainous leg), so editing leg altitudes never
	// recomputes but a VFR/IFR flip does. Null per leg while computing.
	const msaOpts = $derived({
		halfWidthNM: routeSettings.minAltCorridorRadiusNM,
		vfr: routeSettings.vfr,
	});
	$effect(() => {
		if (legMinFt !== undefined || route.waypoints.length < 2) {
			return;
		}
		ensureRouteMsa(route.id, route.waypoints, msaOpts);
	});
	const minFt = $derived(legMinFt ?? routeMsaLegs(route.id, route.waypoints, msaOpts) ?? []);
	const minLoading = $derived(legMinFt === undefined && routeMsaLoading(route.id));
	...
	const airportRadioByIdent = $derived.by(() => {
		...
		// Over the planned flight, the range the schedule column's radios read
		// (scheduleRadios): which frequency to set is a question about the
		// flight, never about the briefing window's unbounded look-ahead.
		const at = plannedFlightAt();
		for (const a of getAirports() ?? []) {
			...
			const radios = ids.has(ident) ? resolveAirportRadios(a, at).radios : a.radios;
...
<!-- Root wrapper carrying the rendition scopes (the A5 kneeboard card, the
     A4 page, the phone's compact grid); display: contents, so it generates
     no box and the sheet's layout (modal body, print cards, fragmentation)
     is byte-identical to the unwrapped markup. -->
<div class="sheet" class:kneeboard class:portrait class:compact={compactMode}>
...
<style>
	...
	/* The wind provenance and its warning are the one part of the title block
	   the compaction above had missed: they kept the base 11px, LARGER than
	   the 9px card around them, so on a 501px card the provenance wrapped to
	   two lines and filled the row, which pushed the generated-on date onto a
	   third. At the card's own 8px it takes one line, and ordering it after
	   the route figures lets the date sit in the dead space beside them. That
	   is worth a whole leg band, because the header is the ONE part of the
	   card the compaction cannot shrink (107px on the A4 page against 106
	   here), which is why a 7-leg log used to miss one card by 13px. */
	.kneeboard .title-line .windline,
	.kneeboard .title-line .windwarn {
		order: 1;
		font-size: 8px;
	}

	/* ---- A4-portrait page rendition (the `portrait` prop) ----------------
	   The printed page's grid, stated here as a class rather than only
	   inside @media print, for the kneeboard block's reason one paper up:
	   navlogMeasure mounts this sheet ON SCREEN at the page's content width
	   to pack the printed document's pages, so the geometry it measures has
	   to be the geometry that prints. The @media print block at the foot of
	   this file states the same rules bare, for the print that was never
	   prepared (a Ctrl+P over the open surface prints the live sheet);
	   tests/navlogPaper.spec.ts fails when the two drift apart. */
	.portrait .scroll {
		overflow: visible;
	}
