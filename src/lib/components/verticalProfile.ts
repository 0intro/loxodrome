...
import { exactFt, formatVLimit, vExtent } from '$lib/vertical/limits';

/** Resolve an airspace's column extents in feet AMSL for a profile drawn
 *  over a point of known ground elevation. AGL / ASFC limits resolve
...
export function columnFeet(
	...
	const lowerFt = a.vLower ? (exactFt(a.vLower, groundFt) ?? a.vLower.ft) : 0;
	const upperFt =
		a.vUpper && !topOpen ? (exactFt(a.vUpper, groundFt) ?? a.vUpper.ft) : Infinity;
...
export function airspaceColumns(
	...
	return list.map((a) => {
		...
		const feet = columnFeet(a, groundFt);
		return {
			id: a.key,
