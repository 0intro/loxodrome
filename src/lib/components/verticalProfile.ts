...
/** Is the DRAWN stack's airspace-above figure still the whole stack's answer?
 *
 *  The figure is arithmetic over the columns the chart drew, and
 *  `airspaceAbove([])` returns the same "nothing above" as a real stack with
 *  nothing counted over you, so a filtered chart can answer "no controlled or
 *  restricted airspace above" for a point under a class A TMA. The honest
 *  question is not "was anything hidden" but "would the answer have been
 *  different", which is this: ask both stacks and compare. Hiding a UTA at
 *  FL 195 over a field under a 1500 ft CTR changes nothing and must not cost
 *  the reading, which is the common case, since the level band ships on at
 *  0-10 000 ft and every stack over France has a slab above it.
 *
 *  Both null compares equal, which is right: a filter that hid nothing
 *  countable leaves an empty sky an empty sky.
 *
 *  The comparison is the BASE, not the column's identity, because the base is
 *  the answer and the column is only what names it. Bases tie constantly:
 *  airspaceAbove clamps every surface-based volume to the ground and keeps the
 *  FIRST on a tie, so a point under a CTR and an R area both starting at the
 *  surface changes column the moment a filter drops whichever came first,
 *  while the height free under it does not move an inch. Comparing identity
 *  there threw away a correct figure and printed the hedge instead. What the
 *  header then names is the DRAWN column, which is the honest one: it is the
 *  volume the chart is actually showing. Pure. */
export function aboveIsComplete(drawn: AirspaceAbove, whole: AirspaceAbove): boolean {
	return drawn.baseFt === whole.baseFt;
}
...
/** The counts state/profile.svelte.ts's ProfileScope carries, taken
 *  STRUCTURALLY rather than imported: this module knows nothing about airspace
 *  state and ProfileScope satisfies the shape, which is the same arrangement
 *  windowNoteText has with WindowColumns one function up. */
...
/** Vocabulary pack for hiddenNoteText; `t.detail` satisfies it. */
export interface HiddenNoteVocab {
	profileHiddenRows: (n: number) => string;
	profileHiddenBand: (n: number) => string;
	profileHiddenScope: (n: number) => string;
}

/** The disclosure the FILTERS owe the reader: how many airspaces over this
 *  point the chart is not drawing, and which control is doing it. The sibling
 *  of windowNoteText, and the one it structurally cannot cover: `hiddenAbove`
 *  / `hiddenBelow` count what the WINDOW dropped from a built column set, so
 *  they are both zero precisely when the filters emptied the set before a
 *  column existed.
 *
 *  EVERY cause that hid something gets its own clause, and the clauses are
 *  ordered BIGGEST FIRST rather than in the order the filters run. Both halves
 *  are measured rather than chosen: the rows are routinely non-zero, aerial
 *  activity and the FIS sectors shipping off, so a rule that named a single
 *  cause only when one cause accounted for the lot read "hidden by the current
 *  filters" for the very case it exists to explain, and a fixed order buried
 *  the 16 the map scope had just taken behind the 2 the rows always take
 *  (measured at LFPN, 16 / 7 / 2 across scope, band and rows).
 *  Undefined when nothing was hidden, so the common chart carries no note.
 *  Pure. */
export function hiddenNoteText(h: HiddenCounts, v: HiddenNoteVocab): string | undefined {
	const clauses: { n: number; text: string }[] = [];
	if (h.hiddenRows > 0) {
		clauses.push({ n: h.hiddenRows, text: v.profileHiddenRows(h.hiddenRows) });
	}
	if (h.hiddenBand > 0) {
		clauses.push({ n: h.hiddenBand, text: v.profileHiddenBand(h.hiddenBand) });
	}
	if (h.hiddenScope > 0) {
		clauses.push({ n: h.hiddenScope, text: v.profileHiddenScope(h.hiddenScope) });
	}
	if (clauses.length === 0) {
		return undefined;
	}
	// Array.sort has been stable since ES2019, so ties keep the filter order.
