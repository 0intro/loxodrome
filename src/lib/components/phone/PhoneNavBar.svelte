<script lang="ts">
	...
	function go(id: PhonePage | 'map'): void {
		const cleared = clearFullSurface();
		if (cleared === 'refused') {
			return;
		}
		if (id === 'map') {
			// Spent the same way: revealing what a full surface covered is
			// answer enough, and going home as well would take the page, both
			// docks and the selection with it on one press.
			if (cleared === 'none') {
				goHome();
			}
			return;
		}
		// A tap that revealed what a full surface covered is spent: toggling
		// the page underneath as well would land the pilot on the map after
		// they asked for the page they were already on.
		if (cleared === 'none' && ui.page === id) {
			closePage();
			return;
		}
		openPage(id);
...
<nav class="phone-bar no-print" aria-label={t.common.navBar}>
