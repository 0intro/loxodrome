<script lang="ts">
	...
	// Over the planned flight (plannedFlightAt): a withdrawal next month is
	// not this flight's, and one on the flight's own day is.
	const fuelAt = (icao: string): FuelOffering<IndexedNotam> | null =>
		fuelNotams.has(icao.toUpperCase())
			? resolveAerodromeFuel(icao, 'fr', plannedFlightAt()).fuel
	 ...
	 *
	 *  The five reasons read differently on purpose. The AIP's three are
	...
	function refusalText(icao: string, refusal: FuelRefusal<IndexedNotam>): string {
		...
		switch (refusal.kind) {
			...
			case 'aipBased':
				return t.flightprep.fuelBasedAt(icao);
			case 'aipGrade':
	...
	/** What the AIP wants arranged before an uplift, in words. A CAUTION and
	 *  not a reason: the fuel is there, and the stop stands once the call is
	 *  made. h24 says nothing, being the absence of a condition. */
	function accessText(icao: string, access: FuelAccess): string | null {
		switch (access) {
			case 'onRequest':
				return t.flightprep.fuelOnRequestAt(icao);
			case 'noHours':
				return t.flightprep.fuelNoHoursAt(icao);
			case 'basedOnly':
				// Unreachable through either caller: fuelRefusal answers
				// aipBased on the same test, and both lists drop a point that
				// carries a refusal. Here for the exhaustive switch, and
				// worded rather than blank so a future caller gets a sentence.
				return t.flightprep.fuelBasedAt(icao);
			case 'h24':
				return null;
		}
	...
	/* The stops that CAN serve this aeroplane and state a condition on doing
	 * it. A refusal outranks a caution, so one aerodrome gets one line: LFCT
	 * is a dry field whose flags were read off its lubricant sentence, and
	 * "no fuel here" is the only thing worth saying about it. */
	const refuelCautions = $derived(
		firstPerIdent(
			refuelPts.flatMap((pt) => {
				if (!pt.ident || pt.refusal || !pt.access) {
					return [];
				}
				const text = accessText(pt.ident, pt.access);
				return text ? [{ ident: pt.ident, text }] : [];
			}),
		),
	);
	...
	{:else}
			...
			<div class="alt-fuel">
				...
				<ul>
					{#each altFuel as a (a.ident)}
						...
						{:else if a.access}
							<li class="caution">{accessText(a.ident, a.access)}</li>
						{/if}
					{/each}
				</ul>
			</div>
		...
		{#if fuel.refuel && fuel.inputs.length > 1}
			<div class="refuel">
				...
				{/if}
				<!-- What the AIP wants arranged at a stop that CAN serve. Under
				     the refusals and in muted ink, never the warning orange:
				     "you cannot fuel here" and "you must ring first" are
				     different facts, and a reader must tell them apart without
				     reading a word. -->
				{#if refuelCautions.length > 0}
					<p class="caution-lead">{t.flightprep.refuelCautionLead}</p>
					<ul class="uplift-caution">
						{#each refuelCautions as c (c.ident)}
							<li>{c.text}</li>
						{/each}
					</ul>
				{/if}
...
<style>
	...
	/* Its own class rather than the global .muted: app.css owns that name, and
	   this needs the gap that separates two lists as much as it needs the ink. */
	.caution-lead {
		margin: 8px 0 0;
		font-size: 12.5px;
