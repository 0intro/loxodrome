<script lang="ts">
	...
	/** The shared empty answer, so a cell with no NOTAM keeps its identity. */
	 ...
	 *  FLIGHT (plannedFlightAt): this page computes that flight's take-off and
	 ...
	 *  The maps come back keyed NORMALISED ('04'); everything on this page keys
	 *  by `RunwayEnd.id`, the dataset's own spelling, hence onPublishedEnds
	 *  below wherever a map leaves this function. */
	function runwaysOf(icao: string): { airport: Airport; res: RunwayResolution } | null {
		const airport = airportByIdent(icao);
	...
	function closedEnds(icao: string): ReadonlyMap<string, IndexedNotam> {
	 ...
	 *  verdicts below are computed from TORA / TODA / ASDA / LDA, so reading
	 *  the NOTAM is what keeps a take-off run measured against the runway
	 *  that is there today (mechanism 18). */
	function distanceOverrides(icao: string): DistanceOverrides {
		const own = runwaysOf(icao);
		const r = own ? onPublishedEnds(own.airport, own.res.distances) : null;
		if (!r || r.size === 0) {
			return NO_DISTANCES;
		}
		// eslint-disable-next-line svelte/prefer-svelte-reactivity -- transient view of a resolved map
	...
	function contestedEnds(icao: string): ReadonlyMap<string, IndexedNotam> {
		const r = runwaysOf(icao);
		return r ? onPublishedEnds(r.airport, r.res.contested) : NO_CLOSURES;
	}

	/** Has a NOTAM shut the FIELD over the flight (state/aerodromeState.svelte.ts)? Said
	 *  over the column rather than acted on: the numbers below stay exactly
	 *  what the aerodrome would give, because a pilot asking what the runway
	 *  yields is asking a question a closure does not answer, and a blanked
	 *  column would say less than a computed one under the word CLOSED. */
	function fieldClosed(icao: string): string | null {
		const airport = airportByIdent(icao);
													...
													{#each cell.ends as end (end.id)}
														<option value={end.id}
															>{cell.closed.has(end.id)
																? t.flightprep.runwayClosedOption(end.id)
																: end.id}</option
														>
													{/each}
												</select>
