<script lang="ts">
	 ...
	 * caveats, caption. Both subjects of the stack list here, a volume and
	 * the ground ahead (docs/terrain-awareness.md), in the fold's one
	 * order. */
	...
	import {
		...
		alertSolid,
		type AlertCaveat,
		type AlertSurfaceModel,
		type SurfaceAlert,
	} from '$lib/nav/alertSurface';
	import { acknowledgeSurfaceAlert } from '$lib/state/alertStack';
	...
	function openDetail(a: SurfaceAlert): void {
		openAlertPanel(a);
		onClose();
	}

	/** A caveat's line and its explanation. */
	function caveatText(c: AlertCaveat): string {
		const n = t.navigation;
		switch (c) {
			case 'airspaces':
				return n.alertNoAirspaces;
			case 'briefing':
				return n.alertNoBriefing;
			case 'terrain':
				return n.alertTerrainUnread;
			case 'obstacles':
				return n.alertObstaclesMissing;
			case 'inhibited':
				return n.alertTerrainInhibited;
		}
	}

	function caveatTip(c: AlertCaveat): string | undefined {
		const n = t.navigation;
		switch (c) {
			case 'airspaces':
				return n.alertNoAirspacesTip;
			case 'briefing':
				return n.alertNoBriefingTip;
			case 'terrain':
				return n.alertTerrainUnreadTip;
			case 'inhibited':
				return n.alertTerrainInhibitTip;
			case 'obstacles':
				return undefined;
		}
	}
...
<HeadOverlay {open} {x} {y} title={t.navigation.alertsSection} minWidthPx={440} maxWidthPx={460} {onClose}>
	...
	<div class="panel" aria-label={t.navigation.alertsSection}>
		...
		{#each model.panel.active as a (a.key)}
			{@const ink = alertInk(a)}
			<div
				class="arow"
				class:ink-danger={ink === 'danger'}
				class:ink-caution={ink === 'caution'}
				class:solid={alertSolid(a)}
				class:planned={alertPlanned(a)}
			>
				{#if alertOpens(a)}
					<button
						type="button"
					...
					>
				{:else}
					<span class="alert-open">{alertLine(a)}</span>
				{/if}
				<button
					type="button"
					class="alert-ack"
					...
					onclick={() => acknowledgeSurfaceAlert(a)}
				>
					<Icon name="x" size={12} />
				</button>
			</div>
		...
		{#each model.panel.caveats as c (c)}
			<p class="caveat" class:lost={c === 'airspaces'} title={caveatTip(c)}>
...
<style>
	...
	/* The banner grammar's row form (NavStrip's .alert family): the body
	   opens the volume's (or the obstacle's) detail panel, the cross
	   acknowledges. Ink per tier through the one --alert-ink device, as the
	   fold decides it (nav/alertSurface.ts alertInk / alertSolid /
	   alertPlanned); planned dashed; acked dimmed. */
	.arow {
