<script lang="ts">
	...
	function agoText(): string {
		...
		// The pass has settled and its trace download has not: the one
		// stage that used to run mute behind a finished-looking sync.
		if (sync.traces !== null) {
			return t.account.statusFetchingTraces(sync.traces.done, sync.traces.total);
		}
		...
		{:else}
			...
			{:else}
				<!-- One truth at a time: the live progress counter while a
				     ...
				     push no payload) or while its traces come down, the
				     pending count only at rest. Side by side they read as a
				     contradiction (53/102 beside 104). -->
				<p class="muted" role="status">
					{agoText()}{#if !sync.syncing && sync.traces === null && pending !== null && pending > 0}
