<script lang="ts">
	...
	import { swipeDismiss } from '$lib/ui/sheet';
	...
	/* The sheet takes focus when it opens and hands it back when it closes,
	 * PopupMenu's rule for the popup (which says why, and why the box rather
	 * than its first control). */
	let sheetEl = $state<HTMLDivElement>();
	$effect(() => {
		if (!open || !ui.isMobile || !sheetEl) {
			return;
		}
		const box = sheetEl;
	...
	/* The sheet's Escape, PopupMenu's rule: only while the keyboard is in the
	 * sheet or nowhere, so a dialog opened over it closes first. */
	function escapeIsMine(): boolean {
		const at = document.activeElement;
		return at === null || at === document.body || (sheetEl?.contains(at) ?? false);
	}
</script>

<!-- Capture phase, the key consumed: PopupMenu's rule (the sheet's Escape
     must not also close the surface under it). -->
<svelte:window
	onkeydowncapture={(e: KeyboardEvent) => {
		if (e.key === 'Escape' && open && ui.isMobile && escapeIsMine()) {
			e.stopPropagation();
			onClose();
		}
	}}
...
<div use:portal>
	{#if ui.isMobile}
		{#if open}
			<!-- CLICK, never pointerdown: a scrim that unmounts on the DOWN
			     leaves the synthesised click to hit-test whatever is under it,
			     and what is under it is the map (measured on the Redmi: the
			     dismissing tap opened EGKK). The surrounding chrome states
			     this rule; this is where it has to hold. -->
			<button
			...
			>
				...
				<div class="sheet-body menu-panel" data-sheet-scroll>
					{@render children()}
				</div>
			</div>
		{/if}
...
<style>
	...
	/* It takes focus so Tab reaches its rows (the popup's rule). */
	.sheet:focus {
		outline: none;
	}
	...
	/* The swipe follows the finger without easing; the spring back is the
	   sheet's own transition, which the swiping class suspends. */
	.sheet:global(.swiping) {
		transition: none;
	}
