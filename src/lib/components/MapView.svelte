<script lang="ts">
	...
	import { createPackChartLayer } from '$lib/map/packChartLayer';
	...
	import { panelNativeZoom, type VacPanelKind } from '$lib/data/vacgeo';
	import {
		buildVacPanelLayer,
		clearVacPanels,
		setVacPanelData,
	...
	let obstaclesBuilt = false;
	let navaidsBuilt = false;
	...
	function syncChartStack(): void {
		...
		layers.chartStack.forEach((id, i) => {
			...
			if (!chartLayerHandles.has(id)) {
				...
				const layer = pack
					? createPackChartLayer(def, paneName, pack)
					: createChartLayer(def, source, paneName);
				if (layer) {
	...
	$effect(() => {
		// Lazy-load and toggle the airspace category overlays.
	...
	$effect(() => {
		...
		// via activeNotamsByObstacle(). No-op until the layer is built.
		const links = activeNotamsByObstacle();
		if (!map || !obstaclesBuilt) {
			return;
		}
		setObstacleCues(new Set(links.keys()));
	});
	...
	$effect(() => {
		...
		// activeNotamsByNavaid(). No-op until the layer is built.
		const links = activeNotamsByNavaid();
		if (!map || !navaidsBuilt) {
			return;
		}
		setNavaidCues(new Set(links.keys()));
	});

	$effect(() => {
		// Dim / grey the navaids an active NOTAM marks unserviceable.
		// Reactive on visibleNotams + notamState.tick + navaidsLoaded via
		// unserviceableNavaids(). No-op until the layer is built.
		const us = unserviceableNavaids();
		if (!map || !navaidsBuilt) {
			return;
		}
		setUnserviceableNavaids(us);
	});
	...
	$effect(() => {
		// Draw the VAC panels over the aerodromes in view.
		//
		// Debounced like the METAR and wind fetches, and for the same
		// reason: each panel not already drawn costs a ranged read of a
		// 195 MB pack and a pdf.js rasterization, and a pan across France
		// must not start one per frame. The render queue answers null the
		// first time and bumps vacRenderState.gen when the picture lands,
		// which is the read that brings this effect back.
		const m = mapState.map;
		void mapState.zoom;
	...
	$effect(() => {
		...
		// Local index for the per-id lookup; the lint rule doesn't flag
		// Map construction inside an effect.
...
<div class="map-wrap">
	...
	{/if}
	<CursorCoords coord={hoveredCoord} />
