<script lang="ts">
	...
	import {
		...
		clearObstacles,
		setObstacleCues,
		setObstacleReferenceAlt,
	} from '$lib/map/obstacleLayer';
	import {
		buildTerrainAwareLayer,
		clearTerrainAwareLayer,
		setTerrainReference,
		setTerrainShadeListener,
		syncTerrainAwareLayer,
	} from '$lib/map/terrainAwareLayer';
	import { setTerrainTileBudget } from '$lib/map/terrain';
	import { clearTerrainThreats, syncTerrainThreats } from '$lib/map/terrainThreatLayer';
	import {
		drainTerrainFires,
		terrainAlerts,
		terrainPrefs,
		terrainThreats,
	} from '$lib/state/terrainAlert.svelte';
	import {
		reportTerrainShade,
		terrainReference,
		terrainTileBudget,
	} from '$lib/state/terrainAwareness.svelte';
	...
	import { installWaypointPins } from '$lib/map/flightPins';
	import {
		noPublishersPushed,
	...
	import { armAlertAudioOnGesture, playFireBatch } from '$lib/nav/alertSounds';
	...
	import { setRouteOnlyAirspaces } from '$lib/state/profile.svelte';
	...
	import { reconcileWxPin, wxValidTimeMs } from '$lib/state/wxTime.svelte';
	...
	import RadarTimeline from './RadarTimeline.svelte';
	import {
	...
	import { t, fmtDecimal } from '$lib/state/i18n.svelte';
	import {
		visibleNotams,
		notamState,
		parseInput,
	...
	import MapButtons from './phone/MapButtons.svelte';
	import { FOLLOW_REARM_MS } from '$lib/nav/followRearm';
	import { pruneRouteMsa } from '$lib/state/routeMsa.svelte';
	...
	let windTip = $state<{
		...
		/** The radar cell under the cursor, when the layer draws one. */
		radar: RadarSample | null;
	} | null>(null);

	/** The badge's lines: the station's or the barb's, then the radar's
	 *  reflectivity with the frame's time and age. Called from the template
	 *  so every t.* read re-renders on a locale switch (docs/i18n.md rule 3). */
	function tipLines(tip: NonNullable<typeof windTip>): string[] {
		const lines = tip.station
			? stationTipLines(tip.station)
			: tip.barb
				? [...barbTipLines(tip.barb, t.weather.barb), ...(barbStamp ? [barbStamp] : [])]
				: [];
		// The frame the VALUE was read from, which the sample carries: the
		// loop may have moved under a resting pointer since. Nothing once the
		// map no longer draws the radar (the feed expired, the layer off, the
		// slot gone from the loop): the sample is the last move's, and a
		// resting pointer kept printing an echo the map had withdrawn.
		const frame = tip.radar && tip.radar.kind !== 'none' ? radarReadingFrame(tip.radar.t) : null;
		if (tip.radar && frame) {
			const words = t.weather.radar;
			lines.push(
				...radarTipLines(tip.radar, {
					radar: words.radar,
					noCoverage: words.noCoverage,
					words: words.words,
					number: fmtDecimal,
				}),
			);
			const age = radarAge(frame);
			lines.push(words.frameAge({ time: slotHHMM(frame.t), age: formatAge(ageMinutes(age.ms), t.weather.metar) }));
		}
		return lines;
	}

	/** The badge's lines as they stand: a badge whose only line was a
	 *  radar reading the map has since withdrawn has nothing left to say, and
	 *  is not drawn as an empty box. */
	const windTipLines = $derived(windTip ? tipLines(windTip) : []);
	...
	onMount(() => {
		// The pin rule's route half (map/flightPins.ts): a hovered feature a
		// waypoint sits on flashes the PIN rather than the symbol it covers.
		installWaypointPins();
		...
		mapState.map = map;
		syncBaseLayer();
		// The corner credit is read once and then gets out of the way
		// (map/attributionCredit.ts, which carries the licence argument): the
		// About modal is where the base maps and the chart layers stand.
		...
		const onMapClick = (e: L.LeafletMouseEvent): void => {
			...
			if (hit?.kind === 'station') {
				...
				// The NOTAM's own areas are non-interactive, so this is where a
				// click inside one lands: featureAt ranked it against every
				// other area under the point and it was the smallest.
		...
		const onMapMouseMove = (e: L.LeafletMouseEvent): void => {
			...
			cursorPending = requestAnimationFrame(() => {
				...
				// The radar reads beside either: an echo under the cursor is worth
				// a line whatever else is there; a dry cell is not. The PIXEL is
				// read first, so the badge names the class whose colour is under
				// the cursor; the position only where the paint has no pixel.
				const radarSample =
					radarSampleAtPixel(Math.floor(cursorPt.x), Math.floor(cursorPt.y)) ?? radarSampleAt(lat, lng);
				const radarLine = radarSample != null && radarSample.kind !== 'none';
				windTip =
					station || barb || radarLine
						? {
								x: cursorPt.x,
								y: cursorPt.y,
	...
	$effect(() => {
		...
		for (const n of notamState.notams) {
			...
			const p = fir ? notamPublisher(fir, n.icaoCodes) : null;
			if (p) {
				wanted.push(p);
			}
		}
	...
	// The airport floors the zoom has passed: the hide rule below reads nothing
	// else about the zoom, so its effect re-runs where a floor is crossed and
	// not at every zoom step.
	...
	$effect(() => {
		...
		// When "Hide airport NOTAM markers" is on, suppress the blue Q-line
		// (fallback) marker of aerodrome NOTAMs whose pin sits ON the field
		// while the field's own symbol is drawn: the symbol and its cue ring
		// stand exactly there and represent them. A pin the NOTAM puts
		// somewhere else (a navaid outage, a zone activation, a closed FIS
		// sector), or beside a symbol the airport layer does not draw at this
		// zoom or under these toggles, is the only thing on the map that says
		// where, so it stays: see pinHeldByAirport. Red position markers, radius
		// circles and area polygons are untouched either way. Reading
	...
	$effect(() => {
		...
		// dataset: the layer indexes the new merge, or it drew the first one
		// while every lookup answered from the second.
	...
	/* The per-route caches follow the route LIST, and nothing else: both
	 * prunes used to ride effects gated on a planning toggle (semicircular,
	 * show-only-route-airspaces), so with those off a deleted route kept its
	 * terrain and MSA entries, and its abort controller, for the session.
	 * Keyed on the ids alone, so an edit to a route the list still holds
	 * costs nothing. */
	const routeIdKey = $derived(routes.list.map((r) => r.id).join('|'));
	$effect(() => {
		void routeIdKey;
		const live = routes.list.map((r) => r.id);
		pruneRouteTerrain(live);
		pruneRouteMsa(live);
	});
	...
	$effect(() => {
		...
		//
		// The iterate-never-name rule and the push-only-what-changed rule are
		// map/publisherFanout.ts's; the sinks are this map's. Reading every
		// flag is what tracks every flag, so it happens before the map guard.
		...
		const moved = pushChangedPublishers(pushedPublisher, want, [
			...
			(p, on) => setNavaidPublisher(m, p, on),
			(p, on) => setObstaclePublisher(m, p, on),
		]);
		if (moved) {
	...
	$effect(() => {
		...
		const keys = routesAirspaceKeysAtAltitude(
			routed.map((r) => r.waypoints),
			all,
			routeSettings.defaultAltitudeFt,
			routed.map((r) => routeTerrainSamples(r.id, r.waypoints)),
		);
		setRouteAirspaceFilter(map, keys);
		// The profiles' on-map scope counts what the map draws (profile.svelte).
		setRouteOnlyAirspaces(keys);
		redrawAirspaceDeco(map);
	});
	...
	// The instant the weather map is read at, resolved ONCE per pass. A
	// $derived on purpose: calling wxValidTimeMs() inside the effects below
	// would splice the flight window's memo signature (every waypoint lat/lon)
	// into their dependency lists, so a waypoint drag would restart the 500 ms
	// lattice debounce on every frame. The derived absorbs that, re-running to
	// return the same number, which notifies nobody, and both effects then read
	// ONE instant, so the fetch window and the sampled hour cannot disagree.
	const wxAt = $derived(wxValidTimeMs());

	// The barbs are drawn for the hour the viewing period states, which can
	// legitimately be another day's, and the hover badge is the one place a
	// wind value is read away from the panel that stamps it. So the hour rides
	// along whenever it is NOT the current one, and stays out of the way when
	// it is. A UTC stamp is locale-invariant, so it needs no catalog key and
	// breaks no rule about what a map module may import.
	const barbStamp = $derived(wxAt === floorHourMs(Date.now()) ? null : formatZulu(new Date(wxAt)));

	$effect(() => {
		// Drop a valid-time pin the viewing period has moved out from under:
		// the period takes the instant back, and it must not revive when that
		// period comes round again. Lives here because the Weather tab is not
		// always mounted and the map draws the barbs either way.
		reconcileWxPin();
	});
	...
	$effect(() => {
		...
		const barbs = on ? windGridBarbs(wxAt) : [];
		const iso = on ? windGridIsotherm(wxAt) : [];
		if (!m) {
			return;
		}
		buildWindLayer(m);
		setWindData({
			...
			isobars: on ? windGridIsobars(wxAt) : [],
		});
		syncWindLayer(m, on);
	});
	...
	/* Follow re-arms by itself FOLLOW_REARM_MS after a pan ends while a
	 * recording runs (nav/followRearm.ts); a replay or the desk keeps the
	 * one-shot suspension. */
	$effect(() => {
		setFollowAutoRearm(nav.recording ? FOLLOW_REARM_MS : 0);
	});
	...
	$effect(() => {
		// The relative-terrain shading (docs/terrain-awareness.md): the ground
		// and the obstacles coloured against the pose's GPS altitude, live
		// while recording, at the playhead in replay. Same no-center/zoom-read
		// rule as the aircraft effect: the layer repaints itself on moveend
		// through the DirectDrawLayer base. The reference is quantised inside
		// both setters, so a fix that moves the altitude a few feet repaints
		// nothing, and the layer is attached only while there is one.
		const m = mapState.map;
		const on = layers.terrainAwareness;
		const ref = on ? terrainReference() : null;
		const budget = terrainTileBudget();
		if (!m) {
			return;
		}
		setTerrainTileBudget(budget);
		buildTerrainAwareLayer(m);
		setTerrainShadeListener(reportTerrainShade);
		setTerrainReference(ref?.altFt ?? null);
		setObstacleReferenceAlt(ref?.altFt ?? null);
		syncTerrainAwareLayer(m, ref != null);
	});
	...
	$effect(() => {
		...
		void ensureAirspaces().catch(() => {});
		void ensureSupaip().catch(() => {});
		// The terrain alerts read the obstacle dataset (the layer's own gates
		// are display toggles and never gate a safety evaluation) and the
		// aerodromes the phase of flight and the runway inhibit are judged
		// against.
		if (terrainPrefs.obstacle) {
			void ensureObstacles().catch(() => {});
		}
	...
	$effect(() => {
		// Where the terrain alert is: the alerting stretch of the corridor or
		// the alerting obstacle, in the alert's ink, whatever the terrain
		// layer's toggle (state/terrainAlert resolves it; empty clears). Same
		// no-center/zoom-read rule as the aircraft effect.
		const m = mapState.map;
		const threats = terrainThreats();
		if (!m) {
			return;
		}
		syncTerrainThreats(m, threats);
	});

	$effect(() => {
		// The attention channel: sound edges drained from the two state-owned
		// queues, live only (a replay debrief stays visual). Each queue hands
		// each edge to exactly one drain however many surfaces read its
		// evaluator, and both drain armed or not, BEFORE the audio gate, so an
		// audio-off session cannot stockpile stale chimes. One figure a pass,
		// the most urgent (nav/alertSounds.ts). A persisted-on channel re-arms
		// on the first gesture after a reload.
		void airspaceAlerts();
		void terrainAlerts();
		if (!nav.recording) {
			return;
		}
		const fires = { airspace: drainAlertFires(), terrain: drainTerrainFires() };
		if (!alertPrefs.audio) {
			return;
		}
		armAlertAudioOnGesture();
		playFireBatch(fires, { caution: alertPrefs.audioCaution });
	});
	...
	$effect(() => {
		// Precipitation radar: debounced viewport ensure (the METAR twin).
		// The view is the base tiles the bounds reach and the pooling the
		// map's resolution asks for; the minute tick re-arms it, but the
		// module asks for the frames index only at the instant the next
		// frame is expected (never blindly per minute) and fetches only
		// the frames and tiles it does not hold. It must NOT track the
		// shown frame, the play state or the opacity: those repaint from
		// cache. Gates itself on showOnMap + display.liveWeather.
		const m = mapState.map;
		void mapState.zoom;
		void mapState.center;
		void radar.showOnMap;
		void radar.loopMin;
		void radar.indexSeq;
		void radar.fetchSeq;
		void radar.retrySeq;
		void display.liveWeather;
		void notamState.tick;
		// The product picks the grid the view is computed on; a switch
		// re-runs this and the pending timeout goes with it.
		const product = radar.product;
		...
		const t = setTimeout(() => {
			...
			const box = gridBoxOfBounds(bounds, grid);
			// The pooling the canvas draws this view at (the one recipe).
			ensureRadar(
				box
					? {
							product,
							tiles: tilesCovering(box, grid),
							p: poolingAt(mm.getZoom(), bounds.south, bounds.north, grid.cellM),
						}
					: null,
			);
		}, 500);
	...
	$effect(() => {
		// Feed the radar canvas; the gated reads keep a hidden layer free.
		// paintSeq is a tile of the shown frame arriving or decoding; the
		// shown frame follows the loop position and the newest arrival; the
		// FEED's expiry (the newest frame past its product's expiry, 30 min
		// for DBZH and 45 for RATE) withdraws the echoes.
		const m = mapState.map;
		const on = radar.showOnMap && display.liveWeather;
		void radar.paintSeq;
		const product = radar.product;
		const shown = on ? shownRadarFrame() : null;
		const feed = on ? radarFeed() : null;
		const frameKey = shown && feed && feed.staleness !== 'expired' ? shown.t : null;
		const opacity = radar.opacityPct / 100;
		const coverage = radar.coverage;
		const noCoverageLabel = t.weather.radar.noCoverage;
		if (!m) {
			return;
		}
		buildRadarLayer(m);
		setRadarData({ product, frameKey, tileAt: radarTileAt, opacity, coverage, noCoverageLabel });
		syncRadarLayer(m, on);
	});
	...
	$effect(() => {
		...
		// A late country's re-merge re-indexes them (the airports effect's
		// rule), or snapping answered from the first merge.
		void dataState.revision.airports;
...
<div class="map-wrap">
	...
	{#if windTip && windTipLines.length > 0}
		<!-- no-print: a readout riding the cursor, and one that can carry
		     the radar's reading, which never reaches paper. -->
		<div
			class="wind-tip no-print"
			class:flip={windTip.flip}
			style:left={`${windTip.x}px`}
			style:top={`${windTip.y}px`}
		>
			{#each windTipLines as line, i (i)}
				<div>{line}</div>
			{/each}
		</div>
	{/if}
	<!-- The bottom-left corner holds the map's two passive readouts, stacked:
	     the terrain legend above the cursor badge. One column, so neither
	     has to know the other's height; pointer-inert, so a pan or a
	     right-click that starts there still reaches the map. -->
	<div class="corner-bl">
		<TerrainLegend />
...
<style>
	...
	/* The bottom-left readouts' column (the terrain legend over the cursor
	   badge). z 450: above the Leaflet panes, under the map's interactive
	   chrome (the map buttons, the radar strip and the band at 470, the alert
	   stack at 471), so any overlap resolves for what takes a finger. The
	   radar strip publishes its height, and the column stands on it rather
	   than under it: it spans a phone's width, and on a desktop map narrower
	   than about 900 px its centred box reaches the legend. The strip stands
	   10 px off the bottom; the min() adds a 6 px gap above it only while it
	   is up (its published height is 0 otherwise). */
	.corner-bl {
		position: absolute;
		left: 8px;
		bottom: calc(8px + var(--radar-strip-h, 0px) + min(var(--radar-strip-h, 0px), 8px));
		z-index: 450;
		display: flex;
	...
	/* The phone layout keeps the map inside the safe areas already (the bar in
	   flow, the landscape rail), the radar strip's own rule. */
	:global(:root.mobile-ui) .corner-bl {
		bottom: calc(10px + var(--radar-strip-h, 0px) + min(var(--radar-strip-h, 0px), 6px));
	}
