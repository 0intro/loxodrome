<script lang="ts">
	...
	function ringPos(ring: RingId): number {
		return resolveRingPos(ring, rings[ring], ringKeys[ring], ringAvail[ring]);
	}
	...
	const trkCell = $derived.by(() => {
		...
		if (trkEntry === 'xtk' && info?.log.xtkNM != null) {
			const v = xtkCompact(info.log.xtkNM, info.log.xtkSide);
	...
	const altCell = $derived.by(() => {
		...
		if (altEntry === 'agl' && agl != null) {
			...
			return { k: 'AGL', v: String(agl), tip: n.aglTip, aria: `${n.aglTip} ${agl} ft`, msa: false };
		}
		if (altEntry === 'msa' && msaLegFt != null) {
			const v = String(Math.round(msaLegFt));
...
<style>
	...
	.contact-row {
		...
		gap: 8px;
		min-width: 0;
		min-height: var(--band-contact-h, 46px);
	}
	...
	.inst-row {
		...
		align-items: stretch;
		min-width: 0;
		min-height: var(--band-inst-h, 40px);
		padding-top: 3px;
	...
	/* The airspace-alert banner (the SkyDemon warning form): the body opens
	   ...
	   lists. The forbidden tier carries the danger ink like a lost
	   position; the clearance / equipment tiers keep the family's alert
	   orange; a caution reads one step calmer in the workbook amber; a
	   planned advisory steps back rather than shouts. Severity has ONE
	   visual threshold, present tense: ahead of the boundary an alert wears
	   its ink as a tint, inside the volume it goes solid in the same ink
