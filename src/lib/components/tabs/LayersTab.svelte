<script lang="ts">
	...
	import { availableChartLayers, CHART_GROUP_ORDER } from '$lib/map/chartOverlays';
	import type { ChartGroup } from '$lib/map/chartOverlays';
	import {
		layers,
		toggleChartLayer,
	...
	import { setShowWindOnMap, windAloft } from '$lib/state/windAloft.svelte';
	import { metarStations, setShowStationsOnMap } from '$lib/state/metarStations.svelte';
	import { setShowSigmetsOnMap, sigmets } from '$lib/state/sigmets.svelte';
	import { radar, setShowRadarOnMap } from '$lib/state/radar.svelte';
	import { STATION_CAT_COLORS } from '$lib/map/metarLayer';
	import { WIND_BARB_INK } from '$lib/map/windLayer';
	import { HAZARD_STYLES } from '$lib/weather/sigmet';
	import { RADAR_SCALE } from '$lib/weather/opera';
	import { terrainSwatches } from '$lib/map/terrainShade';
	import Segmented from '../Segmented.svelte';

	/* The weather group's swatches take the layers' own inks; the radar's is
	   the scale itself, light to extreme. */
	// The same hue ramp for both products (opera.ts RATE_SCALE), so one swatch.
	const RADAR_SWATCH = `linear-gradient(90deg, ${RADAR_SCALE.steps.map((s) => s.color).join(', ')})`;
	/* The terrain row's swatches are the map's own inks at the shading's own
	   opacity (map/terrainShade.ts), so the key cannot drift from the map. */
	const TERRAIN_SWATCH = terrainSwatches();
	const TERRAIN_ROW_SWATCH = `linear-gradient(90deg, ${TERRAIN_SWATCH.red} 50%, ${TERRAIN_SWATCH.yellow} 50%)`;
	...
	/* The charts group grew past twenty toggles, so it is sub-sectioned by
	   publisher. Presentational only: the STACK order is the order the
	   boxes were ticked (MapView gives each its own pane), never this. */
	const chartGroupTitle: Record<ChartGroup, 'chartsFrance' | 'chartsEurope' | 'chartsUs'> = {
		france: 'chartsFrance',
		europe: 'chartsEurope',
		us: 'chartsUs',
	};
	const chartSections = CHART_GROUP_ORDER.map((g) => ({
		group: g,
		defs: chartLayers.filter((d) => d.group === g),
...
<div class="tab-panel">
	...
	<fieldset class="group">
		...
		{#if devMode}
			...
			<!-- i18n-ignore-end -->
		{/if}
		{#each chartSections as sec (sec.group)}
			<p class="chart-section">{t.layers[chartGroupTitle[sec.group]]}</p>
			{#each sec.defs as def (def.id)}
	...
	<!-- The relative-terrain shading (docs/terrain-awareness.md): the ground
	     and the obstacles coloured against the aircraft's GPS altitude while a
	     recording or a replay supplies one. A persisted layers field, so bound
	     like the SUP AIP row; the key under it is the map legend's. -->
	<fieldset class="group">
		<legend>{t.layers.terrain.legend}</legend>
		<label class="check">
			<input type="checkbox" bind:checked={layers.terrainAwareness} />
			<span class="swatch" style:background={TERRAIN_ROW_SWATCH}></span>
			<span>{t.layers.terrain.toggle}</span>
		</label>
		<div class="terrain-key">
			<span class="swatch" style:background={TERRAIN_SWATCH.red}></span>
			<span>{t.layers.terrain.red}</span>
			<span class="swatch" style:background={TERRAIN_SWATCH.yellow}></span>
			<span>{t.layers.terrain.yellow}</span>
		</div>
		<p class="muted">{t.layers.terrain.note}</p>
		<p class="muted">{t.layers.terrain.advisory}</p>
	</fieldset>

	<!-- The weather overlays' switches, mirrored from the Weather tab, which
	     keeps every setting, legend and status: every EFB puts the radar
	     switch in the map's layer picker, and on the phone this page is one
	     tap from the map. Through the setters (not bind:), each persists. -->
	<fieldset class="group">
		<legend>{t.layers.weatherLayers.legend}</legend>
		<label class="check">
			<input
				type="checkbox"
			...
			/>
			<span class="swatch swatch--radar" style:background={RADAR_SWATCH}></span>
			<span>{t.layers.weatherLayers.radar}</span>
		</label>
		<label class="check">
			<input
				type="checkbox"
			...
			/>
			<span class="swatch swatch--wind" style:background={WIND_BARB_INK}></span>
			<span>{t.layers.weatherLayers.windBarbs}</span>
		</label>
		<label class="check">
			<input
				type="checkbox"
			...
			/>
			<span class="swatch swatch--station" style:background={STATION_CAT_COLORS.VFR}></span>
			<span>{t.layers.weatherLayers.metarStations}</span>
		</label>
		<label class="check">
			<input
				type="checkbox"
			...
			/>
			<span class="swatch swatch--sigmet" style:background={HAZARD_STYLES.TS.color}></span>
			<span>{t.layers.weatherLayers.sigmets}</span>
		</label>
		{#if !display.liveWeather}
			<p class="status">{t.layers.weatherLayers.off}</p>
		{/if}
		<p class="muted">{t.layers.weatherLayers.note}</p>
	</fieldset>
...
<style>
	/* Sub-heading inside the charts group. A <p> rather than a nested
	   <legend>, because the fieldset already carries the group's real
	   legend and a second one would re-announce the whole group. */
	.chart-section {
		margin: 10px 0 2px;
	...
	/* The terrain tiers' key, under its checkbox: swatch and words in pairs. */
	.terrain-key {
		display: grid;
