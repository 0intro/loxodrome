<script lang="ts">
	...
	import { revealMap, selectSigmet } from '$lib/state/ui.svelte';
	import {
		OPACITY_MAX_PCT,
		OPACITY_MIN_PCT,
		RADAR_LOOPS,
	...
	const modelInUse = $derived(wxModelInUse());
	...
	// The instant the map is drawn for, and why: the viewing period's own
	// unless the pilot pinned one (state/wxTime.svelte.ts).
	const wx = $derived(wxValidTime());
	const dateStr = $derived(new Date(wx.ms).toISOString().slice(0, 10));
	const timeStr = $derived(new Date(wx.ms).toISOString().slice(11, 16));

	// The two hours worth one tap, as a selector rather than a pair of
	// buttons: which one is in force is a STATE, and the control that sets it
	// should show it. The period's own hour is offered whenever it is not the
	// current hour (under a Now period the two are the same instant, so there
	// is nothing to choose), worded by what the period states, so a flight
	// with no departure time offers its FLIGHT and never a departure nobody
	// gave. An hour typed by hand matches neither and leaves both unpressed,
	// which is what the line underneath then explains.
	// One reading of the clock for the two deriveds below, on the shared minute
	// tick: asking twice lets them straddle an hour boundary and disagree about
	// what "now" is inside one pass.
	const nowHour = $derived.by(() => {
		void notamState.tick;
		return floorHourMs(Date.now());
	});

	const anchorOptions = $derived.by(() => {
		const opts = [{ value: 'now', label: t.weather.now }];
		if (wx.periodSource !== 'now' && wx.briefedMs !== nowHour) {
			opts.push({
				value: 'period',
				label:
					wx.periodSource === 'flight'
						? t.weather.departure
						: wx.periodSource === 'flight-day'
							? t.conditions.flight
							: t.weather.rangeStart,
			});
		}
		return opts;
	});

	// Derived from the VALUE shown, never from a stored bit, so the selector
	// cannot claim one thing while the field shows another: pressing Now under
	// a flight period pins the current hour, and Now is what stays pressed.
	// A clamped period still reads as the period, the shortfall being the
	// forecast's, not the pilot's.
	const anchorValue = $derived.by(() => {
		if (wx.source !== 'pin') {
			return wx.periodSource === 'now' ? 'now' : 'period';
		}
		return wx.ms === nowHour ? 'now' : '';
	});

	const validSource = $derived(
		wx.source === 'pin'
			? t.weather.validPinned
			: wx.source === 'flight'
				? t.weather.validFromFlight
				: wx.source === 'flight-day'
					? t.weather.validFromFlightDay
					: wx.source === 'custom'
						? t.weather.validFromCustom
						: t.weather.validFromNow,
	);

	// Why the map may be answering for another hour, or drawing nothing at all.
	// Advisory ink, not the fetch error's red: the data is not missing, the
	// forecast simply does not go that far.
	const reachNote = $derived(
		wx.clamped
			? t.weather.validClamped(fmtUtc(wx.briefedMs))
			: wx.ms > wx.reachEndMs
				? t.weather.validBeyondReach(fmtUtc(wx.reachEndMs))
				: null,
	);
	...
	function onAnchor(v: string): void {
		if (v === 'period') {
			releaseWxValidTime();
		} else {
			// The hour in force, not the next one: this is an instant something
			// is READ at. Under a Now period it equals the default, so it hands
			// the field back to the period rather than pinning it.
	...
	const slider = $derived.by(() => {
		// Anchored on the animation's own frozen instant while it is engaged,
		// else on the hour shown: the span must contain the thumb, and it must
		// NOT follow the scrub (the fetch window is built from it, and its
		// floored hours are the lattice cache namespace). The end stays
		// now-relative, so track the shared minute tick.
		void notamState.tick;
		return animationWindow(modelInUse, windAloft.animAnchorMs ?? wx.ms, Date.now());
	});

	// The hour, not the instant: a stated ETD lands mid-hour and the slider,
	// stepping in hours from its own start, would round the thumb to the NEXT
	// one and show an hour later than the field. The floored hour is also the
	// one actually fetched, the lattice window opening there.
	const sliderValue = $derived(
		Math.min(Math.max(floorHourMs(wx.ms), slider.startMs), slider.endMs),
	);

	function onSlider(e: Event): void {
		startWxAnimation();
		setWxValidTime(Number((e.currentTarget as HTMLInputElement).value));
	}
	...
	function togglePlay(): void {
		...
		if (wx.ms < slider.startMs || wx.ms >= slider.endMs) {
			setWxValidTime(slider.startMs);
		}
	}
	...
	$effect(() => {
		...
		const id = setInterval(() => {
			const next = floorHourMs(wxValidTime().ms) + 3600_000;
			setWxValidTime(next > win.endMs ? win.startMs : next);
		}, 700);
		return () => clearInterval(id);
	});

	// The play mode is global state but its tick loop lives in this component:
	// pause on unmount (like the pause button; the frozen anchor keeps the slider
	...
	function onSigmetRow(s: Sigmet): void {
		...
		// The phone page steps aside for the advisory's own sheet and the
		// map under it (NotamsTab's rule).
		revealMap();
	}

	// Precipitation radar: the loop and product pickers' labels are
	// invariant tokens read off the catalog per locale; the legend rows come
	// off the product's scale (the one table the canvas paints from) with
	// the Marshall-Palmer equivalent in the other unit under each bound, and
	// the four words summarise the ICAO levels.
	const loopOptions = $derived(
		RADAR_LOOPS.map((m) => ({ value: String(m), label: t.weather.radar.loopLabels[m] })),
	);
	const productOptions = $derived(
		OPERA_PRODUCTS.map((p) => ({ value: p, label: t.weather.radar.productLabels[p] })),
	);
	const radarScale = $derived(RADAR_SCALES[radar.product]);
	/** A rain rate in the locale: whole above 10, one decimal above 1, else two. */
	function fmtMmH(v: number): string {
		return fmtDecimal(v, v >= 10 ? 0 : v >= 1 ? 1 : 2);
	}
	/** A scale bound: whole numbers as they are, 0.4 in the locale. */
	function fmtBound(v: number): string {
		return Number.isInteger(v) ? String(v) : fmtDecimal(v, 1);
	}
	/** The chip tooltip: the bound and its Marshall-Palmer twin. */
	function chipTitle(min: number): string {
		return radarScale.unit === 'dBZ'
			? `${min} dBZ, ${fmtMmH(marshallPalmerMmH(min))} mm/h`
			: `${fmtBound(min)} mm/h, ${Math.round(marshallPalmerDbz(min))} dBZ`;
	}
	// Called from the template (tracked t.* reads): "Light 15 - 30, moderate
	// 30 - 40, heavy 40 - 50, extreme 50+ dBZ." (or the mm/h bounds).
	function radarWordLine(): string {
		const scale = radarScale;
		const order: RadarWord[] = ['light', 'moderate', 'heavy', 'extreme'];
		const parts = order.map((w) => {
			const steps = scale.steps.filter((s) => s.word === w);
			const lo = steps[0].min;
			const next = scale.steps.find((s) => s.min > steps[steps.length - 1].min);
			const word = t.weather.radar.words[w];
			return next ? `${word} ${fmtBound(lo)} - ${fmtBound(next.min)}` : `${word} ${fmtBound(lo)}+`;
		});
		const line = `${parts.join(', ')} ${scale.unit}.`;
		return line.charAt(0).toLocaleUpperCase() + line.slice(1);
	}
	const radarFeedLine = $derived.by(() => {
		if (!display.liveWeather) {
			return { status: t.weather.liveWeatherOff };
		}
		if (!radar.showOnMap) {
			return { status: t.weather.radar.hidden };
		}
		if (radar.status === 'error' && radar.error) {
			return { status: '', error: radar.error() };
		}
		if (radar.status === 'outside') {
			return { status: t.weather.radar.outside };
		}
		const feed = radarFeed();
		if (!feed) {
			return { status: radar.status === 'loading' ? t.weather.radar.loading : t.weather.radar.noFrames };
		}
		if (feed.staleness === 'expired') {
			return {
				status: '',
				error: t.weather.radar.expired({
					time: slotHHMM(feed.newest.t),
					min: OPERA_PRODUCT_INFO[radar.product].staleness.expired,
				}),
			};
		}
		const time = slotHHMM(feed.newest.t);
		const age = formatAge(ageMinutes(feed.ms), t.weather.metar);
		const pub = feed.newest.publishedMs;
		return {
			status:
				pub != null
					? t.weather.radar.newestFrame({ time, published: hhmmZ(pub), age })
					: t.weather.radar.newestFrameNoPub({ time, age }),
			ageText: t.weather.radar.framesHeld({ loaded: radar.loaded, wanted: radar.wanted }),
		};
	});
...
<div class="tab-panel">
	...
	<fieldset class="group">
		...
		<div class="range">
			...
			{#if anchorOptions.length > 1 || wx.source === 'pin'}
				<!-- Its own row, under the fields it sets: the date needs its
				     whole width, and at the 44 px touch floor a selector cannot
				     share a line with two inputs on a phone sheet. Absent while
				     the period IS the current hour and nothing is pinned, there
				     being no other hour to choose. -->
				<div class="range-label anchor-row">
					<Segmented
						options={anchorOptions}
						value={anchorValue}
						onSelect={onAnchor}
						ariaLabel={t.weather.anchorAria}
					/>
				</div>
			{/if}
			<div class="range-label">
			...
			<p class="muted small slider-time">
				{t.weather.showing(fmtUtc(wx.ms))}
				{validSource}
			</p>
			{#if reachNote}
				<p class="small reach-note">{reachNote}</p>
			{/if}
		</div>
	</fieldset>
	...
	<!-- Nothing of the radar reaches paper (docs/precipitation-radar.md): it is
	     supplementary awareness outside the regulated briefing. The PANE is
	     hidden by app.css's print block, but this section's
	     TEXT is not: a desktop Ctrl+P prints what is on screen, so without
	     this the colour scale, the intensity words and the frame's age and
	     publication time went onto a sheet that looks like flight
	     documentation. -->
	<fieldset class="group no-print">
		<legend>{t.weather.radar.legend}</legend>
		<label class="check">
			<input
				type="checkbox"
				checked={radar.showOnMap}
				onchange={(e) => setShowRadarOnMap(e.currentTarget.checked)}
			/>
			<span>{t.weather.radar.showRadar}</span>
		</label>
		<label class="check">
			<input
				type="checkbox"
				checked={radar.coverage}
				onchange={(e) => setRadarCoverage(e.currentTarget.checked)}
			/>
			<span>{t.weather.radar.coverageLimits}</span>
		</label>
		<label class="range-label row">
			<span>{t.weather.radar.opacity}</span>
			<input
				type="range"
				min={OPACITY_MIN_PCT}
				max={OPACITY_MAX_PCT}
				step="5"
				value={radar.opacityPct}
				aria-label={t.weather.radar.opacity}
				oninput={(e) => setRadarOpacity(Number(e.currentTarget.value))}
			/>
			<!-- i18n-ignore: % is locale-invariant -->
			<span class="hint">{radar.opacityPct}%</span>
		</label>
		<div class="range-label row">
			<span>{t.weather.radar.loopLabel}</span>
			<Segmented
				options={loopOptions}
				value={String(radar.loopMin)}
				onSelect={(v) => setRadarLoop(Number(v) as RadarLoopMin)}
				ariaLabel={t.weather.radar.loopAria}
			/>
		</div>
		<!-- The product: reflectivity or rain rate, two composites on two
		     grids at two cadences, the same hue ramp under either legend. -->
		<div class="range-label row">
			<span>{t.weather.radar.productLabel}</span>
			<Segmented
				options={productOptions}
				value={radar.product}
				onSelect={(v) => setRadarProduct(v as OperaProduct)}
				ariaLabel={t.weather.radar.productAria}
			/>
		</div>
		<!-- The legend at 100 %: one chip per scale step, the bound on it
		     and its Marshall-Palmer twin in the tooltip, then the hatch. -->
		<p class="cat-legend radar-legend">
			{#each radarScale.steps as s (s.min)}
				<!-- i18n-ignore: unit tokens, locale-invariant -->
				<span class="cat" title={chipTitle(s.min)}><span class="cat-dot" style:background={s.color}></span>{fmtBound(s.min)}</span>
			{/each}
			<span class="cat"><span class="cat-dot hatch"></span>{t.weather.radar.noCoverage}</span>
		</p>
		<p class="muted small radar-words">{radarWordLine()} {t.weather.radar.scaleUnits[radar.product]}</p>
		<p class="muted small">{t.weather.radar.caveat[radar.product]}</p>
		<FeedStatus {...radarFeedLine} onRefresh={refreshRadar} disabled={!radar.showOnMap || !display.liveWeather} />
		<!-- The producer differs per composite (the frames' own creator_name:
		     CIRRUS by Météo-France, NIMBUS by GeoSphere Austria). -->
		<p class="muted small">{t.weather.radar.credit[radar.product]}</p>
	</fieldset>
...
<style>
	...
	/* The date takes the width its own value needs, not the whole row: with
	   the anchors moved off this line there is nothing left to stretch for,
	   and a box three times its content reads as a missing field. It shrinks
	   before it truncates, so a narrow sheet still fits. */
	.range-label input[type='date'] {
		flex: 0 1 auto;
	}

	/* Aligned under the fields it sets, past the label gutter. */
	.anchor-row {
		padding-left: 56px;
	}

	/* Why the map answers for another hour, or draws nothing: the nav log's
	   own advisory ink, never the fetch error's red. The forecast is not
	   missing, it simply does not reach that far. */
	.reach-note {
		margin: 0;
	...
	/* The no-coverage chip: the same hatch the canvas draws. */
	.cat-dot.hatch {
		background: repeating-linear-gradient(135deg, #78909c 0 2px, transparent 2px 6px);
		border: 1px solid var(--border);
	}
