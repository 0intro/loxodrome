<script lang="ts">
	...
	import { revealMap } from '$lib/state/ui.svelte';
	import Icon from '../Icon.svelte';

	/** The phone's Flight page shows one section of this tab at a time
	 *  (PhonePages: flight / alerts / replay); the desktop passes nothing
	 *  and renders the whole tab. The groups outside the section are hidden,
	 *  not unmounted, so the recorder's controls and the replay transport
	 *  keep their state across the sub-header. */
	type NavSection = 'flight' | 'alerts' | 'replay';
	const { section = null }: { section?: NavSection | null } = $props();
	const hide = (s: NavSection): boolean => section !== null && section !== s;
	...
	const terrainGaps = $derived(terrainInputGaps());
	...
	//
	// A SHALLOW COPY, and it is load-bearing. state/navMotion hands back ONE
	// fold per trace and extends it IN PLACE, and nav.points keeps its
	// identity across an append, so this derived re-executes on every fix and
	// returns a value that is === the last one. Svelte bumps a derived's
	// write version only when the value DIFFERS (deriveds.js update_derived),
	// so nothing downstream ever re-ran: with the tab left open the card
	// never appeared at the takeoff, and its landing, block and airborne rows
	// never filled in during the auto-stop grace window, which is exactly
	// when they are read. Copying is what makes the identity change with the
	// contents; the two committed-instant arrays ride along by reference and
	// are read through this same fresh object.
	...
	/** The trace profile opens in the PANE on a phone, and the Flight page
	 *  this button sits on covers the pane: the page steps aside for it, the
	 *  way the Plan page's own Log and Profile rows do. Desktop is
	 *  unaffected, revealMap only collapsing a sidebar the chart is not in. */
	function openTraceProfile(): void {
		// Only on the way OPEN: this is a toggle, and a tap that puts the
		// chart away has no reason to take the page with it.
...
<div class="tab-panel">
	...
	<fieldset class="group" hidden={hide('flight')}>
		...
		{#if hasTrace && motion.takeoffMs != null}
			<dl class="readout stats">
				{#if motion.firstMoveMs != null && motion.blockOutObserved}
				...
				{#if motion.takeoffObserved}
					<dd>{fmtClockUtc(motion.takeoffMs)}Z</dd>
				{:else}
					<!-- The trace opened already fast: its takeoff is before the
					     first fix, and the first fast fix is no departure. The
					     block-off, airborne and block figures that hang off an
					     unseen instant stay off the card the same way. -->
					<dd class="muted" title={t.navigation.navlogTakeoffBeforeTraceTip}>
						{t.navigation.navlogTakeoffBeforeTrace}
					</dd>
				{/if}
				{#if motion.landingMs != null}
					...
					{#if motion.firstMoveMs != null && motion.lastMoveMs != null && motion.blockOutObserved}
	...
	<fieldset class="group" hidden={hide('alerts')}>
		...
		<div id="{uid}-alert-settings">
			...
			<!-- The ground ahead (docs/terrain-awareness.md): its own two rows
			     and the pilot's inhibit, after the airspace settings, which the
			     terrain look-ahead (one minute, fixed) does not read. -->
			<label class="check sub" title={t.navigation.alertTierTerrainTip}>
				<input
					type="checkbox"
					checked={terrainPrefs.terrain}
					onchange={(e) => setTerrainTier('terrain', (e.target as HTMLInputElement).checked)}
				/>
				{t.navigation.alertTierTerrain}
			</label>
			<label class="check sub" title={t.navigation.alertTierObstacleTip}>
				<input
					type="checkbox"
					checked={terrainPrefs.obstacle}
					onchange={(e) => setTerrainTier('obstacle', (e.target as HTMLInputElement).checked)}
				/>
				{t.navigation.alertTierObstacle}
			</label>
			<label class="check sub" title={t.navigation.alertTerrainInhibitTip}>
				<input
					type="checkbox"
					checked={terrainInhibited()}
					disabled={!terrainPrefs.terrain && !terrainPrefs.obstacle}
					onchange={(e) => setTerrainInhibit((e.target as HTMLInputElement).checked)}
				/>
				{t.navigation.alertTerrainInhibit}
			</label>
			{#if terrainGaps?.unread}
				<p class="alert-gap" title={t.navigation.alertTerrainUnreadTip}>
					{t.navigation.alertTerrainUnread}
				</p>
			{/if}
			{#if terrainGaps?.obstacles}
				<p class="alert-gap">{t.navigation.alertObstaclesMissing}</p>
			{/if}
			...
			<p class="muted small">{t.navigation.alertTerrainAdvisory}</p>
		</div>
		{/if}
	</fieldset>
	...
	<fieldset class="group" hidden={hide('replay')}>
