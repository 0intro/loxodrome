...
<script lang="ts">
	...
	import { budgetSections, orderedNotamOwnerSections } from '$lib/state/notamOrder.svelte';
	import { requestNotamPrint } from '$lib/state/notamPrint.svelte';
	import { decodeQ, t } from '$lib/state/i18n.svelte';
	import { revealMap, selectNotam, ui } from '$lib/state/ui.svelte';
	...
	let {
		rulesFromRoute = false,
		loader,
		maxRows = Number.POSITIVE_INFINITY,
		showLoader = $bindable(false),
	}: {
		/** Does the flight-rules filter mirror a planned route? The shell that
		 *  owns the route workspace answers; an app with none leaves it false,
		 *  and the chip simply says nothing about where the setting came from. */
		rulesFromRoute?: boolean;
		/** The empty state: how THIS app gets a briefing. A snippet rather than
		 *  an import, because the two apps get one differently and the import
		 *  would be the tie: Loxodrome's loader reaches the route workspace and
		 *  the map viewport, neither of which the NOTAM Viewer has. */
		loader: Snippet<[{ onLoaded: () => void; onGaps: () => void }]>;
		/** How many rows to build before closing with a "show the rest" row.
		 *  UNBOUNDED by default, which is what Loxodrome gets: a corridor
		 *  briefing is a few hundred rows and
		 *  docs/performance-2026-08.md declined capping for it on the grounds
		 *  that it changes what the UI does.
		 *
		 *  An app whose briefings are continental needs the cap, and measured
		 *  rather than guessed: a world dump shows 9 148 rows in the default
		 *  period, which is 79 363 DOM nodes and a 4.5 s task to build. */
		maxRows?: number;
		/** Is the fetch view up over the list? Local unless a shell BINDS it,
		 *  which is the NOTAM Viewer: its phone bar carries Load as its own
		 *  destination, so the bar and the head's button have to be two views
		 *  of one state rather than two states. Loxodrome does not bind it and
		 *  is unchanged: the head button remains the only way in, and toggling
		 *  it mutates this locally exactly as the private state it replaces
		 *  did. */
		showLoader?: boolean;
	} = $props();
	...
	/* The row budget, spent by the pure fold in state/notamOrder. `revealAll`
	 * is the escape hatch; the search box is the other one. */
	let revealAll = $state(false);
	const budgeted = $derived(
		budgetSections(sections, revealAll ? Number.POSITIVE_INFINITY : maxRows),
	);
	...
	const showFetch = $derived(notamState.notams.length === 0 || showLoader);
	...
	function onSelect(index: number): void {
		...
		// On a phone the page steps aside so the map, and the NOTAM's sheet
		// opening in the pane, are in view (the Airports tab's own rule):
		// without it the row's whole answer lands behind an opaque page and
		// the tap reads as doing nothing at all.
		revealMap();
	}
...
<div class="tab-panel" class:fills-panel={showFetch}>
	<div class="tab-head">
		{#if showFetch}
			{#if total > 0}
	...
	{#if showFetch}
		...
		{/if}
		{@render loader({ onLoaded: showList, onGaps: openFetch })}
	{:else}
		<input
			class="search"
			type="search"
		...
		{:else}
			{#each budgeted.sections as sec (sec.key)}
				{#if budgeted.sections.length > 1}
					<h3 class="cat-head">{sectionLabel(sec.owner)}</h3>
				{/if}
				{@render notamRows(sec.items, false)}
			{/each}
			{#if budgeted.hidden > 0}
				<div class="hidden-note">
					<p class="muted">{t.notam.moreRows(budgeted.hidden)}</p>
					<button class="btn" onclick={() => (revealAll = true)}>{t.filter.showAll}</button>
				</div>
			{/if}
		{/if}
...
<FiltersPopover
	...
	fromRoute={rulesFromRoute}
/>

<style>
	/* THE LOADER CARD IS GIVEN A DEFINITE HEIGHT, and only it.

	   The card is a form ending in Display on map, and its paste box is the
	   one part that can give. For the box to take what is LEFT, something
	   above it has to know how much there is: a percentage or a flex share
	   resolves against nothing when every ancestor is content-sized, which is
	   why the box first had to be sized by a hand-measured constant, and why
	   that constant went wrong the moment a route was loaded and the route
	   section grew from 62px to 187 (measured; the card's fixed part went 363
	   to 462 and the page overflowed by 43).

	   The panel body already HAS a definite height, being a flex item of a
	   sized column, so one `block-size: 100%` here hands it down and the
	   ordinary flex rules do the rest.

	   Only while the loader is up (`showFetch`). The LIST is the other
	   occupant of this element and is meant to scroll: it has no business
	   being capped at the panel's height, and nothing about it needs a
	   definite one. */
	.tab-panel.fills-panel {
		block-size: 100%;
	}
