...
let pending = false;
/** Which request the sample belongs to. The synchronous ladder can answer
 *  while a fetch for an EARLIER position is still out, and that fetch would
 *  otherwise land last and put the aircraft's ground back where it was a
 *  mile ago, which reads as null once it is two miles behind. */
let seq = 0;
...
export function ensurePoseGround(lat: number, lon: number): void {
	...
	/* The synchronous ladder FIRST, and outside the in-flight slot: it reads
	 * tiles already decoded, so it neither needs the slot nor may be blocked
	 * by one (a fetch over poor cellular can outlast the two miles that make
	 * the sample stale, and the band's AGL and the ALERT engine's ground go
	 * with it). Guarded, because a throw out of the decoded-tile math used to
	 * leave the flag set and kill AGL silently for the rest of the session. */
	let peek: number | null | undefined;
	try {
		peek = peekElevationFtAt(lat, lon);
	} catch {
		peek = undefined;
	}
	if (peek !== undefined) {
