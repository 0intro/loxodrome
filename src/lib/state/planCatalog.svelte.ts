...
export async function removeStoredPlan(id: string): Promise<void> {
	...
	// The one user-intent path a stored plan leaves the catalog by, so the
	// sync tombstone is recorded here; the device-WIPE paths go to
	// flightsDb directly on purpose (docs/accounts-sync.md, delete vs
	// device-wipe).
	void recordSyncTombstone('plans', id);
	derived.delete(id);
