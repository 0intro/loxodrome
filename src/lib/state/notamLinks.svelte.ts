 ...
 *  activation spec. */
// One extraction per NOTAM. The scan is a matchAll over the whole body plus
// the RTBA zone read, and half a dozen callers run it over the entire
// briefing (the activation map, the alert volumes, the profile overlays, the
// per-airspace panels), several of them once a minute. Keyed on the object,
// so a re-parse hands out fresh NOTAMs and therefore a fresh cache.
const activatedIdsCache = new WeakMap<Notam, { src: Airspace[] | null; ids: string[] }>();

export function activatedAirspaceIds(notam: Notam): string[] {
	if (!isActivationQCode(notam.qCode)) {
		return [];
	}
	// Keyed on the dataset identity as well as the NOTAM: the detached-suffix
	// candidates are confirmed against the loaded rows, so an answer computed
	// before those arrived must not outlive them. The array stays the SAME
	// while the data is unchanged, which the downstream memos depend on.
	const src = getAirspaces();
	const hit = activatedIdsCache.get(notam);
	if (hit && hit.src === src) {
		return hit.ids;
	}
	const ids = extractActivatedAirspaceIds(notam);
	activatedIdsCache.set(notam, { src, ids });
	return ids;
}

function extractActivatedAirspaceIds(notam: Notam): string[] {
	// FR-only by design (the grammar is the French prefixes'), so only a
	// French NOTAM activates by it, or one with no Q-line to say which.
	// French is the FIR's, else the A) location's where no dataset belongs to
	// the FIR (notamPublisher: the Antilles are filed under Q) TTZP, A) TFFF).
	// Read over another State's text it finds French ids in prose once the
	// spaced forms are allowed ("LF-R 174", "LF R174"): a US TFR NOTAM writes
	// "TFR 5/2345", the 5 being the year's last digit, and TFR5 is a real
	// Martinique zone. The citation tier below is family-scoped by the same
	// rule for the same reason.
	const fir = notam.qualifier?.fir;
	if (fir && notamPublisher(fir, notam.icaoCodes) !== 'fr') {
		return [];
	}
	const out: string[] = [];
	...
	// The SIA's detached sub-lettering ("LF-R368 A B C1 C2"): candidates until
	// a loaded row carries the id, since a lone letter is prose too.
	const index = airspaceIdIndex();
	if (index) {
		const candidates = [
			...extractSpacedSubZoneIds(notam.fullContent),
			...extractBareDesignatorIds(notam.fullContent),
		];
		for (const c of candidates) {
			if (seen.has(c.id) || !index.has(c.id)) {
				continue;
			}
			if (isReferenceContext(notam.fullContent, c.index)) {
				continue;
			}
			seen.add(c.id);
			out.push(c.id);
...
export function activatesAirspaces(notam: Notam): Airspace[] {
	...
	const out: Airspace[] = [];
	for (const id of activatedAirspaceIds(notam)) {
...
// id → Airspace over the loaded set, rebuilt only when the array itself is
// replaced (the merge hands out a new one whenever a country arrives), the
// firRowsForIdent idiom. It used to be built INSIDE activatesAirspaces, i.e.
// once per NOTAM: ten thousand Map inserts each, for every activation NOTAM
// in the briefing, on every repaint of the activation hatch.
let idIndexSrc: Airspace[] | null = null;
let idIndex: Map<string, Airspace> | null = null;

/** The loaded airspaces keyed by their shared `id`, or null before the
 *  dataset arrives. Reads getAirspaces(), so reactive callers track the load.
 *  Several rows can share an id (MOA parent + exclusion rings); the last
 *  wins, exactly as the per-call build it replaces did. Exported for the
 *  activation-hatch effect, which needs the same index per tick. */
export function airspaceIdIndex(): Map<string, Airspace> | null {
	const all = getAirspaces();
	if (!all) {
		return null;
	}
	if (all !== idIndexSrc || !idIndex) {
		// eslint-disable-next-line svelte/prefer-svelte-reactivity -- not reactive state
...
/** A sector number ends here: the next character is not a digit, not a
 *  point with a digit behind it, and not a comma with TWO digits behind it.
 *
 *  Each refusal is the narrowest that holds, because refusing too much fails
 *  in the worst direction: citing NOTHING means "the whole base", which links
 *  every sector of the base and raises a spurious flag on each one that does
 *  not publish the withdrawn value. A bare '.' is a sentence period ("SIV
 *  SEINE 4 ET 5.", "FIS AREA 2."), so only a point FOLLOWED BY A DIGIT
 *  continues a number (the SIA dots its sector designators in both languages,
 *  "SIV 1, 1.1, 1.2"). A comma is the French decimal separator AND a list
 *  connector, so "4,5" is read as the list it always was and only a comma
 *  carrying two digits or more ("0,16NM") continues a number. A frequency
 *  never reaches the comma at all: three integer digits fail the first test. */
const NOT_PART_OF_A_NUMBER = '(?!\\d)(?!\\.\\d)(?!,\\d{2})';
 ...
 *  ("TOULOUSE TWR ...") matching the same-named SIV.
 *
 *  SIV is accepted beside FIS as the marker word, because the SIA's own
 *  French half writes the service that way and nothing else does: "SIV 2
 *  BEAUVAIS FERME" where the English half writes "'BEAUVAIS' FIS AREA 2
 *  CLOSED" (A5453/26), "FREQ INFO SIV PROVENCE 5" against "FIS PROVENCE 5
 *  INFO FREQ" (A5595/26). Without it the two halves of one NOTAM link
 *  different sectors. Exported for the FIS spec. */
export function fisServiceNamed(token: string, textUpper: string): boolean {
	...
	// Marker before the name: "FIS SEINE", "FIS AREA 'BASTIA SUD'", "FIS 2
	// 'LILLE", "SIV 2 BEAUVAIS".
	const before = new RegExp(
		`\\b(?:FIS|SIV)(?:\\s+(?:AREAS?|SECTORS?|SECTEURS?))?(?:\\s+\\d{1,2}(?:\\.\\d)?)?\\s+['"]?${esc}(?:[^A-Z0-9]|$)`,
	);
 ...
 *  an FIS keyword ("FIS 4 AND 5", "FIS 2", "SECTOR 2", "SEINE 4 AND 5",
 *  "SIV 1 ET 4", "FIS AREA 2"). Connectors: AND / ET / & / , / /. Ranges
 ...
 *  ("... LILLE FIS 2") cannot narrow this one.
 *
 *  A number must END where it is read (`NOT_PART_OF_A_NUMBER`), which is what
 *  keeps a FREQUENCY from reading as a sector. The 1-2 digit shape alone does
 *  not: it matches the HEAD of one, so "SIV BASTIA SUD 135.135MHZ" cited
 *  sector "13", no row is designated 13, and the NOTAM linked NOTHING
 *  (A6001/26; A6090/26 and F1936/26 the same way through "'STRASBOURG' APP
 *  134.575MHZ"). Exported for the FIS spec. */
export function citedSectors(token: string, textUpper: string): Set<string> | null {
	const esc = token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
	const num = `\\d{1,2}(?:\\.\\d)?${NOT_PART_OF_A_NUMBER}`;
	const list = `${num}(?:\\s*(?:AND|ET|&|,|/)\\s*${num})*`;
	const anchor = new RegExp(
		`(?:FIS|SIV|SECTORS?|SECTEURS?|${esc})(?:\\s+(?:AREAS?|SECTORS?|SECTEURS?))?\\s*:?\\s*(${list})`,
		'g',
	);
...
/** English spellings of the trailing cardinal a French FIS sector name
 *  carries. The row is named in French ("BASTIA SUD") and the English half of
 *  the same NOTAM says "FIS BASTIA SOUTH AREA" (A6001/26), so without this the
 *  two halves of one bilingual NOTAM link different sectors. Only the TRAILING
 *  word is translated, and only for the FIS branch: an airspace whose name
 *  merely contains a cardinal is not the same thing. */
const CARDINAL_EN: Record<string, readonly string[]> = {
	NORD: ['NORTH', 'NORTHERN'],
	SUD: ['SOUTH', 'SOUTHERN'],
	EST: ['EAST', 'EASTERN'],
	OUEST: ['WEST', 'WESTERN'],
...
/** The spellings of an FIS base name to look for, the published one first. */
function fisNameVariants(token: string): string[] {
	const m = token.match(/^(.*)\s(NORD|SUD|EST|OUEST)$/);
	const en = m ? CARDINAL_EN[m[2]] : undefined;
	return en ? [token, ...en.map((w) => `${m![1]} ${w}`)] : [token];
}

/** A clause that DESIGNATES a unit to call rather than reporting on it.
 *
 *  The comma is a clause fence here, not just the full stop: the SIA separates
 *  these with commas and bullets, and in French the comma is ALSO the decimal
 *  separator, so without it the scan runs back through a substitute's own
 *  frequency into its CONTACTER. Measured on the real shape
 *  "FREQUENCE 119,800MHZ INDISPONIBLE, CONTACTER PARIS INFO 125,700MHZ,
 *  SIV 2 BEAUVAIS FERME": the subject sector read as substitute-only and did
 *  not link at all, so no dashed sector, no NOTAM in its panel and the nav log
 *  went on handing out the withdrawn frequency. The English half of the same
 *  NOTAM was saved only by the dot inside "125.700MHZ". */
const SUBSTITUTE_LEAD = /\b(?:CONTACT(?:ER|EZ)?|CALL|APPELER|APPELEZ)\b[^.;:,]*$/;

/** True when EVERY callsign-adjacent mention of `token` sits inside a
 *  "contact ..." clause, i.e. the NOTAM names this service as the one to call
 *  INSTEAD, not as one it concerns.
 *
 *  A5453/26 withdraws BEAUVAIS 2's frequency and says "CONTACT 'PARIS INFO'
 *  125.700MHZ". PARIS INFO is the answer, not the subject, and listing the
 *  three FIC PARIS sectors as affected was wrong twice over: the panel and the
 *  map highlight named five airspaces for a NOTAM about one, and because
 *  resolveAirspaceRadios applies closures per row, each of those rows raised a
 *  spurious "closure could not be applied" flag in the nav log, the schedule
 *  and its own panel. The test is ALL mentions, so a NOTAM that both reports a
 *  service and designates it ("X INFO NOT AVBL, CONTACT X INFO ON ...") still
 *  links. Exported for the FIS spec. */
export function fisSubstituteOnly(token: string, textUpper: string): boolean {
	const esc = token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
	...
	let seen = 0;
	let substitutes = 0;
	for (const m of textUpper.matchAll(mention)) {
		seen++;
		if (SUBSTITUTE_LEAD.test(textUpper.slice(Math.max(0, (m.index ?? 0) - 45), (m.index ?? 0) + 1))) {
			substitutes++;
		}
	}
	return seen > 0 && substitutes === seen;
}
 ...
 *  it scopes to (notamPublisher: its FIR's, else its A) locations' where no
 *  dataset belongs to the FIR), its upper-cased E) text, and whether this is
 *  the FIS/SIV
...
function computeNameMatchContext(notam: Notam): NameMatch | null {
	...
	const source = notamPublisher(notam.qualifier.fir, notam.icaoCodes);
	if (!source) {
		return null;
	}
	// Folded to single spaces: a line break in an E) text is a WRAP, and a
	// name it falls inside would otherwise not match ('RENNES\nSUD' in
	// A6082/26). Only the name matching folds; the fuel, runway and
	// aerodrome grammars read the line as the clause and must not.
	const eText = notamEText(notam).toUpperCase().replace(/\s+/g, ' ');
	if (!eText) {
		return null;
	}
...
/** True when `token` sits beside the airspace's TYPE WORD in `textUpper`,
 *  in EITHER order: "TMA LORIENT", "CTR DE BIARRITZ" (French articles
 *  tolerated), the conjunction hop "CTR ET TMA LORIENT" (the CTR row matches
 *  across "ET TMA"), and the SIA's own quoted reversal, "'LOGNES
 *  EMERAINVILLE' RMZ", "'LE HAVRE' RMZ". The reversal is what the RMZ and TMZ
 *  NOTAMs are written in: their subject (QAZ, aerodrome traffic zone) maps to
 *  no type of its own, so the adjacency is the only link they have, and
 *  reading one order only left them attached to nothing at all. Exported for
 *  the name-match spec. */
export function typeAdjacentNameInText(
	...
	// The name carries its own quotes in the reversed form, and the type word
	// follows it directly; nothing may sit between, which is what keeps a
	// neighbouring sentence from pairing a name with someone else's type.
	const after = new RegExp(`['"]?\\b${esc}['"]?\\s+${type}\\b`);
	return before.test(textUpper) || after.test(textUpper);
}
...
function airspaceMatchesName(a: Airspace, ctx: NameMatch): boolean {
	...
	// The spelling the NOTAM actually used, so the sector narrowing below
	// looks for numbers beside THAT one (an English half writes BASTIA SOUTH
	// where the row is named BASTIA SUD).
	const named = fisNameVariants(token).find((v) => fisServiceNamed(v, ctx.eText));
	if (named === undefined) {
		return false;
	}
	// Named only as the unit to call instead: the answer, not the subject.
	if (fisSubstituteOnly(named, ctx.eText)) {
		return false;
	}
	const cited = citedSectors(named, ctx.eText);
	if (cited === null) {
		return true;
	}
...
export function isActiveTrigger(notam: Notam): boolean {
	...
	// In the NOTAM's OWN publisher's grammar: a Spanish activation cites its
	// supplement bare ("SUP 149/26"), which the French grammar never reads.
	if (extractActivatedSups(notam.fullContent, notamSupPublisher(notam) ?? 'fr').length > 0) {
		return true;
	}
	return false;
}

/** The supplement keys a NOTAM cites, in ITS publisher's grammar and
 *  namespaced by that publisher: France and Spain each publish a
 *  "093/2026", and a briefing spanning both must never pair a French
 *  framework with a Spanish activation over the shared number. */
function supKeysOf(notam: Notam, activatedOnly: boolean): Set<string> {
	const pub = notamSupPublisher(notam) ?? 'fr';
	const refs = activatedOnly
		? extractActivatedSups(notam.fullContent, pub)
		: extractAipSupsFor(notam.fullContent, pub);
	return new Set(refs.map((r) => `${pub}:${supKey(r)}`));
}
...
export function notamTriggeredBy(notam: Notam): IndexedNotam[] {
	...
	const keys = supKeysOf(notam, false);
	...
	for (const it of filteredNotams()) {
		...
		for (const k of supKeysOf(it.notam, true)) {
			if (keys.has(k)) {
...
export function notamTriggers(notam: Notam): IndexedNotam[] {
	...
	const keys = supKeysOf(notam, true);
	if (keys.size === 0) return [];
	const out: IndexedNotam[] = [];
	// eslint-disable-next-line svelte/prefer-svelte-reactivity -- not reactive state
	const seen = new Set<string>();
	for (const it of filteredNotams()) {
		...
		for (const k of supKeysOf(it.notam, false)) {
			if (keys.has(k)) {
				seen.add(it.notam.id);
				out.push(it);
				break;
...
export function activatedAirspaceLinks(): Map<string, IndexedNotam[]> {
	// Every reactive input, read unconditionally and before the cache checks
	// (the notamCorridor idiom), so a caller inside $derived / $effect tracks
	// exactly what a derived would. getAirspaces() tracks the lazy dataset
	// load, so the overlay hatches the moment it resolves rather than waiting
	// for a filter change or the tick.
	const { from, to } = activeEvalWindow();
	const cands = candidates(filteredNotams(), airspaceIdIndex());
	if (windowCache && windowCache.candidates === cands && windowCache.from === from && windowCache.to === to) {
		return windowCache.out;
	}
	// eslint-disable-next-line svelte/prefer-svelte-reactivity -- not reactive state
	...
	for (const c of cands) {
		...
		// value list is "NOTAMs activating this airspace").
		if (c.rtba) {
			// eslint-disable-next-line svelte/prefer-svelte-reactivity -- not reactive state
			const seen = new Set<string>();
			for (const z of c.rtba) {
				if (z.start.getTime() <= to && z.end.getTime() >= from && !seen.has(z.airspaceId)) {
					seen.add(z.airspaceId);
					push(z.airspaceId, c.item);
				}
			}
			continue;
		}
		if (!isActiveDuring(c.item.notam, from, to)) {
			continue;
		}
	...
	windowCache = { candidates: cands, from, to, out };
	return out;
}

/** One activation NOTAM reduced to what the window test needs: either its
 *  anchored RTBA zone windows, or the loaded airspace ids it activates for
 *  its whole B)-C) validity. */
interface ActivationCandidate {
	item: IndexedNotam;
	/** Non-null for RTBA NOTAMs; each zone carries its own anchored window. */
	rtba: RtbaActivation[] | null;
	/** Loaded ids only, so an RTBA zone or a cited code with no matching
	 *  airspace row is never recorded. */
	ids: string[];
}

// The two stages. The extract walks the whole briefing, reads each activation
// NOTAM's text and resolves its ids; it depends only on the briefing and the
// loaded airspaces, so it survives the minute tick. The window pass over its
// result is a pair of date comparisons per candidate, which is what the tick
// actually needs to redo.
let candCache: { items: IndexedNotam[]; index: Map<string, Airspace> | null; out: ActivationCandidate[] } | null = null;
let windowCache: { candidates: ActivationCandidate[]; from: number; to: number; out: Map<string, IndexedNotam[]> } | null = null;

function candidates(items: IndexedNotam[], index: Map<string, Airspace> | null): ActivationCandidate[] {
	if (candCache && candCache.items === items && candCache.index === index) {
	...
	for (const item of items) {
		...
		if (isRtbaActivationNotam(n)) {
			const zones = rtbaZoneActivations(n).filter((z) => index?.has(z.airspaceId));
			if (zones.length > 0) {
				out.push({ item, rtba: zones, ids: [] });
			}
			continue;
		}
		const ids: string[] = [];
		...
		if (ids.length > 0) {
			out.push({ item, rtba: null, ids });
		}
	}
	candCache = { items, index, out };
	return out;
}
