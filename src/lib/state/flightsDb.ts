...
/** Whether a points row EXISTS, without reading it. The trace download's
 *  own skip test used to be `getPoints(id)?.length`, which pulls a whole
 *  stored trace into memory per candidate: over a library-wide scan that
 *  is every trace the device holds, read to look at one number. An
 *  unreadable store answers false, the same posture as getPoints: not
 *  landed, so retry. */
export async function hasPoints(id: number): Promise<boolean> {
	try {
		const d = await db();
		return (await reqDone(d.transaction(POINTS).objectStore(POINTS).getKey(id))) !== undefined;
	} catch {
		return false;
	}
}

/** Every outing id whose points are stored, in ONE transaction. The
 *  trace download's prune asks this of the whole library at once, where
 *  a lookup per candidate would be a transaction per candidate. */
export async function pointIds(): Promise<Set<number>> {
	try {
		const d = await db();
		const keys = await reqDone(d.transaction(POINTS).objectStore(POINTS).getAllKeys());
		return new Set(keys.filter((k): k is number => typeof k === 'number'));
	} catch {
		return new Set(); // unreadable reads as nothing landed, the retry posture
	}
}
