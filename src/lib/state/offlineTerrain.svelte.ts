 ...
 * contract in docs/offline-maps.md. One action pins every z12 Terrarium tile
...
import { tileUrl } from '$lib/map/terrain';
import {
	computePinOps,
...
export async function downloadTerrainPins(): Promise<void> {
	...
	const tiles = corridorTerrainTiles(planRoutePoints(), pinRadiusNM());
	if (tiles.length === 0) {
		return;
	}
