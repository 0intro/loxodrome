...
import { completeLevel, ensureTerrainRegions, terrainLevels, tileUrl } from '$lib/map/terrain';
 ...
 *  the profile's own line, an airspace floor); every level from there down
 *  to the complete one, which the terrain alert's corridor reads (its
 *  preferred level is z11 north of about 49 N, and a corridor falls back as
 *  far as the complete level, where the sea is answered); plus whatever
 *  coarser level each route's corridor reduction picks for its bins, derived
 *  from `corridorPlan`, the same description the reduction runs on. Pinned
 *  without them, a plan flown offline read "terrain not read" all the way. */
export function pinLevels(): number[] {
	...
	const zs = new Set<number>([l.deepest]);
