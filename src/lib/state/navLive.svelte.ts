...
import { freqChangeIdents, resolveAirportRadios, type ResolveAt } from './freqOverride.svelte';
...
export function airportContactUnit(a: Airport, at?: ResolveAt): NavContactUnit | null {
	...
	// flight there is nowhere else the pilot would see it. Resolved at the
	// caller's instant (the band passes its display instant, as the airspace
	// schedule below is), else at drawnStateAt(): never the briefing window,
	// whose unbounded look-ahead handed the band a conversion's NEW channel a
	// month before the tower moved to it.
	const resolved = freqChangeIdents().has(a.ident.toUpperCase())
		? resolveAirportRadios(a, at)
		: null;
...
function aerodromeUnit(wp: Waypoint | undefined, at: ResolveAt): NavContactUnit | null {
	if (!wp || wp.kind !== 'airport' || !wp.ident || !dataState.airportsLoaded) {
		return null;
	}
	const a = airportByIdent(wp.ident);
	return a ? airportContactUnit(a, at) : null;
}
...
function legCrossingFolds(routeId: string) {
	...
	// the outing's takeoff, which happened legs ago. The GATE takes the
	// takeoff whatever the trace saw of it (a trace that opens in flight
	// still has a leg to stamp); the departure row's ACTUAL is the junction
	// for a chained leg and otherwise liveNavlogAt's own rule, the takeoff
	// only when the trace holds it (motion.takeoffObserved).
	const gateMs = chained && segment ? segment.fromMs : motion.takeoffMs;
	const departureMs = chained && segment ? segment.fromMs : undefined;
	...
	extendCrossings(leg.fold, checkpoints, boundaries, gateMs, chained);
	return {
		route,
		segment,
		chained,
...
function scheduleFoldFor(
	...
	extendCrossings(sched.fold, f.checkpoints, sched.atNMs, f.gateMs, f.chained);
	return sched;
}
...
export function navLiveFor(routeId: string): NavLiveInfo | null {
	...
	const etdMs = hasStatedEtd() ? routeDepartureMs(route.id) : null;
	...
	if (airspaces) {
		...
		// live and replay alike. The same instant answers the sectors' own
		// operating hours, where a planning surface asks over the whole
		// planned flight: in the air there is nothing to plan for.
		const resolved = resolveScheduleRadios(
			schedule,
			airspaces,
			{ fromMs: tMs, toMs: tMs },
			{ fromMs: tMs, toMs: tMs },
		...
		const chain = contactChainAt({
			...
			departure: aerodromeUnit(route.waypoints[0], { fromMs: tMs, toMs: tMs }),
