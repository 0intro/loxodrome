...
export function activeNotamsByNavaid(): Map<string, IndexedNotam[]> {
	...
	if (!navaids || navaids.length === 0) return out;
...
export function unserviceableNavaids(): Set<string> {
	...
	const { from, to } = activeEvalWindow();
	for (const item of filteredNotams()) {
		if (item.notam.serviceStatus !== 'unserviceable') continue;
		if (!isNavaidQCode(item.notam.qCode)) continue;
		if (!isActiveDuring(item.notam, from, to)) continue;
		for (const n of matchNavaidsToNotam(item.notam, navaids, byIdent)) {
			out.add(n.id);
		}
	}
	return out;
