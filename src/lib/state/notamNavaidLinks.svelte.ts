 ...
 *
 * The navaid's own "Affecting NOTAMs" list reads the evaluation window (the
 * period condition's range, or now to the future; see activeEvalWindow); the
 * unserviceable mark, a drawn state, reads drawnStateAt (unserviceableNavaids);
...
import { filteredNotams, activeEvalWindow, drawnStateAt, type IndexedNotam } from './notam.svelte';
...
/** Navaids a NOTAM marks unserviceable (out of service) at the DRAWN state's
 *  instant (drawnStateAt): its B)/C) validity and its D) schedule, the gate
 *  every resolver of a drawn state shares (state/notamActive.ts). Not the
 *  briefing window, whose look-ahead is unbounded by default: an outage dated
 *  next month dimmed the VOR today, and a weekday one dimmed it on a Sunday.
 *  A schedule outside the parsed subset marks it, the NOTAM saying the aid
 *  is out at SOME time and the panel listing it. Returns a Set<navaidId> for
 *  O(1) membership in the detail tag, the navaid lists, and the map dim;
 *  gates on the parsed `serviceStatus` field. */
export function unserviceableNavaids(): Set<string> {
	...
	for (const m of navaidMatches()) {
		...
		if (!validIn(m.item.notam, at) || activeIn(m.item.notam, at) === false) continue;
		for (const n of m.navaids) {
			out.add(n.id);
		}
	}
