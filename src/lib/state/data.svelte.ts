...
const SUPAIP_SOURCES: { url: string; meta: () => Promise<unknown> }[] = [
	{ url: SUPAIP_URL, meta: loadSupAipMeta },
	{ url: BE_SUPAIP_URL, meta: loadBeSupAipMeta },
	{ url: ES_SUPAIP_URL, meta: loadEsSupAipMeta },
];
...
export function ensureSupaip(): Promise<SupAip[]> {
	...
	supaipPromise = (async () => {
		...
		// Each publisher's set loads alone: a fetch of one that was never
		// cached (offline after an update added a publisher) must not sink
		// the French and Belgian sets with it. Only every source failing
		// is the dataset failing.
		const settled = await Promise.allSettled(SUPAIP_SOURCES.map((s) => loadSupAip(s.url)));
		const parts: SupAip[][] = [];
