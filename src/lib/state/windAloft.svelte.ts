...
export function animationWindow(
	...
	const reachMs = Math.min(
		nowHour + windModel(model).horizonH * 3600_000,
		forecastRangeEndMs(nowMs),
	);
...
/** The hour window ONE lattice fetch asks for: the animation's whole frozen
 *  span while it is engaged, so a sweep replays from cache, else the shown
...
export function ensureWindGrid(view: GridView, showMs: number, nowMs = Date.now()): void {
	...
	const window = gridWindow(model, showMs, anchor, nowMs);
	...
	if (window.startMs > forecastRangeEndMs(nowMs)) {
		gridAbort?.abort();
		gridAbort = null;
		gridInflightKey = null;
		untrack(() => {
