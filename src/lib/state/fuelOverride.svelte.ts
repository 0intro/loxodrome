...
/** The panel's default: drawnStateAt(), the instant the map draws. Never the
 *  briefing window, whose look-ahead is unbounded by default and would take a
 *  withdrawal next month as today's; the flight-prep fuel plan passes the
 *  planned flight's span (timeWindow.svelte.ts plannedFlightAt). */
