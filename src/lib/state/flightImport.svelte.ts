...
/** Whether a batch import runs right now: the sync applier defers its
 *  pass behind it (docs/accounts-sync.md, "Apply passes serialize
 *  behind the same guards the batch importer uses"); one direction is
 *  enough, the importer's own writes being id-keyed upserts. */
export function flightImportRunning(): boolean {
	return importing;
}
...
async function runImportInner(items: readonly NamedText[]): Promise<void> {
	...
	for (const rf of read) {
		...
		if (rf.kind === 'routes') {
			...
			// A plan in an interchange format (FPL / GPX / KML / PLN) lands here
			// too, and stays unsupported ON PURPOSE: those files carry a route
			// and nothing else, no aircraft, no times, nothing this library
			// files, and the way to open one is the Route tab
			// (docs/route-files.md).
