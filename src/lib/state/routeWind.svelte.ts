...
/** The hour window ONE route fetch asks for: the flight's own span, widened
 *  to the animation slider's while that is engaged so a sweep never refetches
 *  the columns. Exported for the spec that pins its endpoint-range cut. */
export function routeFetchWindow(
	...
	if (anchor !== null) {
		...
		// derives from this window and stays stable across the sweep. The
		// slider's anchor is read as plain data on purpose: resolving it here
		// would mean importing state/wxTime.svelte.ts, which imports this
		// module for the departure below.
...
function routeWindKeyFor(waypoints: Waypoint[], departureMs: number, cruiseKt: number | null): { key: string; model: WindModelId; startMs: number; endMs: number } {
	...
	const { startMs, endMs } = routeFetchWindow(waypoints, departureMs, cruiseKt, model);
	return {
...
export function firstDepartureMs(): number {
	...
	return Number.isFinite(dayMs) ? Math.max(dayMs, nextHourMs(Date.now())) : nextHourMs(Date.now());
}
