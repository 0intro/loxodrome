...
import { currentPose, nav } from './navRecording.svelte';
...
export function overflownAirport(): OverflightInfo | null {
	...
	// At the pose's own instant, the band's display instant (navLive's tMs),
	// so a replay reads the frequency the flight had and a frequency change
	// dated next month does not reach the live cell.
	const unit = airportContactUnit(hit.airport, { fromMs: nav.playheadMs, toMs: nav.playheadMs });
