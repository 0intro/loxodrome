 ...
 * download, the resume, the quota check, the manifest and the one offline
 * queue they are both jobs on, and differ only in what a pack IS read
 * with. Everything here that is not in that file is
...
/** This document set's job on the offline queue. */
const jobRef = (id: DocPackId): JobRef => ({ kind: 'doc', id });
...
/* SAFETY, now that a download is a queued job: promotion renames
 * fr-vac-next onto fr-vac, so it must never overlap a run against either.
 * It cannot today, being called from exactly one place, inside
 * ensureOfflineDocs's initPromise at boot, before any UI exists to press
 * Download, and runDocPack awaits that same promise first. That invariant is
 * an ORDERING rather than a type: if promotion ever moves onto a tick (an
 * app left open across an AIRAC turnover is a real case), it must first
 * cancelDownload both doc jobs and await the drain. */
...
export async function ensureDocPackSizes(): Promise<void> {
	...
	/* `remote` is a plain Map, so WHETHER A PACK IS PUBLISHED
	 * (docPackPublished, the pre-release slot's whole test) is not reactive
	 * on its own. Without this bump a derived that asked the question before
	 * the HEAD answered never asked again, and the pre-release VAC row stayed
	 * off the manager for the whole month the SIA publishes it. `gen` already
	 * means "a pack appeared or went"; one becoming KNOWN is that. */
	if (learned) {
		offlineDocs.gen++;
	}
 ...
 *  nothing else. The screen wake lock is deliberately NOT here any more: the
 *  queue holds one for the whole drain, so a chart archive gets the same
 *  protection a document pack always had. */
async function runDocPack(
	id: DocPackId,
	archive: string,
	signal: AbortSignal,
	...
	await ensureOfflineDocs();
	const v = view(id);
	// At DEQUEUE: the jobs ahead of this one have been writing to the same
...
/** Ask for this document set. Enqueues and returns at once; the queue runs
 *  it in turn. Synchronous to its last line, like its chart sibling. */
export function downloadDocPack(id: DocPackId): void {
	if (!offlineDocs.supported) {
		const v = view(id);
		v.error = 'unsupported';
		v.status = 'error';
	...
	enqueueDownload(jobRef(id), {
		...
		run: (signal, onProgress) => runDocPack(id, def.archive, signal, onProgress),
	});
}

/** Stop this set's download: an abort while it runs (the part survives), a
 *  plain removal while it waits. */
export function cancelDocPack(id: DocPackId): void {
	cancelDownload(jobRef(id));
}

export function docPackQueued(id: DocPackId): boolean {
	return isQueued(jobRef(id));
}

export function docPackQueuePosition(id: DocPackId): number {
	return queuedAt(jobRef(id));
}

export function docPackRunning(id: DocPackId): boolean {
	return isRunning(jobRef(id));
}

export async function removeDocPack(id: DocPackId): Promise<void> {
	// Also drops a WAITING job, or deleting a queued set would leave the
	// queue about to download it again.
	cancelDocPack(id);
