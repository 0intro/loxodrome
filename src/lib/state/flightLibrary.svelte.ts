...
import { summarizeFlights, type LogbookCsvRow, type SummaryDeps } from '$lib/nav/logbook';
...
export async function archiveOuting(
	...
	if (!persistAsked) {
		...
		try {
			...
			// Android WebView, harmless everywhere.
...
// --- Module init -----------------------------------------------------------
// The hook registration (this module is statically imported through
// App.svelte -> FlightsModal, so it precedes any runtime stop) and the
