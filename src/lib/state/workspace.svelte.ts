...
/* The phone pane's cap: while it answers true (App registers the
 * recording), nothing may OPEN the pane at its full detent, so a chart, a
 * log or a followed link never takes the map from a pilot in flight
 * (docs/workspace-surfaces.md "Phones"). What the pilot's own hand does is
 * not capped: an explicit drag or handle tap is obeyed in flight exactly as
 * on the ground, because a gesture that silently springs back reads as a
 * broken control, and the map is theirs to cover if they ask. A hook rather
 * than an import, so this module never reads navRecording (the
 * setOutingSettledHook idiom). */
let paneCapped: () => boolean = () => false;

export function setPaneCapHook(fn: () => boolean): void {
	paneCapped = fn;
}

/** True for a surface that renders in the phone pane: map-coupled, on the
 *  phone layout. */
function inPane(id: SurfaceId): boolean {
	return ui.isMobile && surfaceDef(id).mapCoupled;
}
...
/** Whether a surface may STAY where it is when a detail opens. On desktop
 *  the detail is an overlay panel that insets for the docks, so a docked
 *  surface keeps the map and stays, which is the whole point of docking. On
 *  a PHONE the detail is the pane's third occupant and claims the very slot
 *  that surface holds, so it is evicted whatever its placement says: a
 *  profile reading only keepsMapVisible there left no breadcrumb back to
 *  itself and the pilot lost the chart they had followed a link out of. The
 *  mapPreview hover reads the same shape. */
export function surfaceSurvivesDetail(id: SurfaceId): boolean {
	return !ui.isMobile && surfaceKeepsMapVisible(id);
}
...
export function openSurface(id: SurfaceId): void {
	if (inPane(id)) {
		// The pane ALWAYS opens at its half detent, whatever was stored: the
		// remembered full-height sheet was the phone's worst defect (one
		// full-height use covered the map on every later open), and a tap
		// on the head's handle is the way up.
		place(id, mobilePlacement('dock-bottom', true, ui.isLandscapePhone));
		return;
	}
	const stored = readItem(placementKey(id));
	place(id, resolvePlacement(surfaceDef(id), stored, ui.isMobile, workspace.stage.w, ui.isLandscapePhone));
}

/** Close a surface: its slot, placement and open-ness.
 *
 *  NEVER call it from a teardown (an onDestroy, an effect's cleanup) that can
 *  run in the same flush as a slot write. Svelte answers a state read inside a
 *  teardown with the value from before the flush (runtime.js get():
 *  is_destroying_effect && old_values), so vacate's slot test compares against
 *  the slots as they WERE: the open flag and the placement go, and a slot the
 *  flush gave this surface stays claimed with nothing open in it. That is how
 *  a phone's pane was stranded by the detail panel's own chart
 *  (state/airspaceProfileModal.svelte.ts, which defers its close for this). */
...
export function frontmostSurface(): SurfaceId | null {
	// The detail has no paper: a bare Ctrl+P on a phone must not print the
	// panel over the surface behind it.
	if (workspace.overlay !== null && workspace.overlay !== 'detail') {
		return workspace.overlay;
	}
	const docks = [workspace.dockBottom, workspace.dockRight].filter(
		(id): id is SurfaceId => id !== null && id !== 'detail',
	);
...
export function movePlacement(id: SurfaceId, placement: Placement): boolean {
	...
	// A move is the user's own hand on the grip or the handle, which is what
	// lifts the in-flight cap; every other caller goes through place().
	return place(
		id,
		ui.isMobile ? resolvePlacement(surfaceDef(id), placement, true, 0, ui.isLandscapePhone) : placement,
		true,
	);
}
...
/** A surface's shell was destroyed: close the surface unless a shell for it
 *  has registered since (a remount). The shell calls it a microtask after its
 *  own teardown, since Svelte answers a state read inside a teardown with the
 *  value from before the flush. Without it, a shell under a condition that
 *  flipped left its slot claimed with no renderer, no closer and no control,
 *  the dock spacer reserving a strip nobody draws in. */
export function releaseOrphanedSurface(id: SurfaceId): void {
	if (isOpen(id) && !closers.has(id)) {
		closeSurface(id);
	}
}
...
function place(id: SurfaceId, requested: Placement, gesture = false): boolean {
	// Half at most while the pane is capped (a recording runs), unless the
	// PILOT asked for it: the full detent lands on the half one for
	// everything that merely opens or re-resolves the pane, and an explicit
	// move is obeyed.
	const capped =
		requested === 'page' &&
		inPane(id) &&
		!gesture &&
		// Nothing is being taken when the pane is already there: a reflow on
		// a rotation re-resolves what the pilot's own move put at full, and
		// capping that would undo their choice a turn of the phone later.
		placementOf(id) !== requested &&
		paneCapped();
	const placement = capped ? mobilePlacement('dock-bottom', true, ui.isLandscapePhone) : requested;
	...
	// The detail surface IS the detail: paging it must not close itself.
	if (placement === 'page' && id !== 'detail') {
		closeDetail();
	}
	return true;
}
...
function openingDockPx(id: SurfaceId, edge: DockEdge): number {
	...
	if (inPane(id)) {
		// The pane's half detent: a share of the stage, never a remembered
		// size (the phone remembers no pane geometry, see openSurface).
		const frac = edge === 'bottom' ? PHONE_DOCK_FRAC : PHONE_LANDSCAPE_FRACS[0];
		return stage <= 0
			? minDockPx(def, edge, true)
			: clampDockPx(def, edge, stage, Math.round(stage * frac), true);
	}
...
export function setDockPx(edge: DockEdge, px: number): void {
	...
	const floor = Math.min(minDockPx(surfaceDef(id), edge, inPane(id)), stage);
...
export function commitDockPx(edge: DockEdge, px: number): void {
	...
	workspace.dockPx[edge] = clampDockPx(surfaceDef(id), edge, stage, px, inPane(id));
	if (!inPane(id)) {
...
export function setStageSize(w: number, h: number): void {
	...
	for (const edge of ['bottom', 'right'] as const) {
		...
		const stage = stagePx(edge);
		if (id && stage > 0) {
			workspace.dockPx[edge] = clampDockPx(
				surfaceDef(id),
				edge,
				stage,
				workspace.dockPx[edge],
				inPane(id),
			);
		}
	}
...
export function reflowSurfaces(): void {
	for (const id of SURFACE_IDS) {
		...
		// The pane keeps its detent across a rotation (the current placement
		// stands in for the stored one, which the phone never writes); every
		// other surface re-resolves its stored choice.
		const target = resolvePlacement(
			surfaceDef(id),
			inPane(id) ? placementOf(id) : readItem(placementKey(id)),
			ui.isMobile,
			workspace.stage.w,
			ui.isLandscapePhone,
		);
		...
		if (incumbent !== null && incumbent !== id) {
			// Close THIS one rather than evict the incumbent: a reflow runs
			// out of the matchMedia handler, where taking a slot from a
			// surface the user never touched would appear out of nowhere.
			// The detail is the one surface whose reason to be open lives
			// outside the slot (ui.detail, which App's bridge re-asserts), so
			// closing the slot alone would have it reopen on the next flush
			// and evict that incumbent after all, which is the very thing
			// this branch refuses.
			closeSurface(id);
			if (id === 'detail') {
				closeDetail();
