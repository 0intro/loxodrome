...
export const routeSettings = $state<{
	...
	editMode: false,
	corridorRadiusNM: DEFAULT_CORRIDOR_RADIUS_NM,
...
/** Direct-To (route/directTo.ts): an ALTERNATE route from the present
 *  position to `target`, inserted right after the route being flown so it
 *  attaches to that trip, the flown route untouched. ONE undo step. Null at
 *  MAX_ROUTES (the caller says so). The caller pins the fold onto the new
 *  route (setNavRoutePin), which this module cannot import. */
export function directTo(
	target: WaypointSnap,
	from: { lat: number; lon: number },
	flownRouteId: string,
): Route | null {
	if (routes.list.length >= MAX_ROUTES) {
		return null;
	}
	recordUndo();
	const here: Waypoint = {
	...
	const r: Route = {
		id: newRouteId(),
		name: null,
		waypoints: [here, there],
		selectedWaypointId: null,
		alternate: true,
	};
	routes.list = insertDirectTo(routes.list, flownRouteId, r);
	return r;
}
