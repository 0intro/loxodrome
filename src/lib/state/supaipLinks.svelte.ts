...
import { supaipPublisherOf, type SupAip, type SupAipPublisher } from '$lib/data/supaip';
import {
	extractAipSupsFor,
 ...
 *  (`<publisher>:<year>/<number>`). Deliberately NOT notamLinks' private
 *  number-first supKey: a mismatch would silently yield zero matches.
 *  The publisher is part of the key because the numbering namespaces
 *  collide: France and Spain both publish a "149/26". */
function refKey(
	ref: { number: number; year: number },
	publisher: SupAipPublisher,
): string {
	return `${publisher}:${ref.year}/${ref.number}`;
}
...
// TRA / TSA join the French ZRT / ZIT / ZDT subjects: they are what
// Spain calls the areas its supplements reserve ("TRA DFN-26 ZONA E"),
// and the French supplements use them too. Candidate tokens only: a
// match still has to name a zone of a supplement the NOTAM itself cites.
const ZONE_PREFIX_RE =
	/\b(?:Z[RID]T(?:\/Z[RID]T)?|TRA|TSA)(?:\s+[A-Za-zÀ-ÿ0-9][A-Za-zÀ-ÿ0-9'’-]*){1,4}/gi;
...
/** True iff `notam` cites the SUP keyed by `key`
 *  (publisher:year/number) anywhere in its body. The NOTAM's own
 *  publisher supplies both the citation grammar and the namespace, so a
 *  Spanish NOTAM's "SUP 149/26" can never match the French 149/26. */
function citesSupKey(notam: Notam, key: string): boolean {
	const pub = notamSupPublisher(notam);
	if (!pub) {
		return false;
	}
	for (const r of extractAipSupsFor(notam.fullContent, pub)) {
		if (refKey(r, pub) === key) {
			return true;
		}
	}
 ...
 *  Only supplements in a citable numbering namespace (the ones
 *  supaipByRef indexes: France's metropole and Spain) are considered,
 *  matching the forward link.
 *
...
export function supZoneActivations(): Map<string, IndexedNotam[]> {
	...
	for (const item of items) {
		...
		const pub = notamSupPublisher(n);
		if (pub) {
			for (const r of extractActivatedSups(n.fullContent, pub)) {
				const s = byRef.get(refKey(r, pub));
				if (s) {
					cited.add(s);
				}
			}
 ...
 *  so a panel lists every activation regardless of its window. Reads the
 *  loaded supplements via supaipByRef, so it fills in once the dataset
 *  loads. */
export function activatesSupZones(notam: Notam): VisibleZone[] {
	...
	for (const r of extractActivatedSups(notam.fullContent, pub)) {
		const sup = supaipByRef(r.number, r.year, pub);
		if (!sup) {
			continue;
		}
 ...
 *  order. */
export function notamsReferencingSup(
	number: number,
	year: number,
	publisher: SupAipPublisher = 'fr',
): IndexedNotam[] {
	void notamState.tick;
	const key = refKey({ number, year }, publisher);
	// eslint-disable-next-line svelte/prefer-svelte-reactivity -- not reactive state
	const activatingIds = new Set(
		activatedNotamsForSup(supaipByRef(number, year, publisher)).map(
			(it) => it.notam.id,
		),
	);
	const out: IndexedNotam[] = [];
