...
// Cached on the obstacle array's identity: the merge replaces the array
// whenever a country arrives, so a reference compare is a sound invalidation
// test (the firRowsForIdent idiom). Without it the whole loaded set was
// re-indexed on every call, including once a minute from the map cue effect.
let nameIndexSrc: Obstacle[] | null = null;
let nameIndex: Map<string, Obstacle> | null = null;

/** Build the name → Obstacle index used by the tier-3 text-mention path. */
function indexByName(obstacles: Obstacle[]): Map<string, Obstacle> {
	if (obstacles === nameIndexSrc && nameIndex) {
		return nameIndex;
	}
	// eslint-disable-next-line svelte/prefer-svelte-reactivity -- not reactive state
...
export function activeNotamsByObstacle(): Map<string, IndexedNotam[]> {
	...
	for (const m of obstacleMatches()) {
		if (!isActiveDuring(m.item.notam, from, to)) continue;
		for (const o of m.obstacles) {
			const arr = out.get(o.id);
			if (arr) arr.push(m.item);
			else out.set(o.id, [m.item]);
		}
	}
	return out;
}

/** One obstacle-subject NOTAM and the obstacles it resolves to. */
interface ObstacleMatch {
	item: IndexedNotam;
	obstacles: Obstacle[];
}

// The matching stage, cached on the briefing and the loaded obstacles: it is
// the expensive half and does not depend on the evaluation window, so the
// minute tick re-runs only the date test above. Mirrors navaidMatches().
let matchCache: { items: IndexedNotam[]; obstacles: Obstacle[]; out: ObstacleMatch[] } | null = null;

function obstacleMatches(): ObstacleMatch[] {
	const obstacles = getObstacles();
	const items = filteredNotams();
	if (!obstacles || obstacles.length === 0) return [];
	if (matchCache && matchCache.items === items && matchCache.obstacles === obstacles) {
		return matchCache.out;
	}
	const byName = indexByName(obstacles);
	const out: ObstacleMatch[] = [];
	for (const item of items) {
		if (!isObstacleQCode(item.notam.qCode)) continue;
		const matched = matchObstaclesToNotam(item.notam, obstacles, byName);
		if (matched.length > 0) out.push({ item, obstacles: matched });
	}
	matchCache = { items, obstacles, out };
	return out;
}
