...
import { minGroundPerLeg, sampleProfile, terrainTileCacheStats, type TerrainSample } from '$lib/map/terrain';
...
export function ensureRouteTerrain(
	...
	untrack(() => {
		// A retry of the same signature keeps its samples on show until the
		// new ones land (the MSA memo's rule).
		const prev = routeTerrain.byRoute[routeId];
	...
	void sampleProfile(waypoints, { signal: ctrl.signal })
		.then((samples) => {
			...
			// A sample with no ground is the sea or a tile that failed; while
			// some tile has failed, arm the failure retry, since a profile
			// settles rather than throws and would otherwise keep the gap for
			// the session (a route over the sea alone, with nothing failed,
			// is not asked again).
			if (samples.some((s) => s.elevFt == null) && terrainTileCacheStats().failed > 0) {
