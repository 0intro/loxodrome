...
import type { FuelType } from '$lib/aircraft/schema';
...
export const airportSearch = $state<{ query: string; grades: FuelType[] }>({
	query: '',
	// Session-only, like every other view filter: a grade ticked to plan one
	// trip should not still be narrowing the list next week.
	grades: [],
});
...
export function searchAirports(keep?: (a: Airport) => boolean): AirportSearchResult {
	return searchAirportsFor(airportSearch.query, keep);
}

/** The same ranked lookup for a caller-supplied query (the search palette),
 *  leaving the Airports tab's own box untouched. */
export function searchAirportsFor(
	query: string,
	keep?: (a: Airport) => boolean,
): AirportSearchResult {
	const q = query.trim();
	...
	for (const a of all) {
		// The predicate runs BEFORE the cap, so a filter cannot be defeated
		// by a broad query whose first 500 matches happen not to pass it.
		// Injected rather than read here: this module stays free of the
		// datasets a caller filters on (the perfWxStops idiom).
		if (keep && !keep(a)) {
			continue;
		}
