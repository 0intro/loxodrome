...
import type { SuspendReason } from '$lib/nav/alertSurface';
import {
	airspaceVolumes,
	notamVolumes,
	supaipVolumes,
...
import { ensurePoseGround, poseGround } from './poseGround.svelte';
import { firstAirborneMs } from './airborne';
...
// The airborne watermark (position-referenced, no route needed) is shared
// with the terrain alerts: state/airborne.ts.
...
let lastEvalMs = 0;
/** The trace the session memory (acks, prev, sound queue, row order)
 *  belongs to; a different array is a different flight. */
...
export function airspaceAlerts(): AlertEvalResult | null {
	...
	// A REPLACED trace is a new session: an acknowledgement made on the
	// previous flight must not suppress this one's warnings, and the
	// presentation memory must not carry across. The backward-jump reset
	// below already covers a replay rewound in place, but an imported trace
	// whose stamps sit LATER than the previous one's slips past it. Array
	// identity is the trace identity (a live recording appends and the
	// MAX_POINTS splice mutates in place; importTrace hands a fresh array),
	// the airborne watermark's idiom (state/airborne.ts). The acks map is cleared without touching
	// ackRev: this runs inside consumers' deriveds, where a $state write is
	// unsafe, and the changed points already re-key the evaluation.
	...
	// The ground under the pose is the shared sample the band's AGL reads
	// too (state/poseGround): one answer for the height the pilot is at. The
	// sample's identity is part of the signature, so a new one re-evaluates.
	ensurePoseGround(pose.lat, pose.lon);
	const g = poseGround(pose.lat, pose.lon);
	const sig: unknown[] = [
		evalNow,
		ack,
		pose.lat,
	...
	// so the reconstruction is deterministic (the sound queue with it; the
	// stack's row order resets on the same rule in state/alertStack).
	if (evalNow < lastEvalMs - 1000) {
		prevState = emptyAlertPrev();
		firedQueue = [];
	}

	const groundKnown = g != null;
