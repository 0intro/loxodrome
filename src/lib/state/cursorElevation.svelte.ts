...
import { elevationFtAt, peekElevationFtAt, terrainPointDue } from '$lib/map/terrain';
...
function runProbe(): void {
	...
	void elevationFtAt(lat, lon)
		...
		.finally(() => {
			...
			// carried across, and none can land on the wrong point. Re-armed
			// only while a read would fetch something: over a tile that just
			// FAILED the peek stays unknown, and re-arming on that alone probed
			// every 150 ms and spent the bucket.
			if (!writeFromPeek() && badgeShown() && wanted && terrainPointDue(wanted.lat, wanted.lon)) {
				armTimer();
			}
		});
}
...
export function probeCursorElevation(lat: number, lon: number): void {
	...
	wanted = { lat, lon };
	if (writeFromPeek() || !terrainPointDue(lat, lon)) {
		// Known, one way or the other, or not askable yet (a tile that failed
		// a moment ago): nothing to ask for now.
		cancelProbe();
		return;
	}
	armTimer();
