...
export function buildTraceProfileDoc(input: TraceProfileInput): TraceProfileDoc {
	...
	//
	// The FIS sectors are dropped HERE rather than left to the filter row,
	// because the row state is shared across the profiles and turning it on
	// from the route chart would otherwise reach this one. They nest, so a
	// band per sector needs the in-force emphasis to say which one a pilot
	// calls (docs/route-profile.md "The sector in force"), and this chart
	// cannot compute it: `traceAsWaypoints` carries `alt: 0` on every vertex
	// and `inBand`'s floor test is strict, so an altitude-aware schedule walk
	// over a trace answers an empty timeline. Two frequencies and no answer
	// is worse than neither, so the trace shows neither until it can carry
	// the recorded altitude into that walk.
	const asWaypoints = traceAsWaypoints(thinned);
	const corridorBands = airspaces
		? computeAirspaceCorridorSpans(asWaypoints, airspaces).filter((b) => b.category !== 'siv')
		: [];
