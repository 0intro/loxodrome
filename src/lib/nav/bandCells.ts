...
/** The side a cross-track is flown on, in the app's own vocabulary: L / R in
 *  English, G / D in French. A pure module states no words, so the caller
 *  hands them in (the MetarWords idiom, docs/i18n.md). */
export interface XtkSides {
...
export function xtkCompact(xtkNM: number | null, side: -1 | 0 | 1, sides: XtkSides): string {
	if (xtkNM == null) {
		return '';
	}
	if (side === 0 || xtkNM < 0.1) {
	...
	return `${v}${side > 0 ? sides.right : sides.left}`;
}
...
export function resolveRingPos(
	...
	/** Which entries APPLY to this subject, as opposed to having a reading
	 *  right now. Defaults to `available`, which is right wherever an entry
	 *  is either there or not for the whole of a subject. It is NOT right
	 *  for a reading with a dependency that can lapse under a standing
	 *  aircraft: a terrain sample between tiles blanks AGL for a second, and
	 *  falling back to the primary there swapped the caption and the number
	 *  under the pilot's eye for a figure thousands of feet different. A
	 *  choice survives a gap in its reading; only a change of SUBJECT takes
	 *  it back. */
	applicable: readonly boolean[] = available,
): number {
