...
function motionWindow(points: TrackPoint[], tMs: number): MotionSample[] {
	...
	const n = points.length;
	if (n === 0 || points[0].timeMs > tMs) {
		return [];
	}
	// Largest index at or before tMs (the positionAt bracket), then walk down
	// through the window. The old scan started at the array's END and skipped
	// every future fix one by one, so a replay tick early in a long trace paid
	// a full-array walk per call, twice per tick (the live selector and the
	// alert evaluator both read this).
	let lo = 0;
	let hi = n - 1;
	while (lo < hi) {
		const mid = (lo + hi + 1) >> 1;
