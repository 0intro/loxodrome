...
export function parseKml(text: string): TrackPoint[] {
	...
	if (tracks.length > 0) {
		...
		// same scan, in document order.
		const points: TrackPoint[] = [];
		for (const body of tracks) {
			points.push(...trackPoints(body));
		}
		if (points.length > 0) {
			return points;
		}
 ...
 *  position after, so adjacency proves nothing. */
function trackPoints(body: string): TrackPoint[] {
	...
	for (let i = 0; i < coords.length; i++) {
		...
		point.timeMs = Number.isFinite(stamp) ? stamp : (clock ?? 0);
		clock = point.timeMs + 1000;
		out.push(point);
	}
	return out;
}
