...
/** Altitude rise (ft) from the takeoff streak's first known altitude that
 *  proves the aircraft left the ground. An ABORTED takeoff roll reaches
 *  50 kt of ground speed and holds it past the sustain (measured on the
 *  real engine-monitor corpus: three of them, each filing a phantom flight
 *  with a takeoff, a landing seconds later and a logbook row for wheels
 *  that never left), and speed alone cannot tell it from a departure. The
 *  rise is judged from the streak's own first known altitude, so the
 *  trace's datum cancels out. */
export const TAKEOFF_RISE_FT = 50;
/** Streak length (ms) past which the takeoff commits with no rise evidence:
 *  no abort keeps 40 kt on the ground this long, and a trace whose
 *  altitudes drop out mid-streak must not lose its departure. */
export const TAKEOFF_ROLL_GUARD_MS = 20_000;
...
/** Hold zone (NM) above the INITIAL seed: a projection identity fresh out
 *  of the seed is unproven (a closed route's departure projects at the
 *  route END and would stamp the destination during the takeoff roll), so
 *  a boundary crossed this close above the seed is stamped PROVISIONALLY:
 *  held in the fold, promoted to ink once the watermark reaches the top of
 *  the zone with the identity intact, discarded when the projection
 *  re-seeds inside it. The stamp's instant is the crossing either way; only
 *  its confirmation lands up to this far later (the takeoff's own "however
 *  late the sustain confirms it" idiom), which is what keeps the ATO of a
 *  first waypoint 2.7 NM out of the field, or of a waypoint a recording
 *  joins just short of, where a plain no-stamp zone dropped both.
 *
 *  A zone top the route can NEVER REACH promotes nothing, and that is
 *  deliberate rather than an oversight. It covers three cases and they are
 *  indistinguishable from inside this fold: the archetype, whose held
 *  destination stamp dies at the climb-out's re-seed and must; a whole
 *  route shorter than the zone; and a recording joined within the zone of
 *  its LAST waypoint. The two innocent ones therefore keep the behaviour
 *  they had before the hold existed, a blank ATO. The only signal that
 *  would separate them is the projection's own `sinceSeedNM` passing
 *  SEED_LOCK_NM, i.e. the identity having outlived the window in which
 *  routeProgress still accepts a backward correction; promoting on it
 *  would rest the archetype's whole protection on that correction always
 *  arriving inside two miles of ground track, and a fabricated ATO in a
 *  flight record is worse than a missing one. Blank is the safe failure,
 *  and it is the one kept. */
...
export interface MotionFold {
	...
	/** First and latest known altitude of the CURRENT fast streak (whichever
	 *  of the two streaks above is open; they are mutually exclusive): the
	 *  ground-roll evidence. Reset when the streak resets. */
	riseBaseAltFt: number | null;
	riseLastAltFt: number | null;
	 ...
	 *  stands. */
	landingsMs: number[];
	/** A KNOWN speed under MOVE_KT / under TAKEOFF_KT has been seen: the
	 *  evidence that the first movement / the takeoff streak has its rising
	 *  EDGE in the trace. A null speed (the first fix of a hop-backfilled
	 *  trace) is not known. */
	stillSeen: boolean;
	slowSeen: boolean;
	/** Whether firstMoveMs is an observed block-off: a fix under MOVE_KT
	 *  precedes it. False for a trace that opens already moving, whose first
	 *  fix merely continues a movement begun before it. */
	blockOutObserved: boolean;
	/** Whether takeoffMs is an observed departure: a fix under TAKEOFF_KT
	 *  precedes the committed streak, so the acceleration through the
	 *  threshold is in the trace. False while no takeoff is committed, and
	 *  for a trace that opens already fast (a recording started in flight, a
	 *  file cut mid-flight): takeoffMs is then the trace's first fast fix,
	 *  the instant the fold considers the flight airborne FROM (the crossing
	 *  gate, the landing watch and the display's airborne state all need
	 *  one), and no departure at all, so a departure row and a readout must
	 *  not print it as one. The re-arm after a landing is always observed (a
	 *  landing precedes it); the flag describes the scalar takeoffMs. Two
	 *  edges to know: a hop-backfilled speed over a repeated timestamp reads
	 *  0 kt (deriveMotion) and would count as seen, which the importer's
	 *  non-advancing-stamp drop keeps off files; and a recording started
	 *  mid-roll above 40 kt reads unobserved and blanks a departure that was
	 *  seconds away, the honest side to err on. */
...
export interface CrossingFold {
	...
	nextB: number;
	/** Top (NM) of the initial seed's hold zone (SEED_GUARD_NM above the
	 *  seed); null once retired: promoted at its top, or discarded by a
	 *  re-seed inside it. Never set on a chained leg. */
	guardTopNM: number | null;
	/** Stamps made inside the hold zone, not yet ink: the boundary index and
	 *  the lerped crossing instant. Promoted whole or discarded whole. */
	held: { b: number; ms: number }[];
	/** crossMs[k] = first time over boundariesNM[k] (waypoint k+1); null =
	 *  not reached, behind the seed (mid-route join), held in the seed zone
	 *  and then discarded by a re-seed, or crossed inside a jump segment
	 *  (passed without an ATO). */
	crossMs: (number | null)[];
}
...
/** Whether the current fast streak has proven itself a departure rather
 *  than a ground roll: it rose TAKEOFF_RISE_FT from the streak's first
 *  known altitude, or the trace carries no altitude to judge by at all
 *  (the null-tolerant fallback: a speed-only trace keeps the old
 *  sustain-alone rule). The INITIAL streak may also prove it by outliving
 *  TAKEOFF_ROLL_GUARD_MS, which no aborted roll keeps 40 kt through; the
 *  RE-ARM streak after a landing may not, because the thing that does hold
 *  40 kt on a flat line for twenty seconds after a flight is the car
 *  driving home under the auto-stop grace, and a second departure always
 *  climbs. The commit instant stays the streak START either way; only the
 *  confirmation may arrive later, the "however late the sustain confirms
 *  it" semantics the fold always had. */
function streakLeftTheGround(
	fold: MotionFold,
	nowMs: number,
	sinceMs: number,
	rearm: boolean,
): boolean {
	if (fold.riseBaseAltFt == null || fold.riseLastAltFt == null) {
		return true;
	}
	if (fold.riseLastAltFt - fold.riseBaseAltFt >= TAKEOFF_RISE_FT) {
		return true;
	}
	return !rearm && nowMs - sinceMs >= TAKEOFF_ROLL_GUARD_MS;
}
	...
	for (let i = fold.idx; i < n; i++) {
		...
		if (speed != null && speed >= MOVE_KT) {
			if (fold.firstMoveMs == null) {
				...
				fold.blockOutObserved = fold.stillSeen;
			}
		...
		if (fold.takeoffMs == null) {
			if (speed != null && speed >= TAKEOFF_KT) {
				if (fold.aboveSinceMs == null) {
					...
					fold.riseBaseAltFt = null;
					fold.riseLastAltFt = null;
				}
				if (p.altFt != null) {
					fold.riseBaseAltFt ??= p.altFt;
					fold.riseLastAltFt = p.altFt;
				}
				if (
					p.timeMs - fold.aboveSinceMs >= TAKEOFF_SUSTAIN_MS &&
					streakLeftTheGround(fold, p.timeMs, fold.aboveSinceMs, false)
				) {
					fold.takeoffMs = fold.aboveSinceMs;
					// A slow fix seen before the streak means the streak ROSE
					// through the threshold inside the trace; none means the
					// trace opened inside it, and the instant is inherited.
					fold.takeoffObserved = fold.slowSeen;
			...
			// continues, so the landing stamp reopens for the real one. The
			// ground-roll gate applies here too: a fast taxi or an aborted
			// go must not resurrect the flight.
			...
			if (
				p.timeMs - fold.fastSinceMs >= TAKEOFF_SUSTAIN_MS &&
				streakLeftTheGround(fold, p.timeMs, fold.fastSinceMs, true)
			) {
...
export function extendCrossings(
	...
	while (fold.idx < n) {
		...
		if (fold.watermarkNM == null) {
			...
			// consumed without an ATO, and the ones just above it stamp into
			// the hold zone until the identity proves itself to the zone's top
			// or a re-seed corrects it.
		...
		// A forward segment longer than CROSS_JUMP_NM is a re-seed / GPS gap,
		// and a SHRINKING sinceSeedNM is the projection fold's own report that
		// it replaced its identity at this checkpoint: it only ever shrinks on
		// a seed (a duplicate fix keeps it, a pin sets Infinity, a hand-built
		// prefix reads Infinity). Either one inside the hold zone means the
		// held stamps were made under an identity now abandoned.
		const jump = dCur - a.distNM > CROSS_JUMP_NM;
		const reseeded =
			c.sinceSeedNM != null && a.sinceSeedNM != null && c.sinceSeedNM < a.sinceSeedNM;
		if (dCur < a.distNM - BACKTRACK_NM) {
			...
			// and survive (the landing flip must not erase the flight). Held
			// stamps are not ink and go with the identity that made them.
			...
			if (fold.guardTopNM != null && (reseeded || jump)) {
				// The identity the hold zone was waiting on is gone: its
				// provisional stamps with it, and the zone with them. A report
				// that REGRESSED (under BACKTRACK_NM, else the branch above took
				// it) also reopens the boundaries the corrected identity has yet
				// to reach, inked ones excepted (the stamping loop never rewrites
				// a stamp). A FORWARD report leaves the watermark alone: the
				// forward branch below consumes what it passed, and moving the
				// watermark first would hand those boundaries a fabricated time
				// at the next segment's start.
				fold.held.length = 0;
				fold.guardTopNM = null;
				if (reseeded && dCur < fold.watermarkNM) {
					fold.watermarkNM = dCur;
					while (fold.nextB > 0 && boundariesNM[fold.nextB - 1] > dCur) {
						fold.nextB--;
					}
			...
			if (dCur > fold.watermarkNM) {
				...
				while (fold.nextB < boundariesNM.length && boundariesNM[fold.nextB] <= dCur) {
					...
					if (!jump && fold.crossMs[fold.nextB] == null) {
						const span = dCur - a.distNM;
						const f = span > 0 ? Math.max(0, Math.min(1, (b - a.distNM) / span)) : 1;
						const ms = a.timeMs + f * (c.timeMs - a.timeMs);
						if (fold.guardTopNM != null && b <= fold.guardTopNM) {
							fold.held.push({ b: fold.nextB, ms });
						} else {
							fold.crossMs[fold.nextB] = ms;
						}
					}
					fold.nextB++;
				}
				fold.watermarkNM = dCur;
				if (fold.guardTopNM != null && dCur >= fold.guardTopNM) {
					// The identity held to the top of the zone: what it stamped
					// there is ink.
					for (const h of fold.held) {
						fold.crossMs[h.b] = h.ms;
					}
					fold.held.length = 0;
					fold.guardTopNM = null;
				}
...
export interface LiveNavlogInput {
	 ...
	 *  the junction departure for a leg chained onto a preceding one. Omitted
	 *  = the outing's takeoff when the motion fold observed it
	 *  (takeoffObserved), else none; an explicit null is no departure. */
...
export function liveNavlogAt(a: LiveNavlogInput): LiveNavlogState {
	...
	// otherwise the takeoff, and only when the trace holds it. A trace that
	// opens in flight has an airborne instant to gate on and no departure to
	// print, so its row stays blank and the chain anchors on the ETD.
	const departure =
		a.departureMs !== undefined
			? a.departureMs
			: a.motion.takeoffObserved
				? a.motion.takeoffMs
				: null;
