...
export async function computeCorridorGround(
	...
	for (let i = 0; i + 1 < n; i++) {
		...
		// A tile the server holds none of at or below the complete level is
		// the open sea (visitTiles counts it apart from the missing), and the
		// sea is sea level: the leg's ground reaches 0 ft there, which the
		// relief heuristic reads, and a leg over nothing else reads sea level
		// rather than unknown. Counted as missing, a leg across open sea had
		// no MSA at all.
		const lost = ground.missing > 0;
		if (!lost && ground.sea > 0) {
			lMax = Math.max(lMax ?? 0, 0);
			lMin = Math.min(lMin ?? 0, 0);
		}
		legMaxFt.push(lost ? null : (lMax ?? 0));
		legMinFt.push(lost ? null : (lMin ?? 0));
