...
/** The corridor half-width a briefing defaults to, in NM, either side of the
 *  track. SOFIA is asked for this as `width` and reports back a `routeWidth` of
 *  twice it; the autorouter corridor selects its aerodromes and FIRs with it.
 *
 *  Exported so it has ONE spelling: the flight app seeds routeSettings from it
 *  and the NOTAM Viewer, which has no route workspace to carry a per-plan
 *  value, sends it directly. A second literal is how the viewer came to brief a
 *  1 NM corridor between two aerodromes, which is a track and not a route. */
export const DEFAULT_CORRIDOR_RADIUS_NM = 15;
