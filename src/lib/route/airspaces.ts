...
export interface RouteAirspaceEvent {
	...
	vUpper: VLimit | null;
	radio: AirspaceRadio[];
	/** The airspace's published operating hours, carried verbatim so the
	 *  radio resolution can ask whether the unit is on watch over the flight
	 *  without a second lookup (route/airspaceHours.ts). */
	workHr: string;
	rmkWorkHr: string;
}
 ...
 *  conservative: a missing side is unbounded, and an AGL/ASFC limit resolves
 *  against the sample's own ground band when the caller has terrain, else
 *  through its conservative endpoint (floor at its minimum, ceiling
 *  unbounded), so a terrain-hugging zone is included until terrain proves
 *  otherwise. The band's two ends are not interchangeable: the FLOOR takes the
 *  lowest ground under the sample and the CEILING the highest, so neither edge
 *  is quietly pushed away from the aircraft by ground it never flew over. The
 *  lower bound is STRICT: flying at the floor (e.g. 1500 ft into a 1500
 *  ft-base zone) does not count as inside, so the schedule only lists zones
 *  whose floor is below the leg altitude. */
function inBand(a: Airspace, altFt: number, ground: GroundBand = GROUND_UNKNOWN): boolean {
	const lo = a.vLower ? floorMinFt(a.vLower, ground.minFt) : -Infinity;
	const hi = a.vUpper ? ceilingMaxFt(a.vUpper, ground.maxFt) : Infinity;
...
/** Ground band per walk sample, matched by nearest distNM against the route's
 *  terrain profile (the caps differ: the walk samples up to 1000 points,
 *  sampleProfile 600, so index mapping would drift). Two-pointer over the two
 *  distance-sorted arrays; a sample carries the LOWEST and HIGHEST ground of
 *  its own footprint, and failed tiles stay null on both ends, which the
 *  vertical predicates treat conservatively. Null when no terrain was supplied
 *  at all. */
function groundAtSamples(
	...
	const out: GroundBand[] = new Array(samples.length) as GroundBand[];
	let j = 0;
	for (let i = 0; i < samples.length; i++) {
		...
		out[i] = { minFt: sampleFloorFt(terrain[j]), maxFt: sampleCeilingFt(terrain[j]) };
	}
	return out;
}
...
/** One stretch of the route over which a given contact is THE one in force. */
export interface ContactStretch {
	key: string;
	fromNM: number;
	toNM: number;
}

/** Which contact is in force over each stretch of the route: the spans cut at
 *  every edge, each interval resolved through the SAME `contactStateAt` ladder
 *  the nav log and the in-flight band read, and adjacent intervals with the
 *  same winner coalesced.
 *
 *  This is what lets a vertical profile draw a FIS sector honestly. The
 *  sectors NEST - at 2000 ft over Melun a flight is inside both SIV SEINE 1
 *  and FIC PARIS SUD - so a band per sector says nothing about which one to
 *  call, and two frequencies with no answer is worse than none. The stretches
 *  say it: the AIP settles it on the FIC row's own remark ("Excluding areas
 *  where the flight information service is provided by an approach control
 *  unit"), `outranksContact` implements it, and the chart emphasises the
 *  sector over the stretch it wins.
 *
 *  `toNM` is clamped to `totalNM`, since a span the flight never leaves
 *  carries Infinity. Pure. */
export function contactStretches(spans: ContactSpan[], totalNM: number): ContactStretch[] {
	const edges = [...new Set(spans.flatMap((s) => [s.enterNM, Math.min(s.leaveNM, totalNM)]))]
		.filter((d) => Number.isFinite(d) && d >= 0 && d <= totalNM)
		.sort((a, b) => a - b);
	const out: ContactStretch[] = [];
	for (let i = 0; i + 1 < edges.length; i++) {
		...
		// Resolve at the MIDPOINT: an edge belongs to the interval that starts
		// there, and contactStateAt's window is [enter, leave).
		const key = contactStateAt(spans, (from + to) / 2).current?.ev.key ?? '';
		if (key === '') {
			continue;
		}
		const last = out[out.length - 1];
		if (last && last.key === key && Math.abs(last.toNM - from) <= 1e-9) {
			last.toNM = to;
			continue;
		}
		out.push({ key, fromNM: from, toNM: to });
	}
	return out;
}
...
export function computeAirspaceCorridorSpans(
	...
	const candidates = airspaces.filter(
		(a) =>
			scheduleRelevant(a) &&
			// Class G airspace is clutter on the profile. Aerial activity and
			// the FIS sectors are NOT dropped here any more: the walk reports
			// what the track crosses and the pilot's filter rows decide what is
			// drawn, which is the better split and the only way those rows can
			// exist. A FIS sector earns its band because 94 of the 110 French
			// ones have their CEILING inside the VFR band, so which sector is
			// in force changes with ALTITUDE - and a vertical profile is the
			// only surface that can show where in a climb that happens.
			a.airClass !== 'G' &&
			bboxesOverlap(box, a.bbox),
	);
 ...
 *  computeAirspaceCorridorSpans it also keeps class G, the only difference
 *  left between the two candidate sets now that the profile walk reports
 *  activity and FIS and lets the filter rows decide. Per-route
