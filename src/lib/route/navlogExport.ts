...
function frequenciesFor(w: Waypoint, airportByIdent: Map<string, Airport>): string | undefined {
	if (w.kind === 'airport' && w.ident) {
		...
		if (ap && contactRadios(ap).length > 0) {
			// Apply any frequency-change NOTAM in force over the flight, so the
			// saved doc reports the same value as the on-screen nav log.
			const radios = resolveAirportRadios(ap, plannedFlightAt()).radios;
			if (radios.length > 0) {
