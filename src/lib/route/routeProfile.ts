...
/** Distance-ordered edge vertices for one span of one limit: flat (two
 *  points) for AMSL / FL / unknown-datum limits or without terrain,
 *  terrain-following for AGL/ASFC ones (height added to each terrain
 *  sample inside the span, span ends anchored on the nearest sample;
 *  failed tiles fall back to the bare height). Shared with the NOTAM
 *  profile bands (route/notamProfile.ts), so their F)/G) AGL limits
 *  follow the terrain exactly like airspace bands. */
export function spanEdge(
	...
	const pts: AltitudeVertex[] = [
		{ distNM: span.enterNM, altFt: terrainFtAt(terrain, span.enterNM) + l.ft },
	];
	for (const s of terrain) {
		if (s.distNM > span.enterNM && s.distNM < span.leaveNM) {
			pts.push({ distNM: s.distNM, altFt: (s.elevFt ?? 0) + l.ft });
		}
	}
	pts.push({ distNM: span.leaveNM, altFt: terrainFtAt(terrain, span.leaveNM) + l.ft });
	return pts;
}
	...
	return bands
		.map((b): PlacedBand => {
			...
			const spans: PlacedSpan[] = b.spans.map((span) => {
				const lowerPts = spanEdge(b.vLower, span, terrain, 0);
				const upperPts = openTop ? [] : spanEdge(b.vUpper, span, terrain, 0);
				let botFt = Infinity;
...
export function terrainFtAt(terrain: TerrainSample[], targetNM: number): number {
