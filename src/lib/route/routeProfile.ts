 ...
 *  else the plan at the floor times the zoom, so the box never shows less
 *  than the floor's scale. */
export function stripPlotWidth(totalNM: number, boxW: number, zoom = 1, floor = MIN_PX_PER_NM): number {
	...
	return Math.max(box, Math.ceil(Math.max(0, totalNM) * floor * Math.max(1, zoom)));
}
