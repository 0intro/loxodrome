...
export function xOf(
	...
	return padL + ((distNM - fromNM) / span) * innerW;
}
...
export function yOf(
	...
	return padT + ((ceilingFt - ft) / span) * innerH;
}

/* ---- the phone strip: a distance axis that scrolls instead of fitting ----
 *
 * On a phone the chart is a strip the pilot pans sideways (docs/route-profile
 * .md "Phones: the strip"): the window model keeps the altitude axis, the
 * distance axis is the scroller's own. These are its pure rules. */

/** The floor on the distance scale, px per NM. Below it a 50 NM plan fits
 *  a phone's width and every waypoint label collides with the next; at it
 *  the approved mock read the pilot's 52 NM plan at a glance, the idents
 *  all kept (tests/routeProfileFit.spec.ts). Pinch zoom multiplies it. */
export const MIN_PX_PER_NM = 20;

/** The strip's drawn width: the box when the whole plan fits at the floor,
 *  else the plan at the floor, and the pinch zoom multiplies whichever of
 *  the two is in force. Multiplying the FLOOR alone left the zoom doing
 *  nothing on a plan short enough to fit, the box winning until the product
 *  overtook it: a 10 NM plan in a 392 px box did not move until zoom 1.96
 *  and then topped out at 4x instead of the ring's 8. */
export function stripPlotWidth(totalNM: number, boxW: number, zoom = 1, floor = MIN_PX_PER_NM): number {
	const box = Math.max(1, Math.round(boxW));
	const base = Math.max(box, Math.ceil(Math.max(0, totalNM) * floor));
	return Math.ceil(base * Math.max(1, zoom));
}

/** Where the scroller rests to hold the aircraft mid-box, clamped to the
 *  strip's ends (an aircraft near an end sits off-centre rather than
 *  scrolling past the plan). */
export function followScrollLeft(playheadPx: number, boxW: number, svgW: number): number {
	const max = Math.max(0, svgW - boxW);
	return Math.min(max, Math.max(0, playheadPx - boxW / 2));
}

/** The scroll position that keeps the NM under a pinch's midpoint at the
 *  same box pixel after the scale changed (zoomWindow's anchor rule for a
 *  scroller), clamped to the strip. */
export function zoomScrollLeft(
	...
	const max = Math.max(0, svgW - boxW);
	return Math.min(max, Math.max(0, anchorNM * pxPerNM - anchorBoxX));
}
...
/** One FIS sector's in-force stretches plus the line to call it by, already
 *  composed (the `obstacles & { tip }` idiom: no catalog reaches the chart).
 *  Shared by the route-profile modal and the dossier print, which must agree
 *  or paper stops matching the screen. */
export interface ContactStretchView {
	spans: PenetrationSpan[];
	line: string;
}
...
export function bandPenetrations(
	...
	// A FIS sector is a SERVICE, and a flight is inside one essentially all
	// the time: every French sector runs from the surface. "You are inside"
	// is not news, and the emphasis would cover the sector's whole extent -
	// swamping the one mark that IS news, the stretch where that sector is
	// the contact in force (docs/route-profile.md "The sector in force").
	// The same reason `bandCrossings` leaves them off the NO-GO strip.
	if (band.category === 'siv') {
		return [];
	}
...
export function bandCrossings(
	...
	for (const band of bands) {
		// A FIS sector is a SERVICE, not a volume to clear: there is nothing
		// to act on in entering one, and this strip is the NO-GO review. The
		// sectors reach the chart as bands (docs/route-profile.md "The sector
		// in force") and would otherwise add a chip per sector here, in force
		// or not, whether or not the pilot has that row on.
		if (band.category === 'siv') {
			continue;
		}
