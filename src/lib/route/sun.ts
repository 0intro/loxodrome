...
import type { SunLookup } from '$lib/notam/schedule';
...
/** A D)-item sun lookup (`notam/schedule.ts`) for a fixed position: the
 *  day's sunrise and sunset in minutes from UTC midnight. Answers null where
 *  the sun does not cross the horizon that day, which leaves an SR/SS
 *  schedule UNKNOWN rather than guessed, the same asymmetry the rest of the
 *  D) reading keeps. */
export function sunLookupFor(latDeg: number, lonDeg: number): SunLookup {
	return (dayStartMs: number) => {
		const t = sunTimesUtc(latDeg, lonDeg, new Date(dayStartMs).toISOString().slice(0, 10));
		if (!t || t.sunrise.kind !== 'time' || t.sunset.kind !== 'time') {
			return null;
		}
		return { sunriseMin: t.sunrise.minutesUtc, sunsetMin: t.sunset.minutesUtc };
	};
}
