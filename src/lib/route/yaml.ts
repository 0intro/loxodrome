...
/** The plan's own `name:` read surgically (withPlanName's sibling; the
 *  sync conflict-copy captioner needs the current caption without the
 *  cost of a full parse). Null on no name, a parse error or a non-map
 *  root. */
export function readPlanName(text: string): string | null {
	const doc = parseDocument(text);
	if (doc.errors.length > 0 || !isMap(doc.contents)) {
		return null;
	}
	const raw = doc.get('name');
	return typeof raw === 'string' ? normalizePlanName(raw) : null;
}
