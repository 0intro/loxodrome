/* DEFLATE both ways, through the platform's own streams.
 *
 ...
 * library is evictable browser storage. A fourth caller, the precipitation
 * radar (weather/radarDecode.ts), inflates the zlib-WRAPPED tiles of a
 * GeoTIFF, the one other flavour these streams speak.
 *
 * "Raw" is deflate with no zlib wrapper, which is what ZIP carries; a TIFF's
 * Deflate tile carries the wrapper (RFC 1950). The streams are present in
 * every target browser and in Node, so this module and everything over it
 * is testable without a harness, and no dependency is added for it. Pure
 * (no Svelte, no I/O). */
...
/** Decompress a zlib-wrapped stream (RFC 1950) under the same cap. */
export async function inflateZlib(data: Uint8Array, maxBytes: number): Promise<Uint8Array> {
	return drain(
