...
/** Is this member the DOCUMENT the archive is packaging, rather than one
 *  entry of a library? A KMZ is a zipped KML and nothing else, so its one
 *  member is the file the user opened, trace or route alike. Everything else
 *  holding one plan IS a library with one plan in it, and treating that as a
 *  loose route file would load it straight into the workspace and replace
 *  what the pilot is flying. */
function isPackagedDocument(m: ArchiveMember): boolean {
	if (m.kind === 'gpx' || m.kind === 'igc' || m.kind === 'kml') {
		return true;
	}
	// A route KML sniffs as a plan; only its own name still says KMZ.
	return m.kind === 'plan' && m.name.toLowerCase().endsWith('.kml');
}
...
export async function readIncoming(name: string, bytes: Uint8Array): Promise<Incoming> {
	...
	if (members.length === 1 && isPackagedDocument(only)) {
