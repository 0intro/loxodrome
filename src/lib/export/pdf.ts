...
export async function exportMapPdf(map: LeafletMap): Promise<void> {
	...
	try {
		const canvas = await html2canvas(container, {
			...
			allowTaint: true,
			logging: false,
			// The radar composite never reaches paper (ICAO Doc 8896 5.3.1.2
			// keeps radar imagery out of flight documentation; the strip that
			// would date it is outside the container anyway).
			ignoreElements: (el) =>
				el.classList.contains('leaflet-control-container') || el.classList.contains('leaflet-radar-pane'),
		});
