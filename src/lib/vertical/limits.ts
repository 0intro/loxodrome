...
/** The ground under a FOOTPRINT: its lowest and highest elevation, feet AMSL.
 *
 *  The two ends travel together because no single scalar is conservative for
 *  both edges of an AGL band: a floor sits at height + ground, so the LOWEST
 *  ground keeps the zone from being hidden above the aircraft, while a ceiling
 *  sits at height + ground too, so the HIGHEST ground keeps it from being cut
 *  off below. One value fed to both silently biased one edge the moment the
 *  ground stopped being a single point. Either end may be null (no terrain),
 *  which the two helpers already read as their conservative endpoint. */
export interface GroundBand {
	minFt: number | null;
	maxFt: number | null;
}

/** The band of a POINT query, which has no footprint: both ends are the same
 *  reading. Naming it keeps a point and a footprint distinguishable at the
 *  call site instead of collapsing back into one scalar. */
export function groundBandAtPoint(ft: number | null | undefined): GroundBand {
	const v = ft ?? null;
	return { minFt: v, maxFt: v };
}

/** Ground with nothing known about it. */
export const GROUND_UNKNOWN: GroundBand = { minFt: null, maxFt: null };
 ...
 *  limits use their conservative endpoints unless a `GroundBand` is given,
 *  the floor reading its low end and the ceiling its high one. */
export function bandIntersects(
	lower: VLimit | null,
	upper: VLimit | null,
	band: FtBand,
	ground: GroundBand = GROUND_UNKNOWN,
): boolean {
	const lo = lower ? floorMinFt(lower, ground.minFt) : -Infinity;
	const hi = upper ? ceilingMaxFt(upper, ground.maxFt) : Infinity;
