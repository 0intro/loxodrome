...
export interface SurfaceDef {
	...
	minWidthPx: number;
	/** The same floor for a PHONE, where a surface may carry a compact
	 *  rendition narrower than its desktop grid (the nav log's compact
	 *  chevron sheet). Read by phoneFitted instead of minWidthPx, so the
	 *  surface docks under the map rather than going full screen. */
	phoneMinWidthPx?: number;
...
const DEFS: readonly SurfaceDef[] = [
	...
	},
	{
		...
		minWidthPx: 812,
		// The compact rendition a phone renders (NavLogSheet's `compact`):
		// its 376px pane plus the body padding, so the log docks under the
		// map on a phone instead of taking the screen.
		phoneMinWidthPx: 400,
		minDockHeightPx: 200,
		defaultDockFrac: 0.4,
	},
	{
		// The feature detail on a PHONE (an aerodrome's sheet, a NOTAM, an
		// airspace): the pane's third occupant beside the nav log and the
		// profiles, so that opening one puts the other away through the
		// workspace's own arbitration instead of a second container fighting
		// for the same edge (docs/workspace-surfaces.md "Phones"). Desktop
		// never opens it: the detail panel there is the side overlay.
		id: 'detail',
		placements: ['dock-bottom', 'dock-right', 'page'],
		mapCoupled: true,
		minWidthPx: 320,
		minDockHeightPx: 160,
		defaultDockFrac: 0.45,
	},
	{
		id: 'flightPrep',
	...
	{
		// The offline manager (docs/offline-maps.md): every download the app
		// can make, in one place. A catalog of rows with actions, so it
		// opens as a page like the flights library. dock-right stays
		// available because deciding WHICH chart to pull is a coverage
		// question the map answers, and because a queue draining for an hour
		// should not have to hold the whole stage. Not a dialog: a modal box
		// cannot be left up beside the map, and the point of a queue is to
		// start it and go on planning.
		id: 'offline',
		placements: ['page', 'full', 'dock-right'],
		mapCoupled: false,
		// The action cluster (two .btn at the 44px touch floor plus their
		// gap) beside a label column wide enough for the widest chart
		// product name and its state line, plus the body's padding. Below
		// that the row's grid stacks the actions under the label and the
		// body scrolls.
		minWidthPx: 420,
		minDockHeightPx: 260,
		defaultDockFrac: 0.5,
	...
	{
		// The account surface (docs/accounts-sync.md, "UI surface"): the
		// management dialog behind the toolbar chip. Dialog-only like
		// About; phones collapse it to full via resolvePlacement.
		id: 'account',
		placements: ['dialog'],
		mapCoupled: false,
		minWidthPx: 340,
...
/** The phone pane's two detents (docs/workspace-surfaces.md "Phones"): in
 *  portrait the half detent is a bottom dock at this share of the stage and
 *  the full one is the page placement (the whole stage, the toolbar and the
 *  bar still visible); in landscape both are right docks, at these shares of
 *  the stage width, since a page there would hide the map and the band. */
export const PHONE_DOCK_FRAC = 0.45;
export const PHONE_LANDSCAPE_FRACS: readonly [number, number] = [0.4, 0.62];

/** How a placement renders on a phone, where there is no room beside the map
 *  and no in-flow sidebar to preserve. A map-coupled surface is the pane:
 *  it docks under the map, or takes the stage as a page at the full detent
 *  (never `full`, which would cover the toolbar and the bar); in landscape
 *  it docks on the right. Everything else collapses to full screen. */
export function mobilePlacement(placement: Placement, mapCoupled = false, landscape = false): Placement {
	if (mapCoupled) {
		if (landscape) {
			return 'dock-right';
		}
...
export function resolvePlacement(
	...
	const collapsed = phoneFitted(def, mobilePlacement(chosen, def.mapCoupled, landscape), stageW);
	if (collapsed === chosen || def.placements.includes(collapsed)) {
		return collapsed;
	}
	return phoneFitted(def, mobilePlacement(defaultPlacement(def), def.mapCoupled, landscape), stageW);
}
...
function phoneFitted(def: SurfaceDef, placement: Placement, stageW: number): Placement {
	...
	return (def.phoneMinWidthPx ?? def.minWidthPx) > stageW * PHONE_DOCK_WIDTH_TOLERANCE ? 'full' : placement;
}
...
/** The phone pane's floor on either edge. The detents decide the pane's
 *  size (docs/workspace-surfaces.md "Phones"), so the content floor, which
 *  for the nav log is the DESKTOP grid's 812 px, must not: at the landscape
 *  narrow detent the compact sheet pans sideways inside the pane, which is
 *  the mock's decision. Below this a pane is a keyhole no detent offers. */
export const PHONE_PANE_MIN_PX = 160;

/** The surface's own floor along a dock axis: its minimum width for a side
 *  dock, its minimum height for a bottom one; the pane's floor on a phone. */
export function minDockPx(def: SurfaceDef, edge: DockEdge, phone = false): number {
	if (phone) {
		return PHONE_PANE_MIN_PX;
	}
...
export function clampDockPx(
	...
	const floor = Math.min(minDockPx(def, edge, phone), ceiling);
...
export function dockRelease(
	...
	return { kind: 'size', px: clampDockPx(def, edge, stagePx, px, phone) };
}
...
export function pageRelease(
	...
	return { kind: 'size', px: clampDockPx(def, 'bottom', stagePx, px, phone) };
}
