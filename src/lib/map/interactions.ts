 ...
 * featureAt resolves the single feature a click should select;
 * contextFeaturesAt gathers every feature stacked there for the right-click
 * menu. Kept out of MapView so the ordering lives in one place (it drives both
 * the click selection and the hover cursor) and can be unit-tested.
 *
 * featureAt is THE arbiter: no map layer resolves its own click. A filled area
 * that kept a Leaflet click handler would shadow everything beneath it
 * whatever its size, which is what a NOTAM area used to do to the SUP AIP zone
 * it activates; only lines and pins, the targets a pilot aims at, keep one
 * (docs/map-hit-testing.md). Two tiers:
 *
 *   1. aimed targets, topmost pane first, first hit wins: METAR station (415)
 *      > airport (400) > nature (395) > navaid (390) > obstacle (380). Each is
 *      a pixel-radius probe against a drawn glyph.
 *   2. areas, SMALLEST DRAWN SHAPE first, whatever kind drew it: NOTAM (450),
 *      SIGMET (435), SUP AIP (365/360), airspace (350). Each kind picks its own
 *      winner and reports the footprint of the shape that contained the point;
 *      ties fall back to pane order, which is the order this ladder used to
...
import { radarSampleAt } from './radarLayer';
import type { RadarSample } from '$lib/weather/opera';
...
import {
	...
	supaipZonesAt,
	type VisibleZone,
} from '$lib/state/supaip.svelte';
import { vacPanelsAt } from '$lib/state/vacGeo.svelte';
import { vacPanelsDrawnAt } from './vacPanelLayer';
import { panelDrawableAt, type VacPanel, type VacPanelKind } from '$lib/data/vacgeo';
import { sigmetAreaAt, sigmetsAt } from '$lib/state/sigmets.svelte';
...
function areaAt(map: L.Map, lat: number, lon: number): FeatureHit | null {
	const candidates: AreaCandidate<FeatureHit>[] = [];
	...
	const sigmet = sigmetAreaAt(lat, lon);
	if (sigmet) {
		candidates.push({
			hit: { kind: 'sigmet', id: sigmet.sigmet.id },
			areaM2: rankArea(sigmet.areaM2),
			z: AREA_PANE_Z.sigmet,
		});
	}
	...
	if (supaip) {
		candidates.push({
			hit: {
				kind: 'supaip',
				id: supaip.zone.sup.id,
				zone: supaip.zone.zoneIndex,
			},
...
/** Resolve the single feature a click should select: the aimed targets first,
 *  topmost pane over the ones below, then the smallest area under the point
 *  (see the two tiers at the head of this file). Null when the point hits
 *  nothing. The point tier short-circuits, which is why an airport stays
 *  clickable through a SUP AIP zone or a NOTAM area. */
...
/** Which sheet of which plate, for comparing a dataset row with a drawn one. */
function panelSheetKey(p: VacPanel): string {
	return `${p.ident}|${p.section}|${p.page}|${p.kind}`;
}

/** One aerodrome's charts over a point, and which sheets it has there. */
export interface ChartHit {
	ident: string;
	/** Most detailed first, the order they stack in. */
	kinds: VacPanelKind[];
}

/** The aerodromes charting this point, most detailed first, and only the
 *  sheets worth asking for at this zoom.
 *
 *  What COVERS what stays ungated, since a sheet another hides entirely is
 *  the very thing a row is for, and so does which field is leading. The
 *  sheet's own zoom WINDOW is the gate, because that is what the row can be
 *  held to: under the floor the layer draws no sheet at all, so the pin the
 *  row sets is dropped by the next pass, and past the ceiling the chart it
 *  would bring up is a blow-up of one engraved for a zoom far coarser than
 *  this. Le Bourget's approach sheet reaches over Lognes, and at zoom 15 a
 *  row offering it promised thirty-five times its own size across the
 *  screen, under a heading naming an aerodrome twenty kilometres away and
 *  nowhere near the view.
 *
 *  A sheet actually PAINTED here is listed whatever its window says: the
 *  field leading the view draws its best sheet however far past its size the
 *  zoom has gone, something being better than nothing where no other chart
 *  answers, and a chart on screen with no row of its own is the same
 *  confusion the other way about.
 *
 *  vacPanelsAt is already in stack order, so the first sighting of an ident
 *  fixes its place and its kinds follow in the same order. */
function chartsAt(map: L.Map, lat: number, lon: number): ChartHit[] {
	const zoom = map.getZoom();
	const painted = new Set(vacPanelsDrawnAt(lat, lon).map(panelSheetKey));
	const out: ChartHit[] = [];
	const byIdent = new Map<string, ChartHit>();
	for (const p of vacPanelsAt(lat, lon)) {
		if (!panelDrawableAt(p, zoom) && !painted.has(panelSheetKey(p))) {
			continue;
		}
		let hit = byIdent.get(p.ident);
		if (!hit) {
			hit = { ident: p.ident, kinds: [] };
			byIdent.set(p.ident, hit);
			out.push(hit);
		}
		if (!hit.kinds.includes(p.kind)) {
			hit.kinds.push(p.kind);
		}
	}
	return out;
}
...
export interface ContextFeatures {
	notams: IndexedNotam[];
	stations: MapStation[];
	/** The precipitation radar's reading under the point (the shown frame's
	 *  pooled cell), null while the layer is hidden or holds no tile there:
	 *  the phone's one way to read an echo, having no hover. */
	...
	supaips: VisibleZone[];
	sigmets: Sigmet[];
	/** The aerodromes whose charts cover the point, from the DATASET: the
	 *  menu answers "what is here" ungated, the way it does for airspaces
	 *  and SUP AIP, so a chart left undrawn because another covers it
	 *  entirely is still one right-click away. Ungated by every test but
	 *  the zoom the sheet reads at, which is what a row would act on
	 *  (chartsAt).
	 *
	 *  One row per AERODROME, because that is what a row does: a pin is an
	 *  ident. A large field files a plate in each Atlas product, so Paris
	 *  CDG has four panels over the same ground and listed four rows all
	 *  reading LFPG and all doing the same thing. */
	charts: ChartHit[];
	waypoints: Waypoint[];
	leg: number | null;
}
