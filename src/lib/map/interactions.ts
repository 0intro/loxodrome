...
import type { VacPanelKind } from '$lib/data/vacgeo';
...
/** The aerodromes charting this point, most detailed first. vacPanelsAt is
 *  already in that order, so the first sighting of an ident fixes its place
 *  and its kinds follow in the same order. */
function chartsAt(lat: number, lon: number): ChartHit[] {
	const out: ChartHit[] = [];
...
export interface ContextFeatures {
	 ...
	 *  entirely is still one right-click away.
	 *
