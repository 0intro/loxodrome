...
function collectAreaFeatures(
	...
	for (const { notam, index } of items) {
		...
		// point. Same reason as the circle in pushCircleFeature.
 ...
 * largest-first sort so the smallest stays visible inside the ones that
 * contain it. Replaces whatever was drawn before and re-applies the current
...
export function renderNotams(
	...
	// Largest first → the smallest feature ends up on top and stays visible
	// inside the ones that contain it. A DRAW order only: the paths are
	// non-interactive and featureAt ranks the click by area itself, so
	// bringing one to the front no longer decides anything.
