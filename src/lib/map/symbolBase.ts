...
/** Red "shut by NOTAM" cross struck over a symbol of the given half-extent.
 *
 *  The ICAO Annex 4 closed-aerodrome X, in the cue-ring red rather than the
 *  chart ink the `closed` TYPE is drawn in, because the two facts have two
 *  lifetimes and the panel already keeps them apart (CLOSED, "disused or
 *  abandoned", against CLOSED BY NOTAM, shut until Sunday). Struck OVER the
 *  field's own symbol rather than replacing it, so a heliport still reads as
 *  a heliport and a military field as military while it is shut.
 *
 *  Sits inside the cue ring drawCue draws at baseSize + 5, the two together
 *  being the whole mark: a ring saying this field has NOTAMs and a cross
 *  saying one of them shuts it. */
export function drawNotamClosed(
	ctx: CanvasRenderingContext2D,
	x: number,
	y: number,
	baseSize: number,
): void {
	const h = baseSize * 0.78;
	const cross = new Path2D();
	cross.moveTo(x - h, y - h);
	cross.lineTo(x + h, y + h);
	cross.moveTo(x + h, y - h);
	cross.lineTo(x - h, y + h);
	// Halo first, so the cross stays legible over the symbol it strikes.
	ctx.lineJoin = 'round';
	ctx.lineCap = 'round';
	ctx.strokeStyle = HALO;
	ctx.lineWidth = 4;
