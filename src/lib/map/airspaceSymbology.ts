...
const ACTIVITY_TYPES = new Set([
	...
	// cmd/fr types these from the SIA's own txtLocalType now, so the kind
	// arrives on the row rather than hiding in its subtype (docs/fr-aip.md).
	'MODEL', 'AEROBATIC', 'UAS',
]);
 ...
 *
 *  The AIXM txtLocalType in `subtype` routes first and the TYPE is the
 *  fallback. Since cmd/fr began typing these rows from that same
 *  txtLocalType the two agree for France (docs/fr-aip.md); the order stays
 *  because the AIXM 5.1 publishers carry no French activity subtype and
 *  reach the type branch. The dept-numbered AP rows ("33-001") are the
 *  GEN 2.3-2 drone zones, and SUR / PRN no longer occur at all - cmd/fr
 *  stopped emitting the nature twins - but the arm is kept for a stale
 *  cached dataset. */
