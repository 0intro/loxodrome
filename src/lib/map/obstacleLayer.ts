 ...
 *
 * While the relative-terrain shading is up (map/terrainAwareLayer.ts), each
 * glyph takes the shading's tier ink from its TOP elevation against the
 * aircraft's altitude (setObstacleReferenceAlt; the rule and the inks are
 * nav/terrainAwareness.ts): Garmin's obstacle red within 100 ft below or
 * above, its darker obstacle yellow from 100 to 1 000 ft below, the chart
 * ink beyond. The group / publisher / zoom gates stay what they are: which
 * obstacles are drawn is the pilot's choice, how a drawn one reads is not. */
...
import { OBSTACLE_TIER_INK, nextReferenceFt, relativeTier } from '$lib/nav/terrainAwareness';
...
/** The halo a tinted glyph takes, one pixel wider than the chart glyph's, so
 *  a red or yellow mark holds its edge over the coloured ground under it. */
const TINT_HALO_PX = 3;

/** The aircraft's altitude the tint is read against (feet MSL, on the shading's
 *  quantum), null while the shading is not up. */
let referenceFt: number | null = null;
...
const inst = createPointLayer<Obstacle>({
	...
	drawSymbol: (ctx, ob, x, y, o) => {
		...
		// The top AMSL, never elev + hgt: `elev` IS the summit (obstacles.ts).
		const tier = referenceFt != null ? relativeTier(ob.elev, referenceFt) : null;
		const ink = tier ? OBSTACLE_TIER_INK[tier] : undefined;
		drawObstacleSymbol(ctx, ob, x, y, size, ink ? { ink, haloPx: TINT_HALO_PX } : {});
		...
		if (ob.type === 'cable' && (o.highlight || o.zoom >= CABLE_LABEL_MIN_ZOOM)) {
			drawCableMark(ctx, x, y, size, ink);
		}
		if (o.cued) {
...
/** Build the obstacle layer from the loaded dataset, indexing it again when
 *  a later merge hands out a new one. Idempotent for one merge; a call with
 *  the same dataset (a toggle flip) is a no-op. */
...
/** The aircraft's altitude the glyphs are tinted against (feet MSL), null
 *  for the chart ink. Quantised like the terrain shading's reference, and a
 *  repaint only when the quantum moves. */
export function setObstacleReferenceAlt(ft: number | null): void {
	const q = ft == null || !Number.isFinite(ft) ? null : nextReferenceFt(referenceFt, ft);
	if (q === referenceFt) {
		return;
	}
	referenceFt = q;
