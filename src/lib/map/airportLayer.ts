...
import { AIRPORT_KINDS, airportDrawnAt } from './airportVisibility';
import { createPointLayer } from './pointLayerFactory';
import { drawCue, drawNotamClosed } from './symbolBase';
...
// Idents a NOTAM currently shuts. A module-level set rather than a second
// factory cue set: the painter belongs to the module (pointLayerFactory's
// contract), and this is the only layer with an operational state to draw.
let closedByNotam = new Set<string>();

/** Mark these aerodromes as shut by NOTAM (upper-case idents), or pass an
 *  empty set to clear. Fed from the map components, which resolve the state
 *  through aerodromeState.svelte.ts.
 *
 *  The cross rides the SYMBOL, so it appears only where the field itself is
 *  drawn: its group is on and the zoom is past that type's floor. That is the
 *  honest scope for a mark struck over a symbol, there being nothing to strike
 *  otherwise; the field stays reachable through the NOTAMs tab and the
 *  right-click menu either way. */
export function setAirportClosedByNotam(idents: Set<string>): void {
	if (idents.size === closedByNotam.size && [...idents].every((i) => closedByNotam.has(i))) {
		return;
	}
	closedByNotam = idents;
	inst.redraw();
}
...
const inst = createPointLayer<Airport>({
	...
	// Each type from its own zoom floor (airportVisibility.ts, which the
	// on-field NOTAM pin rule asks too).
	drawnAt: (a, zoom) => airportDrawnAt(a, zoom, airportVisible(a)),
	...
	drawSymbol: (ctx, a, x, y, o) => {
		...
		// Struck after the ring, so the cross sits on top of both.
		if (closedByNotam.has(a.ident.toUpperCase())) {
			drawNotamClosed(ctx, x, y, size);
		}
	},
...
// The merged dataset the index was built from: the filter below hands the
// factory a new array every call, so the identity it tests is kept here.
let indexedFrom: Airport[] | null = null;

function build(map: L.Map, airports: Airport[]): void {
	if (inst.built() && airports === indexedFrom) {
		return;
	}
	indexedFrom = airports;
	// Skip types without a zoom threshold (unknown types), as the
	// pre-factory index did.
	inst.build(map, airports.filter((a) => AIRPORT_KINDS[a.type] !== undefined));
}

/** Show the airport overlay, building it from the data on first call and
 *  indexing it again when a later merge (a country loading late) hands out
 *  a new dataset. */
 ...
 *   - the per-type zoom thresholds (airportVisibility.ts).
 *
 ...
 *  toggled off. Idempotent for one merge; a later showAirports just attaches
 *  the canvas. */
