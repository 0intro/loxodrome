 ...
 *
 * While the relative-terrain shading is up, an obstacle whose top is within
 * reach of the aircraft takes that shading's tier ink instead (the caller
 * passes it: obstacleLayer.ts, nav/terrainAwareness.ts), with a wider halo so
 * the glyph holds over the coloured ground; the art is never altered. */
...
/** How a glyph is inked: the chart ink and halo unless the caller says
 *  otherwise (the relative-terrain tint). */
export interface ObstacleInk {
	ink?: string | undefined;
	haloPx?: number | undefined;
}
 ...
 *  selection highlight). `opt` re-inks the glyph (the terrain tint). */
export function drawObstacleSymbol(
	...
	const ink = opt.ink ?? OBSTACLE_INK;
	const halo = opt.haloPx ?? HALO_PX;
