...
/** Every chart drawn at this position, TOPMOST FIRST.
 *
...
export function vacPanelsDrawnAt(lat: number, lon: number): VacPanel[] {
	const out: VacPanel[] = [];
	for (let i = fades.length - 1; i >= 0; i--) {
		...
		if (panelContains(f.item.panel, lat, lon)) {
			out.push(f.item.panel);
		}
	}
	return out;
}

/** The chart drawn at this position, or null: the one a click pins, which is
 *  the one the reader is pointing at. */
export function vacPanelAt(lat: number, lon: number): VacPanel | null {
	return vacPanelsDrawnAt(lat, lon)[0] ?? null;
}
