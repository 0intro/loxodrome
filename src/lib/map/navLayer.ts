...
/* Follow re-arms by itself after a pan while a recording runs
 * (nav/followRearm.ts): the timer starts when the pan ENDS and any new pan
 * gesture cancels it; 0 (the desk, a replay) keeps the one-shot behaviour,
 * where only the recentre control or the checkbox re-arm. */
let autoRearmMs = 0;
let rearmTimer: ReturnType<typeof setTimeout> | null = null;

export function setFollowAutoRearm(ms: number): void {
	autoRearmMs = ms;
	if (ms <= 0) {
		cancelRearm();
	}
	// Nothing to start here: a pan that PREDATES the flight is already
	// settled by the time this runs, `goFlying` calling rearmFollow (or
	// recenterNav) before the effect that arms, and both clear the pan. The
	// countdown belongs to a pan that ENDS while the interval stands.
}
...
function scheduleRearm(): void {
	cancelRearm();
	if (autoRearmMs <= 0 || !userPanning) {
		return;
	}
	rearmTimer = setTimeout(() => {
		rearmTimer = null;
		if (userPanning && autoRearmMs > 0) {
			rearmFollow();
		}
	}, autoRearmMs);
}
...
function onMapKeyDown(e: L.LeafletKeyboardEvent): void {
	...
	if (k === 'ArrowUp' || k === 'ArrowDown' || k === 'ArrowLeft' || k === 'ArrowRight') {
		onPanGesture();
		// A key pan has no `dragend` to close it, so it starts its own
		// countdown; a held key restarts it through onPanGesture's cancel.
		scheduleRearm();
	}
}
...
export function buildNavLayer(m: L.Map): void {
	...
	if (!dragBound) {
		...
		// The pan's END is what the in-flight re-arm counts from.
		m.on('dragend boxzoomend', scheduleRearm); // i18n-ignore: Leaflet event names, not display text
		dragBound = true;
	}
}
...
export function clearNavLayer(m: L.Map): void {
	if (dragBound) {
		...
		m.off('dragend boxzoomend', scheduleRearm); // i18n-ignore: Leaflet event names, not display text
		m.off('keydown', onMapKeyDown);
		dragBound = false;
	}
	// A pan's re-arm timer outlives the map it would recentre, and the flag
	// its callback reads is cleared below, so the countdown has to go with
	// the layer: `dragBound` is what stops a rebuild double-binding, and a
	// teardown that cleared it while leaving the pan-end binding attached
