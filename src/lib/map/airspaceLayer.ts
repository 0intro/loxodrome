...
function visibleEntriesAt(map: L.Map, lat: number, lon: number): Entry[] {
	...
	for (const c of candidates) {
		...
		// NOTAM references (highlight clones, linkedClones), the sectors whose
		// radio a NOTAM has withdrawn (airspaceClosureLayer clones), the
		...
		// with their category toggle off: the user sees them drawn and expects
		// to click them. The linked, closed, contact and alert clones are keyed
		// by row `key`, activated by shared `id`.
		//
