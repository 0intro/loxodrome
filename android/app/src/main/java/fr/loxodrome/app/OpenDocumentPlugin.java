...
/**
 * Opens a file the app has written in whichever app on the phone handles that
 * kind of file: a stored AIP document (a VAC plate, an AIP supplement) in a
 * PDF viewer, an exported flight plan in SDVFR or another navigation app.
 *
 ...
 * and hands this plugin the resulting file:// URI; an export is written to the
 * same cache. The cache is already a declared provider root
 * (res/xml/file_paths.xml), so no manifest path is added here; the
 * &lt;queries&gt; intents in the manifest ARE needed, one per type, without
 * which resolveActivity answers null on Android 11+ even when an app that
 * opens the file is installed.
 *
...
public class OpenDocumentPlugin extends Plugin {
    ...
    public void view(PluginCall call) {
        ...
        // The AIP documents predate the parameter and still call without it.
        String mime = call.getString("mime", PDF_MIME);
        ...
        view.setDataAndType(shared, mime);
        view.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
        if (view.resolveActivity(getContext().getPackageManager()) == null) {
            call.reject("no app opens this kind of file");
            return;
        }
