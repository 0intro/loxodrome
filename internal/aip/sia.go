...
// VacRow is the part of an fr-adcharts.json row the plate readers need:
// which ident, and which Atlas VAC products publish a plate for it.
type VacRow struct {
	Ident string
	Vac   VacKind
}

// ReadVacRows reads the Atlas VAC membership column out of fr-adcharts.json,
// dropping the idents no product publishes.
//
// It lives here rather than in one of its callers because THREE commands now
// decide which plates exist and they must not drift: cmd/aipdocs packs them,
// cmd/vacgeo georeferences them, and cmd/fuel reads their avitaillement
// item. The membership is data the SIA states in its own index, which
// cmd/adcharts records; deriving it from the ident instead would 404 on
// every field the atlas omits.
func ReadVacRows(path string) ([]VacRow, error) {
	p, err := ReadPositional(path)
	if err != nil {
		return nil, err
	}
