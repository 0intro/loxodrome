...
/** A row whose panel reads at its printed size at `native`, over Lognes.
 *
 *  panelNativeZoom is a function of the clip height against the degrees it
 *  spans, so a test that wants a sheet engraved for a given zoom states the
 *  zoom and lets the corners follow. */
function rowAtNativeZoom(
	ident: string,
	page: number,
	kind: 'APP' | 'ATT' | 'GMC',
	native: number,
): unknown[] {
	const clip: [number, number, number, number] = [50, 33, 403, 484];
	const lat = 48.8225;
	const ptPerDeg = 2 ** native * (256 / 360 / Math.cos((lat * Math.PI) / 180));
	const dLat = (clip[3] - clip[1]) / ptPerDeg;
	const dLon = ((clip[2] - clip[0]) / ptPerDeg) * 1.5;
	return axisRow(
		ident,
		page,
		kind,
		clip,
		[lat - dLat / 2, 2.625 - dLon / 2],
		[lat + dLat / 2, 2.625 + dLon / 2],
	);
}
...
describe('vacPanelsIn', () => {
	...
	// Chelles' approach sheet, coarser still and covering Lognes whole: the
	// shape a held lead takes on the way in.
	const lfphApp = axisRow('LFPH', 1, 'APP', [50, 33, 403, 484], [48.7, 2.45], [49.0, 2.8]);
	...
	/** The same, as panels, for a case that is about WHICH edition. */
	const at2 = (view: typeof overLfpl, z: number) =>
		vacPanelsIn(view, ['APP', 'ATT', 'GMC'], z, panelNativeZoom);
	...
	it('answers nothing when nothing legible is in view', async () => {
		// Over Lognes at a zoom under every sheet's own floor: the coarsest
		// of these three is engraved for 10.1, so nothing is left by 6.
		await loadPanels([lfplApp, lfplAtt, lfpoApp]);
		expect(at(overLfpl, 6)).toEqual([]);
	});
	...
	it('holds a sheet back until its own floor', async () => {
		await loadPanels([lfplApp]);
		const native = panelNativeZoom(
			panelOf('LFPL', 1, 'APP', [50, 33, 403, 484], [48.766, 2.543], [48.912, 2.717]),
		);
		// A sheet well under its engraved size is not a chart to read, but
		// it is one to SEE COMING, and an overlay that arrives only once
		// you are on top of the aerodrome cannot say the aerodrome is
		// there. The floor is set so that every landing sheet is on the map
		// at zoom 11, a planning zoom; under it the map wins.
		expect(at(overLfpl, native - PANEL_ZOOM_SLACK)).toEqual(['LFPL APP']);
		expect(at(overLfpl, native - PANEL_ZOOM_SLACK - 0.5)).toEqual([]);
	});

	it('hands over from the approach sheet to the landing sheet as the view closes', async () => {
		await loadPanels([lfplApp, lfplAtt]);
		// Far out, under the landing sheet's own floor: the approach sheet
		// alone.
		expect(at(overLfpl, 8)).toEqual(['LFPL APP']);
	...
	it('trades a blow-up for a sheet the floor has just admitted', async () => {
		// The floor and the ceiling interact the other way round too, and
		// this direction is a TRADE rather than a loss. A field showing one
		// sheet far past its own size is showing it only because it is the
		// nearest, which is what exempts it from the ceiling; a sheet the
		// floor then admits can be nearer still, and it takes the exemption
		// with it.
		//
		// Swept over the whole corpus from zoom 8 to 16, widening the floor
		// from 1.3 to 3.7 did this three times, at Toulouse-Francazal,
		// Saint-Yan and L'Alpe d'Huez. Each traded a blow-up for a sheet
		// nearer its engraved size, and no view went blank, which is the
		// property that matters: picks are non-empty, so a nearest exists.
		const coarse = rowAtNativeZoom('LFPL', 1, 'APP', 12 - 4);
		const fine = rowAtNativeZoom('LFPL', 2, 'ATT', 12 + 3);
		await loadPanels([coarse, fine]);
		// Under the fine sheet's own floor: the blow-up is all there is,
		// and it draws.
		expect(at(overLfpl, 12 + 3 - PANEL_ZOOM_SLACK - 0.5)).toEqual(['LFPL APP']);
		// Once the fine sheet is admitted it is the nearer of the two, so
		// it leads and the blow-up loses its exemption.
		expect(at(overLfpl, 12)).toEqual(['LFPL ATT']);
	});
	...
	it('picks the edition by scale, not by which one the view has reached', async () => {
		// CDG files its landing sheet in both Atlas products, at zoom 11.2
		// and 12.0, the coarser one covering more ground. The picks used to
		// run over the panels IN VIEW, a cost rule, so north of Chelles at
		// zoom 13 the coarse edition was the only one the screen reached
		// and was drawn across the top of it at three and a half times its
		// size; panning up to read it brought the finer one into view,
		// which won the nearest-scale test and replaced it. The chart could
		// not be read at all.
		const fine = axisRow('LFPG', 2, 'ATT', [14, 352, 367, 550], [48.972, 2.493], [49.048, 2.616]);
		const coarse = axisRow('LFPG', 2, 'ATT', [14, 352, 367, 550], [48.946, 2.435], [49.088, 2.65]);
		// The same sheet in the helistation product, which is how the pair
		// is filed: same ident, same page, the other Atlas section.
		coarse[1] = 3;
		await loadPanels([fine, coarse]);
		// A view south of the fine edition, which only the coarse one
		// reaches: the sheet to draw is still the fine one, so nothing of
		// CDG is drawn here rather than the wrong edition of it.
		const south = { south: 48.894, west: 2.431, north: 48.95, east: 2.678 };
		expect(at(south, 13)).toEqual([]);
		// A little north, where both are in view, and three levels down,
		// where the coarse edition IS the nearer one and answers for the
		// field: nothing is out of reach, it is reached by its own zoom.
		const over = { south: 48.95, west: 2.431, north: 49.04, east: 2.678 };
		expect(at(over, 13)).toEqual(['LFPG ATT']);
		expect(at2(over, 13)[0].section).toBe(2);
		expect(at2(over, 10)[0].section).toBe(3);
	});

	it('lets the field the reader named answer with the edition that reaches here', async () => {
		// The same pair, and the same view the automatic pick answers with
		// nothing at: only the coarse edition covers this ground. A row in
		// the right-click menu offers the field there, because the DATASET
		// charts the point, so acting on that row has to bring up a chart
		// over it. Choosing by scale alone brought up the finer edition,
		// which is not over the point and can be off the screen entirely,
		// and a pass that cannot honour the pin makes MapView drop it: the
		...
		const south = { south: 48.894, west: 2.431, north: 48.95, east: 2.678 };
		expect(at(south, 13)).toEqual([]);
		const pinned = vacPanelsIn(south, ['APP', 'ATT', 'GMC'], 13, panelNativeZoom, [], {
			pinned: 'LFPG',
		});
		expect(pinned.map((p) => `${p.ident} ${p.kind}`)).toEqual(['LFPG ATT']);
		// The coarse one, filed in the helistation product: the edition over
		// the middle of the view, not the one nearer this zoom.
		expect(pinned[0].section).toBe(3);
		// And the automatic pick is untouched by the pin's rule, which is
		// what keeps it from following the viewport as the map pans.
		expect(at2(south, 13)).toEqual([]);
	});
	...
	it('lets go once the view has zoomed past the chart it was holding', async () => {
		// The share bounds the hold across the screen, and zooming in only
		// ever grows it, so nothing bounded it across the zoom: Chelles'
		// approach sheet, engraved for zoom 10 and held on the way in,
		// covered Lognes at zoom 15 at thirty times its size, and covered
		// it WHOLLY, so Lognes' own sheets, engraved for that very zoom and
		// under the middle of the view, were dropped as invisible under it.
		await loadPanels([lfplApp, lfplAtt, lfphApp]);
		// Inside its own ceiling it is still the chart being read, whatever
		// it hides.
		expect(held(overLfpl, 13, ['LFPH'])).toEqual(['LFPH APP']);
		// Past it, it is a blow-up rather than a chart, and the field under
		// the middle answers.
		expect(held(overLfpl, 14, ['LFPH'])).toEqual(['LFPL ATT', 'LFPL APP']);
	});
	...
	it('ignores a pin it cannot honour', async () => {
		...
		const tiny = vacPanelsIn(overLfpl, ['APP', 'ATT', 'GMC'], 6, panelNativeZoom, [], {
			pinned: 'LFPL',
		});
	...
	it('draws a neighbour at the size it would draw the chosen field at', async () => {
		...
		const wide = { south: 48.7, west: 2.06, north: 48.98, east: 3.2 };
		expect(at(wide, native - PANEL_ZOOM_SLACK)).toEqual(['LFPL APP', 'LFXX APP']);
		// And the floor is a floor for both: under it the map wins,
		// neighbour and chosen field alike.
		expect(at(wide, native - PANEL_ZOOM_SLACK - 0.5)).toEqual([]);
	});

	it('looks past a field that has nothing legible to show', async () => {
		// East of Le Plessis-Belleville the nearest panel is a hospital
		// helipad whose approach sheet is engraved several levels finer
		// than the basin's. Letting it answer for the screen, and so
		// answering nothing, blanked every approach chart around it.
		...
		expect(at(gap, 8)).toEqual(['LFPL APP']);
	});
