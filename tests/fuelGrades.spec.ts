...
/* What a VISITING aeroplane has to do to uplift. One answer per aerodrome,
 * because the flags are about the pump and not about a grade. */
describe('fuelAccess', () => {
	it('gives the strongest condition, and nothing for none', () => {
		expect(fuelAccess(['based'])).toBe('basedOnly');
		expect(fuelAccess(['or'])).toBe('onRequest');
		expect(fuelAccess(['hx'])).toBe('noHours');
		expect(fuelAccess(['h24'])).toBe('h24');
		expect(fuelAccess([])).toBeNull();
		expect(fuelAccess(undefined)).toBeNull();
	});

	it('lets prior notice reopen a field, and hours not', () => {
		// LFKG: "réservé aux ACFT basés - disponible avec préavis ou sur
		// demande". O/R is the door; HX qualifies the reservation rather than
		// excusing it ("HX réservé aux ACFT basés", LFGE / LFHJ / LFIT).
		expect(fuelAccess(['or', 'based'])).toBe('onRequest');
		expect(fuelAccess(['hx', 'based'])).toBe('basedOnly');
	});

	it('is one answer where the row states several', () => {
		// Ten rows carry O/R and HX together. Two lines about one aerodrome
		// would be two facts where the AIP stated one service.
		expect(fuelAccess(['hx', 'or'])).toBe('onRequest');
		expect(fuelAccess(['h24', 'or'])).toBe('onRequest');
		expect(fuelAccess(['h24', 'hx', 'or'])).toBe('onRequest');
	});

	it('reads the shipped corpus as the wording assumes', () => {
		const at = doc.fields.indexOf('flags');
		const flags = (ident: string): FuelCondition[] =>
			(doc.rows.find((r) => r[0] === ident)?.[at] ?? []) as FuelCondition[];
		// The one row prior notice reopens, and the three it does not.
		expect(fuelAccess(flags('LFKG'))).toBe('onRequest');
		for (const id of ['LFGE', 'LFHJ', 'LFIT']) {
			expect(fuelAccess(flags(id)), id).toBe('basedOnly');
		}
		// No row reserves its fuel to based aircraft AND claims a round-the-
		// clock pump; if one ever does, the tier order needs re-reading.
		const both = doc.rows.filter((r) => {
			const f = (r[at] ?? []) as string[];
			return f.includes('based') && f.includes('h24');
		});
		expect(both).toEqual([]);
	});
});
...
describe('fuelRefusal', () => {
	...
	it('blames the aerodrome where its fuel is not for a visitor', () => {
		// The AIP says "réservé aux aéronefs basés" at 16 fields. That is a
		// fact about the aerodrome, so it holds whatever the aeroplane burns,
		// holds with no aeroplane selected, and holds at the three fields that
		// reserve their fuel while naming no grade at all (LFEP, LFHR, LFOY).
		const based = { cond: ['based'] as const };
		expect(fuelRefusal({ ...offering([['100LL', '']]), ...based }, ['100LL'])).toEqual({
			kind: 'aipBased',
		});
		expect(fuelRefusal({ ...offering([]), ...based }, ['100LL'])).toEqual({ kind: 'aipBased' });
		expect(fuelRefusal({ ...offering([['100LL', '']]), ...based }, [])).toEqual({
			kind: 'aipBased',
		});
		expect(sellsGrade({ ...offering([['100LL', '']]), ...based }, ['100LL'])).toBe(false);
		// LFAW publishes the unqualified AVGAS, so a 100LL aeroplane is
		// already aipGrade there. The reservation is the stronger answer: it
		// holds for every aeroplane, and the grade reason holds for one.
		expect(fuelRefusal({ ...offering([['AVGAS', '']]), ...based }, ['100LL'])).toEqual({
			kind: 'aipBased',
		});
		// Prior notice reopens it (LFKG), and a dry field outranks it (LFCT).
		expect(
			fuelRefusal({ ...offering([['100LL', '']]), cond: ['or', 'based'] }, ['100LL']),
		).toBeNull();
		expect(
			fuelRefusal({ ...offering([], { avail: 'no' }), ...based }, ['100LL']),
		).toEqual({ kind: 'aipNone' });
	});
	...
	it('agrees with sellsGrade in every case', () => {
		const cases = [
			...
			offering([['100LL', '', 'N2']]),
			offering([]),
			{ ...offering([['100LL', '']]), cond: ['based'] as const },
			{ ...offering([['100LL', '']]), cond: ['or', 'based'] as const },
		];
