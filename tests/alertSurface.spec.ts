...
import {
	...
	alertSolid,
	alertSurface,
	type AlertSurfaceInput,
} from '../src/lib/nav/alertSurface';
import type { VolumeAlert } from '../src/lib/nav/airspaceAlert';
import type { AlertVolume } from '../src/lib/nav/alertVolumes';
import type { TerrainAlert, TerrainAlertKind } from '../src/lib/nav/terrainAlert';
...
/* A terrain row: a ridge 45 s ahead, 320 ft under the en-route clearance. */
function ter(kind: TerrainAlertKind, over: Partial<TerrainAlert> = {}): TerrainAlert {
	return {
		subject: 'terrain',
		key: `terrain:${kind}`,
		kind,
		threat: kind === 'terrain' ? 'terrain' : 'obstacle:o1',
		level: 'caution',
		etaSec: 45,
		alongM: 2800,
		toM: 3400,
...
describe('the terrain subject', () => {
	it('puts a terrain warning above everything, a prohibited volume the aircraft is inside included', () => {
		const m = alertSurface(
			input({
				alerts: [al('r', { severity: 'inside' }), ter('terrain', { level: 'warning', etaSec: 20 })],
			}),
		);
		expect(m.banner?.key).toBe('terrain:terrain');
		expect(m.order).toEqual(['terrain:terrain', 'r']);
		expect(m.chip).toEqual({ kind: 'alerts', ink: 'danger', solid: true });
	});

	it('ranks a terrain caution as an approaching entry of the avoid tier', () => {
		const behind = (sev: VolumeAlert['severity']): string[] =>
			alertSurface(input({ alerts: [ter('terrain'), al('r', { severity: sev })] })).order;
		// A prohibited volume the aircraft is inside, or about to enter, leads.
		expect(behind('inside')).toEqual(['r', 'terrain:terrain']);
		expect(behind('imminent')).toEqual(['r', 'terrain:terrain']);
		// A mere proximity does not.
		expect(behind('proximity')).toEqual(['terrain:terrain', 'r']);
		// Every contact, equipment, caution and planned row sits below it.
		const m = alertSurface(
			input({
				alerts: [
					al('ctr', { action: 'clearance', severity: 'inside' }),
					al('rmz', { action: 'radio', severity: 'inside' }),
					al('d', { action: 'caution', severity: 'inside' }),
					al('p', { action: 'clearance', severity: 'inside', planned: true }),
					ter('terrain'),
				],
			}),
		);
		expect(m.order[0]).toBe('terrain:terrain');
		expect(m.chip).toEqual({ kind: 'alerts', ink: 'caution', solid: false });
	});

	it('keeps an approaching tie in its previous order', () => {
		const first = alertSurface(input({ alerts: [ter('terrain'), al('r')] }));
		const second = alertSurface(
			input({ alerts: [al('r'), ter('terrain', { etaSec: 30 })], prevOrder: ['terrain:terrain', 'r'] }),
		);
		expect(first.order).toEqual(['r', 'terrain:terrain']);
		expect(second.order).toEqual(['terrain:terrain', 'r']);
	});

	it('wears the caution amber tinted and the warning red solid, never the planned form', () => {
		expect(alertInk(ter('terrain'))).toBe('caution');
		expect(alertSolid(ter('terrain'))).toBe(false);
		expect(alertInk(ter('obstacle', { level: 'warning' }))).toBe('danger');
		expect(alertSolid(ter('obstacle', { level: 'warning' }))).toBe(true);
		expect(alertPlanned(ter('terrain'))).toBe(false);
		expect(alertPlanned(al('p', { action: 'clearance', planned: true }))).toBe(true);
		expect(alertInk(al('p', { planned: true }))).toBe('alert');
		expect(alertSolid(al('r', { severity: 'inside', planned: true }))).toBe(false);
	});

	it('escapes the hidden strip at either level', () => {
		expect(alertSurface(input({ alerts: [ter('terrain')] })).bannerEscapes).toBe(true);
		expect(alertSurface(input({ alerts: [ter('obstacle', { level: 'warning' })] })).bannerEscapes).toBe(true);
	});

	it('leaves the banner once acknowledged, the volume beneath taking it', () => {
		const m = alertSurface(
			input({ alerts: [ter('terrain', { level: 'warning', acked: true }), al('ctr', { action: 'clearance' })] }),
		);
		expect(m.banner?.key).toBe('ctr');
		expect(m.badge).toEqual({ unacked: 1, acked: 1 });
		expect(m.panel.acked.map((a) => a.key)).toEqual(['terrain:terrain']);
	});

	it('lists the terrain caveats in the panel and raises no chip for them', () => {
		const unread = alertSurface(input({ terrainGaps: { unread: true, obstacles: true } }));
		expect(unread.chip).toBeNull();
		expect(unread.regions).toEqual([]);
		expect(unread.panel.caveats).toEqual(['terrain', 'obstacles']);
		const inhibited = alertSurface(
			input({
				gaps: { airspaces: false, briefing: true },
				terrainGaps: { unread: true, obstacles: false },
				terrainInhibited: true,
			}),
		);
		// An inhibited evaluation reads nothing, so "not read" would be noise.
		expect(inhibited.panel.caveats).toEqual(['briefing', 'inhibited']);
	});
});
