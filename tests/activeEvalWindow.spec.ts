...
import { installFlightScope } from '$lib/state/flightScope';
import { routes, type Route, type Waypoint } from '$lib/state/route.svelte';

// The flight branch is a PROVIDER, not an import (state/planScope.svelte.ts):
// the app registers it in main.ts's boot(), which no spec runs. This file is
// the one that judges that branch, so it registers Loxodrome's own scope; the
// unregistered answer is pinned in planScope.spec.ts instead.
beforeAll(() => {
	installFlightScope();
});
...
describe('activeEvalWindow', () => {
	...
	it('gives a flight with no departure time its whole day', () => {
		...
		// Its last millisecond, not the next midnight: every gate compares
		// both ends inclusively, and Monday's first minute is not Sunday's.
		expect(new Date(w.to).toISOString()).toBe('2026-09-20T23:59:59.999Z');
	});
