...
describe('resolvePlacement', () => {
	...
	it('keeps a bottom dock on phones, and renders a map-coupled full screen as the page detent', () => {
		expect(resolvePlacement(route, 'dock-bottom', true)).toBe('dock-bottom');
		// The pane's full detent is the page (the toolbar and the bar stay);
		// `full` would cover both, so a stored one lands on the detent.
		expect(resolvePlacement(route, 'full', true)).toBe('page');
		expect(resolvePlacement(route, 'page', true)).toBe('page');
		// A surface that is not the pane still collapses to full screen.
		expect(resolvePlacement(surfaceDef('flights'), 'full', true)).toBe('full');
	});

	it('docks every map-coupled surface on the right on a landscape phone', () => {
		for (const stored of [null, 'dock-bottom', 'page', 'full'] as const) {
			expect(resolvePlacement(route, stored, true, 0, true)).toBe('dock-right');
			expect(resolvePlacement(surfaceDef('detail'), stored, true, 0, true)).toBe('dock-right');
		}
		expect(resolvePlacement(surfaceDef('flights'), 'page', true, 0, true)).toBe('full');
	});
	...
	it('gives a phone full screen rather than a keyhole dock', () => {
		...
		// while the surface can show its content in it. A 700px grid (the
		// desktop nav log's, before its compact rendition) against a 414px
		// phone cannot; a profile's 420px floor can, and the strip of map
		// above it is the point of docking at all.
		const wide = { ...surfaceDef('routeProfile'), minWidthPx: 700 };
		expect(resolvePlacement(wide, null, true, 414)).toBe('full');
		expect(resolvePlacement(wide, 'dock-bottom', true, 414)).toBe('full');
		expect(resolvePlacement(wide, 'dock-right', true, 414)).toBe('full');
		...
		// A phone wide enough for the grid to read keeps the dock, the stored
		// choice is untouched either way, and an interactive move (no stage
		// measurement) is never overridden.
		expect(resolvePlacement(wide, null, true, 600)).toBe('dock-bottom');
		expect(resolvePlacement(wide, 'dock-bottom', true, 0)).toBe('dock-bottom');
		expect(resolvePlacement(wide, 'dock-bottom', true)).toBe('dock-bottom');
	});

	it('docks the nav log on a phone by its compact rendition, not its desktop grid', () => {
		// The desktop grid is 812px, the compact chevron sheet 376px plus
		// padding: the phone floor is the one a phone dock is judged by.
		const navlog = surfaceDef('navlog');
		expect(navlog.phoneMinWidthPx).toBe(400);
		expect(resolvePlacement(navlog, null, true, 392)).toBe('dock-bottom');
		expect(resolvePlacement(navlog, 'dock-right', true, 392)).toBe('dock-bottom');
		expect(resolvePlacement(navlog, null, true, 240)).toBe('full');
	});
		...
		for (const id of SURFACE_IDS) {
			...
			for (const s of stored) {
				for (const mobile of [false, true]) {
					...
					expect(mobile ? mobilePlacement(p, def.mapCoupled) : p).toBe(p);
...
describe('the phone pane', () => {
	it('registers the detail as a map-coupled surface that never takes full screen', () => {
		const detail = surfaceDef('detail');
		expect(detail.mapCoupled).toBe(true);
		expect(detail.placements).not.toContain('full');
		expect(defaultPlacement(detail)).toBe('dock-bottom');
		expect(detail.defaultDockFrac).toBe(PHONE_DOCK_FRAC);
	});

	it('collapses a map-coupled surface onto the pane and the rest onto full screen', () => {
		expect(mobilePlacement('dock-right', true)).toBe('dock-bottom');
		expect(mobilePlacement('page', true)).toBe('page');
		expect(mobilePlacement('full', true)).toBe('page');
		expect(mobilePlacement('dialog', true)).toBe('page');
		expect(mobilePlacement('dock-bottom', true)).toBe('dock-bottom');
		expect(mobilePlacement('page', false)).toBe('full');
		expect(mobilePlacement('dialog', false)).toBe('full');
		expect(mobilePlacement('dock-right', false)).toBe('dock-bottom');
		// The one-argument form is the old collapse, for callers with no surface.
		expect(mobilePlacement('page')).toBe('full');
	});

	it('docks the pane on the right in landscape whatever was stored', () => {
		for (const p of ['dock-bottom', 'dock-right', 'page', 'full', 'dialog'] as const) {
			expect(mobilePlacement(p, true, true)).toBe('dock-right');
		}
		expect(mobilePlacement('page', false, true)).toBe('full');
	});

	it('states the two detents', () => {
		expect(PHONE_DOCK_FRAC).toBe(0.45);
		expect(PHONE_LANDSCAPE_FRACS).toEqual([0.4, 0.62]);
	});
});
