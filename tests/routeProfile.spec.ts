...
describe('computeAirspaceCorridorSpans', () => {
	...
	it('KEEPS aerial-activity zones: the walk reports, the filter decides', () => {
		// The walk used to drop these as clutter, which is a display
		// judgement and belongs to the pilot's filter row (which ships off,
		// so the chart reads as it always did until they ask).
		const glider = box({ key: 'GLD', latMin: -1, lonMin: 0, latMax: 1, lonMax: 2, category: 'activity' });
		expect(computeAirspaceCorridorSpans([wp('a', 0, -1), wp('b', 0, 3)], [glider])).toHaveLength(1);
	});
	...
	it('KEEPS FIS sectors: which one is in force changes with ALTITUDE', () => {
		// 94 of the 110 French sectors have their ceiling inside the VFR
		// band, so a climb can leave one - and a vertical profile is the
		// only surface that can show where. The row ships off, so the chart
		// reads as it always did until the pilot asks.
		const siv = box({ key: 'SIV1', latMin: -1, lonMin: 0, latMax: 1, lonMax: 2, category: 'siv' });
		expect(computeAirspaceCorridorSpans([wp('a', 0, -1), wp('b', 0, 3)], [siv])).toHaveLength(1);
	});
...
describe('buildBands', () => {
	...
	it('draws each edge against its own end of the ground band', () => {
		// One sample, a footprint spanning 1000..3000 ft under the track. A
		// SFC..800 ft ASFC band can be anywhere from 1000 ft (its floor over
		// the low ground) to 3800 ft (its roof over the ridge), and drawing
		// both edges off one figure would slice that.
		const terrain: TerrainSample[] = [
			{ distNM: 0, elevFt: 2000, minFt: 1000, maxFt: 3000 },
			{ distNM: 1, elevFt: 2000, minFt: 1000, maxFt: 3000 },
		];
		const [p] = buildBands(
			[mkBand({ key: 'LFR45S2', type: 'R', lower: ['HEI', '0', 'FT'], upper: ['HEI', '800', 'FT'] })],
			terrain,
		...
		expect(p.spans[0].lowerPts.map((v) => v.altFt)).toEqual([1000, 1000]);
		expect(p.spans[0].upperPts.map((v) => v.altFt)).toEqual([3800, 3800]);
		expect(p.minFt).toBe(1000);
		expect(p.maxFt).toBe(3800);
	});
...
describe('bandPenetrations / clipSpanToRange', () => {
	...
	it('never flags a FIS sector as a penetration', () => {
		// A flight is inside one essentially all the time (they run from the
		// surface), so "you are inside" is not news - and the emphasis would
		// cover the whole extent and swamp the stretch that IS news, where
		// that sector is the contact in force.
		const siv = buildBands(
			[mkBand({ key: 'SIV|S1', type: 'SIV', category: 'siv',
				lower: ['ALT', '0', 'FT'], upper: ['ALT', '6500', 'FT'],
				spans: [{ enterNM: 0, leaveNM: 20 }] })],
			terrain,
			EN_TYPE_LABELS,
		)[0];
		const path = [
		...
		expect(bandPenetrations(siv, path)).toEqual([]);
	});

	it('leaves FIS sectors out of the strip: a service is not a crossing', () => {
		// The sectors are drawn as bands now (docs/route-profile.md "The
		// sector in force"), so without this every French leg would carry a
		// chip per sector on a strip that exists for the NO-GO review.
		const mk = (key: string, category: 'siv' | 'controlled') =>
			buildBands(
				[mkBand({ key, type: category === 'siv' ? 'SIV' : 'TMA', category,
					lower: ['ALT', '0', 'FT'], upper: ['ALT', '6500', 'FT'],
					spans: [{ enterNM: 0, leaveNM: 20 }] })],
				terrain,
				EN_TYPE_LABELS,
		...
		expect(bandCrossings([mk('SIV|S1', 'siv'), mk('TMA|D', 'controlled')], path).map((c) => c.key)).toEqual([
			'TMA|D',
		]);
	});
