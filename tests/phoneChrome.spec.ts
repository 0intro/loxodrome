...
describe('the app menu is a bottom sheet', () => {
	...
	it('keeps the phone chrome off paper', () => {
		/* The bar, the pages and the map buttons carry `no-print`; the
		 * toolbar and the map did not, because a DESKTOP print with nothing
		 * claimed is the app's own Ctrl+P over what is on screen. A phone has
		 * no such flow, so a system print raised with only the detail up (the
		 * one occupant frontmostSurface never claims) printed a sheet of
		 * toolbar and map tiles. */
		const css = read('src/app.css');
		expect(css).toMatch(/:root\.mobile-ui header\.toolbar,\n\t:root\.mobile-ui \.map-slot \{[^}]*display: none/s);
		expect(read('src/lib/components/phone/PhoneNavBar.svelte')).toContain('class="phone-bar no-print"');
		expect(read('src/lib/components/phone/PhonePages.svelte')).toContain('class="phone-page no-print"');
	});
});
