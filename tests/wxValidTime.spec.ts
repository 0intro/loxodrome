...
describe('periodAnchor', () => {
	...
	it('marks a flight with no departure time as a day, never a departure', () => {
		...
		reset();
		routes.list = [flyableRoute()];
		// A day still ahead, counted from today: a flight on the CURRENT day
		// anchors on the current hour by design, so a fixed date that arrives
		// makes this case read as a departure (it did on 2026-09-20).
		const day = new Date(Date.now() + 10 * 86_400_000).toISOString().slice(0, 10);
		flightPrep.dossier.flightDate = day;
		setWindowMode('flight');
		const a = periodAnchor(Date.now());
		expect(a.source).toBe('flight-day');
		expect(new Date(a.ms).toISOString()).toBe(`${day}T00:00:00.000Z`);
	});
