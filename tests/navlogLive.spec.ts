 ...
 *  the crossing sub-fold the wanted distances. The default 100 kt from the
 *  very first fix makes every motion folded over it an INHERITED takeoff
 *  (takeoffObserved false): the crossing pins read crossMs, whose gate is
 *  takeoffMs, so nothing there minds; an assembly test built over it would
 *  get a blank departure row (use airborneAt for those). */
function synth(
	ds: number[],
...
function airborneAt(takeoffMs: number): MotionFold {
	return { ...newMotionFold(), idx: 2, takeoffMs, takeoffObserved: true };
}
...
describe('takeoff detection', () => {
	...
	it('backfills missing speeds from the hop (GPX traces)', () => {
		...
		// ...and no fix under TAKEOFF_KT precedes it: the takeoff is before
		// the trace, so the instant is the airborne gate, not a departure.
		expect(f.motion.takeoffObserved).toBe(false);
	});
...
describe('passage fold', () => {
	...
	it('holds a stamp inside the initial seed zone until the identity holds to its top', () => {
		// Joining just below a boundary: the fresh identity is unproven, so
		// the crossing is held, then promoted once the watermark reaches
		// SEED_GUARD_NM above the seed, at the instant it was crossed (the
		// lerp inside 11 -> 12.5, not the promotion instant).
		const { points, checkpoints } = synth([10, 11, 12.5, 14]);
		const f = foldPassages(points, checkpoints, [11.5]);
		expect(Math.round(f.crossings.crossMs[0]!)).toBe(13_333);
		expect(f.crossings.held).toHaveLength(0);
		expect(f.crossings.guardTopNM).toBeNull();
	});

	it('keeps a held stamp out of the ink until the promotion', () => {
		// The boundary is crossed but the watermark (12.8) stops short of the
		// zone's top (13): live, the row shows no ATO yet.
		const { points, checkpoints } = synth([10, 11, 12.5, 12.8]);
		const f = foldPassages(points, checkpoints, [11.5]);
		expect(f.crossings.crossMs[0]).toBeNull();
		expect(f.crossings.held).toHaveLength(1);
		expect(f.crossings.held[0].b).toBe(0);
		expect(Math.round(f.crossings.held[0].ms)).toBe(13_333);
		expect(f.crossings.guardTopNM).toBe(13);
	});

	it('discards the held stamps on a backward re-seed and stamps the genuine crossing', () => {
		// Held inside 11 -> 12.5, dropped by the rewind to 5 (which reopens
		// the boundary), then stamped for real inside 9 -> 12.
		const { points, checkpoints } = synth([10, 11, 12.5, 5, 9, 12, 20]);
		const f = foldPassages(points, checkpoints, [11.5]);
		expect(Math.round(f.crossings.crossMs[0]!)).toBe(48_333);
		expect(f.crossings.held).toHaveLength(0);
	});

	it('discards the held stamps on a jump', () => {
		// The 12.5 -> 19 segment is a re-seed / GPS gap: the held 11.5 goes
		// with the abandoned identity and 15 is consumed with no stamp.
		const { points, checkpoints } = synth([10, 11, 12.5, 19]);
		const f = foldPassages(points, checkpoints, [11.5, 15]);
		expect(f.crossings.crossMs).toEqual([null, null]);
		expect(f.crossings.held).toHaveLength(0);
		expect(f.crossings.guardTopNM).toBeNull();
	});

	it('discards the held stamps when the projection reports a forward re-seed', () => {
		// sinceSeedNM shrinking is the projection fold saying it replaced its
		// identity: the stamp held under the old one is a fabrication.
		const ds = [10, 11, 12.5, 13, 14, 16];
		const since = [0, 1, 2.5, 0, 1, 3];
		const { points, checkpoints } = synth(ds);
		checkpoints.forEach((c, i) => (c.sinceSeedNM = since[i]));
		const f = foldPassages(points, checkpoints, [11.5]);
		expect(f.crossings.crossMs[0]).toBeNull();
		expect(f.crossings.held).toHaveLength(0);
		expect(f.crossings.guardTopNM).toBeNull();
	...
	it('a regressing re-seed report reopens the boundary the corrected identity has yet to reach', () => {
		// A correction under BACKTRACK_NM: the small rewind reopens 11.5, and
		// the genuine crossing inside 11.4 -> 12 stamps it.
		const ds = [10, 11, 12.5, 11.2, 11.4, 12];
		const since = [0, 1, 2.5, 0, 0.2, 0.8];
		const { points, checkpoints } = synth(ds);
		checkpoints.forEach((c, i) => (c.sinceSeedNM = since[i]));
		const f = foldPassages(points, checkpoints, [11.5]);
		expect(Math.round(f.crossings.crossMs[0]!)).toBe(41_667);
		expect(f.crossings.held).toHaveLength(0);
	});
...
describe('the observed edges', () => {
	...
	it('a parked start observes the block-off and the takeoff', () => {
		const f = foldFor([
			tp(0, 0, 0, 0),
			tp(0, 0, 10_000, 0),
			tp(0, 0.001, 20_000, TAKEOFF_KT + 10),
			tp(0, 0.002, 20_000 + TAKEOFF_SUSTAIN_MS, TAKEOFF_KT + 20),
		]);
		expect(f.motion.takeoffMs).toBe(20_000);
		expect(f.motion.takeoffObserved).toBe(true);
		expect(f.motion.firstMoveMs).toBe(20_000);
		expect(f.motion.blockOutObserved).toBe(true);
	});

	it('a taxiing start observes the takeoff but not the block-off', () => {
		const f = foldFor([
			tp(0, 0, 0, 5),
			tp(0, 0.0001, 10_000, 5),
			tp(0, 0.001, 20_000, TAKEOFF_KT + 10),
			tp(0, 0.002, 20_000 + TAKEOFF_SUSTAIN_MS, TAKEOFF_KT + 20),
		]);
		expect(f.motion.takeoffMs).toBe(20_000);
		expect(f.motion.takeoffObserved).toBe(true);
		// The first fix is already moving: the block-off is before the trace.
		expect(f.motion.firstMoveMs).toBe(0);
		expect(f.motion.blockOutObserved).toBe(false);
	});

	it('a trace that opens already fast inherits its takeoff', () => {
		// The first fast fix is the airborne instant, not a departure: the
		// acceleration through TAKEOFF_KT happened before the trace.
		const f = foldFor([
			tp(0, 0, 0, 90),
			tp(0, 0.001, 10_000, 90),
			tp(0, 0.002, 20_000, 90),
		]);
		expect(f.motion.takeoffMs).toBe(0);
		expect(f.motion.takeoffObserved).toBe(false);
		expect(f.motion.firstMoveMs).toBe(0);
	...
	it('a spike between slow fixes leaves the later real takeoff observed', () => {
		const f = foldFor([
			tp(0, 0, 0, 90),
			tp(0, 0.0001, 10_000, 4),
			tp(0, 0.0002, 20_000, 4),
			tp(0, 0.001, 30_000, TAKEOFF_KT + 10),
			tp(0, 0.002, 30_000 + TAKEOFF_SUSTAIN_MS, TAKEOFF_KT + 20),
		]);
		expect(f.motion.takeoffMs).toBe(30_000);
		expect(f.motion.takeoffObserved).toBe(true);
	});

	it('an inherited takeoff leaves the departure row blank while the log runs', () => {
		const legs = [
			{ legNM: 60, cumNM: 60 },
			{ legNM: 60, cumNM: 120 },
		];
		const checkpoints: RouteProgressPoint[] = [
			{ timeMs: 0, distNM: 30, offM: 0 },
			{ timeMs: 100_000, distNM: 34, offM: 0 },
			{ timeMs: 200_000, distNM: 38, offM: 0 },
		];
		const at = (takeoffObserved: boolean) =>
			liveNavlogAt({
				legs,
				checkpoints,
				motion: { ...airborneAt(0), takeoffObserved },
				crossings: extendCrossings(newCrossingFold([60, 120]), checkpoints, [60, 120], 0),
				tMs: 150_000,
				gsLiveKt: 120,
				cruiseKt: 120,
				legGsKt: [null, null],
		...
		const inherited = at(false);
		// Airborne (the gate stands), on leg 0, the target estimating live...
		expect(inherited.takeoffMs).toBe(0);
		expect(inherited.currentLegIdx).toBe(0);
		expect(inherited.wpts[1].etoMs).toBe(150_000 + ((60 - 36) / 120) * 3_600_000);
		// ...but the departure row claims nothing, and neither does the delta.
		expect(inherited.wpts[0].atoMs).toBeNull();
		expect(inherited.planDeltaMin).toBeNull();
		// The same fold with the takeoff observed prints it.
		const observed = at(true);
		expect(observed.wpts[0].atoMs).toBe(0);
		expect(observed.planDeltaMin).not.toBeNull();
	});
});
