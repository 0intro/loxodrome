...
describe('buildTraceProfileDoc airspace bands', () => {
	/** A rectangle over the whole 48N track, of whichever category asked. */
	function box(key: string, type: string, category: AirspaceCategory): Airspace {
		return {
			id: key,
			key,
			type,
			...
			source: 'fr',
			area: 1,
			bbox: { minLat: 47, minLon: -1, maxLat: 49, maxLon: 4 },
		};
	}

	// The FIS sectors are dropped in the trace's OWN walk, not left to the
	// filter row: the row state is shared with the route chart, so a pilot
	// who turns FIS on there must not get a nest of unemphasised sectors
	// here. This chart cannot compute which one is in force, its vertices
	// carrying no planned altitude for the contact ladder to read.
	it('drops FIS sectors from its own walk, and keeps everything else', () => {
		const doc = buildTraceProfileDoc({
			thinned: thinTrace(track),
			airspaces: [
				box('SIV|1', 'SIV', 'siv'),
				box('FIC|1', 'FIC', 'siv'),
				box('TMA|1', 'TMA', 'controlled'),
			],
			terrain: [],
			route: null,
			airports: null,
		...
		const keys = doc.placedBands.map((b) => b.key);
		expect(keys).toContain('TMA|1');
		expect(keys).not.toContain('SIV|1');
		expect(keys).not.toContain('FIC|1');
	});
});
