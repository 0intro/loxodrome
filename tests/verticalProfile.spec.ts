...
import { fromTriple, groundBandAtPoint, type VerticalLimit } from '$lib/vertical/limits';
...
describe('columnFeet', () => {
	it('passes AMSL limits through as feet', () => {
		const f = columnFeet(extent(lim('ALT', '1000', 'FT'), lim('ALT', '4500', 'FT')), groundBandAtPoint(null));
		expect(f).toEqual({ lowerFt: 1000, upperFt: 4500, known: true, topOpen: false });
	});

	it('compares flight levels in feet', () => {
		const f = columnFeet(extent(lim('STD', '115', 'FL'), lim('STD', '195', 'FL')), groundBandAtPoint(null));
		expect(f.lowerFt).toBe(11500);
		expect(f.upperFt).toBe(19500);
	});

	it('converts metres', () => {
		const f = columnFeet(extent(lim('ALT', '600', 'M'), lim('ALT', '1000', 'M')), groundBandAtPoint(null));
		expect(f.lowerFt).toBeCloseTo(1968.5, 0);
	});

	it('resolves an ASFC floor against the point ground', () => {
		const f = columnFeet(extent(lim('HEI', '800', 'FT'), lim('ALT', '4500', 'FT')), groundBandAtPoint(500));
		expect(f.lowerFt).toBe(1300);
	});

	it('draws an AGL limit at face value with no ground known', () => {
		const f = columnFeet(extent(lim('HEI', '800', 'FT'), lim('ALT', '4500', 'FT')), groundBandAtPoint(null));
		expect(f.lowerFt).toBe(800);
	});

	it('sits an SFC floor on the ground', () => {
		const f = columnFeet(extent(lim('SFC', '0', 'FT'), lim('ALT', '2500', 'FT')), groundBandAtPoint(500));
		expect(f.lowerFt).toBe(500);
	});

	it('reports a published UNL ceiling as open-topped', () => {
		const f = columnFeet(extent(lim('SFC', '0', 'FT'), lim('UNL', '0', 'FT')), groundBandAtPoint(null));
	...
	it('distinguishes an unknown ceiling from a published UNL one', () => {
		...
		const f = columnFeet(extent(lim('ALT', '1000', 'FT'), null), groundBandAtPoint(null));
...
/* The sibling windowNoteText structurally cannot make: its counts come from
 * splitting BUILT columns against the window, so they are both zero exactly
 * when a filter emptied the set before a column existed. */
describe('hiddenNoteText', () => {
	const none = { hiddenRows: 0, hiddenBand: 0, hiddenScope: 0 };

	it('says nothing when the filters hid nothing', () => {
		expect(hiddenNoteText(none, en)).toBeUndefined();
	});

	it('names the one cause that accounts for the lot, in both locales', () => {
		expect(hiddenNoteText({ ...none, hiddenScope: 17 }, en)).toBe('17 not drawn on the map');
		expect(hiddenNoteText({ ...none, hiddenScope: 17 }, fr)).toBe('17 non tracés sur la carte');
		expect(hiddenNoteText({ ...none, hiddenRows: 2 }, en)).toBe('2 hidden by the airspace filter');
		expect(hiddenNoteText({ ...none, hiddenBand: 1 }, en)).toBe('1 outside the level band');
	});

	it('gives every cause its own clause, biggest first', () => {
		// The LFPN reading the docstring records, 16 / 7 / 2 across scope, band
		// and rows: the rows are routinely non-zero (aerial activity and the FIS
		// sectors ship off), so a single-cause rule read "the current filters"
		// for the very case it exists to explain, and the filters' own order
		// (rows, band, scope) buried the 16 the map scope had just taken
		// behind the 2 the rows always take.
		const lfpn = { hiddenRows: 2, hiddenBand: 7, hiddenScope: 16 };
		expect(hiddenNoteText(lfpn, en)).toBe(
			'16 not drawn on the map, 7 outside the level band, 2 hidden by the airspace filter',
		);
		expect(hiddenNoteText(lfpn, fr)).toBe(
			'16 non tracés sur la carte, 7 hors de la bande de niveaux, 2 masqués par le filtre d\u2019espaces',
		);
	});

	it('keeps the filter order on a tie', () => {
		expect(hiddenNoteText({ hiddenRows: 2, hiddenBand: 2, hiddenScope: 2 }, en)).toBe(
			'2 hidden by the airspace filter, 2 outside the level band, 2 not drawn on the map',
		);
	});
});

/* The gate on the header's airspace-above figure. Asking "was anything
 * hidden" was tried first and is the wrong question: it suppressed the figure
 * over every French aerodrome, the level band shipping on at 0-10 000 ft with
 * a UTA slab above every stack. */
describe('aboveIsComplete', () => {
	const col = (id: string, lowerFt: number): VerticalColumn =>
		({ id, lowerFt, upperFt: lowerFt + 2000, knownExtent: true, category: 'controlled' }) as VerticalColumn;

	it('holds when the filters did not move the answer', () => {
		const whole = [col('ctr', 1500), col('uta', 19500)];
		const drawn = [col('ctr', 1500)];
		// The UTA is hidden and irrelevant: the CTR is still the one above.
		expect(aboveIsComplete(airspaceAbove(drawn, 500), airspaceAbove(whole, 500))).toBe(true);
	});

	it('fails when a hidden volume IS the one above', () => {
		const whole = [col('ctr', 1500), col('uta', 19500)];
		const drawn = [col('uta', 19500)];
		expect(aboveIsComplete(airspaceAbove(drawn, 500), airspaceAbove(whole, 500))).toBe(false);
	});

	it('fails when the chart was emptied, the case it exists for', () => {
		// airspaceAbove([]) is {column: null}, the same value as a genuinely
		// open sky: this is what stopped a filtered chart reading "no
		// controlled or restricted airspace above" under a class A TMA.
		const whole = [col('tma', 2500)];
		expect(aboveIsComplete(airspaceAbove([], 500), airspaceAbove(whole, 500))).toBe(false);
	});

	it('holds when both are empty', () => {
		expect(aboveIsComplete(airspaceAbove([], 500), airspaceAbove([], 500))).toBe(true);
	});

	it('holds when a filter only changed WHICH volume ties at the same base', () => {
		// Both are surface-based, so airspaceAbove clamps them to the ground
		// and keeps the first; dropping that one leaves the height free under
		// the answer identical. The figure is the answer, the column only
		// names it, so this must not cost the reading.
		const whole = [col('r-area', 500), col('ctr', 500)];
		const drawn = [col('ctr', 500)];
		const a = airspaceAbove(drawn, 500, 0);
		const b = airspaceAbove(whole, 500, 0);
		expect(a.column?.id).not.toBe(b.column?.id);
		expect(a.freeFt).toBe(b.freeFt);
		expect(aboveIsComplete(a, b)).toBe(true);
	});
});
