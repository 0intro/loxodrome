...
describe('committed fr-aerodrome-facilities.json', () => {
	...
	/* The AD 2.4 fuel entry, whole. cmd/fuel reads this column and nothing
	 * else of the AIXM, so its shape is a contract across the language
	 * boundary: null where the AIP files no such record, else an
	 * [avail, text] pair whose text joins the record's three fields per
	 * language rather than taking the first that survives. */
	it('carries the AD 2.4 fuel entry, NIL records included', () => {
		let entries = 0;
		let stated = 0;
		for (const r of doc.rows) {
			const col = r[9];
			...
			const [avail, text] = col as [string, string];
			expect(['yes', 'no']).toContain(avail);
		...
		// The explicit "no fuel here" records the services list drops as an
		// absent facility. Most of them are overseas, where no VAC plate
		// exists to say it instead.
		expect(stated).toBeGreaterThan(10);

		// Bourg-Ceyzeriat states its grades in the remark and its hours in
		// the service-time field; taking the first non-empty field alone
		// showed the hours and lost the grades.
		const lfhs = doc.rows.find((r) => r[0] === 'LFHS')?.[9] as [string, string];
		expect(lfhs[0]).toBe('yes');
		expect(lfhs[1]).toContain('UL 91');
		expect(lfhs[1]).toContain('0900-1600');
	});
