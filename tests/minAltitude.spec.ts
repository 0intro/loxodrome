 ...
 * filter, and the terrain swath geometry. The async terrain sampling
 * (elevationAt) needs a browser and is exercised manually. */
...
describe('corridorSamplePoints', () => {
	...
	it('keeps every sample within the corridor half-width', () => {
		const pts = corridorSamplePoints(a, b, 5, 5, 2.5);
		expect(pts.length).toBeGreaterThan(0);
		// ~1% slack: the planar cos(lat) offset uses the leg's mid-latitude, so the
		// corridor edge is fuzzy by ~0.5% (erring slightly wide, which is safe).
		const limit = 5 * NM_TO_METERS * 1.01;
		for (const p of pts) {
			expect(pointToSegmentDistanceM(p.lat, p.lon, a.lat, a.lon, b.lat, b.lon)).toBeLessThanOrEqual(
				limit,
			);
		}
	});
	...
	it('includes centerline samples (distance ~0 to the leg)', () => {
		const pts = corridorSamplePoints(a, b, 5, 5, 2.5);
		const onLine = pts.filter(
			(p) => pointToSegmentDistanceM(p.lat, p.lon, a.lat, a.lon, b.lat, b.lon) < 1,
		);
		expect(onLine.length).toBeGreaterThan(0);
	});

	it('builds a grid of (2*cross+1) lines wide', () => {
		// 5 NM half-width at a 2.5 NM cross step -> 2 each side -> 5 cross lines.
		const pts = corridorSamplePoints(a, b, 5, 5, 2.5);
		expect(pts.length % 5).toBe(0);
	});
});
