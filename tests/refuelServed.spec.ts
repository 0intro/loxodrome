...
import type { FuelAccess } from '$lib/data/fuel';
...
describe('firstPerIdent', () => {
	it('folds an aerodrome the plan leaves twice', () => {
		// The uplift points are the DEPARTURE of each trip, so an out-and-
		// back-and-out plan (A-B, B-A, A-B) has A in two columns. Both lists
		// under the fuel table are keyed by ident, and Svelte throws on a
		// duplicate key in production as well as in dev; the same sentence
		// printed twice was the milder half of the same bug.
		const rows = [{ ident: 'LFGS', n: 1 }, { ident: 'LFAC', n: 2 }, { ident: 'LFGS', n: 3 }];
		expect(firstPerIdent(rows)).toEqual([
			{ ident: 'LFGS', n: 1 },
			{ ident: 'LFAC', n: 2 },
		]);
		expect(firstPerIdent([])).toEqual([]);
	});
});
...
describe('servedRecommendation', () => {
	...
	it('is never moved by a caution', () => {
		// O/R and HX say the fuel is there and wants arranging, not that it
		// is out. An advisory that demoted a strategy would be exactly the
		// silent change this feature refuses to make.
		for (const a of ['onRequest', 'noHours', 'h24'] as const) {
			const pts = points([true, true, true], [a, a, a]);
			expect(plan.strategies.every((s) => strategyIsServed(s, pts)), a).toBe(true);
			const r = servedRecommendation(plan, pts);
			expect(r.best, a).toBe(plan.recommended);
			expect(r.demoted, a).toBeNull();
		}
	});

	it('routes around a field that reserves its fuel for based aircraft', () => {
		// The fifth reason reaches here through `serves`, exactly as a dry
		// field does, so nothing in this module knows it is new.
		const pts = points([true, true, false]);
		pts[2] = { ...pts[2], refusal: { kind: 'aipBased' }, access: 'basedOnly' };
		const r = servedRecommendation(plan, pts);
		expect(r.demoted!.stops).toEqual([1]);
		expect(r.demotedAt?.refusal?.kind).toBe('aipBased');
		expect(r.best!.stops).toEqual([0]);
	});
