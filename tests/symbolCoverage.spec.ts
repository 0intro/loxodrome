...
const TYPE_FAMILY: Record<string, string> = {
	...
	// activity family: official pictogram. MODEL / AEROBATIC / UAS are the
	// kinds cmd/fr reads off the SIA's own txtLocalType instead of folding
	// into the generic ACTIVITY bucket (docs/fr-aip.md).
...
describe('airspace dataset coverage', () => {
	...
	it('carries no generically-typed activity row, and no nature twin', () => {
		// cmd/fr reads the kind off the SIA's own txtLocalType now, so a
		// French row never falls to the generic bucket; and the PRN / SUR
		// nature-protection zones are fr-nature.json's alone, no longer
		// published a second time as airspace around a 0.5 NM bubble the
		// AIP never drew (docs/fr-aip.md).
		const fr = [...combos.values()].filter((a) => a.source === 'fr');
		expect(fr.filter((a) => a.type === 'ACTIVITY').map((a) => a.id)).toEqual([]);
		expect(fr.filter((a) => a.subtype === 'PRN' || a.subtype === 'SUR').map((a) => a.id)).toEqual(
			[],
		);
	});
