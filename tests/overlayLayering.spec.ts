...
const pages = read('src/lib/components/phone/PhonePages.svelte');
...
describe('overlay z bands', () => {
	...
	it('a phone page sits over the shared surfaces and under the popup band', () => {
		// The pane (a docked or paged surface) lives on under a page and
		// comes back on Back; a sheet raised from the page must still clear
		// the page.
		const page = zOf(pages, '.phone-page');
		expect(page).toBeGreaterThan(sharedBox);
		expect(page).toBeLessThan(zOf(head, '.scrim'));
	});
