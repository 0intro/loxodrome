...
describe('About data-source coverage', () => {
	...
	/* The credits are classified by TYPE of data, one section each, and the
	 * sequence is the contract: what a source IS is what files it, so the
	 * next publisher goes in the section its data belongs to rather than
	 * after whatever happens to be last. Appending a section, or moving one,
	 * fails here until the order below is updated deliberately. */
	it('renders the credit sections by data type, in order', () => {
		const modal = src('lib/components/AboutModal.svelte');
		const sections = [
			'aipHeading',
			'adChartsHeading',
		...
		const at = sections.map((key) => {
			const hits = [...modal.matchAll(new RegExp(`<h3>\\{t\\.about\\.${key}\\}</h3>`, 'g'))];
			expect(hits.length, `${key} should head exactly one section`).toBe(1);
			return { key, at: hits[0]?.index ?? -1 };
		});
		expect(
			[...at].sort((a, b) => a.at - b.at).map((s) => s.key),
			'the credit sections are out of the documented order, the classification by data type',
		).toEqual(sections);
	});

	/* The delivery-shaped headings the taxonomy replaced. Their absence is
	 * what keeps a new source from being filed by how it is fetched. */
	it('no longer groups sources by how they are delivered', () => {
		const modal = src('lib/components/AboutModal.svelte');
		for (const gone of ['dataSetsHeading', 'liveServicesHeading', 'bundledModelsHeading']) {
			expect(modal, `${gone} is back; the credits group by data type now`).not.toContain(gone);
		}
	});
});
