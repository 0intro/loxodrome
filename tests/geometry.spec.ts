...
/* notamMarkerBbox is the box for framing MANY entries: where they are drawn,
 * never how far they reach. The distinction is the difference between a usable
 * fit and a useless one. Measured over three real briefings, the radius-padded
 * union was the IDENTICAL 18.1 x 23.4 degrees each time, because two 460 NM
 * navaid outages reach into every French briefing; the marker union was 5.2 x
 * 5.9, 5.0 x 11.3 and 6.5 x 8.6. A box that cannot tell three routes apart is
 * not framing any of them. */
describe('notamMarkerBbox', () => {
	it('ignores a radius the size of a country', () => {
		const wide = entry({ coordinates: [coord('psn', 48, 2, 460)] });
		const b = notamMarkerBbox(wide);
		// A bare point, nudged open so bbox edge-equality does not bite; the
		// 460 NM is nowhere in it.
		expect(b?.maxLat).toBeLessThan(48.01);
		expect(b?.minLat).toBeGreaterThan(47.99);
		// What the other one does with the same entry, for contrast.
		expect(notamFocusBbox(wide)?.maxLat).toBeGreaterThan(55);
	});

	it('still spans a polygon, which IS drawn where its vertices are', () => {
		const b = notamMarkerBbox(
			entry({
				isPolygon: true,
				coordinates: [coord('psn', 48, 2, 10), coord('psn', 49, 2), coord('psn', 49, 3)],
			}),
		);
		expect(b?.minLat).toBeCloseTo(48, 6);
		expect(b?.maxLat).toBeCloseTo(49, 6);
		expect(b?.maxLon).toBeCloseTo(3, 6);
	});

	it('answers null for an entry with no coordinates at all', () => {
		expect(notamMarkerBbox(entry({ coordinates: [] }))).toBeNull();
	});
});
...
/* The footprint measures the click resolver ranks by (docs/map-hit-testing.md):
 * every area kind has to arrive in the same unit before the smallest drawn
 * shape can be picked out of a mixed stack. */
describe('footprint measures', () => {
	it('scales a degree² shoelace to metres at its own latitude', () => {
		// One square degree at the equator is (111320 m)²; at 60° N the
		// longitude degrees are half as wide.
		expect(degAreaToM2(1, 0)).toBeCloseTo(M_PER_DEG * M_PER_DEG, 0);
		expect(degAreaToM2(1, 60) / degAreaToM2(1, 0)).toBeCloseTo(0.5, 6);
	});

	it('measures a ring and a circle of the same ground area alike', () => {
		// A 0.1° × 0.1° box at 48 N, and the circle of equal area.
		...
		const boxM2 = ringAreaM2(ring);
		expect(boxM2).toBeCloseTo(degAreaToM2(0.01, 48.05), 0);
		expect(circleAreaM2(Math.sqrt(boxM2 / Math.PI))).toBeCloseTo(boxM2, 0);
	});

	it('is zero for a ring that is not one', () => {
		expect(ringAreaM2([[48, 2], [48, 3]])).toBe(0);
	});
});

/* notamShapes is what the click resolver indexes: ONE entry can draw several
 * disjoint circles, and the one under the cursor is the one whose size decides
 * the click. */
describe('notamShapes', () => {
	it('gives a polygon entry its single ring, measured whole', () => {
		const shapes = notamShapes(
			entry({
				isPolygon: true,
				coordinates: [
					coord('psn', 48, 2),
		...
		expect(shapes[0].kind).toBe('ring');
		expect(shapes[0].bbox).toEqual({ minLat: 48, minLon: 2, maxLat: 48.1, maxLon: 2.1 });
		expect(shapes[0].areaM2).toBeGreaterThan(0);
	});

	it('gives a position entry one circle per radius, each its own size', () => {
		const shapes = notamShapes(
			entry({ coordinates: [coord('psn', 48, 2, 1), coord('psn', 45, 5, 30)] }),
		);
		expect(shapes.map((s) => s.kind)).toEqual(['circle', 'circle']);
		// Ranked apart: the wide one is 900 times the narrow one.
		expect(shapes[1].areaM2 / shapes[0].areaM2).toBeCloseTo(900, 0);
		// And each answers for its own ground, so a click inside the wide
		// circle ranks as the wide circle and not as the entry's smallest.
		expect(shapeContainsPoint(shapes[1], 45.1, 5)).toBe(true);
		expect(shapeContainsPoint(shapes[0], 45.1, 5)).toBe(false);
	});

	it('leaves out the Q-line radius circle and a bare pin', () => {
		expect(notamShapes(entry({ coordinates: [coord('qualifierLine', 48, 2, 50)] }))).toEqual([]);
		expect(notamShapes(entry({ coordinates: [coord('psn', 48, 2)] }))).toEqual([]);
	});

	it('bounds a circle by its own radius, longitude widened by latitude', () => {
		const [c] = notamShapes(entry({ coordinates: [coord('psn', 48, 2, 10)] }));
		const padLat = (10 * NM_TO_METERS) / M_PER_DEG;
		expect(c.bbox.maxLat).toBeCloseTo(48 + padLat, 6);
		expect(c.bbox.maxLon).toBeCloseTo(2 + padLat / Math.cos((48 * Math.PI) / 180), 6);
		// The bbox is the index's prefilter, so it must never be tighter than
		// the containment test it stands in front of.
		expect(shapeContainsPoint(c, c.bbox.maxLat - 1e-9, 2)).toBe(true);
	});
});
