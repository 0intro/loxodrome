...
const ALLOWED: Record<string, string[]> = {
	// France files paraglider winching under WP as well as parachuting:
	// W1970/26 is "PARAGLIDING WINCHING OVER BEGANNE - SITE 2", and the
	// classifier is right to read the craft the text names.
	WP: ['parachute', 'paragliding'],
	WB: ['voltige'],
...
describe('classifier vs Q-subject over the France corpora', () => {
	...
	it('keeps the activity subjects classified (unclassified counts)', () => {
		...
		// The eighteenth OB entry is a French obstacle NOTAM that used to be
		// shadowed by another state's NOTAM of the same bare number.
		...
		expect(counts).toEqual({
			...
			'world-fr-20260610.txt': { OB: 18, WU: 1 },
		});
	});
});
