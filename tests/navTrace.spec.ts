...
describe('smoothedMotionAt', () => {
	...
	it('reads the window bracket, not the whole array (causal + truncation)', () => {
		// The window walk starts at the bracket now instead of scanning down
		// from the array's end, so an evaluation at any playhead must equal
		// one over a trace cut just past it: fixes after tMs never mattered,
		// and fixes before the 30 s window never mattered either.
		const pts: TrackPoint[] = [];
		for (let i = 0; i < 10_000; i++) {
			// A gently turning, accelerating track with a duplicate-stamp pair
			// every 97 fixes (an imported trace may carry equal stamps).
			const t = i * 400 - (i % 97 === 0 && i > 0 ? 400 : 0);
			pts.push({
				lat: 48 + i * 1e-5,
				lon: 2 + i * 2e-5,
				altFt: 1000 + i * 0.1,
		...
		const ts = [0, 1, 399, 400, 30_000, 31_000, 200_000, 1_999_999, 4_000_000, 4_100_000];
		for (let k = 0; k < 40; k++) {
			ts.push(Math.floor((k * 4_000_000) / 40) + (k % 3));
		}
		for (const t of ts) {
			const whole = smoothedMotionAt(pts, t);
			const cutIdx = pts.findIndex((p) => p.timeMs > t);
			const cut = smoothedMotionAt(cutIdx < 0 ? pts : pts.slice(0, cutIdx), t);
			expect(cut).toEqual(whole);
			const from = pts.findIndex((p) => p.timeMs >= t - 31_000);
			const windowed = smoothedMotionAt(
				pts.slice(Math.max(0, from - 1), cutIdx < 0 ? pts.length : cutIdx),
				t,
			);
			if (whole != null && from > 1) {
				expect(windowed?.speedKt).toBeCloseTo(whole.speedKt, 6);
				expect(windowed?.trackDeg ?? NaN).toBeCloseTo(whole.trackDeg, 6);
			}
		}
		expect(smoothedMotionAt(pts, -1)).toBeNull();
	});
});
