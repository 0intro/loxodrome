...
/** A route KML, the shape route/routeKml.ts writes: a line and a pin per
 *  place, which files/detect.ts sniffs as a PLAN rather than a trace. */
const ROUTE_KML =
	'<?xml version="1.0"?><kml xmlns="http://www.opengis.net/kml/2.2"><Document>' +
	'<name>LFPL-LFQB</name>' +
	'<Placemark><LineString><coordinates>2.62,48.82,457 2.87,48.79,457</coordinates></LineString></Placemark>' +
	'<Placemark><name>LFPL</name><Point><coordinates>2.62,48.82,457</coordinates></Point></Placemark>' +
	'<Placemark><name>LFQB</name><Point><coordinates>2.87,48.79,457</coordinates></Point></Placemark>' +
	'</Document></kml>';
const FPL =
	'<?xml version="1.0"?><flight-plan xmlns="http://www8.garmin.com/xmlschemas/FlightPlan/v1">' +
	'<waypoint-table><waypoint><identifier>LFPL</identifier><type>AIRPORT</type><country-code/>' +
	'<lat>48.82</lat><lon>2.62</lon><comment/></waypoint></waypoint-table>' +
	'<route><route-name>X</route-name><flight-plan-index>1</flight-plan-index>' +
	'<route-point><waypoint-identifier>LFPL</waypoint-identifier>' +
	'<waypoint-type>AIRPORT</waypoint-type><waypoint-country-code/></route-point></route></flight-plan>';
...
describe('readIncoming', () => {
	...
	/* A KMZ is a KML container whatever the KML holds: Google Earth zips a
	 * route the same way it zips a tour, and the one member IS the document
	 * the user opened. */
	it('reads a KMZ holding a route KML as that route', async () => {
		const got = await readIncoming(
			'LFPL-LFQB_plan.kmz',
			archive([{ name: 'doc.kml', body: ROUTE_KML }]),
		);
		expect(got.kind === 'single' && [got.name, got.text]).toEqual([
			'LFPL-LFQB_plan.kmz',
			ROUTE_KML,
		]);
	});

	/* But the rule above it stands for every other plan: a ZIP holding one
	 * .fpl is a library with one plan in it, not a container around it. */
	it('reads a one-FPL archive as a bundle, not as a route file', async () => {
		const got = await readIncoming('one.zip', archive([{ name: 'plan.fpl', body: FPL }]));
		expect(got.kind).toBe('bundle');
		expect(got.kind === 'bundle' && got.members.map((m) => m.kind)).toEqual(['plan']);
	});
