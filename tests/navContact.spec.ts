...
/* The stretches the vertical profile emphasises. This is the fold that
 * makes a FIS band honest: the sectors NEST, so a band per sector shows
 * two frequencies and answers neither, and only the stretch says which
 * one a pilot calls where. */
describe('contactStretches', () => {
	// SIV SEINE 1 (an APP-run sector) sitting inside FIC PARIS SUD (the
	// FIR-level blanket), which is the real Melun shape.
	const fic = ev('enter', 0, 'FIC|PARIS SUD', { type: 'FIC', category: 'siv', radio: FIS });
	const siv = ev('enter', 20, 'SIV|SEINE 1', { type: 'SIV', category: 'siv', radio: FIS });
	const sivOut = ev('leave', 60, 'SIV|SEINE 1', { type: 'SIV', category: 'siv', radio: FIS });

	it('gives the nested sector the stretch it wins, and the blanket the rest', () => {
		const spans = buildContactSpans([fic, siv, sivOut], false);
		expect(contactStretches(spans, 100)).toEqual([
			{ key: 'FIC|PARIS SUD', fromNM: 0, toNM: 20 },
			{ key: 'SIV|SEINE 1', fromNM: 20, toNM: 60 },
			{ key: 'FIC|PARIS SUD', fromNM: 60, toNM: 100 },
		]);
	});

	it('clamps a span the flight never leaves to the route length', () => {
		// buildContactSpans leaves such a span at Infinity.
		const spans = buildContactSpans([fic], false);
		expect(contactStretches(spans, 42)).toEqual([{ key: 'FIC|PARIS SUD', fromNM: 0, toNM: 42 }]);
	});

	it('coalesces the stretches either side of an unrelated edge', () => {
		// A sector entered and left inside the blanket must not chop the
		// blanket into three rows where two would do... and it does chop it,
		// because the winner genuinely changes in the middle. What must NOT
		// happen is a repeated key across a single boundary.
		const spans = buildContactSpans([fic, siv, sivOut], false);
		const out = contactStretches(spans, 100);
		for (let i = 1; i < out.length; i++) {
			expect(out[i].key === out[i - 1].key && out[i].fromNM === out[i - 1].toNM).toBe(false);
		}
	});

	it('answers nothing when no span carries a usable frequency', () => {
		const mute = ev('enter', 0, 'SIV|MUTE', { type: 'SIV', category: 'siv', radio: [] });
		expect(contactStretches(buildContactSpans([mute], false), 50)).toEqual([]);
	});
});
