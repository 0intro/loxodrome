...
// Every tile fails to load (offline): a tile the server answered it does not
// hold is the sea at a complete level, an ANSWER, and is its own case.
vi.mock('$lib/offline/passiveStore', () => ({
	passiveFetchBlob: (): Promise<Blob | null> => Promise.reject(new Error('offline')),
}));
