...
describe('stripPlotWidth', () => {
	...
	it('zooms the scale in force, not the floor, on a plan that fits', () => {
		// A short plan is drawn at the BOX's scale, above the floor; zooming
		// the floor instead left the pinch inert until its product overtook
		// the box, and capped the real magnification at a fraction of the
		// ring's own maximum.
		expect(stripPlotWidth(10, 392, 2)).toBe(784);
		expect(stripPlotWidth(10, 392, 8)).toBe(392 * 8);
	});
});
