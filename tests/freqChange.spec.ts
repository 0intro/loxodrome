...
/* The SIA quotes the value it is stating, the way it quotes a callsign.
 * C1108/26 LFLY (2026-09-18) is a QCACS 8.33 kHz conversion, so it states no
 * CF/ME condition either: with the quotes unread, nothing moved AND nothing
 * was flagged, which is the one outcome the resolver doctrine forbids. */
describe('quoted values', () => {
	it('reads a replacement whose values are quoted', () => {
		const quoted = "FREQ SAINT EX TWR MODIFIED:\nREAD '120.455MHZ' (INSTEAD OF '120.450MHZ')";
		expect(allReplacements(quoted)).toEqual([{ freq: '120.455', was: '120.450' }]);
		// Identical to the unquoted form, which is the point.
		const plain = 'FREQ SAINT EX TWR MODIFIED:\nREAD 120.455MHZ (INSTEAD OF 120.450MHZ)';
		expect(allReplacements(quoted)).toEqual(allReplacements(plain));
	});

	it('reads a quoted labelled assignment', () => {
		expect(parseFreqAssignments("ATIS: '120.005MHZ' REPLACES '120.000MHZ'")).toEqual([
			{ label: 'ATIS', freq: '120.005', was: '120.000' },
		]);
	});

	it('still reads the quoted callsign form it has always read', () => {
		// A3345/26: the quotes sit on the SERVICE name, not the value.
		expect(allFreqs("'BEAUVAIS' INFO FREQ 119.800MHZ NOT AVBL")).toEqual(['119.800']);
	});
});
