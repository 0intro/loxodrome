...
/* The French phrase, which for want of three letters read as unserviceable in
 * English and as nothing in French: `\bNO\s+` cannot reach across the N of
 * NON, so the Spanish alternation never covered it. The two halves of one
 * bilingual NOTAM must classify alike (tests/bilingual.spec.ts). */
describe('the French NON DISPONIBLE', () => {
	it('reads as unserviceable, singular and plural', () => {
		// D3065/26 LFKO, whose English half is "AVGAS 100LL NOT AVBL."
		expect(serviceStatusFromText('AVGAS NON DISPONIBLE')).toBe('unserviceable');
		// M1339/26 LFKS.
		expect(serviceStatusFromText('AVITAILLEMENT EN 100LL (F18) NON DISPONIBLE.')).toBe(
			'unserviceable',
		);
		// A0081/26 Goma, the plural form.
		expect(serviceStatusFromText('SERVICE CA ET D,AEDROME NON DISPONIBLES.')).toBe(
			'unserviceable',
		);
	});

	it('keeps the Spanish form it was mistaken for', () => {
		expect(serviceStatusFromText('SERVICIO NO DISPONIBLE')).toBe('unserviceable');
	});

	it('does not fire on a plain availability statement', () => {
		expect(serviceStatusFromText('JET A1 DISPONIBLE AVEC 01HR DE PREAVIS')).toBe('');
	});
});

describe('French agrees its adjectives', () => {
	// One letter decides whether a service reads as out. The English half of
	// each of these says NOT AVBL and classifies; the French half must too,
	// or a FIS closure or a fuel withdrawal reaches only one of the two
	// briefings (tests/bilingual.spec.ts pins the whole tier).
	it('reads the plural outage adjectives', () => {
		expect(serviceStatusFromText('POSTES PO-14 A PO-16 INDISPONIBLES.')).toBe('unserviceable');
		expect(serviceStatusFromText('DEP OMNIDIRECTIONNEL RWY 14/32 INUTILISABLES')).toBe(
			'unserviceable',
		);
		expect(serviceStatusFromText('SERVICES DU CONTROLE ET D\u2019INFORMATION DE VOL INDISPONIBLES')).toBe(
			'unserviceable',
		);
		// The singular still reads, and so does the Spanish it sits beside.
		expect(serviceStatusFromText('AVGAS INDISPONIBLE')).toBe('unserviceable');
		expect(serviceStatusFromText('VOR NO DISPONIBLE')).toBe('unserviceable');
	});
});
