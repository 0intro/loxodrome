...
describe('typeAdjacentNameInText (the widened, any-subject form)', () => {
	...
	it('matches the SIA reversal, name then type', () => {
		// E4227/26 and C3494/26 (2026-09-18): the RMZ NOTAMs are written this
		// way round, and their QAZ subject maps to no type of its own, so this
		// adjacency is the only link they have.
		expect(
			typeAdjacentNameInText('RMZ', 'LOGNES EMERAINVILLE', "'LOGNES EMERAINVILLE' RMZ ATS SKED"),
		).toBe(true);
		expect(typeAdjacentNameInText('RMZ', 'LE HAVRE', "AFIS SKED ('LE HAVRE' RMZ)")).toBe(true);
	});
	...
	it('rejects names without their type word adjacent', () => {
		...
		// The reversal is adjacency too: a type word a sentence away is not it.
		expect(typeAdjacentNameInText('CTR', 'MELUN', 'MELUN INFO 120.500MHZ, CTR ACTIVE')).toBe(
			false,
		);
	});
});
