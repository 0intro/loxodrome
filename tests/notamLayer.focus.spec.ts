...
describe('fitToNotams', () => {
	it('unions where the entries are DRAWN, not how far they reach', () => {
		// Framing many is not framing one. A NOTAM's radius of influence
		// routinely covers a country, and over three real briefings the
		// radius-padded union came out at the identical 18.1 x 23.4 degrees
		// because two 460 NM navaid outages reach into every French briefing:
		// a box that cannot tell three routes apart frames none of them
		// (notamMarkerBbox in notam/geometry.ts). So the 10 NM below must NOT
		...
		// The 1e-4 is coordsBbox's own nudge on a single-point box, which each
		// of these entries is; it is four orders below the 10 NM padding the
		// assertion below rules out.
		const [bounds, opts] = fitBounds.mock.calls[0];
		expect(bounds[0][0]).toBeCloseTo(44 - 1e-4, 6);
		expect(bounds[1][0]).toBeCloseTo(48 + 1e-4, 6);
		// Well inside where the 10 NM padding would have put it.
		expect(bounds[1][0]).toBeLessThan(48 + PAD_10NM_DEG / 2);
		// The CAP still reads the drawn extent: this list has a radius circle,
		// so it may frame closer than a list of bare pins.
