...
async function loadCommitted(file = 'fr-supaip.json'): Promise<SupAip[]> {
...
const esSupsPromise = loadCommitted('es-supaip.json');
...
describe('SUP AIP corpus rendering (2026 onward)', () => {
	it('draws every zone as exactly its stated shape', async () => {
		...
		// plus the MAKEMO ALPHA / BRAVO and KATIU circles; the 2026-09-01
		// full rebuild then added metropole 185 to 193, nine supplements
		// carrying 13 zones between them, none by reference; the
		// 2026-09-06 rebuild onto AIRAC 2026-09-03 then added metropole
		// 194 to 200, seven supplements carrying 20 zones between them,
		// none by reference; the 2026-09-13 publish catch-up then added
		// metropole 201 to 208, eight supplements carrying 32 zones
		// between them, none by reference; the corpus audit that followed
		// then moved it by THREE, the only time this number has moved for
		// a parser fix rather than a refresh: 206/2026 gained its two ZRT
		// DEDA ALPHA columns and 181/2026 its ZRT 3, all three lost to a
		// row break falling inside a coordinate, 080/2026 gained ZRT
		// LIBECCIU C and 100/2026 ZRT LESSAY, two circles whose centre
		// broke the same way, and 193/2026 gave up the one zone it never
		// had, an apron stand table read as a boundary. 029/2026 is the
		// third: its four ZRT FOX columns stopped sharing one fused name,
		// and a duplicate read short now dedupes against its twin); the
		// 2026-09-18 rebuild then added metropole 209 to 214, six
		// supplements carrying 14 zones between them, none by reference
		// and none from the arc fix that rebuild carried, which moved
		// 195/2026's ZRT COUPE ICARE from the minor arc to the major one
		// without changing its zone count.
		expect(zones).toBe(807);
...
describe('ES SUP AIP corpus (cmd/es)', () => {
	it('draws every zone as exactly its stated shape', async () => {
		const sups = await esSupsPromise;
		expect(sups.length).toBeGreaterThan(150);
		let zones = 0;
		for (const sup of sups) {
			expect(sup.region).toBe('es');
			for (const z of sup.zones) {
				zones++;
				expect(z.geometry).not.toBeNull();
				expect(z.bbox).not.toBeNull();
				const g = z.geometry;
				if (g === null) {
					continue;
				...
				polygons.length = 0;
				circles.length = 0;
				makeSupGeometryLayer(g, {});
				if (g.type === 'polygon') {
					expect(g.ring.length).toBeGreaterThanOrEqual(4);
					expect(g.ring[0]).toEqual(g.ring[g.ring.length - 1]);
					expect(polygons).toHaveLength(1);
					expect(polygons[0].latlngs).toEqual(g.ring);
				} else if (g.type === 'multipolygon') {
					expect(polygons).toHaveLength(1);
		...
		// The corpus size, moved only with a verified dataset change.
		expect(zones).toBeGreaterThan(400);
	});

	it('keeps every vertex inside Spain and inside its own bbox', async () => {
		for (const sup of await esSupsPromise) {
			for (const z of sup.zones) {
				const g = z.geometry;
				const b = z.bbox;
				if (!g || !b) {
				...
				const pts =
					g.type === 'polygon'
						...
						: g.type === 'multipolygon'
							...
							: [g.center];
				for (const [lat, lon] of pts) {
					// A vertex outside Spanish territory would be a misread
					// column or a swapped pair.
	...
	it('carries the local subject, both editions and a parseable validity', async () => {
		const sups = await esSupsPromise;
		const dfn = sups.find((s) => s.number === 149 && s.year === 2026);
		expect(dfn).toBeDefined();
		// descriptionFr is the LOCAL-language slot: Spanish here.
		expect(dfn!.descriptionFr).toContain('FIESTA NACIONAL');
		expect(dfn!.descriptionEn).not.toBe('');
		expect(dfn!.urlPdf).toMatch(/LE_SUP_A_2026_149_es\.pdf$/);
		expect(dfn!.urlPdfEn).toMatch(/_en\.pdf$/);
		expect(dfn!.zones).toHaveLength(4);
		// The SPA hides a zone whose validity does not parse, so an EST
		// end date must still be a plain ISO day.
		for (const s of sups) {
			if (s.validFrom !== null) {
				expect(s.validFrom).toMatch(/^\d{4}-\d{2}-\d{2}$/);
			}
			if (s.validTo !== null) {
				expect(s.validTo).toMatch(/^\d{4}-\d{2}-\d{2}$/);
			}
		}
	});
});
