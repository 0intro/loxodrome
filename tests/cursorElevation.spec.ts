...
/** Tiles whose load FAILED: nothing decoded, and nothing to ask for until the
 *  cache's retry delay is over. */
...
vi.mock('$lib/map/terrain', () => ({
	...
	terrainPointDue: (lat: number, lon: number): boolean => {
		const tile = tileOf(lat, lon);
		return !decoded.has(tile) && !failed.has(tile);
	},
...
describe('the cursor elevation probe', () => {
	...
	it('asks once over a tile whose load failed, not once a pass', async () => {
		// The failed tile peeks as unknown like one in flight, and re-arming on
		// that alone probed every 150 ms of rest and spent the bucket.
		probeCursorElevation(LAT, LON);
		failed.add(tileOf(LAT, LON));
		for (let i = 0; i < 20; i++) {
			await rest();
		}
		expect(asked).toEqual([tileOf(LAT, LON)]);
		expect(cursorElevation.ft).toBeNull();
		// Nor does a new rest on it ask again before the retry delay.
		probeCursorElevation(LAT, LON + 0.01);
		await rest();
		expect(asked).toEqual([tileOf(LAT, LON)]);
		// Fresh ground elsewhere still answers.
		probeCursorElevation(LAT, LON + NEW_TILE_DEG);
		await rest();
		expect(cursorElevation.ft).toBe(groundFt(LAT, LON + NEW_TILE_DEG));
	});
