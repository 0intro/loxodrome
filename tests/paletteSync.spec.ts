...
describe('the activation hatch defs', () => {
	// They moved out of MapView into a component BOTH maps render: the
	// patterns resolve by id across the document, so a map that omitted them
	// painted an unresolvable fill and drew nothing, silently, which is what
	// the NOTAM Viewer did (tests/notamViewerSite.spec.ts pins the two mounts).
	it('keeps the SUP AIP hatch literal at #c2185b', () => {
		const svelte = read('src/lib/components/HatchDefs.svelte');
		const m = svelte.match(/id="hatch-supaip"[\s\S]{0,200}?fill="(#[0-9a-fA-F]{6})"/);
		expect(m?.[1].toLowerCase()).toBe('#c2185b');
	});

	it('leaves no second copy behind in either map', () => {
		// Two copies is how the literal above drifts from the palette in one
		// app and not the other.
		for (const f of [
			'src/lib/components/MapView.svelte',
			'src/lib/components/NotamMapView.svelte',
		]) {
			expect(read(f), f).not.toContain('id="hatch-supaip"');
		}
	});
});
