...
describe('parseServiceClosure', () => {
	...
	it('reads a substitute whose name wraps onto the next line', () => {
		const one = "BEAUVAIS INFO 119.800MHZ NOT AVBL, FIS AREA 2 CLOSED, CONTACT 'PARIS INFO' 125.700MHZ";
		const wrapped = "BEAUVAIS INFO 119.800MHZ NOT AVBL, FIS AREA 2 CLOSED, CONTACT 'PARIS\nINFO' 125.700MHZ";
		expect(parseServiceClosure(wrapped)).toEqual(parseServiceClosure(one));
		expect(parseServiceClosure('BEAUVAIS INFO 119.800MHZ\nNOT AVBL, CONTACT PARIS\nINFO\n125.700MHZ').substitutes).toEqual([
			{ label: 'PARIS INFO', freq: '125.700' },
		]);
	});
...
describe('the unit', () => {
	it('reads a decimal frequency stated without MHZ', () => {
		// C3443/26 EDHL: the publisher left the unit off, so the closure could
		// only ever be flagged, never applied.
		expect(parseServiceClosure('LUEBECK GROUND FREQ 121.78 U/S.').freq).toBe('121.78');
		expect(parseServiceClosure("'RENNES' FIS 126,950 NOT AVBL").freq).toBe('126.950');
	});

	it('still requires the unit after a whole number', () => {
		expect(parseServiceClosure('SECTOR 123 NOT AVBL').freq).toBe(null);
		expect(parseServiceClosure('FREQ 123 MHZ NOT AVBL').freq).toBe('123');
	});

	it('refuses a value carrying a distance unit', () => {
		expect(parseServiceClosure('RADAR SERVICE WITHIN 100.5 NM NOT AVBL').freq).toBe(null);
	});

	it('refuses the TAIL of a longer number', () => {
		// With the unit optional, a left guard matters as much as the right
		// one: the stated value SELECTS which published rows close.
		expect(parseServiceClosure('AD ELEV 1132.5 NOT AVBL').freq).toBe(null);
		expect(parseServiceClosure('FREQ 132.5 NOT AVBL').freq).toBe('132.5');
	});
});
...
describe('applyClosures', () => {
	...
	it('says nothing more of a second closure of a row already closed', () => {
		// A month's closure and a weekend one, both in force: the row closes
		// on the first, and the second once raised "could not be applied"
		// for a withdrawal that IS applied. The first keeps the credit.
		const r = applyClosures([SIV2, SIV1], [cand(), cand({ source: 'A5001/26' })]);
		expect(r.flags).toEqual([]);
		expect(r.radios[0].closed).toBe(true);
		expect(r.radios[0].closedBy?.source).toBe('A4694/26');
		expect(r.radios[1].closed).toBeUndefined();
	});
...
/* F1698/26 LFBO (2026-09-18), the Toulouse QSELT family: the English half
 * hedges with MAY BE where the French half writes POTENTIELLEMENT. Both must
 * reach the flag, or the value is named to a French reader and withheld from
 * an English one. */
describe('the hedge both languages use', () => {
	it('names the value through either hedge', () => {
		const en = parseServiceClosure(
			'FLT INFO SERVICE ON TOULOUSE INFO 121.250MHZ MAY BE UNAVAILABLE : - AN AUTOMATIC INFO TRANSMITTER IS ACT ON THIS FREQ',
		);
		const fr = parseServiceClosure(
			"SERVICE D'INFORMATION DE VOL SUR TOULOUSE INFO 121.250MHZ POTENTIELLEMENT INDISPONIBLE : - UN REPONDEUR AUTOMATIQUE D'INFORMATION EST ACTIF SUR CETTE FREQUENCE",
		);
		expect(en?.freq).toBe('121.250');
		expect(fr?.freq).toBe('121.250');
	});
});
