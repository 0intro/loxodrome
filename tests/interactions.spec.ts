...
import { PANEL_ZOOM_CEILING, PANEL_ZOOM_SLACK, type VacPanel, type VacPanelKind } from '$lib/data/vacgeo';
...
const m = vi.hoisted(() => ({
	...
	getAirspaces: vi.fn(),
	vacPanelsAt: vi.fn(),
	vacPanelsDrawnAt: vi.fn(),
}));

vi.mock('$lib/state/vacGeo.svelte', () => ({ vacPanelsAt: m.vacPanelsAt }));
vi.mock('$lib/map/vacPanelLayer', () => ({ vacPanelsDrawnAt: m.vacPanelsDrawnAt }));
...
vi.mock('$lib/map/airspaceLayer', () => ({ airspaceAreaAt: m.airspaceAreaAt }));
vi.mock('$lib/map/metarLayer', () => ({ stationNear: m.stationNear }));
vi.mock('$lib/map/radarLayer', () => ({ radarSampleAt: () => null }));
...
vi.mock('$lib/state/notamHit.svelte', () => ({
	notamAreaAt: m.notamAreaAt,
	notamAreasAt: m.notamAreasAt,
}));
// The four area gatherers are stubbed together: featureAt's second tier asks
// every one of them and ranks the answers, so a test that says nothing about a
// kind has to get a miss from it rather than whatever the real module would
// make of an empty app.
vi.mock('$lib/state/supaip.svelte', () => ({
	supaipAreaAt: m.supaipAreaAt,
	supaipZonesAt: m.supaipZonesAt,
}));
vi.mock('$lib/state/sigmets.svelte', () => ({
	sigmetAreaAt: m.sigmetAreaAt,
	sigmetsAt: m.sigmetsAt,
}));
vi.mock('$lib/state/route.svelte', () => ({ activeRoute: m.activeRoute }));

import { contextFeaturesAt, featureAt } from '$lib/map/interactions';

// getZoom because the chart gather reads one: a chart row is offered only
// where the layer would draw a chart. High enough that every fixture panel
// below clears its own floor unless the test says otherwise.
const map = mapAtZoom(14);

/** A map stub reporting one zoom, for the chart gather's legibility test. */
function mapAtZoom(zoom: number): L.Map {
...
/** A north-up panel engraved for a chosen zoom. The fixture states the zoom
 *  the sheet reads at rather than an affine, which is the same statement:
 *  panelNativeZoom reads that zoom back off the affine. */
function mkPanel(ident: string, kind: VacPanelKind, native: number, page = 1): VacPanel {
	const lat = 49;
	const lon = 2.5;
	const pxPerDegLatAtZ0 = 256 / 360 / Math.cos((lat * Math.PI) / 180);
	const perPt = 1 / (2 ** native * pxPerDegLatAtZ0);
	// The envelope is what the affine and the clip imply, never a stated
	// box beside them: a fixture must not be able to say a panel covers
	// ground its own mapping puts elsewhere.
	return {
		ident,
		section: 2,
		page,
...
/* The four area-tier candidates, as their gatherers hand them up: the row plus
 * the footprint in m² of the shape that contained the point. Sizes below are
 * written as NM² so a case reads as the map does; 1 NM² is about 3.43e6 m². */
const NM2 = 1852 * 1852;

function notamCand(index: number, areaNm2: number) {
	return { it: { notam: { id: `LFFA-R000${index}/26` }, index }, areaM2: areaNm2 * NM2 };
}
function supCand(id: string, zoneIndex: number, areaNm2: number) {
	return { zone: { sup: { id }, zoneIndex, zone: {} }, areaM2: areaNm2 * NM2 };
}
function sigCand(id: string, areaNm2: number) {
	return { sigmet: { id }, areaM2: areaNm2 * NM2 };
}
function aspCand(key: string, category: string, areaM2: number) {
	return { airspace: { key, category }, areaM2 };
}
function aspNm2(key: string, category: string, areaNm2: number) {
	return aspCand(key, category, areaNm2 * NM2);
}
...
beforeEach(() => {
	...
	m.airspaceAreaAt.mockReturnValue(null);
	m.notamAreaAt.mockReturnValue(null);
	m.supaipAreaAt.mockReturnValue(null);
	m.sigmetAreaAt.mockReturnValue(null);
	m.supaipZonesAt.mockReturnValue([]);
	m.sigmetsAt.mockReturnValue([]);
	...
	m.vacPanelsDrawnAt.mockReturnValue([]);
...
describe('featureAt', () => {
	...
	it('prefers an airport and short-circuits navaid/obstacle/airspace', () => {
		...
		m.airspaceAreaAt.mockReturnValue(aspCand('k', 'restricted', 1e9));
		...
		expect(m.airspaceAreaAt).not.toHaveBeenCalled();
	});
	...
	it('falls back to an airspace, carrying its category', () => {
		m.airspaceAreaAt.mockReturnValue(aspCand('LF-R-1|R1', 'fir', 1e10));
		expect(featureAt(map, 1, 2)).toEqual({
			kind: 'airspace',
			key: 'LF-R-1|R1',
			category: 'fir',
...
describe('featureAt: the area tier', () => {
	it('selects the smallest drawn shape, whatever kind drew it', () => {
		// The case this tier exists for: a NOTAM area covering a small SUP AIP
		// zone used to make it unreachable, the NOTAM's own path taking every
		// click inside it. Pane order would still say NOTAM; size says ZRT.
		m.notamAreaAt.mockReturnValue(notamCand(1, 40 * 25));
		m.supaipAreaAt.mockReturnValue(supCand('metropole-2026-018', 0, 7 * 7));
		m.airspaceAreaAt.mockReturnValue(aspNm2('LFPG9|TMA PARIS 9', 'controlled', 15 * 12));
		expect(featureAt(map, 1, 2)).toEqual({
			kind: 'supaip',
			id: 'metropole-2026-018',
			zone: 0,
		});
	});

	it('keeps a small NOTAM circle over a big airspace', () => {
		m.notamAreaAt.mockReturnValue(notamCand(2, 3 * 3));
		m.airspaceAreaAt.mockReturnValue(aspNm2('LFFF|FIR PARIS', 'fir', 300 * 300));
		expect(featureAt(map, 1, 2)).toEqual({ kind: 'notam', index: 2 });
	});

	it('ranks a SIGMET by size like everything else', () => {
		m.sigmetAreaAt.mockReturnValue(sigCand('SIG-1', 20 * 20));
		m.supaipAreaAt.mockReturnValue(supCand('sup', 0, 60 * 60));
		expect(featureAt(map, 1, 2)).toEqual({ kind: 'sigmet', id: 'SIG-1' });
	});

	it('lets an aimed target beat every area, without asking them', () => {
		m.airportHit.mockReturnValue({ ident: 'LFPZ' });
		m.notamAreaAt.mockReturnValue(notamCand(3, 0.5));
		expect(featureAt(map, 1, 2)).toEqual({ kind: 'airport', id: 'LFPZ' });
		expect(m.notamAreaAt).not.toHaveBeenCalled();
		expect(m.supaipAreaAt).not.toHaveBeenCalled();
	});

	it('falls back to pane order when two shapes measure the same', () => {
		// An activation NOTAM routinely republishes the ring of the zone it
		// activates; within a tenth of a percent the two are one shape on
		// screen, and the one drawn on top answers, as the ladder always did.
		m.notamAreaAt.mockReturnValue(notamCand(4, 100));
		m.supaipAreaAt.mockReturnValue(supCand('sup', 1, 100.05));
		expect(featureAt(map, 1, 2)).toEqual({ kind: 'notam', index: 4 });
	});

	it('gives a hair of difference to the smaller one', () => {
		m.notamAreaAt.mockReturnValue(notamCand(5, 100));
		m.supaipAreaAt.mockReturnValue(supCand('sup', 2, 90));
		expect(featureAt(map, 1, 2)).toEqual({ kind: 'supaip', id: 'sup', zone: 2 });
	});

	it('ranks a measureless shape last, and still selects it alone', () => {
		m.supaipAreaAt.mockReturnValue({
			zone: { sup: { id: 'degenerate' }, zoneIndex: 0, zone: {} },
			areaM2: Number.NaN,
		});
		expect(featureAt(map, 1, 2)).toEqual({
			kind: 'supaip',
...
describe('contextFeaturesAt', () => {
	it('gathers all feature lists without reordering them', () => {
		...
		// supaips and sigmets default to a miss in beforeEach, as does the VAC
		// gather above: this test is about the shape of the gather and the
		// order it keeps, not about those two lists.
	...
	it('offers a chart row only inside the sheet\u2019s own zoom window', () => {
		// A row brings a chart to the front, so it is worth offering while
		// that chart would be worth looking at. Under the floor the layer
		// draws no sheet at all and the pin the row set is dropped by the
		// next pass; past the ceiling the row promises a blow-up of a sheet
		// engraved for a far coarser zoom, under a heading naming a field
		// that may be nowhere near the view.
		//
		// vacPanelsAt hands them over most detailed first, which is the
		// order the rows keep.
		const ground = 15;
		const approach = 11;
		m.vacPanelsAt.mockReturnValue([
			mkPanel('LFPL', 'GMC', ground),
			mkPanel('LFPG', 'APP', approach),
		] as never);
		// Past the approach sheet's ceiling, inside the ground chart's floor.
		expect(
			contextFeaturesAt(mapAtZoom(approach + PANEL_ZOOM_CEILING + 0.5), 49, 2.55).charts,
		).toEqual([{ ident: 'LFPL', kinds: ['GMC'] }]);
		// Under the ground chart's floor, inside the approach sheet's window.
		expect(
			contextFeaturesAt(mapAtZoom(ground - PANEL_ZOOM_SLACK - 0.5), 49, 2.55).charts,
		).toEqual([{ ident: 'LFPG', kinds: ['APP'] }]);
		// Under both floors.
		expect(
			contextFeaturesAt(mapAtZoom(approach - PANEL_ZOOM_SLACK - 0.5), 49, 2.55).charts,
		).toEqual([]);
	});

	it('lists a sheet that is painted here whatever its window says', () => {
		// The field leading the view draws its best sheet however far past
		// its size the zoom has gone, something beating nothing where no
		// other chart answers. A chart on screen with no row of its own is
		// the same confusion the window rule exists to stop, the other way
		// about.
		const blownUp = mkPanel('LFPG', 'APP', 11);
		const past = 11 + PANEL_ZOOM_CEILING + 2;
		m.vacPanelsAt.mockReturnValue([blownUp] as never);
		expect(contextFeaturesAt(mapAtZoom(past), 49, 2.55).charts).toEqual([]);
		m.vacPanelsDrawnAt.mockReturnValue([blownUp] as never);
		expect(contextFeaturesAt(mapAtZoom(past), 49, 2.55).charts).toEqual([
			{ ident: 'LFPG', kinds: ['APP'] },
		]);
	});
