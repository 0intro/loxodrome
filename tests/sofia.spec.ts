...
describe('retryBudget', () => {
	...
	it('tries a refusal once like anything else, since SOFIA meant it', () => {
		// This pair was worth two attempts while
		// "Le format d'un des paramètres n'est pas bon : [departure_date,
		// departure_time, lang]" was read as a spurious refusal clearing on
		// retry. It is SOFIA declining a departure already behind the clock,
		// refused three times out of three when measured (2026-09-06), so the
		// extra attempt was never anything but another doomed POST at a
		// flood-prone service. The departure is clamped at the source now
		// (sofiaDepartureMs), and `refused` means again what failure.ts says:
		// SOFIA read the request and would refuse it again.
		expect(retryBudget('refused')).toBe(1);
		expect(retryBudget('malformed')).toBe(1);
	});
...
describe('sofiaDepartureMs', () => {
	// 19:41:23Z on the day of the reported failure.
	const NOW = Date.parse('2026-09-06T19:41:23Z');

	// SOFIA refuses a departure earlier than the current UTC minute and would
	// refuse it again. The app was handing it the EVALUATION WINDOW's start,
	// which is a display instant: the Flight period opens an hour before the
	// ETD, so every briefing made within an hour of departure was refused, and
	// with no ETD stated at all the start was always in the past.
	it('never asks for a departure SOFIA has already refused', () => {
		// The reported case: the window read 19:00Z, the clock read 19:41Z.
		const reported = Date.parse('2026-09-06T19:00:00Z');
		expect(sofiaDepartureMs(reported, NOW)).toBeGreaterThan(NOW);
		// And the ordinary one, a period that merely floors to the past minute.
		expect(sofiaDepartureMs(NOW - 23_000, NOW)).toBeGreaterThan(NOW);
	});

	// SOFIA caps no look-ahead (measured accepted at +30 days), so a plan made
	// for next week must still be briefed for next week, not for now.
	it('leaves a window that already opens in the future exactly where it is', () => {
		const nextWeek = Date.parse('2026-09-13T08:00:00Z');
		expect(sofiaDepartureMs(nextWeek, NOW)).toBe(nextWeek);
	});

	it('lands on a whole minute, since the body prints HHMM', () => {
		// buildNarrowRouteBody truncates to the minute, so anything inside a
		// minute would travel as that minute's start, which can be past again.
		expect(sofiaDepartureMs(0, NOW) % 60_000).toBe(0);
		expect(sofiaDepartureMs(NOW - 1, NOW) % 60_000).toBe(0);
	});

	it('keeps a minute to two ahead, so a request survives its own flight time', () => {
		for (const offset of [0, 1, 999, 30_000, 59_999]) {
			const now = NOW + offset;
			const ahead = sofiaDepartureMs(0, now) - now;
			expect(ahead).toBeGreaterThanOrEqual(SOFIA_DEPARTURE_GUARD_MS);
			expect(ahead).toBeLessThan(SOFIA_DEPARTURE_GUARD_MS + 60_000);
		}
	});

	// Resolved per ATTEMPT, and a briefing paces its routes, retries them and
	// waits on cold PIBs for minutes; a value frozen at the start went stale
	// and took every route behind the next minute boundary with it.
	it('never moves backwards as a briefing runs on', () => {
		let previous = 0;
		for (let elapsed = 0; elapsed <= 5 * 60_000; elapsed += 7_000) {
			const asked = sofiaDepartureMs(NOW - 3600_000, NOW + elapsed);
			expect(asked).toBeGreaterThanOrEqual(previous);
			expect(asked).toBeGreaterThan(NOW + elapsed);
			previous = asked;
		}
	});

	// Total on purpose: a window start that failed to resolve must fall back to
	// briefing from now, not reach Date.toISOString inside the body builder and
	// throw there, where it would surface as an unclassified route failure.
	it('briefs from now when the window start did not resolve', () => {
		const asked = sofiaDepartureMs(Number.NaN, NOW);
		expect(Number.isFinite(asked)).toBe(true);
		expect(asked).toBeGreaterThan(NOW);
	});

	it('reaches the wire as a departure ahead of the request that carries it', () => {
		const body = buildNarrowRouteBody(['LFPL', 'LFEM'], {
			widthNM: 15,
			radiusADNM: 10,
			validFrom: new Date(sofiaDepartureMs(Date.parse('2026-09-06T19:00:00Z'), NOW)),
		});
		const p = new URLSearchParams(body);
		expect(p.get('departure_date')).toBe('06-09-2026');
		expect(Number(p.get('departure_time'))).toBeGreaterThan(1941);
	});
});
