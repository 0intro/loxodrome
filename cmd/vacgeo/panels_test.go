...
func TestParseLabelsPrintedForms(t *testing.T) {
	...
	// A run set at 90 degrees is anchored the same way, along its OWN
	// baseline: half an advance in the direction it runs, and on it.
	cases = append(cases, struct {
		...
		name: "turned",
		runs: []textRun{{x: 100, y: 40, text: "002°  40'", adv: 22, ax: 0, ay: 22, size: 7, em: 7}},
		want: []label{{x: 100, y: 51, min: 160, lon: true}},
	})
	for _, c := range cases {
...
// A plain decimal number is not a coordinate, and the separator is matched
// as any run of non-digits, so nothing else tells one from a degree mark.
func TestParseLabelsRejectsAPlainDecimal(t *testing.T) {
	// Brive Souillac prints the leg distances of its VFR arrivals as
	// "3.3", "5.4", "7.8", set at an angle along each leg. All six parsed
	// as latitudes between 3 and 7 degrees, two of them formed a family
	// that ran in order, lay on a line and read a plausible scale, and the
	// sheet was fitted 4649 km from Brive.
	for _, s := range []string{"3.3", "5.4", "7.8", "12,5", "0.5"} {
		if got := parseLabels(mkRuns(textRun{x: 100, y: 100, text: s})); len(got) != 0 {
			t.Errorf("%q read as a coordinate: %+v", s, got)
		}
	}
	// And a real label with decimal SECONDS still reads, the decimal being
	// inside the seconds rather than the whole of the number.
	got := parseLabels(mkRuns(textRun{x: 100, y: 100, text: "45 02 23.0"}))
	if len(got) != 1 || math.Abs(got[0].min-(45*60+2+23.0/60)) > 1e-6 {
		t.Errorf("decimal seconds: %+v", got)
	}
}
