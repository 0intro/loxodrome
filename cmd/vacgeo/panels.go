...
// off the affine in the ellipsoid's own frame. A point of PAPER is the unit
// this command argues in: the draughtsman's pen is the floor on everything
// a plate states, and a threshold in metres means one thing on a pad detail
// and another on an approach chart.
...
func fitPanel(c pageContent, hemi hemisphere) (*panelFit, string) {
	...
	case latAsg == nil && lonAsg == nil:
		// Neither axis has a comb its labels can be tied to, so the
		// projection has no anchor to lend either of them. The LABELS are
		// the anchor of last resort: each axis carries its own points per
		// arcminute from the span of its own labels, and `deriveAxis`
		// looks for a comb at the intervals the SIA prints against that.
		// The scale is then the labels' and only the PHASE is the ticks',
		// which is the weakest fit this reader makes and the reason it is
		// last.
		if seedLat > 0 {
			latFit, latAsg = deriveAxis(latFam, seedLat, seedLat, lats, false)
		}
		if seedLon > 0 {
			lonFit, lonAsg = deriveAxis(lonFam, seedLon, seedLon, lons, true)
		}
	...
	//
	// The two readings separate the corpus cleanly. Of the 143 panels where
	// a comb with a scale of its own was displaced, 119 have the re-fitted
	// axis landing on its labels (the comb was a misread and the constraint
	// repaired it, which is what it is for) and 24 have it landing as far
	// from them as the correction was worth. Of the 29 that reach the
	// agreement test at all, the other 114 being outside the stretch band,
	// the highest refused agreement is 0.481 and the lowest overruled one
	// 0.763, so the threshold is not delicate.
	//
	...
	//
	// An axis with no comb of its own was re-derived from the projection and
	// has no first reading to fall back on.
	if latSpan == nil {
		latSpan = latFit
	}
	if lonSpan == nil {
	...
	// The comb the TICKS gave, which is not always the one above: a re-fitted
	// axis sits on a different grid and its own extent describes nothing,
	// which is why the cut below reads these. Villefranche's latitude axis is
	// forced and its re-fitted comb reaches past the frame its own page
	// draws. Handing THIS box to the readers above instead was measured and
	// is not kept (docs/fr-vacgeo-coverage.md, "Measured and not kept").
	read := box{x0: lonSpan.lo, x1: lonSpan.hi, y0: latSpan.lo, y1: latSpan.hi}
	clip := graticuleBox(latFit, lonFit, c.media)
	frame := neatline(c, ticks)
	if frame == nil {
		// A comb that reaches past its own map contains no frame, and the
		// fallbacks below then publish the sheet border. The LABELS are on
		// the map by construction, a label being what names a graticule
		// line, so a box they span is one a frame can still hold.
		//
		// As a FALLBACK and never as a band: bounding the ticks by the
		// labels wherever the full extent already finds a frame narrows 25
		// panels, Perpignan among them, whose 26 longitude labels sit
		// inside a 40 km chart that lands the coast from Port Leucate to
		// Saint-Cyprien-Plage. Here it runs only where the alternative is
		// the sheet border.
		frame = neatline(c, labelBox(lats, lons))
	}
	if frame == nil {
		frame = strokeFrame(c, ticks)
	}
	if frame != nil {
		if b := boxedFrame(c, read, labelBox(lats, lons), *frame); b != nil {
			frame = b
		}
	}
	if frame != nil {
		if b := labelBand(c, *frame, labelExtent(lats, lons)); b != nil {
			frame = b
		}
	}
	whole := clip
...
// labelBox is the rectangle the two axes' labels span, which is inside the
// map wherever the labels name its own graticule.
func labelBox(lats, lons []label) box {
	b := box{x0: math.MaxFloat64, y0: math.MaxFloat64, x1: -math.MaxFloat64, y1: -math.MaxFloat64}
	for _, l := range lats {
		b.y0, b.y1 = math.Min(b.y0, l.y), math.Max(b.y1, l.y)
	}
	for _, l := range lons {
		b.x0, b.x1 = math.Min(b.x0, l.x), math.Max(b.x1, l.x)
	}
	return b
}

// labelExtent is the rectangle EVERY label sits in, both of its coordinates
// counted. `labelBox` is the other thing: it takes each axis's labels along
// their own axis alone, which is what a frame has to bracket, and it says
// nothing about where a longitude label sits up the page. Deciding which
// band of a stacked sheet the graticule is on needs the whole position of
// every label, since a meridian's label is printed on the neatline the
// parallels' labels are not on.
func labelExtent(lats, lons []label) box {
	b := box{x0: math.MaxFloat64, y0: math.MaxFloat64, x1: -math.MaxFloat64, y1: -math.MaxFloat64}
	for _, l := range append(append([]label(nil), lats...), lons...) {
		b.x0, b.x1 = math.Min(b.x0, l.x), math.Max(b.x1, l.x)
		b.y0, b.y1 = math.Min(b.y0, l.y), math.Max(b.y1, l.y)
	}
	return b
...
func rephase(f *combFit, marks []float64, labels []float64) *combFit {
	...
	// tick at the most.
	//
	// And the EXTENT may NARROW here and never widen, for the same reason.
	// The marks offered are every candidate on the axis, pooled across the
	// families on purpose, so they hold the ones hanging off every other
	// line the sheet draws; the reach belongs to the family that was chosen
	// AS the graticule. Chatellerault Targe's latitude comb is read from
	// the marks on its own neatline, 196.6 to 476.6 inside a frame drawn at
	// 102.5 to 485.0, and comes back out of the re-phase reaching 23.2 to
	// 542.9, which is the page: `neatline` is then asked for a rectangle
	// containing the whole sheet, finds none, and the fallback publishes
	// the sheet border, header band, second aerodrome diagram, runway table
	// and all, onto the ground at the map's scale.
	//
	// Narrowing is kept because it is the picks saying they are not on the
	// new phase. Holding the reach RIGID instead costs three panels their
	// own frame, Pont Saint-Vincent's growing to the page edge over a
	// single pick 40 points past a frame whose step is 49.5
	// (docs/fr-vacgeo-coverage.md, fault 41).
	out := atPhase(f, sortedPicks(picks), f.p0+best)
	out.lo, out.hi = math.Max(out.lo, f.lo), math.Min(out.hi, f.hi)
	out.coreLo, out.coreHi = math.Max(out.coreLo, f.coreLo), math.Min(out.coreHi, f.coreHi)
	return out
}
...
// boxedFrame is the frame a page drew as a closed PATH rather than as an `re`
// rectangle, where that is tighter than anything the readers above found.
//
// The three frame readers take different candidates: `neatline` and
// `frameOnly` see the rectangles and the knockout windows, `panelFrames` sees
// those and the closed paths that FILL their own bounding box. So a frame
// painted as a path is invisible to two of the three, and Villefranche is
// what that costs: it draws its frame at (50.7, 105.2)-(403.6, 454.0) as a
// path, `neatline` found nothing, `strokeFrame` answered a box 76 % of the
// sheet with its bottom on the page trim, and the panel published its own
// header, a second diagram and its instructions block onto the ground at the
// map's scale.
//
// It is not offered as a candidate BESIDE the others, and the measurement is
// why. Admitted to `neatline` as a band it changes 186 clips and lands 166 of
// them on the frame their own page draws, Villefranche included; and it
// publishes the INSET of every composite sheet whose outer frame holds both
// panels. Grenoble Alpes Isere's landing sheet was opened over orthophotos
// and takes its aerodrome diagram, hangars, tower and all, onto the farmland
// south of the field; Biscarrosse Parentis, Nancy, Dijon Darois, Soulac,
// Monaco and Orly are the same shape. What holds those pages today is the
// answer the readers above already have, so this one only ever REPLACES that
// answer, and only where it is strictly tighter.
//
// And it must hold the LABELS as well as the ticks. Montpellier Candillargues
// draws two nested frames and paints the band between them, and that band
// holds the comb this reader fitted while every label it read sits 55 to 250
// points above it. A box that excludes the labels that named the graticule is
// not the graticule's frame, so the band is refused and the page takes its
// own INNER frame instead, which is what its ticks and its labels are both
// inside: it read 306 m against the ground published to the outer one and
// reads 116 m published to the inner.
func boxedFrame(c pageContent, ticks, labels, have box) *box {
	want := box{
		x0: math.Min(ticks.x0, labels.x0), y0: math.Min(ticks.y0, labels.y0),
		x1: math.Max(ticks.x1, labels.x1), y1: math.Max(ticks.y1, labels.y1),
	}
	var best *box
	for _, p := range c.polys {
		...
		if shoelaceArea(p.pts)/(b.width()*b.height()) < minFrameFill {
			continue
		}
		if b.x0 > want.x0+1 || b.y0 > want.y0+1 || b.x1 < want.x1-1 || b.y1 < want.y1-1 {
			continue
		}
		if b.width()*b.height() >= have.width()*have.height() {
			continue
		}
		// A side on the page trim is the sheet's own clip path and not a
		// neatline: no map runs to the paper's edge, there is always a
		// title band. `panelFrames` makes the same test across the width.
		// Orleans-Bricy draws a second box that fills itself, holds the
		// graticule and starts at x = 0, and taking it costs the panel
		// three of the labels it was fitted from once the cut has run.
		const trim = 3.0
		if b.x0 < c.media.x0+trim || b.y0 < c.media.y0+trim ||
			b.x1 > c.media.x1-trim || b.y1 > c.media.y1-trim {
			continue
		}
		if best == nil || b.width()*b.height() < best.width()*best.height() {
			b := b
...
// labelBand cuts a frame that holds more than one of its sheet's panels down
// to the one the graticule is on.
//
// A composite plate stacks its panels and the rules across the frame are the
// cuts, which is what `stackedPanels` reads and what `fitRunway` has done
// with it since it was written: a ground chart keeps the band holding the
// longest drawn runway. The graticule reader had no equivalent, so a frame
// enclosing the map AND a second diagram published both, the second one
// stretched onto the ground at the map's own scale.
//
// The band to keep is the one the LABELS are on. A label names a graticule
// line, so it is on the map by construction, and it is the only thing on
// these sheets that says which band is the map: `cutPanel` below cuts at the
// graticule's own outermost lines and cannot see a band with no graticule in
// it at all. The labels must name exactly ONE band, or the rule they crossed
// is not a divider and nothing is cut.
//
// It reaches four panels in the corpus and each publishes a real inset:
// Chatellerault Targe 81 points of its runway table and second diagram,
// Barcelonnette Saint-Pons 87, Dijon Darois 58 of its aerodrome diagram, and
// Montpellier Mediterranee 183, which is nearly half its clip.
func labelBand(c pageContent, frame, labels box) *box {
	bands := stackedPanels(c)
	if len(bands) < 2 {
		return nil
	}
	var keep *box
	for i := range bands {
		b := bands[i]
		if labels.y0 < b.y0-1 || labels.y1 > b.y1+1 {
			continue
		}
		if keep != nil {
			return nil
	...
	out := frame
	out.y0, out.y1 = math.Max(out.y0, keep.y0), math.Min(out.y1, keep.y1)
	if out == frame || out.height() < minPanelPt {
		return nil
	}
	return &out
}
