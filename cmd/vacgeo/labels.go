...
var (
	...
	// A plain decimal number is not a coordinate, and the loose separator
	// cannot tell one from a degree mark. Brive Souillac's approach sheet
	// prints its terrain spot heights as "3.3", "3.8", "3.4", "5.4",
	// "7.8" and "4.8"; all six parsed as latitudes between 3 and 7
	// degrees, two of them formed a family that ran in order, lay on a
	// line and read a plausible scale, and the sheet was fitted **4649 km**
	// from Brive with a comb residual of 0.151 pt and its axes in the
	// projection's own ratio to two decimals. Only the ARP gate caught it.
	//
	// The separator stays unpinned, for the reason the file header gives.
	// What it may not be is the whole of a decimal number: a coordinate
	// carries a degree mark or is set with spaces, and the SIA writes
	// none of the four printed forms with a decimal point in it.
	labDecimal = regexp.MustCompile(`^\d+[.,]\d+$`)
)
...
// Where a label SITS is the middle of its own baseline, which `baseMid`
// walks along the run's direction. Adding half the advance to x is the same
// point only while the run is upright, and 704 runs in the corpus parse as
// a label while set at an angle. A label half an advance out is a label
// tied to the wrong tick, which is a whole arcminute of panel.
//
...
func parseLabels(runs []textRun) []label {
	...
	for i, r := range runs {
		...
		if labDecimal.MatchString(strings.TrimSpace(r.text)) {
			continue
		}
		t := strings.Join(strings.Fields(r.text), " ")
		if m := labDMS.FindStringSubmatch(t); m != nil {
			lx, ly := r.baseMid()
			out = append(out, mkLabel(lx, ly, m[1], m[2], m[3]))
			used[i] = true
			continue
		}
		if m := labDM.FindStringSubmatch(t); m != nil {
			...
			lx, ly := r.baseMid()
			out = append(out, mkLabel(lx, ly, m[1], m[2], sec))
			used[i] = true
			continue
		}
		...
		used[i] = true
		lx, _ := r.baseMid()
		out = append(out, mkLabel(lx, (lo+hi)/2, m[1], parts[0], sec))
	}
	return out
}
