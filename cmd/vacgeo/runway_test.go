...
func TestScaleBarPtPerM(t *testing.T) {
	...
	got, ok := scaleBarPtPerM(runs, wholePage)
	if !ok {
	...
	if _, ok := scaleBarPtPerM([]textRun{{x: 10, y: 10, text: "hello"}}, wholePage); ok {
	...
	longest, ok := scaleBarPtPerM([]textRun{
	...
	near, ok := scaleBarPtPerM([]textRun{
