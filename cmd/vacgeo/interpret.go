...
type textRun struct {
	...
	// not.
	ax, ay float64
	// The em in PAGE space, which `size` is not: `size` is the text
	// matrix's a coefficient and so reads 0.00 on a run set at 90
	// degrees, and one run in seven on these plates is turned. Anything
	// measuring a glyph's HEIGHT wants this; the readers that group
	// labels by baseline keep using `size`, which is what they were
	// calibrated on.
	em   float64
	text string
}

// height is the run's em in PAGE space, which is what a glyph's size
// actually is. `size` is the text matrix's a coefficient, so it reads 0.00
// on a run set at 90 degrees and one run in seven on these plates is
// turned; anything gating on how BIG a glyph is wants this. The fallback
// is for a fixture that states a size and no matrix.
func (r textRun) height() float64 {
	if r.em > 0 {
		return r.em
	}
	return r.size
...
// axes are the run's own frame: the unit vector ALONG its baseline and the
// one ACROSS it, the way the glyphs stand up. A run with no advance has no
// direction, so it answers the page's.
//
// One run in seven on these plates is turned, so a reader that adds the
// advance to x and the size to y is not reading upright text, it is
// reading a run whose frame it has assumed. This is the frame.
func (r textRun) axes() (ux, uy, vx, vy float64) {
	if r.adv <= 0 {
		return 1, 0, 0, 1
	}
...
// baseMid is the middle of the run's BASELINE: half an advance along it,
// and on it. That is the point a label is anchored at, and adding half the
// advance to x is the same point only while the run is upright.
//
// On the baseline and not at the ink's centre, because every reader that
// matches a label to a tick was calibrated on the baseline: lifting a
// label by a third of its cap height moves all seven thousand of them.
func (r textRun) baseMid() (float64, float64) {
	ux, uy, _, _ := r.axes()
	return r.x + ux*r.adv/2, r.y + uy*r.adv/2
}

// beside reports where o sits in r's own frame: along its baseline and
// across it, both in points, measured from r's origin.
func (r textRun) beside(o textRun) (along, across float64) {
	ux, uy, vx, vy := r.axes()
	...
	for _, a := range args {
		...
		case pdf.String:
			...
			for i := 0; i < len(raw); i++ {
				...
				glyph := (width*w.g.tfs + w.g.tc) * w.g.th
				if code != ' ' {
					adv += glyph
					continue
				}
				gap := w.g.tw * w.g.th
				// A word space of several ems is not a word space: it is
				// the pen crossing the page inside one show string, and
				// what stands either side of it are separate runs.
				if w.g.tfs <= 0 || w.g.tw <= maxWordSpaceEm*w.g.tfs {
					adv += glyph + gap
					continue
				}
				// The glyph at code 32 belongs to the run that ends here.
				// It is a SPACE only by its code: these subset fonts map
				// 32 to the prime mark, which is why the gap is set with
				// word spacing in the first place.
				w.emitRun(raw[start:i+1], adv+glyph)
				w.tm = mat{1, 0, 0, 1, gap, 0}.mul(w.tm)
				start, adv = i+1, 0
			}
			w.emitRun(raw[start:], adv)
...
// maxWordSpaceEm is how much word spacing is still word spacing. A space is
// about a third of an em and Tw ADDS to it, so a Tw of two ems is not a gap
// between words at any size; it is a jump, and the text on the far side of
// it is somewhere else on the page.
//
// docs/fr-vacgeo-coverage.md, fault 48, has the plates: over the 2289 pages
// of a cycle exactly 70 show strings set a Tw this wide, and 28 of them are
// graticule labels printed two to a string.
const maxWordSpaceEm = 2

// emitRun records one piece of a show string and advances the text matrix
// past it. The advance is measured in PAGE space, between the piece's own
// origin and the pen after it: a label's width is what tells the grammar
// whether the next run abuts it.
func (w *walker) emitRun(raw string, adv float64) {
