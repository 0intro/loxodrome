...
type quad struct {
	...
	length float64
	width  float64
	// Whether the shape arrived as a rectangle rather than as a closed
	// outline. The two streams are equally good runways and are not
	// equally good evidence of where the MAP is, which is the one place
	// the distinction is read.
	boxed bool
}
...
func runwayQuads(c pageContent) []quad {
	...
	// And the RECTANGLES, which are the same shape drawn with a different
	// operator and which this reader had never looked at.
	//
	// A plate draws a runway as a closed subpath or as an `re`, and the
	// walk keeps the two apart: `padSquares` has read both streams since
	// it was written and this one read only the first. What it buys is
	// small and it is not what it was looked for. The `runway:no-match`
	// block went from 20 pages to 19: Nice draws 04R/22L as an `re` of
	// 406.0 by 6.17 points where the published strip is 2963 m, which the
	// sheet's own bar makes 407 points at an aspect of 66 against the
	// rectangle's 65.8, and Lyon's two are 315.8 and 210.9 points at
	// aspects 89.3 and 59.6 against published 89 and 59, and both pages
	// are still refused, because what fails on them is `orient` and not
	// the search.
	//
	// What it does buy is a page whose whole diagram is rectangles, Cuers
	// Pierrefeu's, and a SECOND runway on three sheets that had one:
	// Bordeaux, Toulouse and Bale Mulhouse each went from a one-runway
	// fit, which
	// is exact and checks nothing, to a two-runway fit whose residual is
	// 13.8, 13.3 and 27.2 m. Grenoble Alpes Isere's aerodrome chart, whose
	// longitude axis is reconstructed rather than read, gets an anchor
	// that moves it 341 m onto its own runway ends for a residual of 13 m,
	// and its ARP falls from 1049 m off the panel centre to 773.
	//
	// The walk keeps an `re`'s EXTENT and not its corners, so a rotated
	// one arrives as the upright box around it and fails the proportion
	// test by construction. That is the right answer for this reader: a
	// rectangle it can believe is one drawn square to the page.
	for _, b := range c.rects {
		lo, hi := math.Min(b.width(), b.height()), math.Max(b.width(), b.height())
		if hi < minRunwayLenPt || lo <= 0.05 || hi/lo < minRunwayAspect {
			continue
		}
		var a, z [2]float64
		if b.width() >= b.height() {
			a = [2]float64{b.x0, (b.y0 + b.y1) / 2}
			z = [2]float64{b.x1, (b.y0 + b.y1) / 2}
		} else {
			a = [2]float64{(b.x0 + b.x1) / 2, b.y0}
			z = [2]float64{(b.x0 + b.x1) / 2, b.y1}
		}
		out = append(out, quad{a: a, b: z, length: hi, width: lo, boxed: true})
	}
...
//
// Where the page prints no such run at all, the BARE number stands in. A
// ground chart labels a threshold the way the paint on it does, and the
// paint carries the suffix only where it has to: Nice's GMC prints "22"
// twice, once at each of 22L's and 22R's ends, and "22L" nowhere. The
// suffix says WHICH of a parallel pair, which the proportions have already
// said; what the label is wanted for is which END, and the bare number says
// that. It is a fallback and not a widening: a page that prints the suffixed
// form is read by it alone.
func designatorAt(runs []textRun, desig string, near [2]float64) (float64, bool) {
	if d, ok := runNearest(runs, strings.TrimSpace(desig), near); ok {
		return d, true
	}
	if bare := bareDesignator(desig); bare != "" {
		return runNearest(runs, bare, near)
	}
	return math.MaxFloat64, false
}

// bareDesignator drops a parallel-runway suffix, and reports nothing when
// there was none to drop.
func bareDesignator(desig string) string {
	d := strings.TrimSpace(desig)
	if n := len(d); n > 1 {
		switch d[n-1] {
		case 'L', 'C', 'R':
			return strings.TrimSpace(d[:n-1])
		}
	}
	return ""
}

func runNearest(runs []textRun, want string, near [2]float64) (float64, bool) {
...
// oneRunBarRe is the whole bar drawn as ONE show string: the zero, the pen's
// jump to the far end rendered as spaces, then the value and its unit.
var oneRunBarRe = regexp.MustCompile(`^\s*0\s{2,}(\d+(?:[.,]\d+)?)\s*(m|km|NM)\s*$`)
...
func scaleBarWithin(c pageContent, runs []textRun, within box, lo, hi float64) (float64, bool) {
	...
	// And the bar drawn as ONE show string, which the loop above cannot
	// see: the jump between the zero and the far label is inside the
	// string, so the walk renders it as spaces and the two marks never
	// exist apart. Cognac Chateaubernard's ground chart draws
	// `0                    500 m` as one run of 81 points, and this reader
	// found no bar on that page at all; ten pages in the corpus draw one
	// this way and nine of them read no bar.
	//
	// Where the labels sit inside such a run is unknown: it states where it
	// starts and how far it advances, and the far label's own width is
	// inside that. So the INK is required here rather than preferred, since
	// the run's advance runs past the end it names by the width of the
	// value and its unit, and a panel placed on a bar read that much long
	// is drawn that much small.
	for _, r := range runs {
		if !within.holds([2]float64{r.x + r.adv/2, r.y}) {
			continue
		}
		m := oneRunBarRe.FindStringSubmatch(r.text)
		if m == nil {
			continue
		}
		v, err := strconv.ParseFloat(strings.Replace(m[1], ",", ".", 1), 64)
		...
		ink, got := barInk(c, r.x, r.x+r.adv, r.y, within)
		if !got {
			continue
		}
		if ratio := ink / v; ratio >= lo && ratio <= hi && (ink > span || !ok) {
			best, span, ok = ratio, ink, true
		}
	}
	return best, ok
}
...
// outlineQuads keeps the candidates that arrived as closed outlines.
func outlineQuads(quads []quad) []quad {
	var out []quad
	for _, q := range quads {
		if !q.boxed {
			out = append(out, q)
...
// pair is one drawn strip tied to one published runway, both ends in order.
type pair struct {
	page   [2][2]float64
	metric [2][2]float64
	lenPt  float64
...
// crossedPairs ties TWO drawn shapes to TWO published runways with no
// designator read at all, by asking which of the four end-orderings a
// similarity can fit.
//
// `orient` refuses a page that does not print its designators beside its own
// ends, because two points fix a similarity in two ways and nothing about
// one strip tells a half-turn from the truth. TWO strips can, when the pair
// is not centrally symmetric: a half-turn maps each drawn strip onto itself
// reversed, so it maps the PAGE configuration to itself, and it maps the
// GROUND configuration to itself only where the two published runways are
// centrally symmetric, which a crossing pair is not and two parallels of
// equal length are. So the residual decides, and where it cannot the page
// is refused, which is the same idiom `runwayAnchor` uses on its own two
// orderings.
//
// It is deliberately the strictest thing in this file. The best fit has to
// be inside the cross-check's own 30 m and the next best has to be three
// times worse, over every assignment of two shapes to two runways and every
// ordering of their ends, so a second reading anywhere near the first
// refuses the sheet rather than choosing.
func crossedPairs(quads []quad, rwys []runwayRef, fr local) (pair, pair, bool) {
	const mustBeat = 3.0
	var best, next float64 = math.MaxFloat64, math.MaxFloat64
	var bestA, bestB pair
	for i := 0; i < len(rwys); i++ {
		// Unordered in the runways and ordered in the shapes: (i, j) and
		// (j, i) are the same configuration and counting it twice would
		// make the second reading equal the first and refuse every page.
		for j := i + 1; j < len(rwys); j++ {
			if rwys[i].aspect() <= 0 || rwys[j].aspect() <= 0 {
				continue
			}
			for qi := range quads {
				for qj := range quads {
					...
					if math.Abs(quads[qi].aspect()/rwys[i].aspect()-1) > maxAspectMismatch ||
						math.Abs(quads[qj].aspect()/rwys[j].aspect()-1) > maxAspectMismatch {
						continue
					}
					mi := [2][2]float64{fr.to(rwys[i].lePos), fr.to(rwys[i].hePos)}
					mj := [2][2]float64{fr.to(rwys[j].lePos), fr.to(rwys[j].hePos)}
					for _, oi := range [2][2][2]float64{{quads[qi].a, quads[qi].b}, {quads[qi].b, quads[qi].a}} {
						for _, oj := range [2][2][2]float64{{quads[qj].a, quads[qj].b}, {quads[qj].b, quads[qj].a}} {
							sim := fitSimilarity(
								[][2]float64{oi[0], oi[1], oj[0], oj[1]},
								[][2]float64{mi[0], mi[1], mj[0], mj[1]})
							if sim == nil || sim.scale() <= 0 {
								continue
							}
							switch {
							case sim.resid < best:
								next = best
								best = sim.resid
								bestA = pair{page: oi, metric: mi, lenPt: quads[qi].length}
								bestB = pair{page: oj, metric: mj, lenPt: quads[qj].length}
							case sim.resid < next:
								next = sim.resid
							}
						}
	...
	if best > maxCrossResidM || next < best*mustBeat {
		return pair{}, pair{}, false
	}
	return bestA, bestB, true
}

// proportioned is the longest candidate a published runway's proportions
// answer, which is the one worth anchoring a choice of panel on.
func proportioned(quads []quad, rwys []runwayRef) *quad {
	for i := range quads {
		for _, r := range rwys {
			if r.aspect() <= 0 {
				continue
			}
			if math.Abs(quads[i].aspect()/r.aspect()-1) <= maxAspectMismatch {
				return &quads[i]
			}
		}
	}
...
// within keeps the candidates drawn inside a box.
func within(quads []quad, b box) []quad {
	var out []quad
	for _, q := range quads {
...
func fitRunway(c pageContent, rwys []runwayRef, arp latLon) (*panelFit, string) {
	...
	if frame == nil {
		...
		// have got one wrong.
		// The box to bracket is the OUTLINES', where the page draws any.
		// The sheet's furniture is drawn with rectangles: a band margin to
		// margin across the head or the foot of the page, 352 to 354
		// points long and 8 to 43 wide, admitted as a candidate because it
		// is long and thin. It stands outside the map, so bracketing it
		// pushes the frame out to the sheet border: La Fleche and Orange
		// published their pictograms, their ALT AD block and their A/A
		// frequency as ground that way. Where the rectangles are all there
		// is, they are what there is to bracket.
		seed := quads
		if outl := outlineQuads(quads); len(outl) > 0 {
			seed = outl
		}
		if f := strokeFrame(c, quadsBox(seed)); f != nil && framePlausible(c, *f) {
			frame = f
		}
	}
	if frame == nil {
	...
	//
	// The candidates are filtered to the frame FIRST, or the longest of
	// them is the sheet's furniture: the header band is an `re` 352 points
	// long across the top of the page, outside every panel, and a band
	// chosen to hold it is no band at all.
	quads = within(quads, *frame)
	if len(quads) == 0 {
		return nil, "no-quad"
	}
	// One band is enough to cut at. Dunkerque les Moeres draws its
	// approach chart and its aerodrome diagram as two SEPARATE boxes, not
	// two bands of one, so `stackedPanels` has a single band to offer and
	// `strokeFrame` had bracketed both: the sheet published its approach
	// panel, drawn at a third of the scale, onto the ground at the
	// diagram's.
	//
	// And the band is chosen to hold a RUNWAY, so the shape that chooses
	// it has to be one: the longest candidate whose proportions answer a
	// published runway, not simply the longest. Falaise Monts d'Eraines
	// draws a 298.7 point shape at an aspect of 12.8 in its upper band and
	// its own 850 by 100 m runway at 8.5 in the lower one, and anchoring
	// on the longer of the two keeps the band the runway is not in.
	if anchor := proportioned(quads, rwys); anchor != nil {
		for _, b := range stackedPanels(c) {
			if !b.holds(anchor.a) || !b.holds(anchor.b) {
				continue
			}
			if b.width()*b.height() >= frame.width()*frame.height() {
				break
			}
			frame, quads = &b, within(quads, b)
			break
		}
	}
	// A runway is drawn ON the map, so a candidate lying outside the frame
	// is the sheet's furniture and not a strip.
	//
	// It reaches two pages over the corpus and both of them draw what it
	// drops as a rectangle. Hyeres Le Palyvestre draws a 352.3 by 7.92
	// point rule along its own frame edge, and the arrival designators 13
	// and 31 sit within the half-length `orient` allows of its two ends,
	// so the rule was confirmed as a runway and the page then failed the
	// two-runway cross-check at 30 m; dropping it keeps the panel.
	// Soissons Courmelles is the same fault with nothing else to catch it:
	// a 204.6 by 13.52 point legend bar 36 points below the map, confirmed
	// by the 07 and 25 the map itself prints, which placed a 15 km arrival
	// chart on a 1.9 km box.
	if len(quads) == 0 {
		return nil, "no-quad"
	}
	fr := newLocal(arp)
	// The printed scale bar, read before the pairing rather than after the
	// fit, because it is the one measurement on the page that owes the fit
	// nothing and it can say a pair is wrong one strip at a time.
	bar, hasBar := scaleBarPtPerM(c, runs, *frame)
	...
	for _, q := range quads {
		for _, r := range rwys {
			...
			// A shape drawn at a length the sheet's own bar does not agree
			// with is not that runway, whatever its proportions and its
			// designators say. The fitted scale has been checked against
			// the bar since the beginning; checking each PAIR against it
			// is the same measurement applied where it can still refuse
			// one reading instead of the whole sheet.
			//
			// It reaches two pages, both of them aerodromes publishing
			// several records a drawn strip answers equally well. Salon
			// Eyguieres draws a strip 71.9 points long and the reader tied
			// it to the 321 m 10/28, which is 0.224 points per metre
			// against a bar printing 0.153: 47 % out, and the two-runway
			// fit then missed by 98.3 m. Saintes Thenac tied a 102.3 point
			// strip to its 1101 m hard runway at 17.7 % out, where the
			// same strip is its 900 m grass one to a per cent. Both lose
			// the false pair and place on the true one.
			if hasBar && r.lengthM() > 0 {
				drawn := math.Hypot(q.b[0]-q.a[0], q.b[1]-q.a[1])
				if math.Abs((drawn/r.lengthM())/bar-1) > maxScaleDev {
					continue
				}
			}
	...
	if len(pairs) == 0 {
		if a, b, ok := crossedPairs(quads, rwys, fr); ok {
			pairs = []pair{
				{page: a.page, metric: a.metric, lenPt: a.lenPt},
				{page: b.page, metric: b.metric, lenPt: b.lenPt},
			}
		}
	}
	if len(pairs) == 0 {
...
// And how far it must move it, in POINTS OF PAPER.
//
// The rule under "smallest move wins" was that a candidate asking for a
// kilometre contradicts the grid and one asking for eighty metres agrees
// with it. The second half of that is wrong, and the ground says so.
//
// Measured over the 183 anchored panels that can be read against the
// ground both ways (docs/fr-vacgeo-coverage.md, fault 36), the anchor is
// worth its keep at the EXTREMES and costs at every scale between:
//
//	move, points of paper   n    better   worse   metres removed
//	         under 0.5      7       4       1           +75
//	          0.5 to 1     14       3       8           -68
//	            1 to 2     49      10      35          -376
//	            2 to 4     57      14      40          -708
...
//	            over 8     27      24       3         +2120
//
// A comb reads to about a tenth of a point, so an anchor asking for two or
// three is not correcting the grid, it is arguing with it: the drawn
// runway and the rest of the plate's line work disagree, and on that band
// the LINE WORK is the better guide. What the anchor is genuinely for is a
// grid that is WRONG, a misread label or a mis-phased comb, and that asks
// for eight points and more.
//
// Six is where the corpus peaks and it is not a knife edge: applying the
// anchor only past it removes 2264 m of disagreement against the 842 m it
// removes with no floor at all, and the plateau runs from 5 (2095) to 10
// (2052).
const minAnchorMovePt = 6.0
...
func runwayAnchor(c pageContent, fit *panelFit, rwys []runwayRef) (dLat, dLon, resid float64, ok bool) {
	...
	cLat, cLon := fit.Geo.at((fit.Clip.x0+fit.Clip.x1)/2, (fit.Clip.y0+fit.Clip.y1)/2)
	mPerPt := groundPerPoint(fit.Geo, newLocal(latLon{lat: cLat, lon: cLon}))
	if mPerPt <= 0 {
		return 0, 0, 0, false
	}
	runs := dedupeRuns(c.runs)
	...
	for _, q := range runwayQuads(c) {
		...
		for ri, r := range rwys {
			...
			for _, o := range [2][2][2]float64{{q.a, q.b}, {q.b, q.a}} {
				...
				if move < mPerPt*minAnchorMovePt {
					near = true
					continue
				}
				if !ok || move < bestMove {
	...
	if near {
		// A shape that answers this panel's runway from where the grid
		// already put it. That is the grid agreeing with the plate, and
		// the panel is not moved: falling through to some OTHER shape
		// that asks for more is how Toulouse Bourg-Saint-Bernard read
		// 80.7 m against the ground where not moving at all reads 18.3.
