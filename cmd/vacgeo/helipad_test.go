...
func TestNorthArrowNeedsAllThreeMarks(t *testing.T) {
	...
	// The letter is gated on how BIG it is, which `size` does not say: it
	// is the text matrix's a coefficient and reads 0.00 on a run set at 90
	// degrees. Six of the sheets refused `no-north` set theirs that way,
	// their letters measuring 0.05 to 3.07 points against an em of 7.96.
	c = arrow(90)
	c.runs[0].size, c.runs[0].em = 0.05, 9
	c.runs[0].ax, c.runs[0].ay = 0, 5
	if _, _, ok := northArrow(c, panel); !ok {
		t.Error("a north letter set at 90 degrees was not read")
	}
	// And a letter genuinely too small for a north arrow is still refused.
	c = arrow(90)
	c.runs[0].size, c.runs[0].em = 1.2, 1.2
	if _, _, ok := northArrow(c, panel); ok {
		t.Error("a 1.2 point letter was read as a north arrow's")
	}
}
