...
func northArrow(c pageContent, panel box) (float64, float64, bool) {
	...
	for _, r := range dedupeRuns(c.runs) {
		// How big the letter is, not how wide its matrix happens to be.
		// `size` is the text matrix's a coefficient and reads 0.00 on a
		// run set at 90 degrees, which is how these sheets set the N on a
		// turned panel: six of the pages refused `no-north` draw an arrow
		// with an N just past its point that only this gate kept out,
		// their letters measuring 0.05 to 3.07 where the em is 7.96.
		em := r.height()
		if strings.TrimSpace(r.text) != "N" || em < 4 || em > 16 {
			continue
		}
		...
		for _, a := range arrows {
			...
			if math.Hypot(r.x-a.hx, r.y-a.hy) > 3*em {
				continue
			}
...
func padMarks(c pageContent, panel box) []padMarkShape {
	...
	// twelve square corners at an aspect between 1.35 and 1.85. Counted
	// over the corpus, the band fires on 45 of the 971 panels it runs on
	// and finds 50 shapes, of which the enclosure below keeps 13: three
	// quarters of what a proportion admits is not a marking. Neither of
	// those two sheets lost a placement to it, both being refused at
	// no-square anyway, but a false marking is the one thing this reader
	// must not have, because the position it would anchor on is the
	// position.
	//
...
func isolatedRun(runs []textRun, i int) bool {
	...
	for j, o := range runs {
		...
		// The neighbour's offset in the RUN's frame. A run set beside this
		// one at any angle is still beside it.
		along, across := r.beside(o)
