...
func build(dataDir, cacheDir, chartsPath, suffix string, ov overrides, filter func(string) bool) (*Artifact, *Meta, error) {
	...
	// What the AIP says about the pads in PROSE, which is a second
	// published size for thirty aerodromes the airports dataset sizes only
	// once (build.go). Absent is not fatal: it is one more reading, and a
	// panel that cannot be placed without it was not being placed anyway.
	if prose, err := padProseSizes(
		filepath.Join(dataDir, "fr-aerodrome-facilities.json")); err == nil {
		for ident, sizes := range prose {
			ref, ok := positions[ident]
			if !ok || len(ref.pads) == 0 {
				continue
			}
			for i := range ref.pads {
				ref.pads[i].sizesM = append(ref.pads[i].sizesM, sizes...)
			}
			positions[ident] = ref
		}
	}
	...
	st := stats{byKind: map[string]int{}, byReason: map[string]int{}, byMethod: map[string]int{}, byAxes: map[string]int{}}
	var panels []panelRow
	...
	meta := &Meta{
		...
		ByKind: st.byKind, ByMethod: st.byMethod, ByAxes: st.byAxes, Overrides: st.overridden,
