...
func TestAuditCorpus(t *testing.T) {
	...
	if meta.Panels < 815 {
		t.Errorf("panels %d, want >= 815", meta.Panels)
	}
	if meta.Aerodromes < 610 {
		t.Errorf("aerodromes %d, want >= 610", meta.Aerodromes)
	}
	...
	if meta.ByMethod["helipad"] < 58 {
		t.Errorf("helipad fits %d, want >= 58", meta.ByMethod["helipad"])
	}
