...
func TestAuditCorpus(t *testing.T) {
	...
	if meta.Panels < 720 {
		t.Errorf("panels %d, want >= 720", meta.Panels)
	}
	if meta.Aerodromes < 570 {
		t.Errorf("aerodromes %d, want >= 570", meta.Aerodromes)
	}
	// The runway fit is the only way a ground-movement chart is placed at
	// all, so losing it would show up nowhere else.
	if meta.ByMethod["runway"] < 20 {
		t.Errorf("runway fits %d, want >= 20", meta.ByMethod["runway"])
	}
	...
	if meta.GateRejected > 40 {
		t.Errorf("gate rejected %d panels, want <= 40", meta.GateRejected)
	}
	...
	if anchored < 230 {
		t.Errorf("runway-anchored %d panels, want >= 230", anchored)
	}
