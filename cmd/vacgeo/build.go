...
// transitRe reads the VFR TRANSIT chart, which three military fields file
// under a header form of their own: Landivisiau prints "AD 2 LFRJ 01T",
// Lanveoc Poulmic "AD 2 LFRL TRAN 01" and Villacoublay Velizy "AD 3 LFPV
// 01T", and none of them names a family kindRe knows. They are the only
// three pages in the corpus whose kind it cannot name.
//
// They are read as APP, which is the family the SIA itself files this
// content under on the civil chain: Moulins Montbeugny's transit routes are
// printed on its "Visual approach AD 2 LFMY APP 01". All three are maps
// with a graticule; Villacoublay's carries the plate's own ARP and lands it
// inside the panel.
var transitRe = regexp.MustCompile(`\bAD\s*\d\s+(?:[A-Z0-9]{3,5}\s+)?(?:0?\dT\b|TRAN\s*0?\d)`)
...
// can go below it. The AIXM form is DDMMSS.ss and **69 per cent of the
// published pad coordinates sit exactly on a whole arcsecond** (160 of 231),
// which at 47 north is 30.9 m of latitude and 21.1 of longitude: rounding
// both to the nearest second is 10.8 m rms on its own, against the 19 m
// these panels are measured to sit from the ground. A helistation pad is twenty to
