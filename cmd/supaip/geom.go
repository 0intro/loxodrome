...
// centreBareDMSRe is bareDMSRe after an arc-centre lead-in, for the same wrap
// falling on a row that is NOT bare: the centre's own phrase stays with it and
// only the hemisphere letter crosses the break.
//
//	centré sur 43°12'57.00''
//	N,002°18'31.00''E
//
// 206/2026 prints its three ZRT DEDA columns that way and the third one's
// centre happens to fit on one line, which is why that supplement shipped one
// zone of the three its own title announces. The native extractor drops the
// spaces and maps "é" to U+FFFD, so the lead-in is matched loosely; what makes
// this safe is the caller's gate, a column whose arc is already pending a
// centre, and the fact that a cell reaching it carries no whole coordinate at
// all.
var centreBareDMSRe = regexp.MustCompile(`centr[^\d]{0,4}sur\s*\(?(\d{1,3})[°º\x{FFFD}](\d{1,2})['′‘’\x00-\x1f](\d{1,2}(?:[.,]\d+)?)\s*["'″‘’\x{FFFD}]*[,;]?\)?$`)
...
func parseGeometry(rows []prow, nav navaidTable) geomResult {
	...
	// The whole-document fallback speculates: with no LIMITES markers it
	// clusters whatever coordinates the pages carry. But a supplement that
	// creates airspace states that airspace's vertical limits, and names it
	// somewhere the document title can reach, so a fallback zone left with
	// NEITHER has found a coordinate table rather than a boundary. 193/2026
	// redesigns Brest's apron, and its parking-stand and INS check-point
	// tables became a 13-point ring lying over the taxiways, drawn with no
	// floor or ceiling at all. Across the 2025-2026 corpus that zone is the
	// only one carrying neither a name nor a vertical limit; the nine other
	// vertical-less zones are all named ones off a LIMITES table, which this
	// leaves alone.
	if fromFallback && len(zones) == 1 && zones[0].name == "" &&
		zones[0].lower == nil && zones[0].upper == nil {
		zones = nil
	}
	if len(zones) == 0 {
		return res
...
type arcSpec struct {
	...
	// from/to are the vertices the arc was printed BETWEEN, which is the only
	// evidence of where it belongs when several edges of the ring lie at its
	// radius from its centre: 006/2026's ZRT BIZERTE prints four vertices all
	// 12 NM from one centre, so EVERY edge qualifies and a geometric search
	// takes whichever it reaches first, putting both arcs on the wrong
	// side and the boundary some 12 NM off. hasEdge is false for the
	// interleaved tables, which build their zones by another path and still
	// rely on the search.
	from, to latlon
	hasEdge  bool
	afterIdx int // vertices seen in this column when the arc was read
}
...
func collectColumns(coordRows []prow, colXs []float64) map[float64]*colGeom {
	...
	pendingVert := map[float64]float64{} // 181/2026: a whole latitude whose LONGITUDE opens the next row
	...
	addArc := func(col float64, a arcSpec) {
		...
		if a.circle {
			...
			// Where the arc sat in the printed vertex sequence; its
			// neighbours are resolved once the column is complete.
			a.afterIdx = len(out[col].verts)
	...
	for _, c := range cells {
		...
		// The same centre can instead arrive with its longitude PRESENT but
		// stripped of its hemisphere, the row having broken before the
		// letter alone ("rayon centré sur" / "44°23'52.00'' N,004°43'01''" /
		// "E", 208/2026's ZRT VALREAS). The magnitude is on the row, so sign
		// it from the column's own longitudes (resolveLonSign refuses
		// anything but unanimous nearby evidence) rather than hold a
		// latitude that the bare "E" row can never complete. Left unread it
		// cost the zone more than its arc: the arc stayed pending into the
		// next row and took THAT row's vertex for a centre, so VALREAS lost
		// a corner to it as well.
			...
			if bm := trailingBareLonRe.FindStringSubmatch(text); bm != nil {
				if v, isLat, ok := convertDMS(bm[1], bm[2], bm[3], "E"); ok && !isLat {
					if sv, ok := resolveLonSign(v, lonSeen[col]); ok {
						p.spec.center = latlon{toks[0].val, sv}
						addArc(col, p.spec)
						delete(pending, col)
						continue
					}
		...
		// the next row (045/2026); hold it for that row. The same break can
		// fall inside an arc's centre phrase rather than on a bare row
		// (206/2026), which only a column already awaiting a centre accepts.
		if len(toks) == 0 {
			trimmed := strings.TrimSpace(text)
			bm := bareDMSRe.FindStringSubmatch(trimmed)
			if bm == nil {
				if p := pending[col]; p != nil && !p.hasLat {
					bm = centreBareDMSRe.FindStringSubmatch(trimmed)
				}
			}
			if bm != nil {
		...
		// Nothing pairable on this row. A column narrow enough breaks every
		// vertex at its comma, latitude and longitude landing on consecutive
		// rows of the SAME column:
		//
		//	48°39'47.30''N,
		//	002°33'49.20''E
		//
		// which is how 181/2026's ZRT 3 Air Legend, a real 2500 ft AMSL /
		// 5000 ft AMSL restriction, went missing while its three siblings
		// (two circles and a by-reference zone) read normally. Both halves
		// sit in this dead end, where the parser drops them today, so the
		// recovery can only add vertices it used to discard and never move
		// one it already reads. A lone latitude is held only when no arc is
		// pending to claim it as a centre, and only a lone longitude
		// completes it; anything else releases the hold rather than pairing
		// across unrelated rows.
		if len(vs) == 0 {
			if lat, held := pendingVert[col]; held {
				delete(pendingVert, col)
				if len(toks) == 1 && !toks[0].isLat {
					addVerts(col, c.x, []latlon{{lat, toks[0].val}})
					continue
				}
			}
			if len(toks) == 1 && toks[0].isLat && pending[col] == nil {
				pendingVert[col] = toks[0].val
			}
			continue
		}
		delete(pendingVert, col)
	...
	// The column is complete, so each arc's printed neighbours are now known:
	// the vertex read before it and the one read after.
	//
	// An arc printed after the LAST vertex is on the closing edge, running
	// from that vertex back to the first, and it is the commonest layout
	// there is: two vertices, then the arc. Eighteen of the corpus's 250
	// arcs sit there, and leaving them to the geometric search is what the
	// printed edge exists to stop, since on a two-vertex ring BOTH edges lie
	// at the radius and the search takes the first, which is the minor one.
	for _, g := range out {
		for i := range g.arcs {
			k := g.arcs[i].afterIdx
			switch {
			case k >= 1 && k < len(g.verts):
				g.arcs[i].from, g.arcs[i].to = g.verts[k-1], g.verts[k]
				g.arcs[i].hasEdge = true
			case k >= 2 && k == len(g.verts):
				g.arcs[i].from, g.arcs[i].to = g.verts[k-1], g.verts[0]
				g.arcs[i].hasEdge = true
			}
		}
	}
...
// Every arc is matched against the ORIGINAL vertex ring and inserted only
// once they all have their edge. Splicing them one at a time searched the
// ring each arc had already grown: a tessellation is a dense run of points,
// and an arc centred near the previous one finds a consecutive PAIR of them
// at its own radius long before it reaches the two vertices it belongs
// between. 104/2026's ZRT COGNAC CHARLIE closes with two 7 NM arcs whose
// centres lie 6 NM apart, so the second landed inside the first and the ring
// self-intersected and was dropped whole. Matching first also keeps two arcs
// off one edge, which the shifting indices used to allow.
func spliceArcs(ring *[]latlon, arcs []arcSpec) int {
	r := *ring
	if len(r) == 0 {
		return 0
	}
	mids := make(map[int][]latlon, len(arcs))
	count := 0
	// fits reports whether edge i can carry arc a: free, and with both ends at
	// the arc's radius from its centre. The radius test guards the printed
	// edge too, so an arc that read a wrong centre still splices nowhere.
	fits := func(a arcSpec, i int) bool {
		if _, taken := mids[i]; taken {
			return false
		}
		j := (i + 1) % len(r)
		di := haversineM(r[i][0], r[i][1], a.center[0], a.center[1])
		dj := haversineM(r[j][0], r[j][1], a.center[0], a.center[1])
		return within(di, a.radiusM, 0.1) && within(dj, a.radiusM, 0.1)
	}
	for _, a := range arcs {
		at := -1
		// The edge the supplement printed the arc between wins; dedupeRing and
		// splitRings may have moved it, so it is matched by POSITION and not
		// by index.
		if a.hasEdge {
			for i := 0; i < len(r); i++ {
				j := (i + 1) % len(r)
				if samePoint(r[i], a.from) && samePoint(r[j], a.to) && fits(a, i) {
					at = i
					break
				}
			}
		...
		j := (at + 1) % len(r)
		pts := geodesy.ArcPoints(r[at][0], r[at][1], r[j][0], r[j][1], a.center[0], a.center[1], a.radiusM, a.clockwise)
		mid := make([]latlon, 0, len(pts))
		for _, p := range pts {
			mid = append(mid, latlon{p[0], p[1]})
		}
		mids[at] = mid
		count++
	}
	...
	// On the closing edge the arc runs back to the first vertex, so its
	// tessellation simply ends the ring; nothing follows it.
	spliced := make([]latlon, 0, len(r))
	for i, v := range r {
		spliced = append(spliced, v)
		spliced = append(spliced, mids[i]...)
	}
	*ring = spliced
	return count
}
...
// samePoint compares two vertices at the datasets' own 5-decimal resolution
// (about a metre), so a vertex still matches after rounding.
