...
// 195/2026's ZRT COUPE ICARE prints the layout the arc-placement fix exists
// for and the one it was reaching least: two vertices, then the arc, then the
// first vertex repeated to close. The arc is therefore on the CLOSING edge,
// and an arc read after the last vertex must be placed there rather than left
// to the geometric search, which on a two-vertex ring finds both edges at the
// radius and takes the first, drawing the minor arc.
//
// The supplement says which one it means in its own words, "cercle de 1.8 NM
// de rayon tronqué au niveau de l'autoroute A41": a circle with a slice cut
// off is 33.7 km2, and the slice alone is 1.2.
func TestCollectColumnsArcAfterLastVertexClosesTheRing(t *testing.T) {
	rows := []prow{
		cellsRow(-1, cell{x: 62, text: "45�18'33''N,005�55'49''E"}),
		cellsRow(-2, cell{x: 62, text: "45�16'52''N,005�54'33''E"}),
		cellsRow(-3, cell{x: 62, text: "Archorairede1,8NMderayoncentr�sur45�18'25\"N,005�53'16\"E"}),
	}
	g := collectColumns(rows, []float64{62})[62]
	if len(g.arcs) != 1 || len(g.verts) != 2 {
		t.Fatalf("arcs = %d, verts = %d, want 1 and 2", len(g.arcs), len(g.verts))
	}
	a := g.arcs[0]
	if !a.hasEdge {
		t.Fatal("arc printed after the last vertex carries no edge; it closes the ring")
	}
	if !samePoint(a.from, g.verts[1]) || !samePoint(a.to, g.verts[0]) {
		t.Errorf("edge = %v -> %v, want %v -> %v", a.from, a.to, g.verts[1], g.verts[0])
	}
	// And it splices as the major arc: the ring is the circle less a slice.
	ring := []latlon{g.verts[0], g.verts[1]}
	if n := spliceArcs(&ring, g.arcs); n != 1 {
		t.Fatalf("spliced %d arcs", n)
	}
	if len(ring) < 40 {
		t.Errorf("ring = %d points, want the major arc", len(ring))
	}
}
