...
func refreshRetained(ctx context.Context, cache *pdfCache, lim *limiter, rows [][]any, retainFrom int, have map[string]bool) []rawSup {
	...
	for _, row := range rows {
		...
		if err != nil || pdf == nil {
			// The SIA withdraws a lapsed supplement's PDF, so the rows
			// retention exists to keep are the ones the network cannot
			// return. A copy already on disk is the same bytes.
			if pdf = cache.fromDisk(filename); pdf == nil {
				continue
			}
		}
		enName := enFilename(filename)
		pdfEn, err := cache.getDirect(ctx, pdfURL(enName), enName, "en:", lim)
		if err != nil || pdfEn == nil {
			pdfEn = cache.fromDisk(enName)
		}
