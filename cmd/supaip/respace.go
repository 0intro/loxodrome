...
// contact units and the manager, from a single pdftotext -layout pass, and
// reads the penetration rule out of that same text (resolvePenetration),
// which respaces the native rule only where the text holds no section. The
// build path uses it in place of respaceZoneNames so contacts come out
// word-spaced too; it no-ops when poppler is unavailable.
func respaceDoc(res *geomResult, data []byte) {
	...
	// The penetration rule is the one field read out of the layout text rather
	// than respaced back into it: its exceptions are what a pilot needs, and
	// the native pass cannot carry them (resolvePenetration).
	res.penetration = resolvePenetration(res.penetration, text, respace)
}
