...
func acquireOffline(o acquireOpts) (acquireResult, error) {
	...
	for _, reg := range regions {
		...
		rows := parseListing(data)
		res.listingSha[reg.key] = listingFingerprint(rows)
...
// fromDisk returns a PDF already in the cache directory, by filename, without
// asking the SIA. It is what lets a parser improvement reach a supplement the
// SIA has STOPPED SERVING: a lapsed row's PDF 404s, so -refresh-retained
// would skip exactly the rows retention exists to keep, and the committed
// dataset is the durable record of what was published. It never fires in CI,
// which starts from an empty cache.
func (c *pdfCache) fromDisk(filename string) []byte {
	b, err := os.ReadFile(filepath.Join(c.dir, filename))
	if err != nil {
		return nil
	}
	return b
