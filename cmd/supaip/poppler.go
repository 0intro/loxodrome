...
// pdftotextTimeout bounds one pdftotext invocation: a malformed upstream PDF
// can loop poppler forever, and a hang would stall the whole refresh run (the
// cached PDF then reproduces it every run). On expiry the process is killed
// and the caller degrades to its documented no-geometry / heuristic-respace
// path, exactly as when pdftotext is absent.
const pdftotextTimeout = 60 * time.Second

// runPdftotext runs pdftotext with the given arguments, feeding data on
// stdin and returning its stdout, bounded by pdftotextTimeout.
func runPdftotext(data []byte, args ...string) ([]byte, error) {
	...
	if err := cmd.Run(); err != nil {
		if ctx.Err() == context.DeadlineExceeded {
			return nil, fmt.Errorf("pdftotext: killed after %v", pdftotextTimeout)
		}
