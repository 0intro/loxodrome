// listing.go: parse a public SIA SUP AIP listing page into rows, and
// fingerprint what those rows advertise. Each table row carries
// "NNN/YYYY <subject>", a "Valide du <from> au <to> IFR VFR AIRAC" cell, and a
// /documents/download/f/d/<N> link that 302-redirects to the real
// lf_sup_*.pdf.
...
// listingFingerprint is the meta's per-region listing hash, taken over the
// PARSED rows rather than the page that carried them. The SIA serves these
// pages from Magento, which stamps every response with a fresh form_key, a
// random element id and a captcha timestamp: hashing the raw HTML produced a
// value that changed on every request, so the field said "the listing moved"
// on a run where nothing had, and could never say the one thing it was there
// to say. This hash changes when, and only when, the listing advertises
// something different.
//
// The rows are sorted first, so a page that re-orders the same supplements
// hashes the same (Build sorts its own output by region/year/number anyway),
// and an offline mirror of a page hashes exactly like the live fetch of it.
// Every parsed field is covered, including the AIRAC badge no row currently
// reads: the fingerprint describes the SOURCE, not this build's appetite.
func listingFingerprint(rows []listingRow) string {
	// Tab-separated, one line per row: nodeText has already collapsed every
	// run of whitespace in the subject to single spaces, so no value can
	// carry a separator and pass one field's bytes off as another's.
	lines := make([]string, 0, len(rows))
	for _, r := range rows {
		lines = append(lines, fmt.Sprintf("%03d/%d\t%s\t%s\t%s\t%t\t%t\t%t\t%s",
			r.number, r.year, r.descFr, r.validFrom, r.validTo,
			r.ifr, r.vfr, r.airacLabel, r.downloadURL))
	}
	sort.Strings(lines)
