...
func TestBuildAirspaces(t *testing.T) {
	...
	// 13 rows: 4 TMA/CTR/D/P + 1 FIR + 3 SIVs + 1 FBZ + 1 LTA + a
	// same-extent TMA pair (the base plus its borrowed class E twin) + the
	// D-OTHER aeromodelling row. The D-OTHER nature row is NOT among them.
	if meta.AirspaceCount != 13 {
		t.Errorf("AirspaceCount = %d, want 13", meta.AirspaceCount)
	}
	// The two D-OTHER rows the txtLocalType rework is about, pinned end to
	// end rather than only through classifyAse: both carry a full record AND
	// a boundary, so neither can pass by accident. AER must arrive under its
	// own type (not the generic ACTIVITY bucket it used to collapse into),
	// and PRN must be absent entirely, nature owning those 341 features
	// (docs/fr-aip.md).
	var sawModel, sawNature bool
	for _, r := range artifact.Rows {
		...
		case "LFXXAER":
			sawModel = true
			if row[1] != "MODEL" {
				t.Errorf("LFXXAER type = %v, want MODEL", row[1])
			}
		case "LFXXPRN":
			sawNature = true
		}
	...
	if !sawModel {
		t.Error("LFXXAER (D-OTHER + txtLocalType AER) missing from the rows")
	}
	if sawNature {
		t.Error("LFXXPRN (D-OTHER + txtLocalType PRN) emitted as airspace")
	}
	...
	// SkippedNoClassify any more. The dropped PRN row must not land there
	// either: that counter reports upstream SCHEMA DRIFT, and a decision of
	// ours showing up in it would make the signal unreadable.
...
func TestClassifyAse(t *testing.T) {
	cases := []struct {
		...
		// D-OTHER by txtLocalType: the SIA's own activity code, which is
		// the RELIABLE field. codeActivity is present on 25 rows in the
		// whole export and contradicts itself on the 18 that also carry
		// TRVL, so txtLocalType wins wherever it says anything
		// (docs/fr-aip.md).
		{"D-OTHER+AER -> MODEL", "D-OTHER", "AER", "", "MODEL", true},
		{"D-OTHER+VOL -> AEROBATIC", "D-OTHER", "VOL", "", "AEROBATIC", true},
		{"D-OTHER+AP -> UAS", "D-OTHER", "AP", "", "UAS", true},
		{"D-OTHER+PJE -> PARACHUTE", "D-OTHER", "PJE", "", "PARACHUTE", true},
		{"D-OTHER+TRPLA -> GLIDER", "D-OTHER", "TRPLA", "", "GLIDER", true},
		{"D-OTHER+TRVL -> PARAGLIDER", "D-OTHER", "TRVL", "", "PARAGLIDER", true},
		{"D-OTHER+TRPVL -> PARAGLIDER", "D-OTHER", "TRPVL", "", "PARAGLIDER", true},
		{"D-OTHER+BAL -> BALLOON", "D-OTHER", "BAL", "", "BALLOON", true},
		// LFV929TOW is the shape that settles it: txtRmk reads "Paragliding
		// winching" and codeActivity says TOWING. The local type wins.
		{"txtLocalType beats a contradicting codeActivity", "D-OTHER", "TRVL", "TOWING", "PARAGLIDER", true},
		// PRN / SUR are the nature-protection zones fr-nature.json carries.
		// They are not airspace and are dropped SILENTLY, not counted.
		{"D-OTHER+PRN dropped", "D-OTHER", "PRN", "", "", false},
		{"D-OTHER+SUR dropped", "D-OTHER", "SUR", "", "", false},
		// codeActivity is still read where the local type says nothing.
		...
		{"D-OTHER with neither -> ACTIVITY", "D-OTHER", "", "", "ACTIVITY", true},
		{"D-OTHER+unknown local type -> ACTIVITY", "D-OTHER", "WEIRD", "", "ACTIVITY", true},
		...
		{"D-OTHER+ pje (mixed case)", "D-OTHER", " Pje ", "", "PARACHUTE", true},
