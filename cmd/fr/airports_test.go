...
func TestBuildAirports(t *testing.T) {
	...
	// Schema shape. The layout is shared with the other airport datasets and
	// read positionally, so "charts" holds index 17 here as it does in
	// be-airports even though this file never fills it.
	if len(art.Fields) != 19 || art.Fields[0] != "ident" || art.Fields[14] != "joint" ||
		art.Fields[15] != "frequencies" || art.Fields[16] != "transition_alt_ft" ||
		art.Fields[17] != "charts" || art.Fields[18] != "pads" {
		t.Errorf("airports fields shape mismatch: %v", art.Fields)
	}
	if len(art.PadFields) != 6 || art.PadFields[0] != "lat" || art.PadFields[2] != "tlof_len_m" ||
		art.PadFields[4] != "fato_len_m" {
		t.Errorf("pad fields shape mismatch: %v", art.PadFields)
	}
	...
	// The helipads. A helistation's TLOF states where the PAD is, which is
	// not always where the aerodrome reference point is, and how big it is:
	// the two things a chart of a pad can be placed by.
	pads := map[string][]any{}
	for _, r := range art.Rows {
		row := r.([]any)
		pads[row[0].(string)] = row[18].([]any)
	}
	if got := pads["LF075"]; len(got) != 1 {
		t.Errorf("LF075 pads = %v, want one", got)
	} else {
		p := got[0].([]any)
		if p[0].(float64) != 48.79889 || p[1].(float64) != 2.45361 {
			t.Errorf("LF075 pad position = %v, %v, want the TLOF's own", p[0], p[1])
		}
		if p[2] != 21.0 || p[3] != 20.0 {
			t.Errorf("LF075 TLOF = %v x %v m, want 21 x 20", p[2], p[3])
		}
		// The final approach area around it is a different square, and a
		// chart may draw either.
		if p[4] != 30.0 || p[5] != 30.0 {
			t.Errorf("LF075 FATO = %v x %v m, want 30 x 30", p[4], p[5])
		}
	}
	// LFPG's TLOF states no size at all; the FATO it names does, with the
	// width left out because the pad is square.
	if got := pads["LFPG"]; len(got) != 1 {
		t.Errorf("LFPG pads = %v, want one", got)
	} else if p := got[0].([]any); p[2] != nil || p[4] != 16.0 || p[5] != 16.0 {
		t.Errorf("LFPG pad sizes = %v/%v x %v, want no TLOF and a 16 x 16 FATO", p[2], p[4], p[5])
	}
	// And an aerodrome that publishes none carries an empty list, never null.
	if got, ok := pads["LFFG"]; !ok || len(got) != 0 {
		t.Errorf("LFFG pads = %v, want an empty list", got)
	}
	if meta.PadCount != 2 {
		t.Errorf("PadCount = %d, want 2", meta.PadCount)
	}
