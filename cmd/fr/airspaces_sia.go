...
//
//   - ActiviteFreqs maps a SIV sub-sector codeId to the frequency values its
//     own <Volume>/<Activite> states, the per-sub-sector assignment neither the
//     AIXM nor the <Frequence> remarks carry. resolveRadio uses it to narrow a
//     sub-sector to the channel its chart prints, and to fill one the AIXM
//     never linked a service to. Corroboration rules and the join are in
//     airspaces_activite.go.
//
//   - FISFreqs indexes every flight-information-service <Frequence> record by
//     its unit's 4-letter location indicator and frequency (fisFreqKey), which
//     is what lets an Activite-filled row carry the publisher's own unit name
//     and call sign instead of one synthesised from the free text.
type SIAPlan struct {
...
// 119.800. Beauvais 2 (East) correctly stays on 119.800 (no override). The
// per-volume <Activite> reads 123.985 for Beauvais 1 too, so the entry is now
// corroborated rather than load-bearing; it stays as the escape hatch a cycle
// that dropped the field would need.
//
// Geneve 1-9 are the other kind: French airspace worked by a SWISS unit, so
// the frequency their <Activite> states appears in no French <Frequence>
// record at all and activiteFillEntries has nothing to key a unit on. The
// value is the one the SIV chart and ENR 6 print, and the nine sub-sectors
// share it.
var sivFreqOverrides = func() map[string][]InjectEntry {
	m := map[string][]InjectEntry{
		"LFOBFS1": {{Freq: "123.985", Unit: "APP BEAUVAIS Approche", Callsign: "BEAUVAIS - APPROCHE"}},
	}
	for _, codeId := range []string{
		"LSAGFS01", "LSAGFS02", "LSAGFS03", "LSAGFS04", "LSAGFS05",
		"LSAGFS06", "LSAGFS07", "LSAGFS08", "LSAGFS09",
	} {
		m[codeId] = []InjectEntry{
			{Freq: "126.350", Unit: "FIS GENEVE Information", Callsign: "GENEVE - INFORMATION"},
		}
	}
	return m
}()
...
func parseSIA(src []byte, aixmSIVByName AIXMSIVByName, aixmTMAByName AIXMTMAByName) (SIAPlan, error) {
	...
	// The <Activite> frequencies, collected by the SIV name their volume's lk
	// carries and resolved to codeIds after the pass, since the AIXM name index
	// is what maps them and a name can cover several volumes (STRASBOURG 4 is
	// two, on different frequencies, and keeps both).
	activiteByName := map[string][]string{}
	...
	if len(src) > 0 {
		...
		for {
			...
			if se.Name.Local == "Volume" {
				var v siaActivityVolume
				if err := dec.DecodeElement(&v, &se); err != nil {
					return plan, fmt.Errorf("decoding Volume: %w", err)
				}
				// A volume with no Activite contributes nothing, and so does
				// the reference child every record carries (same lk, empty
				// payload), which is why the text is required before the lk is
				// read at all.
				if v.Activite == "" {
					continue
				}
				if freqs := activiteFreqs(v.Activite); len(freqs) > 0 {
					for _, key := range activiteSIVKeys(v.Lk) {
						activiteByName[key] = append(activiteByName[key], freqs...)
					}
				}
				continue
			}
			if se.Name.Local != "Frequence" {
				continue
			}
			var f siaFreq
			...
			// The FIS index an Activite fill is corroborated by and takes its
			// unit and call sign from, keyed on the unit's location indicator
			// and the frequency. Keep-first: a unit publishing one channel
			// twice names it the same way both times, and only the two-letter
			// stems are 4-character indicators (the FIR-level services key
			// "[LF][FIR PARIS]", which no SIV codeId can match).
			if isFIS {
				if ind := stemFromLk(lk); len(ind) == 4 {
					if v, err := parseFreq(freq); err == nil {
						if key := fisFreqKey(ind, v); plan.FISFreqs[key].Freq == "" {
							plan.FISFreqs[key] = InjectEntry{Freq: freq, Unit: unit, Callsign: callsign}
						}
					}
				}
			}
	...
	// Resolve the Activite frequencies to codeIds through the AIXM name index.
	// Deduplicated by VALUE and sorted: a name reached by both the raw and the
	// "partie"-elided key would otherwise carry its channel twice, a SIV split
	// into several vertical volumes states the same one on each, and map
	// iteration order must not reach the artifact.
	for key, freqs := range activiteByName {
		for _, codeId := range aixmSIVByName[key] {
			for _, f := range freqs {
				if !hasFreqValue(plan.ActiviteFreqs[codeId], f) {
					plan.ActiviteFreqs[codeId] = append(plan.ActiviteFreqs[codeId], f)
				}
			}
		}
	}
	for codeId, freqs := range plan.ActiviteFreqs {
		sort.Slice(freqs, func(i, j int) bool {
			a, _ := parseFreq(freqs[i])
			b, _ := parseFreq(freqs[j])
			return a < b
		})
		plan.ActiviteFreqs[codeId] = freqs
	}
