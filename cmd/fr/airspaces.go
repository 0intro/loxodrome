...
//
// The first outcome carries a DELIBERATE drop as well as an uninteresting
// codeType: PRN / SUR are dropped there precisely so they stay out of
// skippedNoClassify, which exists to report schema drift and would be
// reporting a decision instead.
//
...
// airspace, RMZ-TMZ = combined zone (one emit). D-OTHER routes on
// txtLocalType, the field the SIA actually fills, and reads codeActivity
// only where that says nothing; PRN / SUR leave as nature rather than
// airspace (docs/fr-aip.md, and the branch itself for the evidence).
func classifyAse(a *Ase) (emit string, wanted bool) {
	...
	case "D-OTHER":
		...
		// FIRST so it never falls to a sporting bucket.
		local := strings.ToUpper(strings.TrimSpace(a.TxtLocalType))
		if local == "LTA" {
			return "LTA", true
		}
		// PRN (parcs et reserves naturels) and SUR (sites sensibles) are
		// NOT airspace. They are the nature-protection zones cmd/fr already
		// publishes as fr-nature.json, and emitting them here produced the
		// same 341 features twice - with a boundary the AIP never drew, the
		// 0.5 NM bubble pointBoundaryRing synthesises around the single
		// vertex the SIA gives. The map declined to mark them and left them
		// unlabelled; now they simply are not airspaces. docs/fr-aip.md.
		if local == "PRN" || local == "SUR" {
			return "", false
		}
		// The ACTIVITY kind is in txtLocalType, not in codeActivity. Only 25
		// rows in the whole export carry a codeActivity at all, and on the 18
		// that also carry txtLocalType TRVL it contradicts itself: PARAGLIDER,
		// PARACHUTE and TOWING all appear on rows whose txtRmk is word for
		// word "Paragliding winching". txtLocalType is the reliable field, the
		// codeId suffix corroborates it (TOW / MOD / AFP / PJE / AP), and the
		// codes are French abbreviations that decode exactly:
		//
		//	TRPLA  TReuillage PLAneur      glider winch launch
		//	TRVL   TReuillage Voile Libre  free-flight (paraglider) winching
		//	TRPVL  both of the above
		//	PJE    parachutage             parachute dropping
		//	AER    aeromodelisme           model aircraft
		//	VOL    voltige                 aerobatics
		//	AP     aeronefs telepilotes    unmanned aircraft
		//	BAL    ballons                 balloons
		//
		// So codeActivity is read only where txtLocalType says nothing.
		switch local {
		case "TRPLA":
			return "GLIDER", true
		case "TRVL", "TRPVL":
...
type AirspacesMeta struct {
	...
	SIAActiviteFilled    int            `json:"siaActiviteFilled"`
	SIAActiviteNarrowed  int            `json:"siaActiviteNarrowed"`
	SIAActiviteRefused   int            `json:"siaActiviteRefused"`
...
func BuildAirspaces(src []byte, opts AirspacesOptions) (AirspacesArtifact, AirspacesMeta, error) {
	...
	meta := AirspacesMeta{
		...
		SIAActiviteFilled:    stats.siaActiviteFilled,
		SIAActiviteNarrowed:  stats.siaActiviteNarrowed,
		SIAActiviteRefused:   stats.siaActiviteRefused,
		Counts:               stats.counts,
	}
...
type buildStats struct {
	...
	siaSectorMapped int
	siaInjected     int
	// The <Volume>/<Activite> path, split three ways: one number could not
	// say next cycle whether a narrow was lost because the SIA changed a
	// frequency or because an earlier rule started vetoing a stated one.
	siaActiviteFilled   int
	siaActiviteNarrowed int
	siaActiviteRefused  int
	sameExtent          int
}
...
func resolveRadio(ase *Ase, idx airspaceIndexes, plan loadedSIAPlan, stats *buildStats) []any {
	...
	// The sub-sector's own <Volume>/<Activite> (airspaces_activite.go), the
	// last statement left and the only per-sub-sector one for these rows.
	stated := plan.ActiviteFreqs[ase.Uid.CodeId]
	// Narrowing is for the APP-run SIVs only. A FIC is the FIR-level blanket,
	// and its frequency list is per-sub-AREA by construction: NOUVELLE
	// CALEDONIE's union carries four FIS channels whose own SIA remarks read
	// "Dans l'espace du SIV situé sous CTR/TMA NOUMEA partie …", i.e. they
	// serve parts of that very sector, while its Activite states only the two
	// general North / South watches. Dropping the four would lose published
	// service on an oceanic FIR.
	if len(stated) > 0 && len(radio) > 0 && !isFIC(ase) {
		kept, agreed := narrowRadioToStated(radio, stated)
		switch {
		case !agreed:
			stats.siaActiviteRefused++
		// A real reduction only, the same shape assignTMASectorRadio uses: an
		// empty set would blank a published sector, and one the size of the
		// union says nothing (the 57 rows where the two sources already
		// agree exactly).
		case len(kept) > 0 && len(kept) < len(radio):
			radio = kept
			stats.siaActiviteNarrowed++
		}
	}
	if len(radio) == 0 {
	...
	// Last: a sub-sector no other path reached at all. After InjectFreqs on
	// purpose, so the rows the SIA's own service records already answer for
	// (Paris, Marseille, Provence) keep the unit and call sign that path gives
	// them.
	if len(radio) == 0 && len(stated) > 0 {
		if entries, ok := activiteFillEntries(stated, sivIndicator(ase.Uid.CodeId), plan.FISFreqs); ok {
			for _, e := range entries {
				radio = append(radio, []string{e.Freq, e.Unit, e.Callsign})
			}
			stats.siaActiviteFilled++
		} else {
			stats.siaActiviteRefused++
		}
	}
...
type loadedSIAPlan struct {
	...
	ActiviteFreqs  map[string][]string
	FISFreqs       map[string]InjectEntry
}
...
func loadSIAPlan(src []byte, aixmSIVByName AIXMSIVByName, aixmTMAByName AIXMTMAByName) (loadedSIAPlan, error) {
	...
	return loadedSIAPlan{
		...
		ActiviteFreqs:  plan.ActiviteFreqs,
		FISFreqs:       plan.FISFreqs,
	}, nil
}
