...
var airportsOutputFields = []string{
	...
	"frequencies",
	"transition_alt_ft",
	// Index 17 is the "charts" column be-airports carries, and this file
	// does not: France's chart index is its own dataset (fr-adcharts.json).
	// The layout is positional and shared, so the column is emitted null
	// rather than skipped, or every reader would take the pads for charts.
	"charts",
	"pads",
}

// airportsPadFields mirrors the PadRow tuple in src/lib/data/airports.ts:
// one helipad, from the AIXM <Tla> and the <Fto> it names. Position first,
// since that is what the pad is FOR here, then the two published sizes in
// metres, which are the only statement of them a machine can read (AD 1.3-2
// carries the same figures as prose, and leaves them out for most pads).
//
// BOTH sizes, because they are two different squares and a chart may draw
// either: 26 pads state a touchdown area of about 11 m inside a final
// approach area of about 20, and 204 state one size for both.
var airportsPadFields = []string{"lat", "lon", "tlof_len_m", "tlof_wid_m", "fato_len_m", "fato_wid_m"}
...
type AirportsMeta struct {
	...
	// PadCount; aerodromes carrying at least one published TLOF.
...
// aixmTla; a helipad's touchdown and lift-off area, linked to its Ahp by
// codeId. It is the only thing in the export that says where a helistation's
// PAD is as opposed to where the aerodrome is, and how big it is: 234 of
// them, every one with a position and all but four with a length.
//
// The position equals the Ahp reference point on all but nineteen, which is
// the AIP defining a helistation's ARP as the centre of its own pad. Where
// they differ the pad is the truth about the pad, and that is what a chart
// draws.
type aixmTla struct {
	Uid struct {
		AhpUid struct {
		...
		TxtDesig string `xml:"txtDesig"` // "LF001-TLOF"
	} `xml:"TlaUid"`
	FtoUid struct {
		AhpUid struct {
			CodeId string `xml:"codeId"`
	...
	ValLen  string `xml:"valLen"`
	ValWid  string `xml:"valWid"`
	UomDim  string `xml:"uomDim"` // M / FT
}

// aixmFto; the final approach and take-off area a Tla sits in. It carries
// dimensions and no position of its own, so it stands in for a TLOF that
// states none: 276 of them against 234 TLOFs.
type aixmFto struct {
	Uid struct {
		AhpUid struct {
			CodeId string `xml:"codeId"`
...
type aixmAirport struct {
	...
	Ifr     bool
	Runways []aixmRunway
	// Pads; the helipads the export states a TLOF for, sorted by
	// designator. Empty for the 600-odd aerodromes that publish none.
	Pads []aixmPad
...
func decodeAixm(src []byte) (aixmStreams, error) {
	...
	for {
		...
		case "Tla":
			var v aixmTla
			if err := dec.DecodeElement(&v, &se); err != nil {
				return aixmStreams{}, fmt.Errorf("decoding Tla: %w", err)
			}
			s.tlas = append(s.tlas, v)
		case "Fto":
			var v aixmFto
			if err := dec.DecodeElement(&v, &se); err != nil {
				return aixmStreams{}, fmt.Errorf("decoding Fto: %w", err)
			}
			s.ftos = append(s.ftos, v)
		case "Rdd":
			var v aixmRdd
			if err := dec.DecodeElement(&v, &se); err != nil {
...
// aixmPad is one helipad as emitted: where it is and how big it is, both
// as the AIP states them.
type aixmPad struct {
	Lat, Lon float64
	// The touchdown and lift-off area, and the final approach and take-off
	// area around it. Either may be unstated.
	TlofLenM, TlofWidM *float64
	FatoLenM, FatoWidM *float64
}

// maxPadFromAirportM is how far a published pad may sit from the aerodrome
// it is filed under before it is not that aerodrome's pad.
const maxPadFromAirportM = 1000

// haversineM is the great-circle distance in metres, enough for a sanity
// gate over a kilometre.
...
// attachPads folds the <Tla> touchdown areas onto their aerodromes, each
// with the dimensions of the <Fto> it names. A pad with no readable
// position is dropped: the position is the whole point of the record here,
// and a size with nowhere to put it says nothing.
func attachPads(out map[string]*aixmAirport, s aixmStreams, icaoByCodeId map[string]string) {
	// Keyed by the aerodrome AND the designator. The designators are
	// aerodrome-prefixed ("LF192-FATO") and all 276 are distinct in this
	// cycle, so this changes no row today; what it removes is the chance
	// that a future cycle repeats one and a pad takes another field's
	// approach area. The Tla names its Fto with that Fto's own AhpUid, and
	// it agrees with the Tla's on all 66 links, so the pair always
	// resolves.
	type ftoKey struct{ icao, desig string }
	fto := map[ftoKey]*aixmFto{}
	for i := range s.ftos {
		f := &s.ftos[i]
		fto[ftoKey{strings.TrimSpace(f.Uid.AhpUid.CodeId), strings.TrimSpace(f.Uid.TxtDesig)}] = f
	}
	for i := range s.tlas {
		t := &s.tlas[i]
		icao := icaoByCodeId[strings.TrimSpace(t.Uid.AhpUid.CodeId)]
		if icao == "" {
			continue
		}
		ap := out[icao]
		...
		lat, ok1 := aip.ParseLat(t.GeoLat)
		lon, ok2 := aip.ParseLon(t.GeoLong)
		if !ok1 || !ok2 {
			continue
		}
		// A pad is on the aerodrome it is filed under. The export states
		// three that are not, and it states them as plainly as the rest:
		// Dax Centre Hospitalier's TLOF is written 001 25 26.42 W where
		// its own reference point is 001 02 25.7 W, the minutes and the
		// seconds transposed, and lands 30.9 km away; Amiens CHU Sud's is
		// 4.4 km out; Monaco's shares its aerodrome's latitude exactly and
		// misses its longitude by 1.275 degrees, which is 102.6 km.
		//
		// The corpus separates them from every real one without argument.
		// The furthest pad that is genuinely its aerodrome's is Calvi at
		// 180 m, then Perigueux at 143 and Paris Issy at 143; the next
		// three are the kilometres above. A pad more than a kilometre from
		// the aerodrome it belongs to is not that aerodrome's pad.
		//
		// It is dropped rather than corrected, because what the AIP states
		// wrongly is not ours to restate: Amiens' sheet then refuses for
		// having no pad instead of being placed 4.4 km out and caught by a
		// downstream gate, which is the difference between a refusal and a
		// near miss.
		if d := haversineM(ap.Lat, ap.Lon, lat, lon); d > maxPadFromAirportM {
			continue
		}
		pad := aixmPad{Lat: aip.Round5(lat), Lon: aip.Round5(lon)}
		pad.TlofLenM, pad.TlofWidM = padDim(t.ValLen, t.ValWid, t.UomDim)
		if f := fto[ftoKey{strings.TrimSpace(t.FtoUid.AhpUid.CodeId), strings.TrimSpace(t.FtoUid.TxtDesig)}]; f != nil {
			pad.FatoLenM, pad.FatoWidM = padDim(f.ValLen, f.ValWid, f.UomDim)
		}
		ap.Pads = append(ap.Pads, pad)
	}
	for _, ap := range out {
		sort.Slice(ap.Pads, func(i, j int) bool {
			if ap.Pads[i].Lat != ap.Pads[j].Lat {
				return ap.Pads[i].Lat < ap.Pads[j].Lat
			}
			return ap.Pads[i].Lon < ap.Pads[j].Lon
		})
	}
}

// padDim reads a pad's published length and width in metres. A square pad
// is stated once and the width left out, which is not a missing value: the
// SIA omits it exactly where it would repeat the length.
func padDim(lenS, widS, uom string) (*float64, *float64) {
	toM := func(s string) *float64 {
		v, err := strconv.ParseFloat(strings.TrimSpace(s), 64)
		if err != nil || v <= 0 {
			return nil
		}
		if strings.EqualFold(strings.TrimSpace(uom), "FT") {
			v *= 0.3048
		}
...
func buildRowFromAixm(ax *aixmAirport) []any {
	...
	return []any{
		...
		nil, // charts: see airportsOutputFields
		padRows(ax.Pads),
	}
}

// padRows returns the helipad tuples, never nil so the JSON column is
// always a (possibly empty) array.
func padRows(pads []aixmPad) []any {
	out := make([]any, 0, len(pads))
	for _, p := range pads {
		out = append(out, []any{p.Lat, p.Lon,
			floatOrNil(p.TlofLenM), floatOrNil(p.TlofWidM),
			floatOrNil(p.FatoLenM), floatOrNil(p.FatoWidM)})
	}
	return out
}

// floatOrNil unwraps an optional measurement for the JSON row.
