...
an aerodrome opens what its AIP publishes: runways, frequencies, the fuel it
sells, the directory entry and the charts.
...
preparation alongside them. Terrain elevation comes from the application's own
tile mosaic, built from the public Copernicus and NASA elevation models and
served from the same worker as the chart tiles. A finished route can be handed to
...
- **Fuel plan**: taxi, trip, procedure and wind allowance, then the reserve
  ...
  refuels, and marks any stop that publishes no grade the aeroplane takes.
...
It is built from this repository (`npm run android:apk`), and a Google Play
release is in testing.

## Privacy

There is no tracking, no analytics and no advertising, and nothing requires
an account. Routes, aircraft, settings and recorded flights live in the
browser's own storage on your device, and the reset in the Settings tab
erases them. An optional account, signed into with a code received by
e-mail, syncs flight plans, flights with their traces, and aircraft between
your devices through the [account service](account-api/), which keeps
Cloudflare's request log, IP addresses included, for 7 days; the account can
be [deleted](https://loxodrome.fr/account-deletion.html) at any time.

Map tiles and the raster charts come from their own tile servers, as with any
web map. Past those, the NOTAM fetches, METAR, TAF and SIGMET, the
precipitation radar and the weather-chart PDFs for the printed dossier go
through the same small [proxy Worker](notam-proxy/), which exists because the
upstreams send no CORS headers. Winds aloft go directly to
...
The [privacy policy](https://loxodrome.fr/privacy.html) is the complete
statement, request by request, and applies to the Android app as well.
...
| [Copernicus DEM](https://spacedata.copernicus.eu/collections/copernicus-digital-elevation-model) GLO-30 and GLO-90 | ground elevation, re-tiled by this project (live, cached) |
| [NASADEM](https://www.earthdata.nasa.gov/) | ground elevation where Copernicus is withheld (live, cached) |
...
Node 24 and, for the dataset tools, Go 1.27. The datasets in `public/data/`
are committed, so the application runs without the Go tools.

```sh
npm install
npm run dev        # http://localhost:5173
npm run dev:notam  # the NOTAM Viewer, at http://localhost:5173/notam/
npm run verify     # the full gate: lint, i18n lint, check, test, both builds
...
| `fuel` | SIA VAC plates + the AD 2.4 entries | `fr-fuel.json` |
...
its own yearly edition.

### The second application

The repository builds a second application from the same `src/lib`: the **NOTAM
Viewer** (`notam.html` + `src/notam/`), which carries the briefing half and
nothing else. No routes, no navigation, no weather, no aircraft. It is
published inside this site at `/notam/` and reads the same `public/data/`, so
the parser, the datasets and the symbology are never maintained twice. Its own
address, `notam-viewer.net`, is served from that path by the worker in
`notam-viewer-net/`.

### The workers

Three self-contained sub-projects, each with its own README and test suite and
each deployed by hand: `notam-proxy/` (the CORS bridge in front of the NOTAM
and weather services), `account-api/` (the optional account and sync service)
and `notam-viewer-net/` (the mask described above).
