...
// so the worker's miss paths (upstream ceiling + fetch) always run. The worker
// is deployed on a custom domain, where that cache is real, so this is a
// deliberately PESSIMISTIC stand-in: it makes each test pay every upstream
// call. What the real cache must do instead is installRecordingCache's
// subject.
...
// A recording edge cache, for the tests that are about caching itself. The
// entries are keyed by URL exactly as Cloudflare's is (it ignores Vary), which
// is why every cached route stores a response carrying NO
// Access-Control-Allow-Origin and stamps one per request: a hit filled by one
// allow-listed origin is handed to the next, and must not carry the first
// one's header. Returns the store so a test can count what was kept.
export function installRecordingCache() {
	const store = new Map();
	// The ranges asked of it (`bytes=a-b` per match carrying a Range header):
	// Cloudflare's cache answers a Range with a 206 when the stored copy
	// carries a Content-Length, which is what the radar's hit path relies on.
	store.ranges = [];
	globalThis.caches = {
		default: {
			async match(key) {
				const hit = store.get(typeof key === 'string' ? key : key.url);
				if (!hit) {
					return undefined;
				}
				const range = typeof key === 'string' ? null : key.headers.get('range');
				...
				const whole = new Uint8Array(await hit.clone().arrayBuffer());
				const headers = new Headers(hit.headers);
				headers.set('content-range', `bytes ${start}-${end}/${len}`);
				headers.set('content-length', String(end - start + 1));
				return new Response(whole.slice(start, end + 1), { status: 206, headers });
			},
			async put(key, res) {
...
// The whole radar OBJECTS the relay stored, and the refusals it remembers
// beside them under their own keys (/opera/refused/...).
export function objectsIn(store) {
	return [...store.keys()].filter((k) => k.includes('/opera/obj/')).length;
}

export function refusalsIn(store) {
	return [...store.keys()].filter((k) => k.includes('/opera/refused/')).length;
}
