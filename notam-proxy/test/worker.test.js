...
test('/tiles bounds-checks coordinates and forwards with a timeout', async () => {
	const worker = await freshWorker();
	const fc = stubFetch((url, init) => {
		assert.ok(init.signal instanceof AbortSignal);
	...
	const ctx = makeCtx();
	const out = await worker.fetch(req('/tiles/terrarium/2/5/1.png'), ENV, ctx); // x=5 >= 2^2
	assert.equal(out.status, 404);
	const q = await worker.fetch(req('/tiles/terrarium/2/1/1.png?x=1'), ENV, ctx);
	assert.equal(q.status, 400);
	assert.equal(fc.length, 0);
	const ok = await worker.fetch(req('/tiles/terrarium/2/3/1.png'), ENV, ctx);
	assert.equal(ok.status, 200);
	assert.equal(ok.headers.get('content-type'), 'image/png');
	assert.equal(fc.length, 1);
	assert.equal(fc[0].url, 'https://elevation-tiles-prod.s3.amazonaws.com/terrarium/2/3/1.png');
});
...
test('/sia/vac serves a second reader from the edge cache', async () => {
	...
	const second = await worker.fetch(req(path, { ip: '203.0.113.9' }), ENV, ctx);
	assert.equal(second.status, 200);
