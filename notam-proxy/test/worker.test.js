...
test('/notam takes the validity epochs in the one spelling the app writes', async () => {
	// Anything Number() would also read (hex, octal, binary, an exponent, a
	// fraction, padding) once filled the entry the app's own request reads,
	// while the upstream received the raw spelling.
	const worker = await freshWorker();
	const fc = stubFetch((url) => {
		if (url === TOKEN_URL) {
			return tokenResponse();
	...
	const ctx = makeCtx();
	const base = '/notam?itemas=%5B%22LFFF%22%5D&limit=100&offset=0&endvalidity=1000086400';
	for (const spelling of ['0x3b9aca00', '0o7346545000', '0b111011100110101100101000000000', '1e9', '1000000000.9', '%201000000000%20', '01000000000', '-1', '']) {
		const res = await worker.fetch(req(base + '&startvalidity=' + spelling, { ip: '198.51.100.66' }), ENV, ctx);
		assert.equal(res.status, 400, spelling);
	}
	const twice = await worker.fetch(req(base + '&startvalidity=1000000000&startvalidity=1000000000'), ENV, ctx);
	assert.equal(twice.status, 400, 'a repeated epoch');
	assert.equal(callsWith(fc, NOTAM_URL).length, 0);
	// The app's own query is forwarded byte for byte.
	const path = base + '&startvalidity=1000000000';
	const ok = await worker.fetch(req(path), ENV, ctx);
	assert.equal(ok.status, 200);
	const sent = callsWith(fc, NOTAM_URL);
	assert.equal(sent.length, 1);
	assert.equal(sent[0].url, NOTAM_URL + path.slice('/notam'.length));
});

test('the /notam and /sofia memos stay inside one byte budget', async () => {
	// 32 answers of 4 MB per memo could hold more than the isolate: an answer
	// over 2 M characters is served and never kept, and the two memos
	// together keep about 24 MB, the oldest going first.
	const worker = await freshWorker();
	const big = 'x'.repeat(1_900_000);
	const fc = stubFetch((url) => {
		if (url === TOKEN_URL) {
			return tokenResponse();
		}
		if (url.startsWith(NOTAM_URL)) {
			...
			return jsonResponse(offset === 999 ? 'y'.repeat(2_100_000) : big);
		}
		throw new Error('unexpected url ' + url);
	});
	const ctx = makeCtx();
	const upstream = () => callsWith(fc, NOTAM_URL).length;
	const at = (offset) => req(`/notam?itemas=%5B%22LFFF%22%5D&limit=100&offset=${offset}`);
	// Too long to keep: asked twice, fetched twice.
	await (await worker.fetch(at(999), ENV, ctx)).text();
	await (await worker.fetch(at(999), ENV, ctx)).text();
	assert.equal(upstream(), 2);
	// Seven answers of 3.8 MB (two bytes a character): six fit the budget, so
	// the first is gone when the seventh lands and the second still replays.
	for (let i = 0; i < 7; i++) {
		await (await worker.fetch(at(i), ENV, ctx)).text();
	}
	assert.equal(upstream(), 9);
	await (await worker.fetch(at(1), ENV, ctx)).text();
	assert.equal(upstream(), 9, 'the second is still held');
	await (await worker.fetch(at(0), ENV, ctx)).text();
	assert.equal(upstream(), 10, 'the oldest went to make room');
});
...
test('an IPv6 client is one client across its /64', async () => {
	...
	// Each request from another address of the same /64: once 2^64 fresh
	// identities, each with a full window.
	for (let i = 0; i < 120; i++) {
		const res = await worker.fetch(req(path, { ip: `2001:db8:1:2::${(i + 1).toString(16)}` }), ENV, ctx);
		assert.equal(res.status, 200);
	}
	const refused = await worker.fetch(req(path, { ip: '2001:0db8:0001:0002:ffff:ffff:ffff:ffff' }), ENV, ctx);
	assert.equal(refused.status, 429);
	// The next /64 is another client.
	const other = await worker.fetch(req(path, { ip: '2001:db8:1:3::1' }), ENV, ctx);
	assert.equal(other.status, 200);
});

test('an IPv4 client reaching an IPv6 socket is that IPv4 client, not the /64 of ::', async () => {
	const worker = await freshWorker();
	stubFetch((url) => {
		if (url === TOKEN_URL) {
			return tokenResponse();
	...
	const path = '/notam?itemas=%5B%22LFFF%22%5D&limit=10&offset=0';
	for (let i = 0; i < 120; i++) {
		const res = await worker.fetch(req(path, { ip: '::ffff:198.51.100.1' }), ENV, ctx);
		assert.equal(res.status, 200);
	}
	// Its plain IPv4 spelling is the same client...
	assert.equal((await worker.fetch(req(path, { ip: '198.51.100.1' }), ENV, ctx)).status, 429);
	// ...and another mapped address is another client, where every mapped
	// address used to share one window.
	assert.equal((await worker.fetch(req(path, { ip: '::FFFF:198.51.100.2' }), ENV, ctx)).status, 200);
});

test('a flood of fresh keys never resets the limiter for the client being limited', async () => {
	// Past 5 000 keys the map was CLEARED, every window with it, so a client
	// could reset its own limit by cycling addresses. The least recently used
	// key goes instead.
	const worker = await freshWorker();
	stubFetch((url) => {
		if (url === TOKEN_URL) {
	...
	const path = '/notam?itemas=%5B%22LFFF%22%5D&limit=10&offset=0';
	for (let i = 0; i < 120; i++) {
		await worker.fetch(req(path, { ip: '198.51.100.1' }), ENV, ctx);
	}
	assert.equal((await worker.fetch(req(path, { ip: '198.51.100.1' }), ENV, ctx)).status, 429);
	// More other clients than the map holds, the limited one still knocking
	// between them.
	for (let i = 0; i < 5200; i++) {
		await worker.fetch(req(path, { ip: `10.${(i >> 16) & 255}.${(i >> 8) & 255}.${i & 255}` }), ENV, ctx);
		if (i % 400 === 399) {
			assert.equal((await worker.fetch(req(path, { ip: '198.51.100.1' }), ENV, ctx)).status, 429, `after ${i + 1}`);
		}
	}
});
...
test('reads in flight buffer together no more than their shared budget', async () => {
	// Each read buffers up to RESP_CACHE_MAX_BODY, but nothing bounded them
	// together: forty concurrent near-cap briefings held about 156 MB of an
	// isolate's 128. Past the shared budget a read passes the rest through
	// (answered at once, never memoized) instead of buffering it.
	const worker = await freshWorker();
	const chunk = new TextEncoder().encode('x'.repeat(975_000));
	let open;
	...
	const pending = Array.from({ length: 16 }, (_, i) =>
		worker
			.fetch(
				req('/sofia?session=ABC', {
					method: 'POST',
					ip: `198.51.100.${i + 1}`,
					headers: { 'content-type': 'application/x-www-form-urlencoded' },
					body: `:operation=postNarrowRoutePibRequest&route[]=LFPG&route[]=LFMN&n=${i}`,
				}),
				ENV,
				ctx,
			)
	...
	// 3.9 MB each: the 32 MB budget holds eight of them, the rest pass through.
	assert.ok(early >= 7, `${early} of 16 answered before their bodies ended`);
	open();
...
test('/wx serves a second origin from the edge cache under its own CORS header', async () => {
	...
	// answer's max-age to four hours); the edge copy carries the minute.
