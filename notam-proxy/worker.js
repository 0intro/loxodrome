 ...
 * CORS headers on the response. It also relays terrain elevation tiles
 * (/tiles/terrarium/, see handleTile), live METAR / TAF / SIGMET weather from
 ...
 *      (gitignored).
 *   4. Copy the worker URL (https://<name>.<account>.workers.dev) into the
 *      Loxodrome build (VITE_NOTAM_PROXY_URL) if it differs from the
 *      default.
 *
...
// Terrain elevation tiles (AWS Terrarium PNG) proxied for CORS; see handleTile.
const TILE_PREFIX = '/tiles/terrarium/';
const S3_BASE = 'https://elevation-tiles-prod.s3.amazonaws.com/terrarium/';
const TILE_RE = /^\/tiles\/terrarium\/(\d{1,2})\/(\d{1,7})\/(\d{1,7})\.png$/;
const TILE_MAX_Z = 15;
...
const RATE_LIMITS = {
	...
	tiles: { limit: 1200, windowMs: 60_000 },
...
const GLOBAL_CEILINGS = {
	...
	tiles: { limit: 6000, windowMs: 60_000 },
...
// Module-global memos (real on *.workers.dev, unlike caches.default). Short
// TTL keeps safety-critical NOTAM data fresh; the entry + body caps bound
// memory (whole-FIR / full-bulletin payloads are large).
...
export default {
	async fetch(request, env, ctx) {
		...
		// Terrain tiles: a path-prefix branch. Served from S3 with CORS.
		if (url.pathname.startsWith(TILE_PREFIX)) {
			return handleTile(url, allowOrigin, ctx);
		}
...
/**
 * Terrain elevation tile proxy. AWS Terrarium tiles (elevation-tiles-prod)
 * carry no CORS header, so the browser can't decode their pixels directly;
 * this serves them with CORS and a long immutable TTL. SSRF-safe: only
 * well-formed {z}/{x}/{y} integers in range are forwarded, always to the
 * hardcoded S3 base. (caches.default is a no-op on *.workers.dev, so there the
 * browser HTTP cache + the immutable header carry caching; the edge cache only
 * helps if this Worker is ever served from a custom domain.)
 */
async function handleTile(url, allowOrigin, ctx) {
	if (url.search !== '') {
		// No query params are meaningful; reject to keep the cache key clean.
	...
	const y = Number(m[3]);
	const n = 2 ** z;
	// The regex already forces non-negative integers; just bound them.
	if (z > TILE_MAX_Z || x >= n || y >= n) {
		return cors(new Response('tile out of range', { status: 404 }), allowOrigin);
	}

	// Edge cache keyed on the normalised URL: an immutable tile is shared by
	// every origin, so the cached copy carries no per-origin CORS header; CORS
	// is layered on per-request below.
	const cache = caches.default;
	const cacheKey = new Request(url.toString(), { method: 'GET' });
	let cached = await cache.match(cacheKey);
	if (!cached) {
		// Tier-2 ceiling before the S3 fetch (same caches.default no-op caveat as
		// /wx on *.workers.dev; the tiles ceiling is high to suit profile bursts).
		const reservation = reserveUpstream('tiles');
		if (!reservation.allowed) {
			return cors(refusal(busyMessage('tiles'), reservation.retryAfterS), allowOrigin);
		}
		let s3;
		try {
			s3 = await fetch(`${S3_BASE}${z}/${x}/${y}.png`, {
				redirect: 'follow',
				signal: AbortSignal.timeout(FETCH_TIMEOUT_MS.tiles),
			});
		} catch (err) {
			logError('upstream-error', { route: 'tiles', error: err.message ?? String(err) });
			return cors(
		...
		if (!s3.ok) {
			// Pass an upstream miss through (e.g. 404 for an absent tile); don't cache it.
			return cors(new Response(null, { status: s3.status }), allowOrigin);
		}
		cached = new Response(s3.body, {
			status: 200,
			headers: {
 ...
 * forced. Responses are edge-cached for WX_TTL_S on the normalised URL
 * (same caches.default caveat as the tiles: a no-op on *.workers.dev, where
 * the browser cache + the max-age header carry it).
 */
async function handleWx(url, allowOrigin, ctx) {
	...
	let cached = await cache.match(cacheKey);
	if (!cached) {
		// Tier-2 ceiling before the upstream call (caches.default is a no-op on
		// *.workers.dev, so there every /wx counts; the wx ceiling is generous).
